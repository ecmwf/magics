...
m = MissingValue(0)
m.apply()
waveperiod_fieldset = FieldSet("wavemeanperiod.grib")
...
for area in arg_dict["area"]:
	...
	box = LegendBox(
		...
		legend_title_text = 'Mean wave period in seconds',
	...
	for step in arg_dict["step"]:
		...
		title = FieldAutoTitle(ice,text = [None,"Mean wave period and direction"])
		layout.plot(s(
				box,
