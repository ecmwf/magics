...
m = MissingValue(0)
m.apply()
waveheight_fieldset = FieldSet("windwave_waveheight.grib")
wavedir_fieldset = FieldSet("windwave_md.grib")
ice_fieldset = FieldSet("waveicedir.grib")

# Set values less then 0.1 to 0.2 purely for area fill purpose.
#for f in waveheight_fieldset:
#    m = f.matrix()
#    m.replaceIfLowerOrEqual(0.1,0.2)
...
for area in arg_dict["area"]:
	...
	box = LegendBox(
		...
		legend_title_text = 'Significant height of wind waves in meters',
	...
	for step in arg_dict["step"]:
		...
		title = FieldAutoTitle(ice,text = [None,"Significant height of wind waves and mean direction"])
		layout.plot(s(
				box,
