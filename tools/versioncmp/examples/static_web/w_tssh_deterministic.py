...
m = MissingValue(0)
m.apply()
waveheight_fieldset = FieldSet("swell_waveheight.grib")
wavedir_fieldset = FieldSet("swell_md.grib")
...
for area in arg_dict["area"]:
	...
	box = LegendBox(
		...
		legend_title_text = 'Significant wave height of total swell in meters',
	...
	for step in arg_dict["step"]:
		...
		wavedir = wavedir_index.constrained_access(wanted=1,step=step,param='mdts')
		print "plotting:",area," step:",step

		title = FieldAutoTitle(ice,text = [None,"Significant wave height of total swell and mean direction"])
		layout.plot(s(
				box,
