...
C     Open MAGICS and set the output device
      ...
      CALL POPEN
      CALL PARSE_COMMAND_LINE ('ukmo_netcdf_03')
...
C     + 'data/rlftoai_co2_A5.nc')
C      CALL PSETC ('NETCDF_TYPE',                    'GEOMATRIX') 
C      CALL PSETC ('NETCDF_LATITUDE_VARIABLE_NAME',  'lat')
C      CALL PSETC ('NETCDF_LONGITUDE_VARIABLE_NAME', 'lon')
C      CALL PSETC ('NETCDF_FIELD_VARIABLE_NAME',     'rlftoai_co2')
C      CALL PSETC ('NETCDF_DIMENSION_SETTING',       'time/1')
