...
C     for wind plotting. Data is in U/V.
...
C     Open MAGICS and set the output device
      ...
      CALL POPEN
      CALL PARSE_COMMAND_LINE ('wind_vector_array')
...
C     Set the field data
      ...
      DO 10 I = 1, NLAT
        ...
        DO 20 J = 1, NLON
          ...
          U(J,I) = (LON / ABS(LON_FIRST + (NLON * LON_STEP))) * 10
  20    CONTINUE
  10  CONTINUE
      ...
      DO 30 I = 1, NLAT
        ...
        DO 40 J = 1, NLON
          LON = LON_FIRST + (J * LON_STEP)
          V(J,I) = (LAT / ABS(LAT_FIRST + (NLAT * LAT_STEP))) * 10
  40    CONTINUE
  30  CONTINUE
...
C     Plot the coastlines and text
      ...
      CALL PSETC ('TEXT_LINE_1',  'Wind vector values from arrays')
      CALL PTEXT
      CALL PCOAST
