...
C     for wind plotting. Data is in speed/direction values.

      PARAMETER (NLON=240, NLAT=121)
      DIMENSION RSPEED (NLON,NLAT)
      DIMENSION RDIR   (NLON,NLAT)
...
C     Open MAGICS and set the output device
      ...
      CALL POPEN
      CALL PARSE_COMMAND_LINE ('wind_polar_array')
...
C     Set the field data
      ...
      DO 10 I = 1, NLAT
        ...
        DO 20 J = 1, NLON
          ...
          RSPEED(J,I) = (LON / ABS(LON_FIRST + (NLON * LON_STEP))) * 10
  20    CONTINUE
  10  CONTINUE
...
C     Pass the data to MAGICS
      ...
      CALL PSET2R ('INPUT_WIND_SPEED',     RSPEED, NLON, NLAT)
      CALL PSET2R ('INPUT_WIND_DIRECTION', RDIR,   NLON, NLAT)
...
C     Plot the coastlines and text
     ...
     x            'Wind speed/direction values from arrays')
      CALL PTEXT
      CALL PCOAST
