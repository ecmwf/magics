...
my @cat_menu    = magml_testsuite_submenus (); 
my @devices     = (); #magml_devices ();
my %proj_info   = projections_info ();
my $max_pages   = 15;
my $lib_version = 'MAGPLUSLIB_SHARED';
my $testsuite   = ($testprogs_type eq "test") ? 1 : 0;
my $page_title  = ($testsuite) ? "Magics++ MagML Test Suite" : "Magics++ MagML Examples";
...
sub magics_lib {
    ...
    # get the version of the MagML_Interpreter executable

    $strResult = $strResult . `which magmlx` . "\n";
    ...
    $magpluslib = substr ($magpluslib, 11, length($magpluslib) - 2);
...
#    local $hFORTRAN;
    ...
    print ("Base: $basename, FORTRAN: $pathtofortran/$basename.f\n");
...
sub location {
    ...
    $default = $default . "<a href=\"index.html\">MagML</a>";
    return $default;
}
...
sub content {
    ...
    my $htmldir    = ($testsuite) ? "html/test/magml" : "html/examples/magml";
    my $progs_dir  = "4_examples/progs";
    my $srcfile    = "$progs_dir/magml/$srcsubdir/$source.magml";
    my $notesfile  = "$progs_dir/magml/$srcsubdir/$source.txt";
    my $vlog_srcn  = "$progs_dir/logs/$source.vlog";
    my $time_ps    = "$htmldir/$source.ps.time";
    my $notes_table = "";
    my $proj_selection = "";
    my $dev_selection  = "";
    my $srccode  = magml_file_to_string ($srcfile);
    my $pic_mnew_thumb    = $source . "_small.gif";
    my $ps_mnew           = "$source.ps";
    my $pdf_mnew          = "$source.pdf";
    my $gif_mnew          = "$source.gif";
    ...
    my $gifsizenew     = filesize ("$htmldir/$gif_mnew");
    
    my $magml_projection = "";

    my $test_suite_selection = ($testsuite) ? test_suite_selection_html("MagML") : "";
   ...
   "<TR>
    ...
    <TR>
        ...
        $ps_extra_text = " ($filesizenew), 
   	                   <A HREF=\"$gif_mnew\">GIF</A> ($gifsizenew), 
   	                   <A HREF=\"$pdf_mnew\">PDF</A> ($pdfsizenew)<BR>[Time: $time_ps_text]<BR>('PS_PDF' is 'BOTH')";
    ...
    foreach my $device (@devices)
    {
        ...
        if ($device ne 'ps')  
        {
            my $lowdev   = lc($device);
            my $page     = 1;
            my $outfile  = "$source" . "$page.$lowdev";
#           my $outfile  = "$source" . "_0" . "$page.$lowdev";
            my $time_mnew  = "$htmldir/$source" . ".$device.time";
            my $filesize = ($testsuite) ? "(" . filesize ("$htmldir/$outfile") . ")" : "";
            my $uselink  = 0;
            my $name     = $device;
            ...
            for ($page = 2; $page < $max_pages; ++$page)
            {
                ...
                $outfile  = "$source" . "_0" . "$page.$lowdev";

                if (-e "$htmldir/$outfile")
                {
                    $filesize = filesize ("$htmldir/$outfile");
   ...
   "<P>
    ...
    <TR>
        <TD align=\"center\"><B>magml and Magics++ library directory (from \$$lib_version)</B></TD>
    </TR>
    <TR>
        <TD>
            $magplusdir
    ...
    <table>
    <tr>
        <td> <a href=\"$ps_mnew\"><IMG SRC=\"$pic_mnew_thumb\" alt=\"MagML Plot\"></a> </td>
        $table_FORTRAN_image_cell
    </tr>
    <tr>
        <td><I>Magics++ output - select a format</I></td>
        $table_FORTRAN_link_cell
    </tr>
    <tr>
        <td> <img src=\"ps_icon.gif\" alt=\"MagML PostScript\">   <a href=\"$ps_mnew\">Postscript</a>$ps_extra_text</td>
        $table_FORTRAN_ps_cell
    </tr>
    $table_log_rows    
    $dev_selection
...
foreach my $example (@menu)
{
    ...
    my $fordocs = $example->{fordocs};
       $cat     = $example->{cat};
    my $htmlsubdir = ($testsuite) ? "html/test/magml" : "html/examples/magml";

print ("$title\n");

    if (($lang eq "magml") && (($testsuite) || ($fordocs eq "yes")))
    {
print ("YES - $title\n");
        $html = "$htmlsubdir/$source.html";
        $/ = undef;
