...
WrepJSon::WrepJSon()
    : missing_(-9999), height_(-9999), mask_(9999), latitude_(0),
      station_longitude_(9999), longitude_(0), api_("v0") {
  ...
  metaMethods_["deterministic_resolution"] =
      &WrepJSon::deterministic_resolution;
...
void WrepJSon::visit(Transformation &transformation) {
  ...
  map<string, TransformationHandler>::iterator handler =
      transformationHandlers_.find(family_);
...
void WrepJSon::eps(Transformation &transformation) {
  ...
  for (vector<CustomisedPoint *>::iterator point = points_.begin();
...
void WrepJSon::eps() {
  ...
  map<string, vector<double>>::iterator intensity =
      values_.values_.find("intensity");
  map<string, vector<double>>::iterator direction =
      values_.values_.find("direction");
  ...
  for (unsigned int i = 0; i < values_.steps_.size(); i++) {
    ...
    double speed =
        (intensity == (values_.values_.end())) ? 0 : intensity->second[i];
    double dir =
        (direction == (values_.values_.end())) ? 0 : direction->second[i];
    ...
    for (map<string, vector<double>>::iterator val = values.begin();
         val != values.end(); ++val) {
...
CustomisedPoint *point_cape(const string &name, InputWrep &input,
                            double &maxy) {
  CustomisedPoint *point = new CustomisedPoint();
...
void WrepJSon::hodograph() {
  ...
  unsigned int ens2 = (hodograph_member_ < 0)
                          ? values_.ensembleValues_["u"].size()
                          : hodograph_member_ + 1;
  double max = 0;
...
void WrepJSon::tephigram() {
  ...
  methods_[param_] =
      profile_quantile_.empty() ? &WrepJSon::param : &WrepJSon::parameter;
  methods_["pres"] = &WrepJSon::levels;
  methods_[keyword_] = &WrepJSon::dig;
  ...
  if (profile_quantile_.empty()) {
    for (unsigned int i = 0; i < values_.levels_.size(); i++) {
      ...
      for (map<string, vector<double>>::iterator val = values.begin();
           val != values.end(); ++val) {
        if (val->first == "hres") {
          double value = (val->second)[i] == missing_
                             ? missing_
                             : correctDetz((val->second)[i]);
    ...
    map<string, vector<double>>::iterator val =
        values_.values_.find(profile_quantile_);
    ...
    map<string, vector<double>>::iterator min = values_.values_.find(
        profile_quantile_ == "lower" ? "min" : "twenty_five");
    map<string, vector<double>>::iterator max = values_.values_.find(
        profile_quantile_ == "lower" ? "max" : "seventy_five");
...
void WrepJSon::customisedPoints(const Transformation &transformation,
...
void WrepJSon::customisedPoints(const std::set<string> &,
                                CustomisedPointsList &out) {
...
void WrepJSon::basic() {
  ...
  try {
    ...
    MagLog::error() << "Could not processed the file: " << file_ << ": "
                    << e.what() << endl;
    // abort();
  }
}
...
void WrepJSon::station(const json_spirit::Value &value) {
  ...
  const json_spirit::Value lat =
      find_value(location, binding(api_, "latitude"));
  const json_spirit::Value lon =
      find_value(location, binding(api_, "longitude"));
...
void WrepJSon::valid_time(const json_spirit::Value &value) {
  ...
  ostringstream vt;
  vt << "from " << from.tostring("%A %e %B %Y %H UTC") << " to "
     << to.tostring("%A %e %B %Y %H UTC") << endl;
  valid_time_ = vt.str();
}
...
void WrepJSon::parameter(const json_spirit::Value &value) {
  ...
  for (vector<Pair>::const_iterator info = param.begin(); info != param.end();
       ++info) {
    ...
    if (info->name_ == "steps") {
      ...
      bool add = true;
      for (vector<string>::iterator i = ignore_keys_.begin();
           i != ignore_keys_.end(); ++i)
        if (*i == info->name_)
          add = false;
      if (!add) {
        continue;
...
MatrixHandler &WrepJSon::matrix() {
  ...
  for (IntervalMap<int>::iterator interval = array.begin();
    ...
    double row = (interval->first.max_ - interval->first.min_) / 2 +
                 interval->first.min_;
  ...
  for (vector<CustomisedPoint *>::const_iterator point = points_.begin();
    ...
    for (IntervalMap<int>::iterator interval = array.begin();
         interval != array.end(); ++interval) {
      matrix_[column + row * matrix_.columnsAxis().size()] =
...
void WrepJSon::cdf() {
  ...
  for (map<string, InputWrep>::iterator info = eps_.begin(); info != eps_.end();
       ++info) {
  ...
  for (map<string, vector<double>>::iterator val = clim_.values_.begin();
       val != clim_.values_.end(); ++val) {
    double value = correctEpsz(val->second[index]);
    if (minx_ > value)
      minx_ = value;
    if (maxx_ < value)
  ...
  for (vector<int>::iterator step = steps_.begin(); step != steps_.end();
       ++step) {
...
void WrepJSon::efi(const json_spirit::Value &value) {
  ...
  /*
  for (map<string,  InputWrep>::iterator info = efi_.begin(); info !=
  efi_.end(); ++info) {
...
void WrepJSon::eps(const json_spirit::Value &value) {
  ...
  /*
  for (map<string,  InputWrep>::iterator info = eps_.begin(); info !=
  eps_.end(); ++info) {
...
void WrepJSon::visit(MetaDataVisitor &visitor) {
  ...
  for (vector<Pair>::iterator entry = object.begin(); entry != object.end();
       ++entry) {
...
Value WrepJSon::temperature_adjustment() {
  ...
  if (correction_ && height_ != -9999) {
    ...
    correction.push_back(
        Pair("deterministic_adjustement", -(height_ - detz_) * 0.0065));
...
void WrepJSon::points_along_meridian(const json_spirit::Value &value) {
  ...
  MagLog::dev() << "found -> points_along_meridian " << points_along_meridian_
                << endl;
}
...
void WrepJSon::visit(TextVisitor &text) {
  ...
  if (param_info_ != "none") {
    ...
    if (step_) {
      ...
      text.update("json", "valid_date",
                  " Valid for " + to.tostring("%A %e %B %Y %H UTC"));
    }
  }
  ...
  if (position_info_) {
    if (param_info_ != "none") {
      ...
      text.update("json", "grid_point",
                  (mask_ < 0.5) ? " (ENS sea point) " : " (ENS land point) ");
      if (height_ != -9999)
  ...
  if ((correction_ && height_ != -9999 && param_info_ != "none")) {
    full_correction << " reduced to " << height_ << " m (station height) from "
                    << maground(detz_) << " m (HRES) and " << maground(epsz_)
                    << " m (ENS)";
    short_correction << " reduced to " << height_ << " m (station height) from "
                     << maground(epsz_) << " m (ENS)";
  }

  text.update("json", "full_temperature_correction_info",
              full_correction.str());
  text.update("json", "short_temperature_correction_info",
              short_correction.str());
  text.update("json", "parameter_info",
              (param_info_ == "none") ? "" : param_info_);
  ...
  text.update("json", "min_max_values",
              "Max = " + tostring(maground(maxClim_)) +
                  ", Min = " + tostring(maground(minClim_)));
}

void WrepJSon::points(const Transformation &transformation,
  ...
  for (vector<CustomisedPoint *>::const_iterator point = points_.begin();
    ...
    // doubble v =  (*point)->find("value") != (*point)->end() ) :
    // (**point)["value"] : 0;
