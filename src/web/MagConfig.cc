...
MagConfigHandler::MagConfigHandler(const string &config, MagConfig &magics) {
  ...
  try {
    ...
    MagLog::error() << "JSON error in file: " << config << ": " << e.reason_
                    << "[line: " << e.line_ << ", column: " << e.column_ << "]"
                    << endl;
  }
}
...
void StyleLibrary::callback(const string &name,
...
void StyleLibrary::init() {
  ...
  for (auto token = paths.begin(); token != paths.end(); ++token) {
    ...
    if (!dir) {
      ostringstream error;
      error << "Trying to open directory " << library << ": "
            << strerror(errno);
...
void PaletteLibrary::callback(const string &name,
                              const json_spirit::Value &value) {
  Palette palette;
  palette.name_ = name;
  json_spirit::Object object = value.get_value<json_spirit::Object>();
...
void UnitConvert::to(const json_spirit::Value &value) { to_ = value.get_str(); }
...
void UnitsLibrary::callback(const string &name,
                            const json_spirit::Value &value) {
...
int Style::score(const MetaDataCollector &data) {
  ...
  for (auto match = criteria_.begin(); match != criteria_.end(); ++match) {
    ...
    for (auto key = match->begin(); key != match->end(); ++key) {
      ...
      int tmpscore = 0;
      for (auto value = key->second.begin(); value != key->second.end();
           ++value) {
  ...
  if (bestscore) {
    if (styles_.empty()) {
      ...
      MagLog::debug() << "----   Found style with score : " << bestscore
                      << " Style --> " << styles_.front() << endl;
      for (auto match = criteria.begin(); match != criteria.end(); ++match) {
        MagLog::debug() << "    " << match->first << " == " << match->second
                        << endl;
      }
      MagLog::debug() << "----------------------------------------------------"
                      << endl;
...
void StyleLibrary::getCriteria(std::set<string> &keys) {
  for (vector<Style>::iterator style = library_.begin();
       style != library_.end(); ++style) {
    style->keywords(keys);
  }
}

bool StyleLibrary::findStyle(const MetaDataCollector &data, MagDef &visdef,
                             StyleEntry &info) {
  int score = 0;
  Style beststyle;
...
string StyleLibrary::getAttribute(const string &style, const string &param,
                                  const string &defval) {
  MagDef visdef;
...
void NetcdfGuess::callback(const string &name,
                           const json_spirit::Value &value) {
  ...
  if (value.type() == json_spirit::obj_type) {
    ...
    for (vector<json_spirit::Pair>::const_iterator entry = object.begin();
      ...
      json_spirit::Array values =
          (entry->value_).get_value<json_spirit::Array>();
      for (unsigned int i = 0; i < values.size(); i++) {
...
void DimensionGuess::init() {
  ...
  try {
    ...
    MagLog::error() << "JSON error in" << definitions_ << ": " << e.reason_
                    << "[line: " << e.line_ << ", column: " << e.column_ << "]"
                    << endl;
  }
}
...
void MagDefLibrary::callback(const string &name,
                             const json_spirit::Value &value) {
  MagDef def;
  def.name_ = name;
  json_spirit::Object object = value.get_value<json_spirit::Object>();
