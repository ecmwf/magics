...
int getOdbColumnIndex(odb::Reader::iterator &firstRowIt,
                      const string &colName) {
  int index = 0;
  for (odb::MetaData::const_iterator it = firstRowIt->columns().begin();
       it != firstRowIt->columns().end(); ++it) {
    string s = (*it)->name();
    if (s == colName)
      return index;
...
double degrees(double val) { return val; }

double radians(double val) { return val * 180 / 3.14; }
...
void OdaGeoDecoder::decode(const Transformation &transformation) {
  ...
  if (info("_datatype") == "ODB_geovectors") {
    ...
    for (const auto &point : points) {
      push_back(new UserPoint((*point)["lon"], (*point)["lat"],
                              (*point)["colour_component"]));
    }
  } else {
    try {
      odb_start();
      ...
      MagLog::info() << "Indices: " << latIndex << " " << lonIndex << " "
                     << valueIndex << endl;
...
void OdaGeoDecoder::visit(TextVisitor &title) {
  ...
  if (stats_.find("value") == stats_.end()) {
    ...
    if (!info("stats::min").empty() && !info("stats::max").empty()) {
      text << "Min: " << info("stats::min") << " Max: " << info("stats::max")
    ...
    if (info("stats::min").empty() && stats_["value"].size() > 0) {
      double min =
          *(std::min_element(stats_["value"].begin(), stats_["value"].end()));
      setInfo("stats::min", tostring(min));
    }
    if (info("stats::max").empty() && stats_["value"].size() > 0) {
      double max =
          *(std::max_element(stats_["value"].begin(), stats_["value"].end()));
      setInfo("stats::max", tostring(max));
    }
    title.addAutomaticTitle("Min: " + info("stats::min") +
                            "  Max: " + info("stats::max") + "    ( " +
                            info("stats::points") + " points) ");
  }
}

static double speed(double x, double y) { return sqrt(x * x + y * y); }

void OdaGeoDecoder::customisedPoints(const std::set<string> &,
                                     CustomisedPointsList &list) {
  if (!customisedPoints_.empty()) {
    for (CustomisedPointsList::iterator point = customisedPoints_.begin();
         point != customisedPoints_.end(); ++point) {
      CustomisedPoint *cp = new CustomisedPoint(
          (*point)->longitude(), (*point)->latitude(), (*point)->identifier());
      for (CustomisedPoint::iterator key = (*point)->begin();
           key != (*point)->end(); ++key)
  ...
  try {
    ...
    for (; it != oda.end(); ++it, row++) {
      ...
      double val = (valueIndex != -1) ? (*it)[valueIndex]
                                      : speed((*it)[xIndex], (*it)[yIndex]);
      ...
      CustomisedPoint *cp = new CustomisedPoint(
          point->longitude(), point->latitude(), point->identifier());
      for (CustomisedPoint::iterator key = point->begin(); key != point->end();
           ++key)
        cp->insert(make_pair(key->first, key->second));
...
void OdaGeoDecoder::position(double &lat, double &lon) {
  ...
  map<string, Converter>::iterator converter =
      converters_.find(lowerCase(unit_));

  if (converter == converters_.end()) {
    MagLog::warning() << "odb_coordinates_unit:" << unit_
                      << " is not valid -->Change back to default:degrees"
                      << endl;
...
void OdaGeoDecoder::customisedPoints(const Transformation &transformation,
  ...
  if (!customisedPoints_.empty()) {
    for (CustomisedPointsList::iterator point = customisedPoints_.begin();
      ...
      CustomisedPoint *cp = new CustomisedPoint(
          (*point)->longitude(), (*point)->latitude(), (*point)->identifier());
      for (CustomisedPoint::const_iterator key = (*point)->begin();
           key != (*point)->end(); ++key)
        cp->insert(make_pair(key->first, key->second));
...
void OdaGeoDecoder::visit(ValuesCollector &points) {
  ...
  for (ValuesCollector::iterator point = points.begin(); point != points.end();
       ++point) {
    vector<int> idxV;
    double lat = (*point).y();
    double lon = (*point).x();
    ...
    // Pre collection - just based on search radius
      ...
      if (info("_datatype") == "ODB_geopoints") {
        ...
        for (int i = 0; i < customisedPoints_.size(); i++) {
          if (fabs(customisedPoints_.at(i)->latitude() - lat) <
                  points.searchRadiusY() &&
              fabs(customisedPoints_.at(i)->longitude() - lon) <
                  points.searchRadiusX()) {
            idxV.push_back(i);
          }
        }
      }
    ...
    if (info("_datatype") == "ODB_geopoints") {
      ...
      if (!points.multiData()) {
        ...
        for (unsigned int i = 0; i < idxV.size(); i++) {
          int idx = idxV[i];
          double d =
              magics::geoDistanceInKm(at(idx)->y(), at(idx)->x(), lat, lon);
          if (d < dist) {
            minIdx = idx;
            dist = d;
          }
        ...
        if (minIdx >= 0)
          (*point).push_back(
              new ValuesCollectorData(at(minIdx)->x(), at(minIdx)->y(),
                                      at(minIdx)->value(), dist, minIdx));
      }
          ...
          for (unsigned int i = 0; i < idxV.size(); i++) {
            ...
            if (fabs(distV[i] - dist) < distEps) {
              (*point).push_back(new ValuesCollectorData(
                  at(idx)->x(), at(idx)->y(), at(idx)->value(), distV[i],
                  dataIndex_[idx]));
            }
          }
        }
      ...
      if (!points.multiData()) {
        ...
        for (unsigned int i = 0; i < idxV.size(); i++) {
          int idx = idxV[i];
          double d = magics::geoDistanceInKm(
              customisedPoints_.at(idx)->latitude(),
        ...
        if (minIdx >= 0) {
          map<string, double>::iterator uIt =
              customisedPoints_.at(minIdx)->find("x_component");
          map<string, double>::iterator vIt =
              customisedPoints_.at(minIdx)->find("y_component");
          if (uIt != customisedPoints_.at(minIdx)->end() &&
            ...
            (*point).push_back(new ValuesCollectorUVData(
                ...
                customisedPoints_.at(minIdx)->latitude(), uIt->second,
                vIt->second, dist, minIdx));
          }
        }
      }
      // collect multiple values
        ...
        if (dist < 1000) // 1000 km
        {
          for (unsigned int i = 0; i < idxV.size(); i++) {
            ...
            if (fabs(distV[i] - dist) < distEps) {
              map<string, double>::iterator uIt =
                  customisedPoints_[idx]->find("x_component");
              map<string, double>::iterator vIt =
                  customisedPoints_[idx]->find("y_component");
              if (uIt != customisedPoints_[idx]->end() &&
                  vIt != customisedPoints_[idx]->end()) {
                (*point).push_back(new ValuesCollectorUVData(
                    customisedPoints_[idx]->longitude(),
                    customisedPoints_[idx]->latitude(), uIt->second,
                    vIt->second, distV[i], dataIndex_[idx]));
              }
            }
          }
        }
...
void OdaXYDecoder::customisedPoints(const Transformation &transformation,
                                    const std::set<string> &,
                                    CustomisedPointsList &list) {
  try {
    odb_start();
    ...
    if (!x_component_.empty() &&
        (xcIndex = getOdbColumnIndex(it, x_component_)) == -1)
      throw exception();
    if (!y_component_.empty() &&
        (ycIndex = getOdbColumnIndex(it, y_component_)) == -1)
      throw exception();

    MagLog::info() << "Indices: " << xIndex << " " << yIndex << " "
                   << valueIndex << " " << xcIndex << " " << xcIndex << endl;
    ...
    for (; it != oda.end(); ++it) {
      ...
      if (transformation.in(x, y)) {
        ...
        if (xcIndex != -1 && ycIndex != -1) {
          ...
          double val = (valueIndex != -1)
                           ? (*it)[valueIndex]
                           : speed((*it)[xcIndex], (*it)[ycIndex]);
          (*point)["colour_component"] = val;
          stats_["value"].push_back(val);
...
void OdaXYDecoder::customisedPoints(const std::set<string> &,
                                    CustomisedPointsList &list) {
  try {
    odb_start();
    odb::Reader oda(path_);
