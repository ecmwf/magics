...
FortranMagics::FortranMagics()
    : drivers_(0), output_(0), action_(0), empty_(true), gribindex_(0),
      legend_todo_(false), symbolinput_todo_(false), matrixinput_todo_(false),
...
int FortranMagics::pclose() {
  ...
  try {
    ...
    MagLog::error() << "Errors reported:" << e.what() << " - No plot produced  "
                    << endl;
    MagLog::error().flush();
    return -1;
  }
  ...
  if (getEnvVariable("MAGPLUS_QUIET").empty() && !silent_) {
    ...
    */
    ...
    MagLog::userInfo() << "\n";
    MagLog::userInfo()
        << "    Any problems or suggestions? Please contact us at\n";
...
void FortranMagics::finish() {
  ...
  if (!legends_.empty() && axisContainer_ && !axisContainer_->items_empty()) {
    legend();
    for (vector<LegendVisitor *>::iterator legend = legends_.begin();
         legend != legends_.end(); ++legend) {
      top()->legend(*legend);
    }
    legends_.clear();
  }
  ...
  // Check any text
  for (vector<BasicSceneObject *>::iterator other = later_.begin();
       other != later_.end(); ++other) {
    top()->push_back(*other);
  }
  later_.clear();

  for (vector<FortranTextVisitor *>::iterator text = texts_.begin();
...
const char *FortranMagics::detect(const string &data, const string &dim) {
  ...
  for (auto check = checks->second.begin(); check != checks->second.end();
       ++check) {
...
template <class T> void param(const string &from, const string &to, T &val) {
...
#else
      MagLog::warning()
          << " Attempt to decode data from GRIB, but GRIB support is disabled!"
          << endl;
#endif
    }
    top()->push_back(action_);
...
#else
      MagLog::warning()
          << " Attempt to decode Wind from GRIB, but GRIB support is disabled!"
          << endl;
#endif
    }
    top()->push_back(action_);
...
void FortranMagics::prepare() { actions(); }
