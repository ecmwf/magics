...
void MultiVisdef::operator()(Data &data, BasicGraphicsObjectContainer &out) {
  ...
  for (vector<Visdef *>::iterator visdef = visdefs.begin();
       visdef != visdefs.end(); ++visdef)
    (**visdef)(data, out);
}

bool MultiVisdef::needLegend() {
  for (vector<Visdef *>::iterator visdef = one_d_.begin();
       visdef != one_d_.end(); ++visdef)
    if ((*visdef)->needLegend())
      return true;
  for (vector<Visdef *>::iterator visdef = two_d_.begin();
       visdef != two_d_.end(); ++visdef)
    if ((*visdef)->needLegend())
      return true;
  return false;
}
...
void MultiVisdef::visit(Transformation &, Data &) { ASSERT(false); }
