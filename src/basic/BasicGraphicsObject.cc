...
bool BasicGraphicsObjectContainer::buildTree(const Layout &parent,
                                             unsigned int frame,
...
void BasicGraphicsObjectContainer::visit(const BaseDriver &driver) const {
  ...
  for (vector<BasicGraphicsObject *>::const_iterator object = last_.begin();
       object != last_.end(); ++object)
    (*object)->redisplay(driver);
}
