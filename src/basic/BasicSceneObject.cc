...
double BasicPositionalObject::adjustDimension(double x, double def,
                                              double parent) {
  // MagLog::dev() << *this << endl;
  // MagLog::dev() << *parent_ << endl;
  if (x == -1) {
    MagLog::dev() << "adjustDimension[" << x << ", " << def << ", " << parent
                  << "]=" << def << "%" << endl;
    return def;
  }
  MagLog::dev() << "adjustDimension[" << x << ", " << def << ", " << parent
...
void EmptySceneObject::visit(SceneLayer &layer,
...
void FrameBackgroundObject::visit(SceneLayer &layer,
                                  vector<LayoutVisitor *> &visitors) {
...
void FrameForegroundObject::visit(SceneLayer &layer,
                                  vector<LayoutVisitor *> &visitors) {
  NoDataLayer *frame = new NoDataLayer(this);
  layer.add(frame);
  for (vector<LayoutVisitor *>::iterator visitor = visitors.begin();
