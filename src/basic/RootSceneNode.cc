...
void RootSceneNode::setPage(RootScenePage *node) {
  ...
  // current_ is a copy of the basic SceneNode with exactly the same dimensions
  // as the Root!
...
RootSceneNode::RootSceneNode()
    : absoluteWidth_(29.7), absoluteHeight_(20.), current_(0), scale_(1),
      newpage_(true), endpage_(false) {
  name_ = "root";
}

RootSceneNode::~RootSceneNode() { delete layout_; }
...
void XmlRootSceneNode::getReady() {
  ...
  if (absoluteWidth_ == -1 || absoluteWidth_ == -1) {
    PaperDimension *dimension =
        MagTranslator<string, PaperDimension>()(format_);
...
void RootSceneNode::release() { return latest_->release(); }
...
RootScenePage::RootScenePage(double width, double height)
    : root_(0), width_(width), height_(height) {
  static int i = 0;
  ostringstream n;
  n << "rootpage" << i;
  name_ = n.str();
