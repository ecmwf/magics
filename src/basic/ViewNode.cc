...
ViewNode::~ViewNode() {
  for (vector<LayoutVisitor*>::iterator component = components_.begin();
...
void ViewNode::text(TextVisitor* text) { texts_.push_back(text); }
...
void ViewNode::visit(MetaDataVisitor& metadata) {
  ...
  viewTransformation_->visit(metadata, left, top, imgwidth, imgheight, width,
                             height);

  BasicSceneObject::visit(metadata);
  for (vector<TextVisitor*>::iterator text = texts_.begin();
       text != texts_.end(); ++text)
    (*text)->visit(metadata);
  if (legend_) legend_->visit(metadata);
}
...
void ViewNode::visit(HistoVisitor& histo) { dispatch(histo); }
...
void ViewNode::visit(SceneLayer& tree) {
  ...
  BasicSceneObject::visit(
      *viewTransformation_);  // to set up the automatic style for mapping!
  ...
  if (MagicsGlobal::compatibility()) needLegend_ = false;
  ...
  push_front(
      new FrameBackgroundObject(blank, Colour(drawing_background_colour_)));

  // Here we have the steps!
  prepareLayout(tree);

  push_back(new FrameForegroundObject(frameIt_, frameColour_, frameStyle_,
                                      frameThickness_));
  ...
  if (needLegend_) {
    for (auto& item : items_) item->getReady(*legend_);
  }

  for (auto& item : items_) item->visit(tree, components_);
...
void XmlViewNode::updateLayout() {
  drawing_bottom_ =
      ((drawing_bottom_ * layout_->height()) / 100) + layout_->y();
...
void XmlViewNode::getReady() {
  ...
  Dimension width(XmlBasicNodeAttributes::width_, parent_->absoluteWidth(),
                  100);
  Dimension height(XmlBasicNodeAttributes::height_, parent_->absoluteHeight(),
                   100);
  ...
  double waa = wab;
  double haa = hab;
  MagLog::dev() << "after aspect ratio -->[" << waa << ", " << haa << "]"
                << endl;
  ...
  layout_->frame(true, border_, *border_colour_, border_style_,
                 border_thickness_, *background_);
...
void FortranViewNode::getReady() {
  ...
  if (predefined_) {
    ...
    viewTransformation_ = MagTranslator<string, Transformation>()(
        area.find("subpage_map_projection")->second);
  ...
  MagLog::dev() << "after aspect ratio -->[" << abswidth << ", " << absheight
                << "]" << endl;
  MagLog::dev() << "[" << abswidth << ", " << absheight << "]" << endl;
  MagLog::dev() << "[" << absoluteWidth() << ", " << absoluteWidth() << "]"
                << endl;
  ...
  layout_->frame(true, frame_, *frame_colour_, frame_line_style_,
                 frame_thickness_, *background_);
...
static SimpleObjectMaker<NoOverlayAnimationRules, AnimationRules> never(
    "never");
static SimpleObjectMaker<LevelAnimationRules, AnimationRules> by_level(
    "by_level");
