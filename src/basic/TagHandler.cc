...
void TagHandler::print(ostream &out) const {
  out << "TagHandler[";
  for (map<string, DefList>::const_iterator fam = definitions_.begin();
       fam != definitions_.end(); ++fam) {
    ...
    for (DefList::const_iterator def = defs.begin(); def != defs.end(); ++def) {
      string sep;
      string value;
      for (std::set<string>::const_iterator v = def->second.begin();
           v != def->second.end(); ++v) {
        if ((*v).empty())
          continue;
        value += sep + *v;
...
void TagHandler::update(const string &family, const string &definition,
                        const string &value) {
...
void TagConverter::visit(const XmlNode &node) {
  ...
  if (function != map_.end())
    ...
    MagLog::error() << "Html Text handler: unknown tag [" << node.name() << "]"
                    << endl;
    MagLog::broadcast();
  }
...
void TagConverter::grib(const XmlNode &node) {
  ...
  string key = (attributes.find("id") != attributes.end())
                   ? attributes.find("id")->second
                   : "";
...
void TagConverter::netcdf(const XmlNode &node) {
  push();
  const map<string, string> &attributes = node.attributes();
  string key = (attributes.find("variable") != attributes.end())
                   ? attributes.find("variable")->second
                   : "";
  if (attributes.find("attribute") != attributes.end()) {
    string result =
        owner_.get("netcdf" + key, attributes.find("attribute")->second);
    if (result.empty()) {
      if (automatic_ == 1)
        automatic_ = 0;
    } else {
...
void TagConverter::magics_param(const XmlNode &node) {
  ...
  const map<string, string> &attributes = node.attributes();
  string key = (attributes.find("param") != attributes.end())
                   ? attributes.find("param")->second
                   : "param not found";
...
void TagConverter::font(const XmlNode &node) {
  ...
  if (attributes.find("colour") != attributes.end()) {
    MagLog::debug() << "set colour" << attributes.find("colour")->second
                    << endl;
...
void TagConverter::entities(ostream &out) {
  ...
  try {
    ...
    MagLog::warning() << "Cannot find entities declaration : " << path
                      << " [Ignore them]" << endl;
  }
}
