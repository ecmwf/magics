...
void setAttribute(const vector<string>& roots, const string& name, shared_ptr<Colour>&, const map<string, string>&);

template <class T>
void setMember(const string& value, shared_ptr<T>& object, const XmlNode& from)
{
	try{
        ...
        object = shared_ptr<T>(new_object);
...
template <class T>
bool acceptNode(const string& node, shared_ptr<T>& object)
{
	try {
...
template <class T>
void setMember(const vector<string>& roots, const string& name, shared_ptr<T>& object, const map<string, string>& params)
{	
	vector<string> keys;
