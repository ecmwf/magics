...
Layer::Layer()
    : visibility_(true), zindex_(1), transparency_(0), state_(new_layer),
      id_(""), parent_(0), object_(0)

{}

Layer::Layer(BasicSceneObject *object)
    : visibility_(true), zindex_(1), transparency_(0), state_(new_layer),
      id_(""), parent_(0), object_(object) {}
...
void Layer::collect(ValuesCollector &values) { values.clear(); }

void Layer::newLayer(const BaseDriver &driver) { driver.newLayer(*this); }

void Layer::closeLayer(const BaseDriver &driver) { driver.closeLayer(*this); }
...
void Layer::print(ostream &out) const {
  out << "layer[" << name_;
  string sep = "";
  for (vector<MetviewIcon>::const_iterator icon = icons_.begin();
       icon != icons_.end(); ++icon) {
    out << ", [" << (*icon).iconName() << ", " << (*icon).iconClass() << ", "
        << (*icon).iconId() << "]";
    sep = "]";
  }
  out << sep << "]";
...
string Layer::timeStamp(const string &fmt) const { return from_.tostring(fmt); }

string Layer::timeBegin(const string &fmt) const { return from_.tostring(fmt); }

string Layer::timeEnd(const string &fmt) const { return to_.tostring(fmt); }

static string iso_time("%Y-%m-%dT%H:%M:00Z");

string Layer::timeBegin() const { return timeBegin(iso_time); }

string Layer::timeEnd() const { return timeEnd(iso_time); }

string Layer::timeStamp() const { return timeStamp(iso_time); }
...
void Layer::collectText(vector<TextVisitor *> &texts, LegendVisitor *legend) {
  ...
  for (vector<TextVisitor *>::iterator text = texts.begin();
       text != texts.end(); ++text) {
    (*text)->visit(*object_);
...
void SingleLayer::collect(ValuesCollector &values) { object_->visit(values); }
...
void SingleLayer::getReady() const {
  ...
  if (!objects_) {
    ...
    for (vector<LayoutVisitor *>::iterator visitor =
             parentLayer_->firstVisitor();
         visitor != parentLayer_->endVisitor(); ++visitor) {
      set(*visitor);
...
Layer *SingleLayer::baseLayer() { return parentLayer_; }

void SingleLayer::histogram(const BaseDriver &driver, const string &visdefName,
                            const string &visdefClass) {
...
void StepLayer::execute(int i, const BaseDriver &driver,
...
void StepLayer::getReady(int i) const { steps_[i]->getReady(); }

void StepLayer::newLayer(const BaseDriver &driver) { driver.newLayer(*this); }

void StepLayer::closeLayer(const BaseDriver &driver) {
  driver.closeLayer(*this);
}

int StepLayer::size() { return 1; }

Layer *StepLayer::get(int i) { return steps_[i]; }
...
void StaticLayer::execute(const BaseDriver &driver) const { redisplay(driver); }
...
void StaticLayer::newLayer(const BaseDriver &driver) { driver.newLayer(*this); }
...
void StaticLayer::clean() { layer_->clear(); }
...
void StaticLayer::histogram(const BaseDriver &driver, const string &visdefName,
                            const string &visdefClass) {
  HistoVisitor histogram;
  MetviewIcon icon(visdefName, visdefClass);
  histogram.dataVisdefIcon(icon);
...
void TextLayer::collectText(vector<TextVisitor *> &texts,
...
SceneLayer::SceneLayer()
    : rules_(0), currentIndex_(0), legend_(0), currentFrame_(0) {
  layout_ = new Layout();
}

SceneLayer::~SceneLayer() {
  for (vector<Layer *>::iterator layer = layers_.begin();
...
bool SceneLayer::buildTree(const Layout &parent, unsigned int frame,
                           const BaseDriver &out) const {
    ...
    if (nb == 0) {
      ...
      for (unsigned int n = 0; n < nb; n++) {
        ...
        if (i < 0 || i >= numberOfSteps()) {
          MagLog::warning()
              << " Export : could not find page " << i + 1 << endl;
          continue;
        }
...
vector<Layer *> &SceneLayer::prepare(int i) const {
  ...
  if (i >= (int)(*rules_).size()) {
    for (vector<Layer *>::const_iterator layer = layers_.begin();
         layer != layers_.end(); ++layer) {
      if ((*layer)->get())
...
vector<Layer *>::iterator SceneLayer::endLayer() const { return layers_.end(); }
...
void SceneLayer::finishText(Layout &layout) {
  for (vector<TextVisitor *>::iterator text = textVisitors_.begin();
       text != textVisitors_.end(); ++text)
    (*text)->finish(layout);
}
...
void SceneLayer::executeInfo(int i, const BaseDriver &) const {
  ...
  for (vector<TextVisitor *>::iterator text = textVisitors_.begin();
    ...
    vector<string> lines;
    (*text)->titles(lines);
    for (vector<string>::reverse_iterator line = lines.rbegin();
         line != lines.rend(); ++line) {
      // for ecah layer!
...
void MetviewIcon::visit(MetaDataCollector &collector) {
  ...
  if (collector.empty()) {
    ...
    for (map<string, string>::iterator key = collector.begin();
         key != collector.end(); ++key) {
...
operator<(const LevelDescription &other) const // DEclaration function in Data.h
{
  if (this->surface_) {
    if (other.surface_) {
...
bool DateDescription::
operator<(const DateDescription &other) const // DEclaration function in Data.h
{
