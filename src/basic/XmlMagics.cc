...
XmlMagics::XmlMagics()
    : root_(0), gribloop_(0), geographical_(true), driversToSet_(true) {
...
void XmlMagics::execute(XmlTree &tree) {
  ...
  /* later
      ...
      if ( filename != meta.end() && !filename->second.empty()) {
          ...
          for (std::map<string, string>::const_iterator d = meta.begin(); d !=
     meta.end(); ++d) { if ( d != filename ) file << d->first << " = " <<
     d->second << "; " << endl;
          }
      }
  */
...
void XmlMagics::magics(const XmlNode &node) {
  ...
  MagLog::debug() << " You are using the " << fixed << setprecision(1)
                  << version_ << " version of the magml interpreter\n";
  string version = node.getAttribute("version");
  if (version.empty()) {
    MagLog::error()
        << " No version defined in your magml file\n"
        << " Compatibilty issue: check your magml file and add a version number"
        << endl;
    return;
  }
  float v;
  ...
  sv >> v;
  if (v < version_) {
    MagLog::error() << " The version defined in the file is " << fixed
                    << setprecision(1) << v
                    << "\n Compatibilty issue: check your magml file and "
...
void XmlMagics::cartesian(const XmlNode &) { geographical_ = false; }

void XmlMagics::geographical(const XmlNode &) { geographical_ = true; }
...
void XmlMagics::gribloop(const XmlNode &) {
  MagLog::warning() << " Attempt to call GribLoop but GRIB support is disabled!"
                    << endl;
}

void XmlMagics::gribinloop(const XmlNode &) {
  MagLog::warning()
      << " Attempt to call GribInLoop but GRIB support is disabled!" << endl;
}

void XmlMagics::grib(const XmlNode &) {
  MagLog::warning()
      << " Attempt to call Grib decoding but GRIB support is disabled!" << endl;
}
#endif
...
void XmlMagics::image(const XmlNode &node) {
  MagLog::warning() << " Image has been deprecated! Please use contour."
                    << endl;
}
...
void XmlMagics::wind(const XmlNode &node) {
  if (geographical_) {
    ...
    MagLog::warning() << " wind not yet implemented for cartesian system"
                      << endl;
  }
#ifdef HAVE_GRIB
  if (gribloop_)
