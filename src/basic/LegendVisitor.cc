...
void LineEntry::rowBox(const PaperPoint& point,
...
void RainbowEntry::rowBox(const PaperPoint& point,
                          BasicGraphicsObjectContainer& out) {
  double x = point.x();
  double y = point.y();
...
void RainbowEntry::columnBox(const PaperPoint& point,
                             BasicGraphicsObjectContainer& out) {
  double x = point.x();
  double y = point.y();
...
void LineEntry::columnBox(const PaperPoint& point,
                          BasicGraphicsObjectContainer& out) {
  double x = point.x();
  double y = point.y();
...
void DoubleLineEntry::rowBox(const PaperPoint& point,
                             BasicGraphicsObjectContainer& out) {
  set(point, out);
}
void DoubleLineEntry::columnBox(const PaperPoint& point,
                                BasicGraphicsObjectContainer& out) {
  set(point, out);
}
...
void LegendVisitor::build() {
  ...
  if (title_) {
    ...
    font.size(font_size);
    Colour colour =
        title_font_colour_->automatic() ? *colour_ : *title_font_colour_;
    font.colour(colour);
  ...
  for (auto& entry : *this) {
    ...
    entry->borderColour(entry_border_ ? *entry_border_colour_
                                      : Colour("automatic"));

    entry->set(*this);
    string text;
    if (entry->needText()) {
      std::map<string, Composition>::iterator composition =
          compositions_.find(lowerCase(composition_));
      string user;
    ...
    (magCompare(direction_, "column"))
        ? (*method_).column(*entry, position->x(), position->y(), *legend,
                            *legend_)
        : (*method_).row(*entry, position->x(), position->y(), *legend,
                         *legend_);
...
void LegendVisitor::horizontal() {
  ...
  if (title_) {
    if (title_position_ == M_AUTOMATIC) title_position_ = M_TOP;
  ...
  if (magCompare(entry_orientation_, "right_left")) {
    ...
    for (auto& entry : *this) std::swap(entry->from_, entry->to_);
    back()->last_ = true;
    front()->last_ = false;
    back()->first_ = false;
    front()->first_ = true;
...
void LegendVisitor::vertical() {
  ...
  if (entriesNumber_ % columns_) rows++;
  ...
  if (title_) {
    if (title_position_ == M_AUTOMATIC) title_position_ = M_LEFT;
    (this->*titleBuilders_[title_position_])();
  }
...
void LegendVisitor::rightTitle() {
  ...
  titlePosition_ =
      PaperPoint(maxx + (newmaxx - maxx) * factor, (maxy + 3 * miny) / 4);
...
void LegendVisitor::grid() {
  ...
  for (auto& entry : *this) {
    entry->set(*this);
    string text =
        (label != lines_.end() && !label->empty()) ? *label : entry->label();
  ...
  std::map<string, Builder>::iterator builder =
      builders_.find(lowerCase(direction_));
...
void SymbolEntry::set(const PaperPoint& point,
                      BasicGraphicsObjectContainer& legend) {
  ...
  Colour colour =
      (borderColour_.automatic()) ? symbol_->getColour() : borderColour_;
...
void SymbolEntry::rowBox(const PaperPoint& point,
                         BasicGraphicsObjectContainer& legend) {
  double width = 1;
  double height = 0.5;
...
void SymbolEntry::columnBox(const PaperPoint& point,
                            BasicGraphicsObjectContainer& legend) {
  double width = computeWidth(0.8) / 2;
  double height = 0.2;
  PaperPoint p = centreSymbolBox(point);
...
Colour SymbolEntry::colour() { return symbol_->getColour(); }

void ArrowEntry::set(const PaperPoint& point,
                     BasicGraphicsObjectContainer& legend) {
...
void ArrowEntry::rowBox(const PaperPoint& point,
                        BasicGraphicsObjectContainer& legend) {
  set(point, legend);
  Text* text = new Text();
...
void ArrowEntry::columnBox(const PaperPoint& point,
                           BasicGraphicsObjectContainer& legend) {
  PaperPoint pos(centreSymbolBox(point));
  double width = computeWidth(0.8) / 2;
  pos.x_ -= width;
...
void FlagEntry::set(const PaperPoint& point,
                    BasicGraphicsObjectContainer& legend) {
  PaperPoint pos = centreSymbolBox(point);
  double width = computeWidth(0.8) / 2;
  pos.x_ -= width;
...
void FlagEntry::rowBox(const PaperPoint& point,
                       BasicGraphicsObjectContainer& legend) {
  set(point, legend);
  Text* text = new Text();
  text->addText(label_, font_);
...
void FlagEntry::columnBox(const PaperPoint& point,
                          BasicGraphicsObjectContainer& legend) {
  set(point, legend);
  PaperPoint pos(centreSymbolBox(point));
  double width = computeWidth(0.8) / 2;
...
Colour BoxEntry::colour() { return box_->getFillColour(); }

void LegendEntry::rowBox(const PaperPoint& point,
                         BasicGraphicsObjectContainer& legend) {
...
void LegendEntry::columnBox(const PaperPoint& point,
                            BasicGraphicsObjectContainer& legend) {
  Polyline* box = new Polyline();
  Colour col = colour();
  if (col == "none") {
...
void EmptyEntry::columnBox(const PaperPoint& point,
                           BasicGraphicsObjectContainer& legend) {}

void BoxEntry::set(const PaperPoint& point,
                   BasicGraphicsObjectContainer& legend) {
  MagLog::debug() << "BoxEntry--->set at " << point << endl;
  double width = computeWidth(0.8) / 2;
  double height = 0.4;
  ...
  LegendVisitor::addLegendInfo("legend_entry_colour",
                               box_->getFillColour().name());
...
void BoxEntry::rowBox(const PaperPoint& point,
                      BasicGraphicsObjectContainer& legend) {
  double width = 1;
  double height = 0.4;
  double x = point.x();
  ...
  // Small check
  Colour colour =
      (borderColour_.automatic()) ? box_->getFillColour() : borderColour_;
...
void BoxEntry::rowHisto(const PaperPoint& point,
                        BasicGraphicsObjectContainer& legend,
                        const Colour& colour) {
  ...
  for (vector<Polyline*>::iterator tick = ticks.begin(); tick != ticks.end();
       ++tick)
  ...
  if (histogram_->mean() && meanSet_) {
    ...
    double pos =
        ((2 * width) / (to_ - from_)) * (meanValue_ - from_) + (x - width);
    symbol->push_back(PaperPoint(pos, y + 1.));
    legend.push_last(symbol);
  }
}
void BoxEntry::columnBox(const PaperPoint& point,
                         BasicGraphicsObjectContainer& legend) {
  double width = computeWidth(0.8) / 2;
  double height = 0.5;
  PaperPoint p = centreSymbolBox(point);
...
void BoxEntry::columnHisto(const PaperPoint& point,
                           BasicGraphicsObjectContainer& legend,
                           const Colour& colour) {
  MagLog::debug() << "BoxEntry--->set at " << point << endl;
  double width = computeWidth(0.8) / 2;
...
Colour LineEntry::colour() { return line_->getColour(); }

Colour DoubleLineEntry::colour() { return line1_->getColour(); }

void LineEntry::set(const PaperPoint& point,
                    BasicGraphicsObjectContainer& legend) {
  double width = computeWidth(0.8) / 2;
  PaperPoint p = centreSymbolBox(point);
  double x = p.x();
...
void CdfEntry::set(const PaperPoint& point,
                   BasicGraphicsObjectContainer& legend) {
...
void DoubleLineEntry::set(const PaperPoint& point,
                          BasicGraphicsObjectContainer& legend) {
  double width = computeWidth(0.8) / 2;
...
void LegendVisitor::getReady() {
  ...
  if (lines_.empty()) check(text_, lines_);
...
void XmlLegendVisitor::getReady() {
  ...
  Dimension width(XmlBasicNodeAttributes::width_,
                  BasicSceneObject::parent_->absoluteWidth(), 100);
  Dimension height(XmlBasicNodeAttributes::height_,
                   BasicSceneObject::parent_->absoluteHeight(), 100);
  ...
  double text_x =
      left.percent() + xl * 100 / BasicSceneObject::parent_->absoluteWidth();
  double text_y =
      bottom.percent() + yb * 100 / BasicSceneObject::parent_->absoluteHeight();
  ;
  ...
  layout_->frame(XmlBasicNodeAttributes::blanking_,
                 XmlBasicNodeAttributes::border_,
...
void FortranPositionalLegendVisitor::getReady() {
  ...
  if (box_height_ != -1)
    layout_->height(box_height_ / BasicPositionalObject::absoluteHeight() *
                    100);

  // adjust the font size!...
  Dimension text(font_dimension_, box_height_, 10);
  font_size_ = text.absolute();
  layout_->Layout::frame(blanking_, border_, *border_colour_,
                         border_line_style_, border_thickness_,
                         Colour("white"));
}
...
void SimpleSymbolEntry::set(const PaperPoint& point,
                            BasicGraphicsObjectContainer& legend) {
...
const string& LegendEntry::label() const {
  if (!label_.empty() || !fromto_) return label_;
  if (userMin_) {
    label_ = minText_;
    return label_;
  }
...
void SimpleSymbolEntry::rowBox(const PaperPoint& point,
                               BasicGraphicsObjectContainer& legend) {
  Polyline* box = new Polyline();
  FillShadingProperties* shading = new FillShadingProperties();
  box->setFillColour(colour());
  ...
  // Small check
  Colour colour =
      (borderColour_.automatic()) ? box->getFillColour() : borderColour_;
...
void SimpleSymbolEntry::columnBox(const PaperPoint& point,
                                  BasicGraphicsObjectContainer& legend) {
  Polyline* box = new Polyline();
  FillShadingProperties* shading = new FillShadingProperties();
  box->setFillColour(colour());
