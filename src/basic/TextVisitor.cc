...
TextVisitor::TextVisitor() : positional_(true) {
  ...
  if (tags_.empty()) {
    ...
    tags_["valid_date"] =
        "grib_info  key='end-date' format='%A %d %B %Y at %H UTC'";
  }
}
...
void TextVisitor::visit() {
  ...
  for (vector<string>::iterator line = lines_.begin(); line != lines_.end();
       ++line) {
...
void TextVisitor::visit(BasicSceneObject &parent) {
  ...
  while (line != lines_.end()) {
    ...
    if (*line != "<magics_title/>") {
      ...
      if (key != end()) {
        for (vector<TextEntry *>::iterator entry = key->second.begin();
             entry != key->second.end(); ++entry)
  ...
  for (vector<string>::reverse_iterator line = lines_.rbegin(); line != lend;
       ++line) {
    ...
    map<string, vector<TextEntry *>>::iterator key = find(*line);
    if (key != end()) {
      for (vector<TextEntry *>::reverse_iterator entry = key->second.rbegin();
           entry != key->second.rend(); ++entry) {
        Text *text = new Text();
...
void TextVisitor::start() { texts_.clear(); }
void TextVisitor::finish(BasicGraphicsObjectContainer &parent) {
  ...
  double last = orientation_ == "horizontal" ? (0.10 / absheight) * 100
                                             : (0.10 / abswidth) * 100; // in %
  double ratio = OutputHandler::patchLineSpacing();
  double gap = height * ratio;

  double angle = 0;

  for (vector<Text *>::iterator text = texts_.begin(); text != texts_.end();
       ++text) {
    ...
    if (orientation_ == "horizontal") {
      angle = 0;
      gap = (*text)->noText()
                ? (font_size_ / absheight) * 100 * ratio
                : (((*text)->getFontMax() / absheight) * 100) * ratio;
      (*text)->push_back(
          PaperPoint(x, last)); // approximate position to be improved
    }
    if (magCompare(orientation_, "bottom_top")) {
      ...
      gap = (*text)->noText()
                ? (font_size_ / abswidth) * 100 * ratio
                : (((*text)->getFontMax() / abswidth) * 100) * ratio;
      (*text)->push_back(
          PaperPoint(last, x)); // approximate position to be improved
    }
...
void TextVisitor::update(vector<Text *> &texts) {
  for (vector<Text *>::iterator text = texts.begin(); text != texts.end();
       ++text) {
...
void XmlTextVisitor::getReady() {
  ...
  layout_->frame(
      TextVisitorAttributes::blanking_, TextVisitorAttributes::border_,
...
void FortranTextVisitor::decode() {
  if (lines_.empty()) {
    ...
    if (first_ > 10) {
      MagLog::warning() << "Invalid value for text_first_line[" << first_
                        << "] : reset to 1 "
                        << "\n";
      first_ = 1;
    }
    ...
    if (count_ > 10) {
      MagLog::warning() << "Invalid value for text_count_line[" << first_
                        << "]  reset to  " << count_ << "\n";
      count_ = 10 - first_ + 1;
    }
    ...
    if (last > 10) {
      count_ = 10 - first_ + 1;
      MagLog::warning() << "Invalid value (" << last << ") for text_first_line["
                        << first_ << "] and  text_count_line[" << count_
                        << "] reset\n";
      last = 10;
    }
...
void FortranTextVisitor::interpret(string &line, stringarray &lines) {
  ...
  for (vector<KeyInfo>::iterator ikey = keys.begin(); ikey != keys.end();
       ++ikey) {
  ...
  if (html_) {
    for (map<string, string>::iterator tag = tags_.begin(); tag != tags_.end();
         ++tag) {
...
void FortranAutomaticTextVisitor::getReady() {
  ...
  font_size_ = text.absolute();
  layout_->frame(blanking_, border_, *border_colour_, border_line_style_,
                 border_thickness_, Colour("white"));
}
...
void FortranPositionalTextVisitor::getReady() {
  ...
  layout_->x(adjustDimension(TextVisitorAttributes::x_, 7.5,
                             BasicPositionalObject::absoluteWidth()));
  layout_->y(adjustDimension(TextVisitorAttributes::y_, 5.,
                             BasicPositionalObject::absoluteHeight()));
  layout_->width(adjustDimension(TextVisitorAttributes::width_, 50.,
                                 BasicPositionalObject::absoluteWidth()));
  layout_->height(adjustDimension(TextVisitorAttributes::height_, 20.,
                                  BasicPositionalObject::absoluteHeight()));
  Dimension text(font_dimension_, absoluteHeight(), 10);
  font_size_ = text.absolute();
  layout_->frame(TextVisitorAttributes::blanking_,
                 TextVisitorAttributes::border_, *border_colour_,
...
void TextVisitor::titles(vector<string> &titles) {
  for (vector<string>::const_iterator line = lines_.begin();
...
void TextVisitor::add(const string &line, TextEntry *entry) {
  map<string, vector<TextEntry *>>::iterator key = find(line);
  if (key == end()) {
    map<string, vector<TextEntry *>>::insert(
        make_pair(line, vector<TextEntry *>()));
    key = find(line);
  }
