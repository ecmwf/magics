...
VisualAction::~VisualAction() {
  if (data_)
    delete data_;
  for (vector<Visdef *>::iterator visdef = visdefs_.begin();
       visdef != visdefs_.end(); ++visdef) {
    if (*visdef)
      delete *visdef;
    *visdef = 0;
  }
...
void VisualAction::print(ostream &out) const { out << "ActionNode"; }

void VisualAction::getReady() { MagLog::dev() << "ActionNode::getReady()\n"; }
...
void VisualAction::visit(DrawingVisitor &drawing) {
  ...
  while (data) {
    ...
    for (vector<Visdef *>::iterator visdef = visdefs_.begin();
      ...
      (**visdef).theme(theme());
      (**visdef)(*data, drawing.layout()); // Now the visualObject ahs the
                                           // responsability to reproject!
    }
    data = data_->next();
  }
}
void VisualAction::visit(FrameVisitor &frame) { frame.blankIt(); }
...
void VisualAction::visit(TopAxisVisitor &top) {
  for (vector<Visdef *>::iterator visdef = this->visdefs_.begin();
       visdef != this->visdefs_.end(); ++visdef) {
    (*visdef)->visit(top);
  }
}
...
void VisualAnimation::visit(SceneLayer &layer,
                            vector<LayoutVisitor *> &visitors) {
  layer.add(layer_);
