...
GeoJsonDriver::GeoJsonDriver()
    : currentTimeBegin_(""), currentTimeEnd_(""), GeoJson_placemark_(false),
...
void GeoJsonDriver::open() {
  ...
  if (!pFile_) {
    MagLog::error()
        << " GeoJsonDriver --> Cannot write output file to what was specified: "
        << fileName_ << endl;
    MagLog::error() << "";
  ...
  /*	pFile_	<< "<?xml version=\"1.0\" encoding=\"UTF-8\"
     standalone=\"no\"?>\n"
          ...
          << " <open>1</open>\n";
      pFile_  << "
     <atom:generator>"<<getMagicsVersionString()<<"</atom:generator>\n";
      if(!author_.empty()) pFile_ << "
     <atom:author><atom:name>"<<author_<<"</atom:name></atom:author>\n";
...
/*!
  ...
  GeoJson_output_resource_list_ ) are put into one zip file. We use minizip (see
  minizip subdirectory to do so) since gnuzip is NOT sufficient!
*/
void GeoJsonDriver::close() {
  ...
  if (zip_ && !GeoJson_output_resource_list_.empty()) {
    ...
    if (zf == 0) {
      ...
      if (buf == 0) {
        MagLog::error() << "Error allocating memory for GeoJson ZIP generation!"
                        << std::endl;
        return;
      }
      ...
      for (; it != itend; it++) {
        if (debug_)
          MagLog::dev() << "GeoJsonDriver.close() > Start adding file " << *it
                        << " to ZIP file.\n";
        FILE *fin;
        int size_read;
        ...
        if (err != ZIP_OK)
          ...
          if (fin == 0) {
            MagLog::error() << "Open file " << filename
                            << " to be added to ZIP FAILED!" << endl;
            return;
          } else {
            do {
              err = ZIP_OK;
              ...
              if (size_read > 0) {
                ...
                if (err < 0) {
                  MagLog::error()
                      << "Could NOT write ZIP file " << fileName_ << endl;
                }
              }
...
GeoJsonDriver::project(const magics::Layout &layout) const {
  // MagLog::dev() << " GeoJsonDriver::project("<<layout.id()<<")"<< endl;
}

MAGICS_NO_EXPORT void
GeoJsonDriver::redisplay(const magics::LegendLayout &layout) const {
...
*/
MAGICS_NO_EXPORT int GeoJsonDriver::setLineParameters(const LineStyle,
...
*/
MAGICS_NO_EXPORT void GeoJsonDriver::renderPolyline(const int n, MFloat *x,
                                                    MFloat *y) const {
...
*/
MAGICS_NO_EXPORT void
GeoJsonDriver::renderSimplePolygon(const int n, MFloat *xx, MFloat *yy) const {
  if (!render_)
    return;
  if (GeoJson_placemark_)
    closePlacemark();
...
*/
MAGICS_NO_EXPORT bool GeoJsonDriver::renderPixmap(MFloat x0, MFloat y0,
                                                  MFloat x1, MFloat y1, int w,
                                                  int h, unsigned char *pixmap,
                                                  int, bool) const {
