...
void QtDriver::open() {
  ...
  const int qtDpiResolution =
      (!scList.isEmpty()) ? scList.at(0)->logicalDotsPerInchY() : 72;
  ...
  // By default the ratio between the physical pixel size according to the
  // external definition and Qt is 1
  ...
  if (scene->dpiResolution() != -1) {
    ...
    dpiResolutionRatio_ = static_cast<float>(qtDpiResolution) /
                          static_cast<float>(scene->dpiResolution());
  }
  // Otherwise we suppose it is the same as given by Qt (thus their ratio will
  // be 1)
  else {
...
void QtDriver::close() {
  ...
  MagLog::debug() << "QtDriver::close -->item num:" << scene->items().count()
                  << endl;
}
...
/*!
  ...
  This method will update the offset and scale according to the new
  PreviewLayout given.
...
QtDriver::project(const magics::PreviewLayout &layout) const {
  MagLog::progress("project");
...
/*!
  ...
  This method will update the offset and scale according to the new
  MagnifierLayout given.

  \sa Layout
*/

MAGICS_NO_EXPORT void
QtDriver::project(const magics::MagnifierLayout &layout) const {
...
MAGICS_NO_EXPORT void
QtDriver::project(const magics::HistoLayout &layout) const {
...
MAGICS_NO_EXPORT void
QtDriver::project(const magics::SceneLayout &layout) const {
...
MAGICS_NO_EXPORT void QtDriver::project(MgQLayoutItem *item) const {
  ...
  MagLog::dev() << "Translate: " << projectX(-minX) << " " << projectY(-minY)
                << endl;
...
MAGICS_NO_EXPORT void QtDriver::newLayer(StaticLayer &layer) const {
  MagLog::debug() << "QtDriver::newLayer (StaticLayer) ---> " << layer.id()
                  << endl;
...
MAGICS_NO_EXPORT void QtDriver::newLayer(StepLayer &layer) const {
  ...
  MgQLayoutItem *parentLayout = layoutItemStack_.top();
  MgQLayerItem *qln =
      new MgQLayerItem(layer, parentLayout, layer.parent()->numberOfSteps());

  newLayer(qln);
}

MAGICS_NO_EXPORT void QtDriver::newLayer(MgQLayerItem *qln) const {
  MagLog::debug() << "QtDriver::newLayer (MgQLayerItem) ---> "
                  << qln->layer().id() << endl;
...
/*!
  ...
  This method will close an existing layer. This includes resets of existing
  boxes.
...
*/
MAGICS_NO_EXPORT int QtDriver::setLineParameters(const LineStyle linestyle,
...
*/
MAGICS_NO_EXPORT void QtDriver::renderPolyline(const int n, MFloat *x,
                                               MFloat *y) const {
...
/*!
  ...
  \param y array of y values
*/
MAGICS_NO_EXPORT void QtDriver::renderPolyline2(const int n, MFloat *x,
                                                MFloat *y) const {
  if (currentColour_ == Colour("none"))
    return;
...
/*!
  ...
  \param y array of y values
*/
MAGICS_NO_EXPORT void QtDriver::renderSimplePolygon(const int n, MFloat *x,
                                                    MFloat *y) const {
  if (currentColour_ == Colour("none"))
    return;
  ...
  if (currentShading_ == M_SH_DOT) {
    ...
    if (indexHatch_ < 1 || indexHatch_ > 6) {
      MagLog::warning() << "QtDriver::renderSimplePolygon > Hatch index "
                        << indexHatch_
...
void QtDriver::renderSimplePolygon(const Polyline &line) const {
  ...
  if (line.beginHoles() != line.endHoles()) {
    ...
    for (Polyline::Holes::const_iterator h = line.beginHoles();
         h != line.endHoles(); h++) {
      vector<double> x;
      vector<double> y;
      line.hole(h, x, y);
...
MAGICS_NO_EXPORT void QtDriver::circle(const MFloat x, const MFloat y,
...
MAGICS_NO_EXPORT void QtDriver::renderImage(const ImportObject &obj) const {
  ...
  MgQPixmapItem *item =
      new MgQPixmapItem(QPixmap::fromImage(img.mirrored(false, true)));
...
*/
MAGICS_NO_EXPORT bool QtDriver::renderPixmap(MFloat x0, MFloat y0, MFloat x1,
                                             MFloat y1, int w, int h,
                                             unsigned char *pixmap,
                                             int landscape,
                                             bool hasAlpha) const {
  MagLog::debug() << "renderPixmap: " << x0 << " " << y0 << " " << x1 << " "
                  << y1 << endl;
  ...
  if (hasAlpha) {
    for (int j = 0; j < h; j++) {
      for (int i = 0; i < w; i++) {
        ...
        pixel = qRgba(pixmap[srcPos], pixmap[srcPos + 1], pixmap[srcPos + 2],
                      pixmap[srcPos + 3]);
    ...
    for (int j = 0; j < h; j++) {
      for (int i = 0; i < w; i++) {
        srcPos = ((h - j - 1) * w + i) * 3;
        pixel =
            qRgba(pixmap[srcPos], pixmap[srcPos + 1], pixmap[srcPos + 2], 0xff);
        img.setPixel(i, j, pixel);
      }
    }
...
void QtDriver::generateSymbolPath(MgQSymbolItem *qSym, svgSymbol sym) const {
  ...
  for (unsigned int i = 0; i < si;
       i++) // for all elements in the symbol description
  {
    if (sym.elements[i].name == "circle") {
      const MFloat r = atof(sym.elements[i].attributes["r"].c_str()) * scaling;
      const MFloat cx =
          atof(sym.elements[i].attributes["cx"].c_str()) * scaling;
      const MFloat cy =
          atof(sym.elements[i].attributes["cy"].c_str()) * scaling;
...
MAGICS_NO_EXPORT void QtDriver::renderSymbols(const Symbol &symbol) const {
  ...
  if (sym_.size() == 0) {
    MagLog::error() << "QtDriver::renderSymbols() --> NO symbols available!"
                    << endl;
    return;
  }
...
MAGICS_NO_EXPORT void QtDriver::snowflake(const MFloat x, const MFloat y,
                                          const MFloat size,
                                          MgQSymbolItem *qSym) const {
...
/*!
  Plotting drizzle
*/
MAGICS_NO_EXPORT void QtDriver::drizzle(const MFloat x, const MFloat y,
                                        const MFloat size,
                                        MgQSymbolItem *qSym) const {
...
*/
MAGICS_NO_EXPORT void QtDriver::lightning(const MFloat x, const MFloat y,
                                          const MFloat size,
                                          MgQSymbolItem *qSym) const {
  MgQPainterPath path(false);
  QVector<QPointF> pp;
...
/*!
  Plotting a triangle
*/
MAGICS_NO_EXPORT void QtDriver::triangle(const MFloat x, const MFloat y,
                                         const MFloat size, const int fill,
                                         const int l,
                                         MgQSymbolItem *qSym) const {
...
MAGICS_NO_EXPORT void QtDriver::executeStep(int step) const {
  ...
  for(vector<Layer*>::const_iterator itL = sl.beginLayer();itL !=
  sl.endLayer(0); ++itL)
  {
...
void QtDriver::executeStep(int step, MgQLayerItem *layerItem) const {
  ...
  scene->currentSceneItem()->sceneLayerItem()->sceneLayer().execute(
      &layerItem->layer(), step, *this);
...
void QtDriver::executeStep(int step, MgQLayerItem *layerItem,
...
MAGICS_NO_EXPORT void
QtDriver::executeMagnifier(Layer *layer,
                           MgQMagnifierLayoutItem *layoutItem) const {
  QGraphicsItem *prevItem = currentItem_;
...
MAGICS_NO_EXPORT void QtDriver::executeHisto(Layer *layer,
                                             MgQHistoItem *layoutItem,
                                             QString visdefName,
...
MAGICS_NO_EXPORT void QtDriver::redisplay(const SceneLayer &layer) const {
  ...
  for (vector<Layer *>::iterator itL = layer.beginLayer();
       itL != layer.endLayer(); ++itL) {
    (*itL)->newLayer(*this);
    (*itL)->execute(*this);
    (*itL)->closeLayer(*this);
  }
  ...
  /*AnimationStep* step = layer.rules()->at(0);
      ...
      for (list<Layer*>::const_iterator itL = layer.begin(); itL != layer.end();
     ++itL)
      {
