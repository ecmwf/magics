...
See <a
href="http://www.w3.org/TR/REC-html40/sgml/entities.html">entities.html</a> for
...
PostScriptDriver::PostScriptDriver()
    : ps_(true), pdf_(false), eps_(false), maxPathSize_(200),
...
/*!
  \brief Destructor
*/
PostScriptDriver::~PostScriptDriver() { relieveFonts(); }
...
MAGICS_NO_EXPORT void PostScriptDriver::startPage() const {
  ...
  *ps << "2 setlinejoin 0 1 SUP 0 10 SF 0 SHA 0 SVA\n"; // MITER is now always
                                                        // on
  *ps << "0 0 0 0 Y n 0 0 m " << dimensionX_ << " 0 rl 0 " << dimensionY_
      << " rl " << -dimensionX_ << " 0 rl cp fill\n"; // force white background
...
/*!
  ...
  This method will update the offset and scale according to the new Layout
  given.

  \sa Layout
*/
MAGICS_NO_EXPORT void
PostScriptDriver::project(const magics::Layout &layout) const {
  ...
  *ps << "gs";
  if (layout.clipp()) {
    *ps << " " << offsetX_ << " " << offsetY_ << " " << dimensionX_ << " "
        << dimensionY_ << " rectclip";
  }
  *ps << " " << X_ << " " << Y_ << " t";
  *ps << "\n";
}
...
*/
MAGICS_NO_EXPORT void
PostScriptDriver::setNewColour(const Colour &colour) const {
...
*/
MAGICS_NO_EXPORT void
PostScriptDriver::setNewLineWidth(const MFloat width) const {
...
*/
MAGICS_NO_EXPORT int
PostScriptDriver::setLineParameters(const LineStyle linestyle,
                                    const MFloat w) const {
  setNewLineWidth(w);
  ...
  case M_DOT:
    *ps << "[" << ((sw) ? width : 4) << " " << ((sw) ? width : 8) << "] 4 sd\n";
    break;
  case M_CHAIN_DASH:
    *ps << "[" << ((sw) ? width * 4 : 16) << " " << ((sw) ? width * 1 : 8)
        << " " << ((sw) ? width * 1 : 4) << " " << ((sw) ? width * 1 : 8) << " "
        << "] 0 sd\n";
    break;
  case M_CHAIN_DOT:
    *ps << "[" << ((sw) ? width * 4 : 12) << " " << ((sw) ? width * 1 : 8)
        << " " << ((sw) ? width * 1 : 4) << " " << ((sw) ? width * 1 : 8) << " "
        << ((sw) ? width * 1 : 4) << " " << ((sw) ? width * 1 : 8) << " "
        << "] 0 sd\n";
    break;
...
*/
MAGICS_NO_EXPORT void PostScriptDriver::renderPolyline(const int n, MFloat *x,
                                                       MFloat *y) const {
...
/*!
  ...
  \param y array of y values
*/
MAGICS_NO_EXPORT void PostScriptDriver::renderPolyline2(const int n, MFloat *x,
                                                        MFloat *y) const {
  if (n != 2 || (currentColour_ == Colour("NONE")))
    return;
  writeColour();
...
void PostScriptDriver::renderSimplePolygon(const magics::Polyline &line) const {
  ...
  *ps << "gs\n";
  ...
  if (currentShading_ == M_SH_DOT) {
    const DotShadingProperties *pro =
        (DotShadingProperties *)currentShadingProperties_;
    ...
    *ps << "/Pat {\n gs 0 0 " << square_size << " " << square_size
        << " rectclip gr gs ";
    if (k == 1.)
      *ps << "0 0 0 1";
    else {
    ...
    *ps << " setcmykcolor 1 1 m 0 " << s << " rl " << s << " 0 rl 0 -" << s
        << " rl cp fill gr "
        << "} bind def\n"
        << "<< /PatternType 1 /PaintType 1 /TilingType 1\n"
        << "/BBox [0 0 " << square_size << " " << square_size << "] /XStep "
        << square_size << " /YStep " << square_size << "\n"
        << "/PaintProc { Pat }\n"
        << ">>\n"
        << "matrix makepattern setpattern\n";
  } else if (currentShading_ == M_SH_HATCH) {
    const HatchShadingProperties *pro =
        (HatchShadingProperties *)currentShadingProperties_;
    indexHatch_ = pro->index_;
    if (indexHatch_ < 1 || indexHatch_ > 6) {
      MagLog::warning()
          << "PostScriptDriver::renderSimplePolygon > Hatch index "
          << indexHatch_ << " is wrong. No hatch shading possible!" << endl;
      if (indexHatch_ == 0)
        MagLog::debug() << "PostScriptDriver::renderSimplePolygon > Hatch "
                           "index is 0. Alternative hatch "
  ...
  *ps << "n ";
  ...
  for (; h != he; ++h) {
    ...
    *ps << pcounter << " " << projectX(x[0]) << " " << projectY(y[0])
        << " F P\n";
  }

  if ((line.get(n - 1).x() == line.get(0).x()) &&
      (line.get(n - 1).y() == line.get(0).y()))
    n--;
...
*/
MAGICS_NO_EXPORT void
PostScriptDriver::renderSimplePolygon(const int n, MFloat *x, MFloat *y) const {
  if (n < 3 || (currentColour_ == Colour("NONE")))
    return;
  ...
  if (currentShading_ == M_SH_DOT) {
    ...
    if (density <= 0) {
      if (density < 0)
        MagLog::warning()
            << "PostScriptDriver::renderSimplePolygon > Dot density " << density
...
MAGICS_NO_EXPORT void PostScriptDriver::renderText(const Text &text) const {
  ...
  *ps << text.getJustification() << " SHA " << text.getVerticalAlign()
      << " SVA ";
  ...
  for (; niceText < niceTextEnd;) {
    ...
    const bool underlined =
        (styles.find("underlined") != styles.end()) ? true : false;
      ...
      if (niceText == text.textBegin()) // if first text string
      {
        ...
        *ps << "gs " << x0 << " " << y0 << " t ";
        ...
        if (counterSubStrings > 1) {
          *ps << "(" << all_text.str()
              << ") stringwidth pop HA mul VA Height mul moveto "
              << "(" << tmp.str() << ") " << showCommand << "\n";
        } else {
...
/*!
  ...
  The meaning of the last parameter <i>s</i> is as follows:
     - 0-8 determines how many quarters of the circle are filled. Starting from
  the top clock-wise.
     - 9 fills the whole circle but leaves a vertical bar empty in the middle of
  the circle.
...
*/
MAGICS_NO_EXPORT void PostScriptDriver::circle(const MFloat x, const MFloat y,
                                               const MFloat r,
                                               const int s) const {
  writeColour();
  std::fstream *ps = getStream();
  ...
  if (s < 8) {
    *ps << "n " << cx << " " << cy << " " << r << " 0 360 arc st\n";
    if (s > 0)
      *ps << "n " << cx << " " << cy << " m " << cx << " " << cy << " " << r
          << " 90 " << 90 - (s * 45) << " arn\n";
  } else
    *ps << "n " << cx << " " << cy << " " << r << " 0 360 ar\n";
  if (s == 9) {
    *ps << "1 1 1 C n " << cx << " " << cy + r - 1 << " m 0 " << -r - r + 2
        << " rl st\n";
...
/*!
  ...
  This method renders pixmaps. These are used for cell shading and raster input
  (GIFs and PNGs).
...
*/
MAGICS_NO_EXPORT bool
PostScriptDriver::renderPixmap(MFloat x0, MFloat y0, MFloat x1, MFloat y1,
                               int width, int height, unsigned char *pixmap,
                               int landscape, bool alpha) const {
  ...
  *ps << "gs /pic " << width * ((col_model == 1) ? 4 : 3) << " string def "
      << x0 << " " << y0 << " t " << dx << " " << dy << " s " << width << " "
      << height << " 8\n"
  ...
  for (int j = height - 1; j >= 0; j--) {
    for (int i = width - 1; i >= 0; i--) {
      ...
      int n =
          (landscape) ? (height * i + j) * 3 : (j * width + width - 1 - i) * 3;
      if (alpha)
        n = (landscape) ? (height * i + j) * 4
                        : (j * width + width - 1 - i) * 4;
...
/*!
  ...
  This method renders cell arrays, also called images in Magics language. These
  are mainly used for satellite data.
...
*/
MAGICS_NO_EXPORT bool
PostScriptDriver::renderCellArray(const Image &image) const {
...
/*!
  ...
  \note This can increase file and log file sizes if you run Magics++ in debug
  mode!
...
*/
MAGICS_NO_EXPORT void
PostScriptDriver::renderSymbols(const Symbol &symbol) const {
  ...
  if (symbolName.find(logo) == std::string::npos) {
    ...
    pFile_ << " " << projectX(symbol[0].x()) << " "
           << projectY(symbol[0].y()) - convertCM(symbol.getHeight() * .3)
           << " t 0.4 0.4 s" << endl;
    pFile_ << "gs\n"
           << "183.473 76.074 m 183.473 63.121 l 146.492 63.121 l 146.492 "
              "48.105 l 180.438 48.105 l 180.438 36.094 "
              "l 146.492 36.094 l 146.492 18.977 l 184.262 18.977\n"
           << "l 184.262 6.023 l 131.086 6.023 l 131.086 76.074 l P 183.473 "
              "76.074 m fill\n"
           << "234.574 57.387 m 233.656 58.859 232.492 60.172 231.141 61.262 c "
              "228.277 63.559 224.711 64.805 "
              "221.039 64.793 c 217.973 64.891 214.934 64.199 212.211\n"
           << "62.793 c 209.84 61.496 207.82 59.645 206.324 57.398 c 204.781 "
              "55.047 203.668 52.441 203.043 49.699 "
              "c 202.359 46.773 202.023 43.773 202.039 40.77 c 202.023\n"
           << "37.875 202.359 34.992 203.043 32.18 c 203.68 29.504 204.793 "
              "26.961 206.324 24.672 c 207.832 22.441 "
              "209.855 20.605 212.219 19.316 c 214.945 17.91 217.984\n"
           << "17.219 221.051 17.316 c 225.762 17.316 229.434 18.754 232.062 "
              "21.629 c 234.844 24.812 236.543 "
              "28.797 236.918 33.012 c 251.832 33.012 l 251.523\n"
           << "28.91 250.508 24.895 248.828 21.141 c 247.281 17.738 245.082 "
              "14.68 242.352 12.129 c 239.625 9.621 "
              "236.422 7.688 232.934 6.445 c 229.121 5.086 225.105\n"
           << "4.41 221.059 4.441 c 216.094 4.363 211.164 5.312 206.586 7.234 "
              "c 202.457 8.988 198.758 11.613 "
              "195.742 14.934 c 192.75 18.289 190.449 22.203 188.977\n"
           << "26.453 c 187.379 31.043 186.582 35.871 186.625 40.73 c 186.578 "
              "45.684 187.375 50.605 188.977 55.293 "
              "c 190.449 59.605 192.746 63.586 195.742 67.016 c\n"
           << "198.734 70.395 202.43 73.074 206.566 74.875 c 214.637 78.219 "
              "223.621 78.641 231.973 76.074 c "
              "235.309 75.016 238.43 73.371 241.191 71.223 c 243.934\n"
           << "69.062 246.215 66.375 247.906 63.324 c 249.742 59.988 250.883 "
              "56.316 251.262 52.531 c 236.344 "
              "52.531 l 236.098 54.254 235.492 55.906 234.574 57.387 c P "
              "234.574 57.387 m fill\n"
           << "279.66 76.074 m 296.047 27.906 l 296.246 27.906 l 311.742 "
              "76.074 l 333.426 76.074 l 333.426 6.004 l "
              "319 6.004 l 319 55.664 l 318.801 55.664 l 301.633\n"
           << " 6.023 l 289.762 6.023 l 272.594 55.176 l 272.395 55.176 l "
              "272.395 6.023 l 257.977 6.023 l 257.977 "
              "76.074 l P 279.66 76.074 m fill\n"
           << "394.41 6.023 m 382.586 53.703 l 382.398 53.703 l 370.715 6.023 "
              "l 355.121 6.023 l 336.578 76.098 l "
              "351.984 76.098 l 363.066 28.418 l 363.27 28.418\n"
           << " l 375.391 76.074 l 389.816 76.074 l 401.816 27.824 l 402.02 "
              "27.824 l 413.488 76.074 l 428.594 "
              "76.074 l 409.758 6.004 l 394.41 6.004 l P 394.41 6.023 m fill\n"
           << "481 76.074 m 481 63.121 l 447.156 63.121 l 447.156 46.938 l "
              "476.496 46.938 l 476.496 34.922 l "
              "447.164 34.922 l 447.164 6.023 l 431.758 6.023 l 431.758\n"
           << " 76.074 l 481.012 76.074 l P 481 76.074 m fill\n"
           << "0.402 36.836 m 34.848 36.836 l 34.848 46.164 l 0.402 46.164 l "
              "-0.137 43.078 -0.137 39.922 0.402 "
              "36.836 c P 0.402 36.836 m fill\n"
           << "115.121 33.59 m 111.5 14.293 94.715 0.258 75.078 0.117 c 69.25 "
              "0.215 63.5 1.465 58.16 3.801 c "
              "52.703 1.328 46.773 0.086 40.781 0.156 c 21.168 0.16\n"
           << " 4.344 14.148 0.762 33.43 c 25.875 33.43 l 28.719 27.184 34.949 "
              "23.176 41.812 23.18 c 49.266 23.312 "
              "55.867 28.02 58.422 35.023 c 60.812 28.211 67.102\n"
           << " 23.539 74.312 23.211 c 81.527 22.883 88.215 26.965 91.215 "
              "33.531 c P 115.121 33.59 m fill\n"
           << "115.121 49.41 m 111.496 68.73 94.676 82.77 75.02 82.883 c "
              "69.207 82.801 63.473 81.543 58.16 79.188 "
              "c 52.715 81.699 46.777 82.961 40.781 82.883\n"
           << "c 21.148 82.891 4.305 68.879 0.742 49.57 c 25.855 49.57 l "
              "28.688 55.84 34.934 59.867 41.812 59.859 "
              "c 49.273 59.723 55.875 55 58.422 47.988 c 60.82 54.793\n"
           << " 67.113 59.465 74.324 59.789 c 81.535 60.117 88.223 56.031 "
              "91.227 49.469 c P 115.121 49.41 m fill\n"
           << "gr" << endl;
  }
}
...
MAGICS_NO_EXPORT void PostScriptDriver::openFile() const {
  ...
  if (!pFile_) {
    MagLog::error() << " PostScriptDriver --> Cannot write output file to what "
                       "was specified: "
                    << fileName_ << endl;
    MagLog::error() << "";
...
MAGICS_NO_EXPORT void PostScriptDriver::writePSFileHeader() const {
  ...
  *ps << getMagicsVersionString() << "\n%%CreationDate: " << info.getTime()
      << "\n%%For: " << info.getUserID() << "@" << info.getHostName() << " "
      << info.getUserName() << "\n";
  ...
  string orientation =
      (getXDeviceLength() < getYDeviceLength()) ? "Portrait" : "Landscape";
  ...
  if (isEPS()) {
    *ps << "%%LanguageLevel: 2\n%%Pages: 1\n"
        << "%%BoundingBox: 0 0 " << static_cast<int>(dimensionX) << " "
        << static_cast<int>(dimensionY) + 1 << "\n";
  } else {
    if (isPDF() && !isPS()) {
      ...
      *ps << "%%Orientation: " << orientation
          << "\n%%LanguageLevel: 2\n%%Pages: (atend)\n"
          << "%%BoundingBox: 0 0 " << static_cast<int>(small) + 1 << " "
          << static_cast<int>(big) + 1 << "\n";
    }
  }
  ...
  *ps << "/S { gr showpage } def /m {moveto} def /st {stroke} def /rl "
         "{rlineto} def /ro {rotate} def /cp {closepath} "
         "def /d { {rmoveto rlineto} repeat stroke} bind def /gr {grestore} "
         "def /gs {gsave} def /n { newpath } def\n"
      << "/sa {save} def /lw {setlinewidth } def /ar {arc fill} def /arn {arcn "
         "fill} def /l { lineto } bind def /c { "
         "curveto } bind def\n"
      << "/sd {setdash} def /C { setrgbcolor } def /Y { setcmykcolor } def  /B "
         "{ moveto rlineto stroke } bind def "
         "/BB { moveto lineto stroke } bind def /t { translate } def /s "
         "{scale} def /K { /UY exch def /UX exch def "
         "/LY exch def \n"
      << "/LX exch def gsave newpath LX LY moveto UX LY lineto UX UY lineto LX "
         "UY lineto closepath newpath } def /lp "
         "{ moveto rlineto } bind def /p { moveto {rlineto} repeat stroke} "
         "bind def /po { moveto {rlineto} repeat } "
         "bind def\n"
      << "/q {moveto rlineto stroke} bind def /f {moveto {rlineto} repeat "
         "fill} bind def /e {moveto {rlineto} repeat "
         "eofill} bind def /F {moveto {rlineto} repeat} bind def /E {eofill} "
         "bind def /P { closepath } bind def\n"
      << "/SAVEMT matrix def\n"
      << "/Degreevec\n"
      << "[\n"
      << "	8#100 /at 8#251 /copyright 8#260 /degree 8#306 /AE 8#301 "
         "/Aacute 8#304 /Adieresis 8#300 /Agrave 8#305 "
         "/Aring 8#303 /Atilde 8#307 /Ccedilla 8#311 /Eacute 8#312 "
         "/Ecircumflex 8#313 /Edieresis 8#310 /Egrave\n"
      << "	8#320 /Eth 8#315 /Iacute 8#316 /Icircumflex 8#317 /Idieresis "
         "8#314 /Igrave 8#321 /Ntilde 8#323 /Oacute "
         "8#325 /Odieresis 8#322 /Ograve 8#330 /Oslash 8#325 /Otilde 8#336 "
         "/Thorn 8#332 /Uacute 8#333 /Ucircumflex\n"
      << "	8#334 /Udieresis 8#331 /Ugrave 8#335 /Yacute 8#341 /aacute "
         "8#342 /acircumflex 8#222 /acute 8#264 "
         "/acute 8#344 /adieresis 8#346 /ae 8#340 /agrave 8#345 /aring 8#343 "
         "/atilde\n"
      << "	8#226 /breve 8#246 /brokenbar 8#237 /caron 8#347 /ccedilla "
         "8#270 /cedilla 8#242 /cent 8#223 "
         "/circumflex 8#244 /currency 8#250 /dieresis 8#227 /dotaccent 8#220 "
         "/dotlessi\n"
      << "	8#351 /eacute 8#352 /ecircumflex 8#350 /egrave 8#360 /eth "
         "8#241 /exclamdown 8#337 /germandbls 8#221 "
         "/grave 8#253 /guillemotleft 8#273 /guillemotright 8#235 "
         "/hungarumlaut\n"
      << "	8#255 /hyphen 8#355 /iacute 8#356 /icircumflex 8#357 "
         "/idieresis 8#354 /igrave 8#254 /logicalnot 8#257 "
         "/macron 8#265 /mu\n"
      << "	8#327 /multiply 8#361 /ntilde 8#363 /oacute 8#364 /odieresis "
         "8#236 /ogonek 8#362 /ograve 8#275 "
         "/onehalf 8#274 /onequarter 8#271 /onesuperior 8#252 /ordfeminine\n"
      << "	8#272 /ordmasculine 8#370 /oslash 8#365 /otilde 8#266 "
         "/paragraph 8#267 /periodcentered 8#261 "
         "/plusminus 8#277 /questiondown 8#256 /registered 8#232 /ring 8#247 "
         "/section 8#243 /sterling\n"
      << "	8#376 /thorn 8#276 /threequarters 8#263 /threesuperior 8#224 "
         "/tilde 8#262 /twosuperior 8#372 /uacute "
         "8#373 /ucircumflex 8#374 /udieresis 8#371 /ugrave 8#375 /yacute "
         "8#377 /ydieresis 8#245 /yen\n"
      << "] def\n"
      ...
      << "	{ exch dup /FID ne\n"
      << "		{ dup /Encoding eq\n"
      << "			{ exch dup length array copy newfont 3 1 roll "
         "put}\n"
  ...
  for (mapit = FontMap_.begin(); mapit != FontMap_.end(); mapit++)
    *ps << "Font " << (*mapit).second.id << " eq { /" << (*mapit).second.ps_name
        << " } if\n";
  ...
  *ps << "ReEncodeSmall /Magicsfontname findfont Height scalefont setfont\n"
      ...
      << "/ST\n"
      << "{ /YPOS exch def /XPOS exch def [ CHUPY CHUPX neg CHUPX CHUPY XPOS "
         "YPOS ] concat} def\n"
      << "/SHA\n"
      << "{ /a exch def a 0 eq { /HA 0 def  } if a 1 eq { /HA -0.5 def } if a "
         "2 eq { /HA -1 def } if } def\n"
      << "/SVA\n"
      << "{ /b exch def\n"
      << "	Font  4 lt { b 0 eq { /VA 0 def } if b 1 eq { /VA -0.7 def } "
         "if b 2 eq { /VA -0.6625 def } if b 3 eq { "
         "/VA -0.33125 def } if b 4 eq { /VA 0 def } if b 5 eq { /VA 0.0375 "
         "def } if\n"
      << "	} if\n"
      << "	Font  8 lt { b 0 eq { /VA 0 def } if b 1 eq { /VA -0.76 def } "
         "if b 2 eq { /VA -0.725 def } if b 3 eq { "
         "/VA -0.3625 def } if b 4 eq { /VA 0 def } if b 5 eq { /VA 0.035 def "
         "} if\n"
      << "	} if\n"
      << "	Font 12 lt { b 0 eq { /VA 0 def } if b 1 eq { /VA -0.7 def } "
         "if b 2 eq { /VA -0.6625 def } if b 3 eq { "
         "/VA -0.33125 def } if b 4 eq { /VA 0 def } if b 5 eq { /VA 0.0375 "
         "def } if\n"
      << "	} if\n"
      << "	Font 12 eq { b 0 eq { /VA 0 def } if b 1 eq { /VA -0.7 def } "
         "if b 2 eq { /VA -0.6625 def } if b 3 eq { "
         "/VA -0.33125 def } if b 4 eq { /VA 0 def } if b 5 eq { /VA 0.0375 "
         "def } if\n"
      << "	} if\n"
      << "	Font 12 gt { b 0 eq { /VA 0 def } if b 1 eq { /VA -0.7 def } "
         "if b 2 eq { /VA -0.6625 def } if b 3 eq { "
         "/VA -0.33125 def } if b 4 eq { /VA 0 def } if b 5 eq { /VA 0.0375 "
         "def } if\n"
      << "	} if\n"
      ...
      << "	4 add /x1 exch def\n"
      << "	text stringwidth pop HA mul VA Height mul moveto\n"
      << "	gs n x1 -2 add HA mul VA Height mul -2 add moveto x1 0 rlineto "
         "0 12 Height add .7 mul rlineto x1 neg 0 "
...
MAGICS_NO_EXPORT void
PostScriptDriver::setDeviceColourModel(const string &m) const {
  if (m.empty())
...
static SimpleObjectMaker<PostScriptDriver, BaseDriver>
    PostScript_driver("PostScript");
