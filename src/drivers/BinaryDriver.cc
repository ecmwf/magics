...
BinaryDriver::BinaryDriver() {
  MagLog::debug() << "BinaryDriver::BinaryDriver needs implementing."
                  << std::endl;
}
...
/*!
  ...
  files are closed.
*/
MAGICS_NO_EXPORT void BinaryDriver::endPage() const { out_.close(); }
...
BinaryDriver::project(const magics::Layout &layout) const {
  char c = 'P';
  out_.write(&c, 1);
...
*/
MAGICS_NO_EXPORT int BinaryDriver::setLineParameters(const LineStyle linestyle,
...
*/
MAGICS_NO_EXPORT void BinaryDriver::renderPolyline(const int n, MFloat *x,
                                                   MFloat *y) const {
  char c = 'H';
  out_.write(&c, 1);
...
/*!
  ...
  \param y array of y values
*/
MAGICS_NO_EXPORT void BinaryDriver::renderPolyline2(const int n, MFloat *x,
                                                    MFloat *y) const {
  char c = 'B';
  out_.write(&c, 1);
  out_.write((char *)(&n), sizeof(int));
...
*/
MAGICS_NO_EXPORT void
BinaryDriver::renderSimplePolygon(const Polyline &line) const {
...
/*!
  ...
  \param y array of y values
*/
MAGICS_NO_EXPORT void BinaryDriver::renderSimplePolygon(const int n, MFloat *x,
                                                        MFloat *y) const {
  char c = 'S';
  out_.write(&c, 1);
  out_.write((char *)(&n), sizeof(int));
...
*/
MAGICS_NO_EXPORT void BinaryDriver::circle(const MFloat x, const MFloat y,
...
*/
MAGICS_NO_EXPORT bool BinaryDriver::renderPixmap(MFloat x0, MFloat y0,
                                                 MFloat x1, MFloat y1, int w,
                                                 int h, unsigned char *pixmap,
                                                 int landscape, bool) const {
  MagLog::debug() << "BinaryDriver::renderPixmap needs implementing."
                  << std::endl;
  return true;
}
...
MAGICS_NO_EXPORT bool BinaryDriver::renderCellArray(const Image &image) const {
  ...
  ColourTable &lt = image.getColourTable();
  int si = 0;
  for (magvector<ColourTableEntry>::const_iterator colour = lt.begin();
       colour != lt.end(); ++colour) {
    si++;
  }
  const int sii = si;
