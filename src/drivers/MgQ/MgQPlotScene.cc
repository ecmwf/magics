...
MgQPlotScene::MgQPlotScene(QObject *parent)
    : MgQScene(parent), ignoreSceneItemChange_(false) {
  dpiResolution_ = 75;
...
MgQSceneItem *MgQPlotScene::currentSceneItem() {
  if (currentSceneItemIndex_ >= 0 &&
      currentSceneItemIndex_ < sceneItems_.count()) {
...
bool MgQPlotScene::identifyPos(QPointF scenePos, MgQSceneItem **sceneItem,
                               MgQLayoutItem **projectorItem) {
...
void MgQPlotScene::updateCache() {
  ...
  if (cacheDevice_->width() < sceneRect().width() ||
    ...
    cacheDevice_ =
        new QPixmap(sceneRect().width() + 100, sceneRect().height() + 100);
    ...
    MagLog::debug()
        << "MgQPlotScene::updateCache() ---> Pixmap size changed to: "
...
void MgQPlotScene::renderForMagnifier(QPainter *painter,
...
float MgQPlotScene::plotScale() { return plotRootItem_->scale(); }
