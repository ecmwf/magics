...
MgQPatternProperties::MgQPatternProperties(Type type)
    : type_(type), bg_(QColor(255, 255, 255, 0)) {}
...
bool MgQPatternProperties::operator==(const MgQPatternProperties &p) const {
  ...
  if (type_ == DotShading) {
    return p.size_ == size_ && p.itemSize_ == itemSize_ && p.bg_ == bg_ &&
           p.colour_ == colour_;
...
MgQPattern::MgQPattern(const MgQPatternProperties &prop)
    : QPixmap(prop.size_), prop_(prop) {
  if (prop.type_ == MgQPatternProperties::DotShading) {
    ...
    if (prop_.itemSize_.width() == 1) {
      ...
      p.fillRect(QRectF((width() - prop_.itemSize_.width()) / 2.,
                        (height() - prop_.itemSize_.height()) / 2.,
      ...
      p.drawEllipse(QRectF((width() - prop_.itemSize_.width()) / 2.,
                           (height() - prop_.itemSize_.height()) / 2.,
                           prop_.itemSize_.width(), prop_.itemSize_.height()));
    }
