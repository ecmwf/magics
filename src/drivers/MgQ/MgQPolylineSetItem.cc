...
MgQPolylineSetItem::MgQPolylineSetItem(QRectF &boundingRect,
...
void MgQPolylineSetItem::paint(QPainter *painter,
  ...
  // qDebug() <<  "POLYLINESET" << polylines_.count() <<
  // painter->clipRegion().boundingRect() << boundingRect_ <<
...
void MgQPolylineSetItem::addPolyline(QVector<QPointF> points, QBrush brush,
                                     QPen pen, bool isPolygon) {
  if (points.count() <= 0)
    return;
