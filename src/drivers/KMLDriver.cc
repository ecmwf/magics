...
bool copy_file(string from, string to) {
  ...
  if (source >= 0 && dest >= 0) {
    ...
    MagLog::error() << "KMZ copy file> file " << from
                    << " could NOT be copied to " << to << endl;
    close(source);
    close(dest);
    return false;
  }
...
*/
KMLDriver::KMLDriver()
    : currentTimeBegin_(""), currentTimeEnd_(""), kml_placemark_(false),
      polyline_begin_(true), polygon_begin_(true), MultiGeometrySet_(false),
      layer_(false), render_(true), ecmwf_logo_(false) {}
...
void KMLDriver::open() {
  ...
  if (!pFile_) {
    MagLog::error() << " KMLScriptDriver --> Cannot write output file to what "
                       "was specified: "
                    << fileName_ << endl;
    MagLog::error() << "";
  ...
  if (!author_.empty())
    pFile_ << " <atom:author><atom:name>" << author_
...
/*!
  ...
  While closing the driver all files generated (stored in
  kml_output_resource_list_ ) are put into one KMZ zip like file. We use minizip
  (see minizip subdirectory to do so) since gnuzip is NOT sufficient!
*/
void KMLDriver::close() {
  ...
  if (ecmwf_logo_) {
    ...
    pFile_ << "<ScreenOverlay id=\"logo\">\n"
           ...
           << "</Icon>\n"
           << "<overlayXY x=\"0\" y=\"0\" xunits=\"fraction\" "
              "yunits=\"fraction\"/>\n"
           << "<screenXY x=\"0\" y=\"0\" xunits=\"fraction\" "
              "yunits=\"fraction\"/>\n"
           << "<size x=\"-1\" y=\"0.1\" xunits=\"fraction\" "
              "yunits=\"fraction\"/>\n"
           << "</ScreenOverlay>\n";
  }
  ...
  if (kmz_ && !kml_output_resource_list_.empty()) {
    ...
    if (zf == 0) {
      ...
      if (buf == 0) {
        MagLog::error() << "Error allocating memory for KMZ generation!"
                        << std::endl;
        return;
      }
      ...
      for (; it != itend; it++) {
        if (debug_)
          MagLog::dev() << "KMLDriver.close() > Start adding file " << *it
                        << " to KMZ file.\n";
        FILE *fin;
        int size_read;
        ...
        err = zipOpenNewFileInZip(zf, filename, 0, 0, 0, 0, 0, 0, Z_DEFLATED,
        ...
        if (err != ZIP_OK)
          ...
          if (fin == 0) {
            MagLog::error() << "Open file " << filename
                            << " to be added to KMZ FAILED!" << endl;
            return;
          } else {
            do {
              err = ZIP_OK;
              ...
              if (size_read > 0) {
                ...
                if (err < 0) {
                  MagLog::error()
                      << "Could NOT write KMZ file " << fileName_ << endl;
                }
              }
...
KMLDriver::redisplay(const magics::LegendLayout &layout) const {
#ifdef HAVE_CAIRO
  // redisplay((const Layout&) legend);
  const string filename = "legend.png";
  cairo_surface_t *surface_ = cairo_image_surface_create(
      CAIRO_FORMAT_ARGB32, layout.width(), layout.height());
  ...
  pFile_
      ...
      << "</ScreenOverlay>\n";
#else
  MagLog::warning() << " KMLDriver> legend could NOT be generated - Cairo "
...
MAGICS_NO_EXPORT void KMLDriver::newLayer() const {
  ...
  pFile_ << "<description><![CDATA[Layer:" << currentLayer_
         << "]]></description>\n";
  polyline_begin_ = true;
  polygon_begin_ = true;
  layer_ = true;
...
*/
MAGICS_NO_EXPORT int KMLDriver::setLineParameters(const LineStyle,
                                                  const MFloat w) const {
  setNewLineWidth(w);

  //	MagLog::debug() << "KMLDriver::setLineParameters needs implementing." <<
  // std::endl;
  return 0;
}
...
*/
MAGICS_NO_EXPORT void KMLDriver::renderPolyline(const int n, MFloat *x,
                                                MFloat *y) const {
  if (render_ && n > 1) {
...
*/
MAGICS_NO_EXPORT void KMLDriver::renderSimplePolygon(const int n, MFloat *xx,
                                                     MFloat *yy) const {
  if (!render_)
    return;
  if (kml_placemark_)
...
*/
MAGICS_NO_EXPORT bool KMLDriver::renderPixmap(MFloat x0, MFloat y0, MFloat x1,
  ...
  if (render_) {
    ...
    MagLog::warning()
        << "Image import is not implemented for the used driver!!!" << endl;
    return false;
#else
    MagLog::dev() << "KML driver Image import uses GD -> for better results "
                     "use Cairo backend."
                  << endl;
    stringstream out;
...
MAGICS_NO_EXPORT bool KMLDriver::renderCellArray(const Image &image) const {
  ...
  if (render_) {
...
#else
    MagLog::dev()
        << "KML driver uses GD -> for better results use Cairo backend."
        << endl;
    stringstream out;
    out << currentPage_;
    const string layer_name =
        (!currentLayer_.empty()) ? currentLayer_ : "default_page";
    const string filename =
        "KML_cell_overlay_" + layer_name + "_" + out.str() + ".png";
    ...
    pFile_ << "ffffff</color>\n"
           ...
           << "   <south>" << image.getOrigin().y() - image.getHeight()
           << "</south>\n"
           << "   <east>" << image.getOrigin().x() + image.getWidth()
           << "</east>\n"
      ...
      for (int j = 0; j < w; j++) {
        ...
        if (!(lt[c] == "undefined")) {
          ...
          if ((r * g * b > 1.) || (r * g * b < 0.)) {
            ...
            b = 1.;
            a = 0.;
            //				MagLog::info()<< "PostScriptDriver->
            // Cellshading colour not defined in table!
            // Colour index: "<<c<< std::endl;
            //    PostScript will always 'overpaint' anything below missing
            //    data!!!!
            //
          }
...
MAGICS_NO_EXPORT void KMLDriver::renderWindArrow(const Arrow &arrow) const {
  ...
  for (unsigned int pts = 0; pts < arrPoNo; pts++) {
    ...
    pFile_ << "<heading>" << 90. + (arr->angle() * -57.29578) << "</heading>\n"
           ...
           << "</Style>\n"
           << "<Point>\n"
           << "<coordinates>" << arr->point_.x() << "," << arr->point_.y()
           << ",0</coordinates>\n"
           << "</Point>\n"
           << "</Placemark>" << endl;
    ++arr;
...
MAGICS_NO_EXPORT void KMLDriver::renderWindFlag(const Flag &flag) const {
  /*
   *	const MFloat thickness = (flag.getThickness()==1) ? 1
   : 2.*flag.getThickness(); const LineStyle style = flag.getStyle(); const
   MFloat length = convertCM(flag.getLength() / coordRatioX_ ); const string
   marker = flag.getOriginMarker(); const MFloat markerHeight =
   convertCM(flag.getOriginHeight() *.5 / coordRatioX_);
  */
