...
/*
 ...
 I currently have a function that accepts a cairo context (with a path already
set) and strokes the path with a pre-set width and color. The context can have
any arbitrary transformation already applied to it. So given this situation, how
would I ensure that I get a uniform line width from the stroke even when they
may have applied a deforming scale to the context? Any ideas would be
appreciated!
...
/*! \file CairoDriver.cc
   ...
   \todo Check how much drivers are dependent on writing temp files in local
   directory (thread safety)
*/
#include <cairo.h>
...
void CairoDriver::open() {
  MagLog::info() << "Cairo version used is: " << cairo_version_string()
...
void CairoDriver::setupNewSurface() const {
  if (magCompare(backend_, "png") || magCompare(backend_, "geotiff")) {
    surface_ = cairo_image_surface_create(CAIRO_FORMAT_ARGB32,
                                          dimensionXglobal_, dimensionYglobal_);
  } else if (magCompare(backend_, "pdf")) {
#if CAIRO_HAS_PDF_SURFACE
    fileName_ = getFileName("pdf");
    surface_ = cairo_pdf_surface_create(fileName_.c_str(), dimensionXglobal_,
                                        dimensionYglobal_);

#else
    MagLog::error() << "CairoDriver: PDF output NOT supported! Enable PDF "
                       "support in your Cairo installation."
                    << std::endl;
#endif
  } else if (magCompare(backend_, "ps")) {
#if CAIRO_HAS_PS_SURFACE
    fileName_ = getFileName("ps");
    const int dimensionXglobal =
        static_cast<int>(getXDeviceLength() * 72 / 2.54);
    const int dimensionYglobal =
        static_cast<int>(getYDeviceLength() * 72 / 2.54);
    surface_ = cairo_ps_surface_create(fileName_.c_str(), dimensionXglobal,
                                       dimensionYglobal);
#else
    MagLog::error() << "CairoDriver: PS output NOT supported! Enable PS "
                       "support in your Cairo installation."
                    << std::endl;
#endif
    ...
    surface_ = cairo_ps_surface_create(fileName_.c_str(), dimensionXglobal_,
                                       dimensionYglobal_);
    cairo_ps_surface_set_eps(surface_, true);
#else
    MagLog::error() << "CairoDriver: EPS output NOT supported! You need at "
                       "least version Cairo 1.5.2.\n"
    ...
    surface_ = cairo_svg_surface_create(fileName_.c_str(), dimensionXglobal_,
                                        dimensionYglobal_);
#else
    MagLog::error() << "CairoDriver: SVG output NOT supported! Enable SVG "
                       "support in your Cairo installation."
                    << std::endl;
#endif
  } else {
    MagLog::error() << "CairoDriver: The backend " << backend_
                    << " is NOT supported!" << std::endl;
  }

  cairo_status_t status = cairo_surface_status(surface_);
  if (status) {
    MagLog::error() << "CairoDriver: the output file (" << backend_
  ...
  if (magCompare(backend_, "ps")) {
    ...
    const string s2 =
        "%%Creator2: " + output_creator_ + " " + getMagicsVersionString();
  ...
  if (magCompare(transparent_, "off") ||
      !(magCompare(backend_, "png") || magCompare(backend_, "geotiff"))) {
...
MAGICS_NO_EXPORT void CairoDriver::startPage() const {
  if (currentPage_ > 0) {
    if (magCompare(backend_, "png") || magCompare(backend_, "geotiff")) {
      ...
      surface_ = cairo_image_surface_create(
          CAIRO_FORMAT_ARGB32, dimensionXglobal_, dimensionYglobal_);
      cr_ = cairo_create(surface_);
      if (magCompare(transparent_, "off")) {
        cairo_set_source_rgb(cr_, 1.0, 1.0, 1.0); /* white */
      } else {
  ...
  if (currentPage_ == 0 ||
      (!magCompare(backend_, "pdf") && !magCompare(backend_, "ps"))) {
...
MAGICS_NO_EXPORT void CairoDriver::endPage() const {
  ...
  if (magCompare(backend_, "eps")) {
    ...
    if (magCompare(palette_, "on")) {
      if (!write_8bit_png()) {
        MagLog::warning() << "CairoDriver::renderPNG > palletted PNG failed! "
                             "Generate 24 bit one ..."
                          << endl;
    ...
    if (status != CAIRO_STATUS_SUCCESS) {
      MagLog::error() << "PNG could NOT be written - "
...
void CairoDriver::newLayer(Layer &) const { cairo_save(cr_); }
...
/*!
  ...
  Only the raw raster (normally written to a PNG) is here written into a
  (Geo)Tiff.
...
MAGICS_NO_EXPORT void CairoDriver::write_tiff() const {
  ...
  if (!tif) {
    MagLog::warning() << "CairoDriver: Unable to open TIFF file " << fileName_
                      << std::endl;
    return;
  }
  ...
  if (!gtif) {
    MagLog::warning() << "CairoDriver: Unable to open GeoTIFF file "
                      << fileName_ << std::endl;
    return;
  }
...
/*!
  ...
  Only the raw raster (normally written to a 32 bit PNG) is here written into a
  8 bit.

*/
//#define PNG_DEBUG 3

MAGICS_NO_EXPORT bool CairoDriver::write_8bit_png() const { return false; }
...
MAGICS_NO_EXPORT void CairoDriver::project(const Layout &layout) const {
  ...
  if (layout.isNavigable() &&
      (magCompare(backend_, "png") || magCompare(backend_, "svg") ||
       magCompare(backend_, "geotiff"))) {
    const double offsetX = projectX(layout.minX());
    const double offsetY = projectY(layout.maxY());
    layout.pushDriverInfo(offsetX, oldHeight + offsetY, dimensionX_,
                          dimensionY_);
  }
}
...
*/
MAGICS_NO_EXPORT int CairoDriver::setLineParameters(const LineStyle linestyle,
                                                    const MFloat width) const {
...
*/
MAGICS_NO_EXPORT void CairoDriver::renderPolyline(const int n, MFloat *x,
                                                  MFloat *y) const {
  ...
  cairo_set_source_rgba(cr_, currentColour_.red(), currentColour_.green(),
                        currentColour_.blue(), currentColour_.alpha());
  cairo_stroke(cr_);
  cairo_restore(cr_);
}
...
/*!
  ...
  \param y array of y values
*/
MAGICS_NO_EXPORT void CairoDriver::renderPolyline2(const int n, MFloat *x,
                                                   MFloat *y) const {
  if (n != 2 || (currentColour_ == Colour("none")))
    return;
...
CairoDriver::renderSimplePolygon(const magics::Polyline &line) const {
...
/*!
  ...
  \param y array of y values
*/
MAGICS_NO_EXPORT void CairoDriver::renderSimplePolygon(const int n, MFloat *x,
                                                       MFloat *y) const {
  if (n < 3 || (currentColour_ == Colour("none")))
    return;
  cairo_save(cr_);
...
MAGICS_NO_EXPORT void CairoDriver::renderSimplePolygon() const {
  ...
  if (currentShading_ == M_SH_DOT) {
    ...
    pat_surface = cairo_surface_create_similar(cairo_get_group_target(cr_),
                                               CAIRO_CONTENT_COLOR_ALPHA,
                                               square_size, square_size);
    cr2 = cairo_create(pat_surface);

    cairo_set_source_rgba(cr2, currentColour_.red(), currentColour_.green(),
                          currentColour_.blue(), currentColour_.alpha());
    ...
    if (indexHatch_ < 1 || indexHatch_ > 6) {
      MagLog::warning() << "CairoDriver::renderSimplePolygon > Hatch index "
                        << indexHatch_
  ...
  if (currentShading_ == M_SH_HATCH || currentShading_ == M_SH_DOT)
    MagLog::error() << "CairoDriver: For hatch and dot shading you need at "
                       "least Cairo 1.2!\n"
...
MAGICS_NO_EXPORT void CairoDriver::renderText(const Text &text) const {
  ...
  for (; niceText < niceTextEnd; niceText++) {
    ...
    if (r < 0 || g < 0 || b < 0) {
      MagLog::debug() << "CAIRO: Text " << (*niceText).text() << " is RGB -1!"
                      << endl;
    } else {
      ostringstream col;
      col << hex << "#";
      ...
      alltext << "<span color=\"" << col.str() << "\" font_family=\""
              << magfont.name() << "\" size=\""
...
/*!
  ...
  \todo check if this is right and correct colour in other drivers for case fill
  = 9!!!
...
*/
MAGICS_NO_EXPORT void CairoDriver::circle(const MFloat x, const MFloat y,
                                          const MFloat r, const int s) const {
#if CAIRO_VERSION >= CAIRO_VERSION_ENCODE(1, 2, 0)
...
#else
  MagLog::warning()
      << "CairoDriver::circle requires at least cairo version 1.2!" << endl;
#endif
}
...
*/
MAGICS_NO_EXPORT bool CairoDriver::renderPixmap(MFloat x0, MFloat y0, MFloat x1,
                                                MFloat y1, int w, int h,
                                                unsigned char *pixmap, int,
                                                bool alpha) const {
...
MAGICS_NO_EXPORT void CairoDriver::renderImage(const ImportObject &obj) const {
  ...
  if (!magCompare(obj.getFormat(), "png")) {
    ...
    MagLog::info() << "CairoDriver::renderImage calling convert ... with: "
                   << cmd << endl;
  ...
  if (image) {
    ...
    const MFloat oow =
        (obj.getWidth() < 0) ? 30. / coordRatioX_ : obj.getWidth();
    const MFloat ooh =
        (obj.getHeight() < 0) ? 30. / coordRatioY_ : obj.getHeight();
    ...
    MagLog::warning() << "CairoDriver-> Could NOT read the image file " << file
                      << " !" << endl;
}
...
MAGICS_NO_EXPORT bool CairoDriver::renderCellArray(const Image &image) const {
  MagLog::debug() << "CD:renderCellArray> " << image.getWidth() << "x"
  ...
  const double wid = projectX(image.getOrigin().x() + image.getWidth()) -
                     projectX(image.getOrigin().x());
  const double hei = projectY(image.getOrigin().y() + image.getHeight()) -
                     projectY(image.getOrigin().y());
  ...
  if ((width > 500) || (height > 500)) // high resoltion enough to use image
  {
    ...
    if (cairo_surface_status(result) != CAIRO_STATUS_SUCCESS) {
      MagLog::warning() << "CAIRO:renderImage> can not create surface ("
                        << width << "x" << height << ")" << endl;
      return result;
    }
...
MAGICS_NO_EXPORT void CairoDriver::renderSymbols(const Symbol &symbol) const {
  ...
  if (symbolName.find(logo) == std::string::npos) {
    ...
    cairo_surface_t *image =
        cairo_image_surface_create_from_png(logofile.c_str());
    cairo_status_t ret = cairo_surface_status(image);

    if (!ret) {
      cairo_save(cr_);
      cairo_translate(cr_, projectX(symbol[0].x()),
                      projectY(symbol[0].y()) -
                          convertCM(symbol.getHeight() * .3));
      ...
      MagLog::warning() << "CairoDriver - Could NOT read logo " << logofile
                        << endl;
  }
}

MAGICS_NO_EXPORT bool
CairoDriver::convertToPixmap(const string &fname, const GraphicsFormat format,
                             const int reso, const MFloat wx0, const MFloat wy0,
                             const MFloat wx1, const MFloat wy1) const {
  if (format == PNG) {
    cairo_save(cr_);
