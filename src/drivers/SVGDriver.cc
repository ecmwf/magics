...
/*!
  \brief Destructor
*/
SVGDriver::~SVGDriver() { relieveFonts(); }
...
MAGICS_NO_EXPORT void SVGDriver::startPage() const {
  ...
  if (!pFile_) {
    MagLog::error()
        << " SVGDriver --> Cannot write output file to what was specified: "
        << fileName_ << endl;
    MagLog::error() << "";
  ...
  pFile_
      ...
      << "\t<dc:author>" << info.getUserID() << " on " << info.getHostName()
      << "</dc:author>\n"
  ...
  if (!output_creator_.empty())
    pFile_ << "\t<dc:creator><cc:Agent><dc:title>" << output_creator_
           << "</dc:title></cc:Agent></dc:creator>\n";
...
MAGICS_NO_EXPORT void SVGDriver::project(const magics::Layout &layout) const {
  ...
  if (layout.clipp()) {
    const double clip_height =
        projectY(layout.maxY()) - projectY(layout.minY());
    pFile_ << "<defs>\n"
           << " <clipPath id=\"clip_" << layout.name() << "\">\n"
           << "  <rect x=\"" << projectX(layout.minX()) << "\" y=\""
           << projectY(setY(layout.minY())) - clip_height << "\" width=\""
...
MAGICS_NO_EXPORT string SVGDriver::buildLayerName(const Layer *layer,
                                                  string type) const {
  string s = layer->name();
  if (s.empty())
    return type;
...
MAGICS_NO_EXPORT void SVGDriver::newLayer() const {
  ...
  if (!currentLayer_.empty()) {
    if (inkscape_)
      pFile_ << " inkscape:groupmode=\"layer\" inkscape:label=\""
             << currentLayer_ << "\"";
...
*/
MAGICS_NO_EXPORT int SVGDriver::setLineParameters(const LineStyle linestyle,
  ...
  if (currentLineType_ == M_DASH)
    ...
    stream << " stroke-dasharray=\"" << 4 * widi << "," << 2 * widi << ","
           << 2 * widi << "," << 2 * widi << "\"";
  else if (currentLineType_ == M_CHAIN_DOT)
    stream << " stroke-dasharray=\"" << 4 * widi << "," << 2 * widi << ","
           << 2 * widi << "," << 2 * widi << "," << 2 * widi << "\"";
...
*/
MAGICS_NO_EXPORT void SVGDriver::renderPolyline(const int n, MFloat *x,
                                                MFloat *y) const {
...
/*!
  ...
  \param y array of y values
*/
MAGICS_NO_EXPORT void SVGDriver::renderPolyline2(const int n, MFloat *x,
                                                 MFloat *y) const {
  ...
  pFile_ << "<path stroke=\"rgb(" << r << "," << g << "," << b << ")\" d=\"M"
         << x[0] << " " << setY(y[0]) << "L" << x[1] << " " << setY(y[1])
         << "\"/>\n";
}
...
/*!
  ...
  \param y array of y values
*/
MAGICS_NO_EXPORT void SVGDriver::renderSimplePolygon(const int n, MFloat *x,
                                                     MFloat *y) const {
  if (currentColour_.alpha() < 0.01 || n < 2 ||
  ...
  if (currentColour_.alpha() < 1.)
    gStream << "fill-opacity=\"" << currentColour_.alpha()
            << "\" stroke-opacity=\"0.01\" ";
  gStream << "stroke=\"rgb(" << r << "," << g << "," << b
          << ")\" stroke-width=\"0.01\" fill-rule=\"evenodd\"";
  ...
  if (count > 2) {
    if (currentShading_ == M_SH_DOT) {
      ...
      pFile_ << "<pattern id=\"D_" << svg_pattern_count
             ...
             << square_size << "cm\" height=\"" << square_size << "cm\">\n"
             << "   <rect x=\"0cm\" y=\"0cm\" width=\"" << s << "cm\" height=\""
             << s << "cm\" stroke=\"none\" fill=\"rgb(" << r << "," << g << ","
             << b << ")\" ";
      if (currentColour_.alpha() < 1.)
        pFile_ << "fill-opacity=\"" << currentColour_.alpha() << "\" ";
      pFile_ << "/>\n"
             << "</pattern>\n"
             << "<path fill=\"url(#D_" << svg_pattern_count
             << ")\" stroke=\"none\" " << stream.str() << "\"/>\n";
      svg_pattern_count++;
      ...
      {
        pFile_ << "  <polyline points=\"0," << density * .5 << " " << density
               << "," << density * .5 << "\"\n/>";
      }
      if (indexHatch_ == 2 || indexHatch_ == 3) {
        pFile_ << "  <polyline points=\"" << density * .5 << ",0 "
               << density * .5 << "," << density << "\"\n/>";
      }
      ...
      if (indexHatch_ == 5 || indexHatch_ == 6) {
        pFile_ << "  <polyline points=\"" << density << "," << density
               << " 0,0\"\n/>";
      }
      pFile_ << " </g>\n</pattern>\n"
             << "<path fill=\"url(#H_" << svg_pattern_count
             << ")\" stroke=\"none\" " << stream.str() << "\"/>\n";
      svg_pattern_count++;
    } else {
...
void SVGDriver::renderSimplePolygon(const magics::Polyline &line) const {
  ...
  if (currentShading_ == M_SH_DOT) {
    ...
    if (indexHatch_ == 2 || indexHatch_ == 3) {
      pFile_ << "  <polyline points=\"" << density * .5 << ",0 " << density * .5
             << "," << density << "\"\n/>";
    }
    if (indexHatch_ == 4 || indexHatch_ == 6) {
      pFile_ << "  <polyline points=\"0,0 " << density << "," << density
...
/*!
  ...
  \note As of version 2.0 there are two forms of describing text in Text.
   ...
   The'dy' parameter is used to shift text vertically because the
  'basline-shift' parameter does not work in Firefox 1.5 + 2.0.
  ...
  for (unsigned int tex = 0; tex < noTexts; tex++) // for all sting COORDINATES
  {
    ...
    for (; niceText < niceTextEnd; niceText++) {
      ...
      if (iter != FontMap_.end())
        ...
        MagLog::warning() << "SVGDriver: Font " << font
                          << " is not registered! Default font is used."
                          << endl;
      }
      ...
      if (niceText == text.textBegin()) {
        if (tex == 0) {
          ostringstream stream;
          stream << "text-anchor=\"" << justification << "\" font-size=\""
                 << dheight << "cm\" font-family=\"" << ttf << "\" ";
          ...
          if (text.getAngle() != 0)
            stream << "transform=\"rotate(" << text.getAngle() * 57.29577951
                   << "," << xxx << "," << yyy << ")\" ";
          stream << "fill=\"rgb("
        ...
        pFile_ << "<text x=\"" << xxx << "\" y=\"" << yyy
               << "\" dominant-baseline=\"" << verticalJustification << "\"";
...
*/
MAGICS_NO_EXPORT void SVGDriver::circle(const MFloat x, const MFloat y,
  ...
  if (s < 8 && s >= 0) {
    ...
    if (s == 0) {
      ...
      if (s == 2)
        pFile_ << "<path d=\"M" << cx << " " << cy - r << " v" << r << " h" << r
               << " a" << r << "," << r << " 0 0 0 " << -r << "," << -r
               << "\" ";
      else if (s == 4)
        pFile_ << "<path d=\"M" << cx << " " << cy - r << " v" << 2 * r << " a"
               << r << "," << r << " 0 0 0 " << 0 << "," << -2 * r << "\" ";
      else if (s == 6)
        pFile_ << "<path d=\"M" << cx << " " << cy - r << " v" << r << " h"
               << -r << " a" << r << "," << r << " 1 1 0 " << r << "," << -r
               << "\" ";
      pFile_ << "fill=\"rgb(" << static_cast<int>(currentColour_.red() * 255)
             << "," << static_cast<int>(currentColour_.green() * 255) << ","
...
*/
MAGICS_NO_EXPORT bool SVGDriver::renderPixmap(MFloat x0, MFloat y0, MFloat x1,
  ...
  for (int i = h - 1; i >= 0; i--) {
    for (int j = 0; j < w; x0 += dx, j++) {
      ...
      if (r * g * b > 0.) {
        ...
        pFile_ << " <rect x=\"" << x0 << "\" y=\"" << setY(y0) << "\" width=\""
               << dx << "\" height=\"" << dy << "\""
               << " fill=\"rgb(" << r << "," << g << "," << b << ")\" "
               << "stroke=\"none\" />\n";
      }
...
MAGICS_NO_EXPORT bool SVGDriver::renderCellArray(const Image &image) const {
...
#endif
  {
    ...
    pFile_
        << "<g pointer-events=\"none\" stroke=\"none\" transform=\"translate("
        << x0 << "," << -y0 << ") scale(" << dx << "," << dy << ")\">\n";
    for (int i = height - 1; i >= 0; i--) {
      for (int j = 0; j < width; j++) {
        ...
        if (!(lt[c] == "undefined") &&
            (lt[c].red() * lt[c].green() * lt[c].blue()) >= 0.) {
    ...
    if (!external_) {
      pFile_ << "<image x=\"" << x0 << "\" y=\"" << setY(y0) << "\" "
             << " width=\"" << projectX(image.getWidth()) << "\" height=\""
             << projectY(image.getHeight()) << "\" xlink:href=\"data:;base64,";
    ...
    } else // if external files
    {
      ...
      if (fin == 0) {
        MagLog::error() << "Open file " << filename
                        << " to be added to SVG FAILED!" << endl;
        return 1;
      } else {
        pFile_ << "<image x=\"" << x0 << "\" y=\"" << setY(y0) << "\" "
               << " width=\"" << projectX(image.getWidth()) << "\" height=\""
               << projectY(image.getHeight()) << "\" xlink:href=\"" << filename
               << "\" />\n";
      }
      if (fin)
        fclose(fin);
...
MAGICS_NO_EXPORT void SVGDriver::renderSymbols(const Symbol &symbol) const {
  ...
  if (symbol.getSymbol() == "logo_ecmwf") {
    if (inkscape_)
      pFile_
          << "<g inkscape:groupmode=\"layer\" inkscape:label=\"ECMWF_logo\">\n"
    ...
    if (!magCompare(logoLocation_, "INLINE")) {
      ...
      pFile_ << "<a xlink:href=\"http://www.ecmwf.int\">"
             << "<image x=\"" << x - (y * 1.35) << "\" y=\""
             << setY(y + (y * .5)) << "\" width=\"" << y * 5.4 << "\" height=\""
             << y << "\" xlink:href=\"" << logofile << "\" />"
             << "</a>\n";
    } else {
      pFile_ << "<g transform=\"translate(" << x - (y * 1.35) << ","
             << setY(y + (y * .5)) << ")\">\n";
      const string s = buildConfigPath("ecmwf_logo_2014.svg");
      ifstream psfile(s.c_str());

      if (!psfile) {
        MagLog::error() << "PostScriptDriver::copyMacro() --> Cannot open "
                           "PostScript Macro file! "
                        << s << " Is MAGPLUS_HOME set correctly?\n";
        return;
      }
      char ch;
      while (psfile.get(ch)) {
