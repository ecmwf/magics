...
*/
GDDriver::GDDriver()
    : currentFont_(""), currentFontSize_(0), currentPageCount_(0),
      lastPage_(-1), scaleFactor_(1), animated_(false), jpg_(false),
      gif_(false), png_(false), clipping_(true), offsetX_(0), offsetY_(0) {
  readFonts();
}
...
/*!
  \brief Destructor
*/
GDDriver::~GDDriver() { relieveFonts(); }
...
void GDDriver::open() {
  ...
  if (gdFontCacheSetup() != 0)
    MagLog::error()
        << "GDDriver::readFonts() --> Cannot initialize font cache!\n";
#endif
...
void GDDriver::close() {
  if (animated_) {
    ...
    MagLog::warning() << "GIF and Animated GIF are not supported in this "
                         "version (Too old version of GD library!)";
    animated_ = false;
    png_ = true;
#else
  ...
  if (jpg_)
    MagLog::info() << "JEPG is not a good format for scientific plots. Please "
                      "consider using GIF or PNG.\n";
...
MAGICS_NO_EXPORT void GDDriver::startPage() const {
  ...
  if (animated_ && cPage == 0) {
    ...
    gdImageGifAnimBegin(currentImage_, outFile_, 1 /*global colormap*/,
                        0 /*infinite loop*/);
  }
#endif
  ...
  gdImageFilledRectangle(currentImage_, 0, 0, dimensionXglobal_,
                         dimensionYglobal_, white);
...
MAGICS_NO_EXPORT void GDDriver::endPage() const {
  ...
  if (animated_) {
    if (currentPage_ == 0)
      ...
      gdImageGifAnimAdd(currentImage_, outFile_, 1, 0, 0, delay_, 1,
                        Images_[currentPage_ - 1]);
  } else
#endif
#endif
  {
    ...
    if (jpg_) {
      ...
      outFile_ = fopen(fileName.c_str(), "wb");
      if (outFile_) {
        gdImageJpeg(currentImage_, outFile_,
                    quality_); // quality = -1 is default
        fclose(outFile_);
        printOutputName(fileName);
      } else
...
*/
MAGICS_NO_EXPORT int GDDriver::setLineParameters(const LineStyle linestyle,
...
*/
MAGICS_NO_EXPORT void GDDriver::renderPolyline(const int n, MFloat *x,
                                               MFloat *y) const {
  if (n < 2)
    return;
  ...
  for (int l = 1; l < n; l++) {
    ...
    gdImageLine(currentImage_, (int)old_x, (int)old_y, (int)xx, (int)yy,
                currentLineStyleIndex_);
...
/*!
  ...
  \param y array of y values
*/
MAGICS_NO_EXPORT void GDDriver::renderPolyline2(const int n, MFloat *x,
                                                MFloat *y) const {
  if (n != 2)
    return;
...
/*!
  ...
  \param y array of y values
*/
MAGICS_NO_EXPORT void GDDriver::renderSimplePolygon(const int nn, MFloat *x,
                                                    MFloat *y) const {
  int n = nn;
...
GDDriver::renderSimplePolygon(const Polyline &line) const {
...
/*!
 ...
 By default line fonts are used if MAGICS_TTF is not set (see configure &
 config.h).

 \note Be careful: TTF text ploting seems to fail in multithread context (AIX
 Jan2006) \sa Text \param text object containing the strings and their
 description
*/
MAGICS_NO_EXPORT void GDDriver::renderText(const Text &text) const {
  ...
  for (; niceText < niceTextEnd; niceText++) {
    ...
    if (iter != FontMap_.end())
      ...
      ttf +=
          FontMap_["sansserif_normal"].ttf_filename; // if not found get default
      MagLog::warning() << "GDDriver: Font " << lowFont
                        << " is not registered! Default font is used." << endl;
    }
    ...
    if (!fontfile) {
      ...
      MagLog::warning() << "GDDriver (pretty): TTF font file " << ttf
                        << " could not be found! Font " << new_font
    ...
    int bbx[8];
    char *err = gdImageStringFT(NULL, &bbx[0], 0, (char *)ttf.c_str(), fontSize,
                                0., 10, 10, (char *)(*niceText).text().c_str());
    if (err)
      MagLog::error() << "GDDriver::renderText() -> Could not determine text "
                         "bounding box for >"
                      << (*niceText).text() << "< !\n";
  ...
  for (; niceText < niceTextEnd; niceText++) {
    ...
    if (err) {
      MagLog::error() << "GDDriver: gdImageStringFT (prettyText) -> " << err
                      << " in " << ttf << endl;
      return;
    } else {
      // W = text_length;
    ...
    {
      const int x0 =
          static_cast<int>(E1xcsE1yss + projectX(text[nT].x())); // lower left
      const int y0 = static_cast<int>(E1xssE1ycs + projectY(text[nT].y()));

      if (text.getBlanking()) {
        err = gdImageStringFT(NULL, &bbx[0], 0, (char *)ttf.c_str(), fontSize,
                              LastAngle_, x0, y0,
        ...
        if (!err) {
          ...
          MagLog::error() << "GDDriver: gdImageStringFT -> " << err
                          << " -> NO blanking!!!" << endl;
      }
      err = gdImageStringFT(currentImage_, &bbx[0], currentColourIndex_,
                            (char *)ttf.c_str(), fontSize, LastAngle_, x0, y0,
                            (char *)(*niceText).text().c_str());
      if (err) {
        MagLog::error() << "GDDriver: gdImageStringFT -> " << err << endl;
        gdImageString(currentImage_, gdFontGetLarge(), x0, y0,
                      (unsigned char *)(*niceText).text().c_str(),
                      currentColourIndex_);
      }
      delete[] err;
    }
...
*/
MAGICS_NO_EXPORT void GDDriver::circle(const MFloat x, const MFloat y,
  ...
  if (s < 8) {
    ...
    if (s > 0)
      gdImageFilledArc(currentImage_, cx, cy, rx, ry, 270, 270 + (s * 45),
                       currentLineStyleIndex_, gdArc);
  } else
    gdImageFilledArc(currentImage_, cx, cy, rx, ry, 0, 360,
                     currentLineStyleIndex_, gdArc);

  if (s == 9) {
    const int col = gdImageColorResolveAlpha(currentImage_, 255, 255, 255, 0);
    gdImageLine(this->currentImage_, cx, static_cast<int>(cy + r - 1), cx,
                static_cast<int>(cy - r + 1), col);
  }
}
...
*/
MAGICS_NO_EXPORT bool GDDriver::renderPixmap(MFloat x0, MFloat y0, MFloat x1,
  ...
  for (int i = height - 1; i >= 0; i--) {
    for (int j = 0; j < width; j++) {
      ...
      if (!(lt[c] == "undefined")) {
        ...
        gdImageFilledRectangle(this->currentImage_, wx, wy,
                               static_cast<int>(wx + dx),
