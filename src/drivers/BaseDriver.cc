...
BaseDriver::BaseDriver()
    : currentPage_(-1), fileName_(""), currentLayer_(""),
      currentLineType_(M_SOLID), currentLineWidth_(-1), currentLineStyle_(1),
      currentColour_(Colour("white")), coordRatioX_(1), coordRatioY_(1),
      ...
      xDeviceLength_(
          MagTranslator<double, double>().magics("SUPER_PAGE_X_LENGTH")),
      yDeviceLength_(
          MagTranslator<double, double>().magics("SUPER_PAGE_Y_LENGTH")),
      obs_distance_(-1.) {}

BaseDriver::~BaseDriver() { FontMap_.clear(); }
...
void BaseDriver::printOutputName(const std::string &str) const {
  if (filelist_) {
    ...
    if (numFiles_ == 0) {
      ...
      fs << "# " << getMagicsVersionString() << " " << info.getHostName() << " "
         << info.getTime() << "\n";
    } else
...
void BaseDriver::readFonts() const {
  ...
  if (psfile) {
    ...
    while (!psfile.eof()) {
      magFont font;
      psfile >> id >> magics_name >> ps_name >> ps_filename >> ttf_filename >>
          css_name;
      font.id = id;
    ...
    MagLog::error() << "BaseDriver::readFonts() --> Cannot open Font file " << s
                    << " ! No text can be plotted.\n";
}
...
*/
string BaseDriver::getFileName(const string &extension,
  ...
  if (numberWidth > 4) {
    numberWidth = 4;
    MagLog::warning() << "Output --> NumberingWidth only allows values between "
                         "1 and 4. 4 is used now.\n";
  }
...
/*!
  ...
  This methods processes the Layout objects. It needs to be checked if a Layout
  is a new page or not. \sa Layout
*/
...
void BaseDriver::redisplay(const Arrow &arrow) const { renderWindArrow(arrow); }

void BaseDriver::redisplay(const Flag &flag) const { renderWindFlag(flag); }
...
void BaseDriver::redisplay(const Text &text) const { renderText(text); }
...
void BaseDriver::redisplay(const TextItem &text,
                           const ComplexSymbol &symbol) const {
  renderTextItem(text, symbol);
}

void BaseDriver::redisplay(const SymbolItem &text,
                           const ComplexSymbol &symbol) const {
  renderSymbolItem(text, symbol);
}

void BaseDriver::redisplay(const FlagItem &text,
                           const ComplexSymbol &symbol) const {
...
double BaseDriver::LSF(MFloat *x, MFloat *y, int i0) const {
  ...
  if ((abs(sxx) > 0.00001)) {
    ...
    angle = 10.;
    MagLog::debug() << "BaseDriver: Devision through zero prevented in "
                       "calculation of Label angle!"
                    << endl;
  }
  return angle;
}
...
void BaseDriver::printLine(const magics::Polyline &line) const {
  ...
  const unsigned int minimum_points_for_labelling =
      25; // must be at least -15 : see function LSF above
  ...
  if ((arrowHead ||
    ...
    MFloat *labelx =
        new MFloat[n]; // in theory, we shouldn't need this many entries...
    MFloat *labely =
        new MFloat[n]; // in theory, we shouldn't need this many entries...

    // Store the minimum x,y corner, pretending that we're going to plot a label
    // there. We won't actually plot a label there though - it's just to make
    // sure that we don't plot one too close to that point.
    int num_labels = 1;
    labelx[0] = staLayouts_.top()->minX();
    labely[0] = staLayouts_.top()->minY();

    // Calculate how far apart the labels should be.
    // Our algorithm is to take the average of the width and height and divide
    // by 2. This should give us approximately 3 labels over the width of the
    // page.
    const MFloat coords_range_x =
        fabs(staLayouts_.top()->maxX() - staLayouts_.top()->minX());
    const MFloat coords_range_y =
        fabs(staLayouts_.top()->maxY() - staLayouts_.top()->minY());
    const MFloat min_distance_between_labels =
        (coords_range_x + coords_range_y) / 4.;
    const MFloat min_square_distance_between_labels =
        (min_distance_between_labels * min_distance_between_labels);
    ...
    while (i < n - minimum_points_for_labelling) {
      ...
      if ((angle < 4.) && (angle2 < 4.) && fabs(angle - angle2) < 0.01) {
        ...
        const double distance_squared =
            ((THIS_X - PREV_X) * (THIS_X - PREV_X)) +
            ((THIS_Y - PREV_Y) * (THIS_Y - PREV_Y));

        if ((arrowHead) ||
            (distance_squared > min_square_distance_between_labels)) {
          if (arrowHead) {
            MFloat pro_x = x[i];
            MFloat pro_y = y[i];
...
*/
void BaseDriver::print(ostream &out) const { out << "BaseDriver"; }
...
bool BaseDriver::renderCellArray(const Image &) const { return true; }

bool BaseDriver::renderPixmap(MFloat, MFloat, MFloat, MFloat, int, int,
                              unsigned char *, int, bool) const {
  return true;
}
...
void BaseDriver::redisplay(const SceneLayer &layer) const {
  for (vector<Layer *>::iterator l = layer.beginLayer(); l != layer.endLayer();
       ++l) {
    (*l)->redisplay(*this);
  }
}
...
*/
void BaseDriver::setDimensionsFromBinary(string mbg_tmpl, MFloat &ratio,
                                         int &width) const {
  ...
  if (in.is_open()) {
    ...
    if (strncmp("MAGICS", mag, 6) != 0) {
      MagLog::error()
          << "Magics number of TEMPLATE binary file " << mbg_tmpl
          << " is wrong!\n"
          << "   Please check endiness and/or if you gave the right filename."
          << endl;
      return;
    }
    ...
    if (checksum != 10) {
      MagLog::error() << "Checksum of TEMPLATE binary file " << mbg_tmpl
                      << " is wrong!\n"
    ...
    if (version != BINARY_VERSION)
      MagLog::error() << " MGB VERSION MISMATCH " << version
                      << " != " << BINARY_VERSION << endl;
    int lengthHeader;
    ...
    MagLog::error() << "TEMPLATE binary file " << mbg_tmpl
...
void BaseDriver::redisplay(const BinaryObject &binary) const {
  ...
  if (in.is_open()) {
    ...
    if (strncmp("MAGICS", mag, 6) != 0) {
      MagLog::error()
          << "Magics number of binary file " << mbg_tmpl.c_str()
          << " is wrong!\n"
          << "   Please check endiness and if you gave the right filename."
          << endl;
      return;
    }
    ...
    in.read((char *)(&checksum), sizeof(int));
    if (checksum != 10) {
      MagLog::error() << "Checksum of binary file " << mbg_tmpl.c_str()
                      << " is wrong!\n"
                      << "   Please check endiness of your file." << endl;
      return;
    }
    ...
    while (in.get(c)) {
      ...
      case 'I': // Images
      {
        ...
        for (int i = 0; i < d; i++)
          object.push_back(
              pixels[i]); // object(std::begin(pixels), std::end(pixels));
