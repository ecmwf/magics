...
void Skewt::init() {
  // When default input visualiser values are specified: we assing a meaningful
  // default
  ...
  if (x_min_ >= x_max_) {
    throw MagicsException("Tephigram: minimum temperature cannot be greater "
                          "than maximum temperature");
  }

  if (y_min_ <= y_max_) {
    throw MagicsException(
        "Tephigram: top pressure cannot be greater than bottom pressure");
  }
...
PaperPoint Skewt::operator()(const UserPoint &xy) const {
  ...
  if (xy.x() >= 1000) {
    ...
    double y = (maxPCY_ - minPCY_) * (log(y_min_) - log(xy.y())) /
               (log(y_min_) - log(y_max_));
    return PaperPoint(x, y, xy.value());
  }

  // Main skewt area
  double t = xy.x(); // t is linear
  double p = (maxPCY_ - minPCY_) * (log(y_min_) - log(xy.y())) /
             (log(y_min_) - log(y_max_));

  double tc = x_min_ + (x_max_ - x_min_) / 2.;
  double x =
      (t - tc) * (maxpcx - minPCX_) / (0.5 * (x_max_ - x_min_)) + (p - minPCY_);
  double y = p;
...
void Skewt::revert(const PaperPoint &pt, UserPoint &point) const {
  double p = exp(log(y_min_) -
                 pt.y() * (log(y_min_) - log(y_max_)) / (maxPCY_ - minPCY_));
  double tc = x_min_ + (x_max_ - x_min_) / 2.;
  double t = tc + (pt.x() - (p - minPCY_)) * (0.5 * (x_max_ - x_min_)) /
                      (maxpcx - minPCX_);
...
void Skewt::revert(const vector<std::pair<double, double>> &in,
                   vector<std::pair<double, double>> &out) const {
  out.reserve(in.size());
  double tc = x_min_ + (x_max_ - x_min_) / 2.;
  for (vector<std::pair<double, double>>::const_iterator it = in.begin();
       it != in.end(); ++it) {
    double p = exp(log(y_min_) - it->second * (log(y_min_) - log(y_max_)) /
                                     (maxPCY_ - minPCY_));
    double t = tc + (it->first - (p - minPCY_)) * (0.5 * (x_max_ - x_min_)) /
                        (maxpcx - minPCX_);
    out.push_back(make_pair(t, p));
  }
}

bool Skewt::needShiftedCoastlines() const { return false; }
...
void Skewt::boundingBox(double &xmin, double &ymin, double &xmax,
                        double &ymax) const {
...
double Skewt::getMinX() const { return -1.5; }

double Skewt::getMinY() const { return y_min_; }
...
double Skewt::getMaxY() const { return y_max_; }
...
double Skewt::getMinPCX() const { return minPCX_; }

double Skewt::getMinPCY() const { return minPCY_; }

double Skewt::getMaxPCX() const { return maxPCX_; }

double Skewt::getMaxPCY() const { return maxPCY_; }

double Skewt::getMaxTestPCX() const { return maxpcx; }
...
static void toxml2(string &out, const map<string, string> &def) {
  ostringstream os;
  string sep = "";
  for (map<string, string>::const_iterator entry = def.begin();
       entry != def.end(); ++entry) {
...
void Skewt::getNewDefinition(const UserPoint &ll, const UserPoint &ur,
                             string &out) const {
...
void Skewt::operator()(const Polyline &from,
