...
template <class P> RasterData<P>::RasterData() : projection_(0) {}

template <class P> RasterData<P>::~RasterData() {
  if (projection_)
    delete projection_;
}
...
*/
template <class P> void RasterData<P>::print(ostream &out) const {
  out << "RasterData<P>[";
  out << "xRes=" << x_;
  out << ", yRes=" << y_;
