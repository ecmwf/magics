...
XmlNode::XmlNode(const XmlNode &from) : name_(from.name_) { copy(from); }
...
void XmlNode::copy(const XmlNode &from) {
  ...
  // From the node..
  for (map<string, string>::const_iterator attr = from.attributes_.begin();
       attr != from.attributes_.end(); ++attr) {
...
XmlNode::XmlNode(const XmlNode &from, const map<string, string> &def) {
  copy(from);
  for (map<string, string>::const_iterator attr = def.begin();
       attr != def.end(); ++attr) {
    attributes_.insert(make_pair(attr->first, attr->second));
  }
}
...
void XmlNode::print(ostream &out) const {
  ...
  for (map<string, string>::const_iterator attr = attributes_.begin();
       attr != attributes_.end(); ++attr) {
    out << " " << attr->first << "='" << attr->second << "'";
  }
  out << ">\n";
  for (vector<string>::const_iterator data = data_.begin(); data != data_.end();
       ++data) {
    out << *data << "\n";
  }
  for (const auto &elt : elements_)
