...
void output(void *, const char *msg) { cout << msg; }

MagLog::MagLog()
    : reporter_(0), devnull_("/dev/null"), debug_(true), dev_(true),
      info_(true), userInfo_(true), warning_(true), error_(true), fatal_(true),
      profiling_(true), warnings_(0) {
...
static void niceprint(int nb, const string &legend, const string &sep,
                      ostream &out) {
  if (!nb)
    return;
...
ostream &MagLog::warning() {
  ...
  if (log_.warning_) {
    if (log_.warnings_++ > 10) {
      // log_.warningstream_ << "Magics-warning: Too many warnings! Stop sending
      // them ..." << endl;
...
void MagLog::progress(const string &msg) {
  for (vector<MagicsObserver *>::iterator observer = log_.observers_.begin();
       observer != log_.observers_.end(); ++observer) {
...
void MagLog::broadcast() {
  bool more = true;
  for (vector<MagLogObserver *>::iterator observer = log_.listeners_.begin();
       observer != log_.listeners_.end(); ++observer) {
...
void MagLog::unregisterObserver(MagicsObserver *observer) {
  vector<MagicsObserver *>::iterator o =
      std::find(log_.observers_.begin(), log_.observers_.end(), observer);
