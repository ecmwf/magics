...
void Data::computeStats() {
  ...
  if (info("statsType") == "scatter" && itX != stats_.end() &&
      itY != stats_.end()) {
    ...
    for (std::map<string, DataStats *>::iterator it = st.begin();
         it != st.end(); it++) {
    ...
    setInfo("stats::correlation", tostring(DataStats::correlation(
                                      itX->second, itY->second, stX, stY)));
  }

  else if (info("statsType") == "vector" && itX != stats_.end() &&
           itY != stats_.end()) {
    DataStats stX(itX->second);
    DataStats stY(itY->second);
    ...
    for (std::map<string, DataStats *>::iterator it = st.begin();
      ...
      if (it->second->hasStDev()) {
        ...
        if (it->second->hasThirdMoment()) {
          setInfo("stats::skewness" + it->first,
                  tostring(it->second->skewness()));
          setInfo("stats::kurtosis" + it->first,
                  tostring(it->second->kurtosis()));
        }
      }
    }
  }
...
DataList::DataList() { current_ = data_.begin(); }
DataList::~DataList() {}

void DataList::setToFirst() { current_ = data_.begin(); }

Data *DataList::current() { return *current_; }

bool DataList::hasMore() { return (current_ != data_.end()); }

void DataList::next() { ++current_; }

void DataList::add(Data *data) { data_.push_back(data); }
