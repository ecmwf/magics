...
void convert(const deque<PaperPoint> &in, ClipperLib::Path &out,
             bool print = false) {
  out.reserve(in.size());
  if (print)
    cout << " -----------start---------------------------" << endl;
  for (auto pt = in.begin(); pt != in.end(); ++pt) {
    if (print)
      cout << "     subj.push_back(MyPoint(" << long(pt->x_ * scale_) << ", "
           << long(pt->y_ * scale_) << ").get());" << endl;
    out.push_back(
        ClipperLib::IntPoint(long(pt->x_ * scale_), long(pt->y_ * scale_)));
  }
  int orientation = ClipperLib::Orientation(out);
  if (print)
    cout << " -------------------" << orientation << "----" << out.size()
         << "---------------------" << endl;
}
...
void MagClipper::clipOpened(const Polyline &subject, const Polyline &clip,
                            vector<Polyline *> &out) {
...
void MagClipper::clip(const Polyline &subject, const Polyline &clip,
                      vector<Polyline *> &out) {
  if (subject.empty())
    return;
  /*
      ...
      cout << " igmore" << endl;
    */
  return subject.closed() ? clipClosed(subject, clip, out)
                          : clipOpened(subject, clip, out);
}

void MagClipper::clip(const Polyline &subject, const PaperPoint &lowerleft,
                      const PaperPoint &upperright,
...
void MagClipper::clipClosed(const Polyline &subject, const Polyline &clip,
                            vector<Polyline *> &out) {
...
void MagClipper::add(const Polyline &subject, const Polyline &clip,
                     vector<Polyline *> &out) {
  // Create Path from outer and holes!
  // First Outer!
  ClipperLib::Path path_subject, hole_subject, path_clip;
...
bool MagClipper::in(const Polyline &poly, const PaperPoint &point) {
  ...
  return ClipperLib::PointInPolygon(
      ClipperLib::IntPoint(long(point.x_ * scale_), long(point.y_ * scale_)),
      path);
}
