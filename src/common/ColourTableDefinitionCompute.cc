...
ColourTableDefinitionCompute::ColourTableDefinitionCompute(
    const string &min, const string &max, const string &method,
...
void ColourTableDefinitionCompute::set(
    const ColourTableDefinitionComputeInterface &attributes) {
...
void ColourTableDefinitionCompute::set(const XmlNode &node) {
  ...
  MagLog::warning() << "ColourTableDefinitionCompute::set(const XmlNode&): to "
                       "be implemented\n";
...
void ColourTableDefinitionCompute::hsl(ColourTable &table, int nb) {
  ...
  */
  ...
  for (int i = 0; i < nb - 1; i++) {
    MagLog::dev() << "ColourTableDefinitionCompute::set->add-->" << Colour(hmin)
                  << endl;
...
void ColourTableDefinitionCompute::set(ColourTable &table, int nb) {
  ...
  MagLog::dev() << "ColourTableDefinitionCompute::set->min-->" << minColour_
                << endl;
  MagLog::dev() << "ColourTableDefinitionCompute::set->max-->" << maxColour_
                << endl;
  ...
  std::map<string, ComputeFunction>::iterator method =
      methods_.find(lowerCase(method_));
...
void ColourTableDefinitionCompute::hcl(const Colour &colour, float &h, float &c,
                                       float &l) {
  float x, y, z;
...
Colour ColourTableDefinitionCompute::rgb(float h, float c, float l,
                                         float alpha) {
  float x, y, z;
  float r, g, b;
...
void ColourTableDefinitionCompute::xyzToRgb(float x, float y, float z, float &r,
                                            float &g, float &b) {
...
void ColourTableDefinitionCompute::xyzToHcl(float x, float y, float z, float &h,
                                            float &c, float &l) {
  x /= REF_X;
  y /= REF_Y;
  z /= REF_Z;
...
void ColourTableDefinitionCompute::hclToXyz(float h, float c, float l, float &x,
                                            float &y, float &z) {
...
void ColourTableDefinitionCompute::rgbToXyz(float r, float g, float b, float &x,
                                            float &y, float &z) {
  if (r > 0.04045)
