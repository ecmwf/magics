...
template <class T>
void TableDefinitionCompute<T>::set(
    const TableDefinitionComputeInterface<T> &attributes) {
  count_ = attributes.getCount();
}

template <class T> inline void fromString(const string &str, T &out) {
  std::stringstream ss(str);
  ss >> out;
}

template <class T> void TableDefinitionCompute<T>::set(const XmlNode &node) {
  MagLog::warning()
      << "TableDefinitionCompute::set(const XmlNode&): to be implemented\n";
  MagLog::dev() << "Node to be interpreted ---> " << node << endl;

  fromString(node.getAttribute("count"), count_);
}

template <class T> void TableDefinitionCompute<T>::adjust(T min, T max) {
