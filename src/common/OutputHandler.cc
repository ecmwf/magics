...
void OutputHandler::set(DriverManager &magics) {
  ...
  for (vector<string>::const_iterator format = formats_.begin();
       format != formats_.end(); ++format) {
  ...
  if (formats_.size() == 1 &&
      (formats_.front() == "png" || formats_.front() == "pdf"))
    lineSpacing_ = 1.;
}
...
void OutputHandler::set(const XmlNode &node, DriverManager &magics) {
  try {
    OutputFactory *factory =
        MagTranslator<string, OutputFactory>()(node.name());
...
static SimpleObjectMaker<GD_GifAnimOutputFactory, OutputFactory>
...
static SimpleObjectMaker<CAIRO_CairoOutputFactory, OutputFactory>
    cairo("cairo");
static SimpleObjectMaker<CAIRO_SvgOutputFactory, OutputFactory>
    csvg("cairo_svg");
static SimpleObjectMaker<CAIRO_PsOutputFactory, OutputFactory> cps("cairo_ps");
static SimpleObjectMaker<CAIRO_EpsOutputFactory, OutputFactory>
    ceps("cairo_eps");
#ifdef HAVE_GEOTIFF
static SimpleObjectMaker<CAIRO_GeoTiffOutputFactory, OutputFactory>
    geotiff("geotiff");
#endif
#endif

static SimpleObjectMaker<KML_KmlOutputFactory, OutputFactory> kml("kml");
static SimpleObjectMaker<GEOJSON_GeoJsonOutputFactory, OutputFactory>
    geojson("geojson");
