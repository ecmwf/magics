...
Colour::Colour() : rgb_(1., 0., 0., 1.), automatic_(false) { init(); }
...
Colour::Colour(float red, float green, float blue, float alpha)
  ...
  ostringstream name;
  name << "rgba(" << int(red * 255) << "," << int(green * 255) << ","
       << int(blue * 255) << "," << alpha << ")"
       << "\n";
  name_ = name.str();
}
    ...
    if (colour == colours_.end()) {
...

      {
        ...
        if (name_.size() == 7) {
          ...
          MagLog::warning()
              << "The color [" << name_ << "] is not valid. Red used.\n";
          name_ = "red";
          rgb_ = Rgb(1., 0., 0., 1.);
        }
      } else {
        MagLog::warning() << "The color [" << name_
                          << "] is not valid. Red used.\n";
        name_ = "red";
        rgb_ = Rgb(1., 0., 0., 1.);
      }
    } else
...
void Colour::print(ostream &out) const {
  out << "[" << rgb_.red_ << ", " << rgb_.green_ << ", " << rgb_.blue_ << ", "
      << rgb_.alpha_ << "] ";
}

Hsl Colour::hsl() const { return rgb_.hsl(); }
