...
MagFont::MagFont(const string &name) : name_(name), size_(0.5) {}

MagFont::MagFont() : name_("sansserif"), size_(0.5) {}

MagFont::~MagFont() { styles_.clear(); }
...
void MagFont::print(ostream &out) const {
  ...
  for (set<string>::const_iterator style = styles_.begin();
       style != styles_.end(); ++style)
    out << *style << ", ";
  out << size_ << "]";
}
