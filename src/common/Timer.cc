...
Timer::Timer(const string &name, const string &detail)
    : name_(name), details_(detail), cpu_(clock()) {
...
Timer::~Timer() {
...

  {
    ...
    profiles_.push_back(
        ProfileInfo(name_, details_, start, stop, e.str(), c.str()));
  }
}
...
ProfileInfo::ProfileInfo(const string &name, const string &details,
    ...
    : name_(name), details_(details), start_(start), stop_(stop),
      elapsed_(elapsed), cpu_(cpu) {}
