...
void Emagram::revert(const PaperPoint &pt, UserPoint &point) const {
  double t =
      x_min_ + (x_max_ - x_min_) * (pt.x() - minPCX_) / (maxpcx - minPCX_);
...
void Emagram::revert(const vector<std::pair<double, double>> &in,
                     vector<std::pair<double, double>> &out) const {
  out.reserve(in.size());
  for (vector<std::pair<double, double>>::const_iterator it = in.begin();
       it != in.end(); ++it) {
    double t =
        x_min_ + (x_max_ - x_min_) * (it->first - minPCX_) / (maxpcx - minPCX_);
...
bool Emagram::needShiftedCoastlines() const { return false; }
...
void Emagram::boundingBox(double &xmin, double &ymin, double &xmax,
                          double &ymax) const {
...
double Emagram::getMinX() const { return -1.5; }

double Emagram::getMinY() const { return y_min_; }
...
double Emagram::getMaxY() const { return y_max_; }
...
double Emagram::getMinPCX() const { return minPCX_; }

double Emagram::getMinPCY() const { return minPCY_; }

double Emagram::getMaxPCX() const { return maxPCX_; }

double Emagram::getMaxPCY() const { return maxPCY_; }

double Emagram::getMaxTestPCX() const { return maxpcx; }
...
void Emagram::getNewDefinition(const UserPoint &ll, const UserPoint &ur,
                               string &out) const {
...
void Emagram::operator()(const Polyline &from,
