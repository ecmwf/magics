...
Symbol::Symbol()
    : marker_(0), height_(0.2), outline_(false), connectLine_(false) {}
...
SymbolProperties::SymbolProperties(Colour colour, double height,
                                   const string &marker, const string &label)
    : colour_(colour), height_(height), marker_(marker), label_(label),
      position_(Symbol::M_NONE), outline_(false), connectLine_(false),
      image_(false) {
  font_.size(height_);
  font_.colour(colour_);
}

SymbolProperties::SymbolProperties(Colour colour, double height, int marker,
                                   const string &label)
    : colour_(colour), height_(height), marker_(Symbol::convert(marker)),
...
SymbolProperties::SymbolProperties()
    : colour_(Colour("none")), height_(0), marker_(Symbol::convert(1)),
...
Symbol *SymbolProperties::symbol(const string &type) const {
  ...
  symbol->connectline(connectLine_, connectLineColour_, connectLineThickness_,
                      connectLineStyle_);
...
void Symbol::redisplay(const BaseDriver &driver) const {
  ...
  for (vector<PaperPoint>::const_iterator point = begin(); point != end();
       ++point) {
  ...
  vector<PaperPoint>::iterator last = std::remove_if(
      symbol.begin(), symbol.end(), WithinFunction(boundingbox_));
  ...
  for (vector<Polyline *>::const_iterator l = lines.begin(); l != lines.end();
       ++l) {
    driver.redisplay(**l);
  }
}
