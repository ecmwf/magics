...
MagException::~MagException() THROW_NOTHING() { first() = next_; }
...
void MagException::reason(const string &w) { what_ = w; }

bool MagException::throwing() { return first() != 0; }
...
FailedSystemCall::FailedSystemCall(const char *msg, int line, const char *file,
                                   const char *proc, int err) {
  cout << "Failed system call: " << msg << " in " << proc << ", line " << line
       << " of " << file << endl;
}
...
AssertionFailed::AssertionFailed(const char *msg, int line, const char *file,
                                 const char *proc) {
  cout << "Assertion failed: " << msg << " in " << proc << ", line " << line
       << " of " << file << endl;
}
...
NotImplemented::NotImplemented(int line, const char *file, const char *proc) {
  cout << "Not implemented: " << proc << ", line " << line << " of " << file
       << endl;
}
