...
pair<int, bool> InfoIndex::index(double pos) const {
  ...
  return std::make_pair(floor((pos - first_) / step_),
                        fmod(pos - first_, step_) == 0);
}
...
ProjectedMatrix::ProjectedMatrix(int rows, int columns)
    : Matrix(rows, columns) {
...
double Matrix::interpolate(double i, double j) const {
  ...
  if (columns() == 0 || j < xleft || j > xright) return missing_;
  if (i < ybottom || i > ytop) return missing_;
  int ii = rowIndex(i);
  if (ii == -1) {
...
#include <limits>
pair<double, double> Matrix::nearest_value(double row, double column,
                                           double& rowOut,
                                           double& colOut) const {
  double col, offset;
  ...
  vector<pair<double, map<double, pair<double, double> >::const_iterator> >
      points;
  ...
  if (row_index != index_.end()) {
    ...
    if (column_index == row_index->second.end() ||
        column_index == row_index->second.begin()) {
      rowOut = missing();
  ...
  for (vector<
           pair<double, map<double, pair<double, double> >::const_iterator> >::
           iterator point = points.begin();
...
int Matrix::nearest_index(double row, double column, double& rowOut,
                          double& colOut) const {
  double col, offset;

  int factor = (column - minX()) / 360.;

  if (column - minX() < 0) factor--;
  ...
  if (row_index != yIndex_.end()) {
    ...
    if (column_index.second) {
      ...
      points.push_back(make_pair(
          row,
          make_pair(xIndex_[row_index->second].value(column_index.first),
                    xIndex_[row_index->second].position(column_index.first))));
      points.push_back(make_pair(
          row,
          make_pair(
              xIndex_[row_index->second].value(column_index.first + 1),
  ...
  if (column_index.second)
    points.push_back(make_pair(row_index->first,
                               make_pair(xindex.value(column_index.first),
                                         xindex.position(column_index.first))));
  else {
    points.push_back(make_pair(row_index->first,
                               make_pair(xindex.value(column_index.first),
                                         xindex.position(column_index.first))));
    points.push_back(make_pair(
        row_index->first, make_pair(xindex.value(column_index.first + 1),
  ...
  double min = numeric_limits<double>::infinity();
  int value = -1;
  for (vector<pair<double, pair<double, int> > >::const_iterator point =
           points.begin();
       point != points.end(); ++point) {
    double dist = geodistance(point->first, point->second.first, row, col);
    if (dist < min) {
...
double Matrix::nearest(double row, double col, double& rowOut,
                       double& colOut) const {
  double xleft = std::min(left(), right());
  double xright = std::max(left(), right());
  double ybottom = std::min(bottom(), top());
  ...
  if (columns() == 0 || col < xleft || col > xright) return missing_;
  if (columns() == 0 || row < ybottom || row > ytop) return missing_;

  double row1, row2, col1, col2;
  int rowIdx1, rowIdx2, colIdx1, colIdx2;

  boundRow(row, row1, rowIdx1, row2, rowIdx2);
  if (rowIdx1 == -1 || rowIdx2 == -1) return missing_;

  boundColumn(col, col1, colIdx1, col2, colIdx2);
  if (colIdx1 == -1 || colIdx2 == -1) return missing_;
...
#include "MatrixHandler.h"
GeoBoxMatrixHandler::GeoBoxMatrixHandler(const AbstractMatrix& matrix,
  ...
  bool global =
      (matrix_.regular_column(columns - 1) - matrix_.regular_column(0)) >
      (360. - 2 * matrix_.XResolution());

  if (!global) {
    lon = matrix_.column(0, 0) - step;
    if (minlon <= lon && lon <= maxlon) lons[lon] = -1;
  ...
  for (int i = 0; i < columns; i++) {
    ...
    if (minlon <= lon && lon <= maxlon) lons[lon] = i;
  ...
  int i = 0;
  for (map<double, int>::const_iterator entry = lons.begin();
       entry != lons.end(); ++entry) {
  ...
  i = 0;
  for (map<double, int>::const_iterator entry = lats.begin();
       entry != lats.end(); ++entry) {
...
Proj4MatrixHandler::Proj4MatrixHandler(const AbstractMatrix& matrix,
                                       const string& proj)
    : MatrixHandler(matrix), projHelper_(proj) {
  internal_ = false;
}

RotatedMatrixHandler::RotatedMatrixHandler(const AbstractMatrix& matrix,
...
double Proj4MatrixHandler::interpolate(double row, double column) const {
  if (internal_) return MatrixHandler::interpolate(row, column);
  double x = column;
  double y = row;
...
double Proj4MatrixHandler::nearest(double row, double column) const {
  if (internal_) return MatrixHandler::nearest(row, column);
  double x = column;
  double y = row;
...
int RotatedMatrix::nearest_index(double lat, double lon, double& nlat,
                                 double& nlon) const {
...
pair<double, double> RotatedMatrixHandler::rotate(double lat_y,
                                                  double lon_x) const {
...
pair<double, double> RotatedMatrixHandler::unrotate(double lat_y,
                                                    double lon_x) const {
  const double cToRadians = M_PI / 180.0;
  double ZRADI = 1. / cToRadians;
  double ZSYCEN = sin(cToRadians * (southPoleLat_ + 90.));
