...
void CartesianTransformation::adjustXAxis(Layout &layout) const {
  MagLog::dev() << "CartesianTransformation::adjustXAxis()--->[" << dataMinX_
                << ", " << dataMaxX_ << "]\n";
...
void CartesianTransformation::adjustYAxis(Layout &layout) const {
  MagLog::dev() << "CartesianTransformation::adjustYAxis()--->[" << dataMinY_
                << ", " << dataMaxY_ << "]\n";
...
MAGICS_NO_EXPORT void CartesianTransformation::operator()(Layout &) const {
  /*
  if (layout["drawing_area"])
  layout["drawing_area"]->setCoordinates(x_->minpc(), x_->maxpc(), y_->minpc(),
  y_->maxpc()); if (layout["top_comment_box"])
  layout["top_comment_box"]->setCoordinates(x_->minpc(), x_->maxpc(), -10, 10);
  if (layout["bottom_comment_box"])
  layout["bottom_comment_box"]->setCoordinates(x_->minpc(), x_->maxpc(), -10,
  10); if (layout["left_comment_box"])
  layout["left_comment_box"]->setCoordinates(-10, 10, y_->minpc(), y_->maxpc());
  if (layout["right_comment_box"])
  layout["right_comment_box"]->setCoordinates(-10, 10, y_->minpc(),
  y_->maxpc()); if (layout["title"]) layout["title"]->setCoordinates(0, 100, 0,
  100); if (layout["legend"]) layout["legend"]->setCoordinates(0, 100, 0, 100);
  */
}
...
void CartesianTransformation::visit(MetaDataVisitor &visitor, double left,
...
void CartesianTransformation::boxes() const {
  ...
  PCEnveloppe_->box(PaperPoint(x_->minpc(), y_->minpc()),
                    PaperPoint(x_->maxpc(), y_->maxpc()));
}

void CartesianTransformation::getNewDefinition(const UserPoint &ll,
                                               const UserPoint &ur,
                                               string &out) const {
...
void CartesianTransformation::reprojectComponents(
    double &x, double &y, pair<double, double> &) const {
  fast_reproject(x, y);
}
