...
void Polyline::newHole() { holes_.push_back(deque<PaperPoint>()); }
...
void Polyline::hole(Holes::const_iterator hole, vector<double> &x,
                    vector<double> &y) const {
  x.reserve(hole->size());
  y.reserve(hole->size());

  for (deque<PaperPoint>::const_iterator h = hole->begin(); h != hole->end();
       ++h) {
    x.push_back(h->x_);
    y.push_back(h->y_);
  }
...
void Polyline::southClean(bool add) {
  if (!add) {
    auto from =
        std::remove_if(polygon_.begin(), polygon_.end(), SouthCleaner());
...
void Polyline::reproject(const Transformation &transformation) {
  auto from = std::remove_if(polygon_.begin(), polygon_.end(),
                             ReprojectHelper(transformation));
  polygon_.erase(from, polygon_.end());
...
Polyline *Polyline::clone() const {
  ...
  // Now the holes!
  for (Holes::const_iterator hole = holes_.begin(); hole != holes_.end();
       ++hole) {
    to->newHole();
    for (auto h = hole->begin(); h != hole->end(); ++h) {
      to->push_back_hole(*h);
...
void feed(const deque<PaperPoint> &points, const Polyline &box,
          vector<Polyline *> &out) {
  Polyline *poly = new Polyline();
...
void Polyline::push_front(Polyline &other) {
  ...
  polygon_.insert(polygon_.begin(), other.polygon_.begin(),
                  other.polygon_.end());
}
