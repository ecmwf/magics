...
Transformation::Transformation()
    : coordinateType_(GeoType), dataMinX_(std::numeric_limits<double>::max()),
      dataMaxX_(-std::numeric_limits<double>::max()),
      dataMinY_(std::numeric_limits<double>::max()),
      dataMaxY_(-std::numeric_limits<double>::min()), topAxis_(true), xTile_(0),
      yTile_(0), zoomLevel_(0) {
...
void Transformation::print(ostream &out) const { out << "Transformation"; }

void Transformation::forceNewArea(double xpcmin, double ypmin, double xpcmax,
                                  double ypcmax, double &width,
                                  double &height) {}
#define PRINT(w, a)                                                            \
...
void Transformation::thin(MatrixHandler &matrix, double x, double y,
...
ViewFilter::ViewFilter(double xmin, double xmax, double ymin, double ymax,
                       double xres, double yres)
    : xmin_(xmin), xmax_(xmax), ymin_(ymin), ymax_(ymax), xres_(xres),
      yres_(yres) {
...
void Transformation::setDataMinMaxX(double minx, double maxx,
...
void Transformation::setDataMinMaxY(double miny, double maxy,
                                    const string &ref) const {
...
void Transformation::visit(MetaDataVisitor &visitor, double left, double top,
                           double width, double height, double imgwidth,
                           double imgheight) {
  ostringstream java;
...
void Transformation::boundingBox(double &minx, double &miny, double &maxx,
                                 double &maxy) const {
...
void Transformation::thin(PointsHandler &points, vector<PaperPoint> &thin,
...
void Transformation::thin(MatrixHandler &points, vector<PaperPoint> &thin,
                          vector<PaperPoint> &all) const {
...
void Transformation::operator()(const UserPoint &geo,
                                magics::Polyline &out) const {
  PaperPoint pp = (*this)(geo);
  if (in(pp))
    out.push_back(pp);
}

void Transformation::reprojectComponents(double &x, double &y,
...
public:
  SortTool(const Transformation &projection, double x, double y)
      : projection_(projection), x_(x), y_(y) {}
...
void Transformation::thin(double step, PaperPoint &origin,
...
void Transformation::thin(double step, PaperPoint &origin, Matrix &matrix,
