...
void ThreadControler::execute() {
  ...
  try {
    ...
    magics::MagLog::error()
        << "** " << e.what() << " Caught in " << here << endl;
#ifndef MAGICS_ON_WINDOWS
    magics::MagLog::error()
        << "** MagException is termiates thread " << pthread_self() << endl;
#else
    pthread_t pt = pthread_self();
    magics::MagLog::error()
        << "** MagException is termiates thread " << pt.p << pt.x << endl;
#endif
  } catch (...) {
    magics::MagLog::error()
        << "** UNKNOWN MagException Caught in " << here << endl;
#ifndef MAGICS_ON_WINDOWS
    magics::MagLog::error()
        << "** MagException is termiates thread " << pthread_self() << endl;
#else
...
void ThreadControler::kill() { pthread_cancel(thread_); }

void ThreadControler::stop() { proc_->stop(); }
