    ...
    for (auto &elt : definitions_.elements())
      if (elt->name() == "definition") {
        for (auto &id : elt->elements()) {
          ...
          if (keyword == "")
            MagLog::warning()
                << "No if defined in group definition : " << id->name() << "\n";
          else
...
XmlNode *XmlTree::newNode(const string &name,
                          const map<string, string> &def) const {
    ...
    for (auto &elt : definitions_.elements())
      if (elt->name() == "definition") {
        for (auto &i : elt->elements()) {
          ...
          if (keyword == "")
            MagLog::warning()
                << "No id defined in group definition : " << i->name() << "\n";
          else
  ...
  for (auto &elt : definitions_.elements())
    if (elt->name() == "definition") {
      for (auto &i : elt->elements()) {
        ...
        if (keyword == "")
          MagLog::warning()
              << "No if defined in group definition : " << i->name() << "\n";
        else
          ids_.insert(make_pair(keyword, i.get()));
      }
      break;
