...
Text::Text()
    : justification_(MCENTRE), blanking_(false), verticalAlign_(MBASE),
      angle_(0) {}
...
double Text::getFontMax() {
  double max = -1;
  for (vector<NiceText>::const_iterator nice = nice_.begin();
       nice != nice_.end(); ++nice) {
...
void Text::print(ostream &out) const {
  ...
  for (Text::const_iterator point = this->begin(); point != this->end();
       ++point)
    out << ", " << *point;
  out << "]";
}
