...
PaperPoint Tephigram::operator()(const UserPoint &xy) const {
  ...
  if (xy.x() >= 1000) { // x = x
    ...
    double y = (maxpcx + (273.15 * (cosinus - sinus))) *
                   ((sinus + (coefficient * cosinus))) /
                   (cosinus - (coefficient * sinus)) -
               2713.15 * (sinus + cosinus);
    MagLog::dev() << p << "-->" << y << "??? " << minPCY_ << "<<" << maxPCY_
                  << endl;
...
bool Tephigram::needShiftedCoastlines() const { return false; }
...
void Tephigram::boundingBox(double &xmin, double &ymin, double &xmax,
                            double &ymax) const {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      double p =
          magics::pressureFromTheta(theta + 273.15, tempe + 273.15) / 100;
...
double Tephigram::getMinX() const { return -1.5; }

double Tephigram::getMinY() const { return y_min_; }
...
double Tephigram::getMaxY() const { return y_max_; }
...
double Tephigram::getMinPCX() const { return minPCX_; }

double Tephigram::getMinPCY() const { return minPCY_; }

double Tephigram::getMaxPCX() const { return maxPCX_; }

double Tephigram::getMaxPCY() const { return maxPCY_; }
double Tephigram::getMaxTestPCX() const { return maxpcx; }
...
void Tephigram::getNewDefinition(const UserPoint &ll, const UserPoint &ur,
                                 string &out) const {
...
void Tephigram::revert(const vector<std::pair<double, double>> &in,
...
void Tephigram::operator()(const Polyline &from,
