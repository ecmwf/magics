...
MetaDataVisitor::MetaDataVisitor() { meta_.push_back(this); }
...
void MetaDataVisitor::start() { start_ = now(); }
...
void MetaDataVisitor::collectMetaData() {
  ...
  try {
    ...
    string s = "";
    for (vector<ProfileInfo>::const_iterator web = Timer::begin();
         web != Timer::end(); ++web) {
      out << s;
      out << *web;
      s = ",\n";
  ...
  if (!javascript_.empty()) {
    try {
      ...
      for (map<string, string>::const_iterator web = web_.begin();
           web != web_.end(); ++web) {
...
void MetaDataVisitor::collect() {
  for (vector<MetaDataVisitor *>::iterator meta = meta_.begin();
       meta != meta_.end(); ++meta)
...
void StyleEntry::print(ostream &s) const {
  ...
  for (auto style = styles_.begin(); style != styles_.end(); ++style) {
    string description = styles.getAttribute(*style, "contour_description",
                                             "description to come");
    string title =
        styles.getAttribute(*style, "contour_title", "title to come");
    s << sep << "{\"name\":\"" << *style << "\"";
    sep = ",\n      ";
