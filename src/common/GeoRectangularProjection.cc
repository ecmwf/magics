...
/*!
  ...
  \todo do we need here a "delete projection_;" as in
...
PaperPoint GeoRectangularProjection::operator()(const UserPoint &point) const {
  if (!projection_) {
    return PaperPoint(point.x(), point.y(), point.value(), point.missing(),
...
PaperPoint GeoRectangularProjection::operator()(const PaperPoint &point) const {
  MagLog::dev()
      << "GeoRectangularProjection::operator()(...) needs implementing."
      << endl;
  return Transformation::operator()(point);
}

void GeoRectangularProjection::revert(const PaperPoint &xy,
...
void GeoRectangularProjection::revert(
    const vector<std::pair<double, double>> &input,
    vector<std::pair<double, double>> &output) const {
  output.reserve(input.size());
  const vector<std::pair<double, double>>::const_iterator in_end = input.end();
  if (!projection_) {
    for (vector<std::pair<double, double>>::const_iterator pt = input.begin();
         pt != in_end; ++pt) {
      output.push_back(*pt);
    }
    return;
...
bool GeoRectangularProjection::needShiftedCoastlines() const { return true; }
...
double GeoRectangularProjection::getMinX() const { return min_longitude_; }

double GeoRectangularProjection::getMinY() const { return min_latitude_; }

double GeoRectangularProjection::getMaxX() const { return max_longitude_; }

double GeoRectangularProjection::getMaxY() const { return max_latitude_; }

void GeoRectangularProjection::setMinX(double xx) { min_longitude_ = xx; }

void GeoRectangularProjection::setMinY(double yy) { min_latitude_ = yy; }

void GeoRectangularProjection::setMaxX(double xx) { max_longitude_ = xx; }

void GeoRectangularProjection::setMaxY(double yy) { max_latitude_ = yy; }

double GeoRectangularProjection::getMinPCX() const { return xpcmin_; }

double GeoRectangularProjection::getMinPCY() const { return ypcmin_; }

double GeoRectangularProjection::getMaxPCX() const { return xpcmax_; }

double GeoRectangularProjection::getMaxPCY() const { return ypcmax_; }
...
void GeoRectangularProjection::gridLongitudes(const GridPlotting &grid) const {
  ...
  for (vector<double>::const_iterator lon = longitudes.begin(); lon != lon_end;
    ...
    for (double lat = min; lat <= max + step; lat += step) {
      (lat > max) ? poly.push_back((*this)(UserPoint(*lon, max)))
...
/*!
 ...
 \sa Text
*/
void GeoRectangularProjection::labels(const LabelPlotting &label,
...
void GeoRectangularProjection::labels(const LabelPlotting &,
...
void GeoRectangularProjection::labels(const LabelPlotting &label,
  ...
  for (vector<double>::const_iterator lat = latitudes.begin(); lat != lat_end;
    ...
    if (*lat > min_latitude_ && *lat < max_latitude_) {
      const double lon =
          min_longitude_ + ((max_longitude_ - min_longitude_) * .1);
      UserPoint point(lon, *lat);
      text = new Text();
      label.add(text);
      text->setText(writeLatitude(point));
...
double GeoRectangularProjection::patchDistance(double res) const { return res; }
...
void GeoRectangularProjection::init() {
  ...
  while (min_longitude_ > max_longitude_) {
    max_longitude_ += 360;
    MagLog::warning() << "lower_left_longitude > upper_right_longitude --> "
                         "upper_right_longitude is change to "
                      << max_longitude_ << endl;
  }

  if (min_latitude_ > max_latitude_) {
    MagLog::warning() << "lower_left_latitude > upper_right_latitude --> swap"
                      << endl;
  ...
  if (max_longitude_ - min_longitude_ < min_area_) {
    ...
    MagLog::warning() << "The geographical area has been extented to respect "
...
MercatorProjection::~MercatorProjection() { delete projection_; }
...
void GeoRectangularProjection::setNewPCBox(double minx, double miny,
...
*/
void GeoRectangularProjection::coastSetting(map<string, string> &setting,
                                            double abswidth,
...
MatrixHandler *
GeoRectangularProjection::prepareData(const AbstractMatrix &matrix) const {
  // return new MatrixHandler(matrix);
  return new GeoBoxMatrixHandler(matrix, *this);
}

void GeoRectangularProjection::populate(double lon, double lat, double value,
...
void GeoRectangularProjection::wraparound(const UserPoint &origin,
                                          stack<UserPoint> &out) const {
...
void GeoRectangularProjection::getNewDefinition(const UserPoint &ll,
                                                const UserPoint &ur,
                                                string &out) const {
...
double MercatorProjection::patchDistance(double) const { return 1000000; }

double GeoRectangularProjection::ratio() const {
  // return domain/fulldomain!

  return ((max_longitude_ - min_longitude_) * (max_latitude_ - min_latitude_)) /
         (360 * 180);
}
