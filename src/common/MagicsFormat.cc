...
bool MagicsFormat::valid(ostream &out) const {
  if (magCompare(format_, "(automatic)") || magCompare(format_, "automatic")) {
    ...
    if (value_ == trVal &&
        static_cast<double>(static_cast<int>(trVal)) == trVal) {
  ...
  /* Added to allow rounded values to appear on a contour with a 'PS' prefix for
   * positive figures and to remove '-' from negative figures */

  if (magCompare(format_, "(aeronautical)") ||
      magCompare(format_, "aeronautical")) {
    char bufr[256];
    double trVal = trunc(value_);
    if (value_ == trVal &&
        static_cast<double>(static_cast<int>(trVal)) == trVal) {
  ...
  while (c != format_.end() && valid) {
    ...
    case 1:
      ...
      break;
    case 2: // Format ---> nX Specifies that "n" spaces will appear in the
            // output field.
      if (isdigit(*c))
        break;
      switch (*c) {
    ...
    case 71: // Format Iw[.m] -->Wait for a .
      ...
      valid = false;
      break;
    case 72: // Format Iw[.m] -->Wait for a number m [the least number of digits
             // in the field]
      specifier = "i";
...
ostream &MagicsFormat::format(ostream &out) const {
  ...
  if (!valid(out)) {
    MagLog::warning() << " Format \"" << format_
                      << "\" is not valid: will be reset to automatic " << endl;
    return out;
  }
  // out.width(20);
