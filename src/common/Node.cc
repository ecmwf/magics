...
public:
  FrameNode(const RootNode &root)
      : width_(root.getWidth()), height_(root.getHeight()), x_(0), y_(0) {
    layout_ = new Layout();
    layout_->set(this);
    layout_->setLevel(1);
    layout_->box();
...
RootNode::RootNode()
    : frameNode_(0), width_(0), height_(0), frame_(0), metaData_(0),
      needsNewPage_(false) {}
...
void RootNode::set(const XmlPageAttributes &params) {
  PaperDimension *dimension =
      MagTranslator<string, PaperDimension>()(params.getFormat());
...
BaseSceneObject *
RootNode::newMetviewNode(const SuperPageAttributes &attributes) {
  set(attributes);
  ParameterManager::set("SUPER_PAGE_X_LENGTH", width_);
  ParameterManager::set("SUPER_PAGE_Y_LENGTH", height_);
