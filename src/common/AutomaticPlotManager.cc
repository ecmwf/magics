...
AutomaticPlotManager::AutomaticPlotManager()
    : x_(-1), y_(-1), pageX_(-1), pageY_(-1), pwidth_(0), pheight_(0),
...
void AutomaticPlotManager::addpage(MagicsManager &magics) {
  ...
  page->setParent(top());
  page->setFromFortran();
  MagLog::dev() << " try to fit page[" << page->width() << ", "
                << page->height() << "]"
                << "\n";
  // calculate position
  pageX_ = x_;
...
static SimpleObjectMaker<AutomaticPlotManager, PlotManager>
    automatic_plot_manager("automatic");
