...
Layout::Layout()
    : owner_(0), animationRules_(0), transformation_(0), width_(100),
      height_(100), x_(0), y_(0), display_(M_DT_INLINE), xmin_(0), xmax_(100),
      ymin_(0), ymax_(100), zoomable_(false), navigable_(false),
      resizable_(false), resolve_(false), clipping_(false) {}
...
void Layout::print(ostream &out) const {
  ...
  out << "]";
  for (vector<DriverInfo>::const_iterator info = driverInfos_.begin();
       info != driverInfos_.end(); ++info)
    out << *info << endl;
}
...
LayoutFrame::LayoutFrame()
    : thickness_(1), style_(M_SOLID), colour_("grey"), blanking_(false),
      visible_(false) {}
...
bool Layout::buildTree(const Layout &parent, unsigned int frame,
                       const BaseDriver &driver) const {
  bool more = false;

  for (vector<BasicGraphicsObject *>::const_iterator object = objects_.begin();
...
void Layout::release() {
  if (resolve_) {
    for (vector<BasicGraphicsObject *>::iterator object = objects_.begin();
         object != objects_.end(); ++object)
      (*object)->release();
  }
}
...
bool BasicLayout::buildTree(const Layout &parent, unsigned int frame,
                            const BaseDriver &driver) const {
  bool more = false;
