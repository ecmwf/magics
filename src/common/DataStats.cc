...
double DataStats::correlation(vector<double> &dataX, vector<double> &dataY,
                              DataStats &stX, DataStats &stY) {
  double corr = 0.;

  if (stX.hasStDev() == false || stY.hasStDev() == false || stX.stDev() <= 0. ||
      stY.stDev() <= 0. || stX.num() != stY.num() ||
      dataX.size() != stX.num() || dataY.size() != stY.num() ||
      stX.num() == 0) {
    return corr;
  }
