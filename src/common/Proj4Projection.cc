...
Proj4Projection::Proj4Projection(const string &definition)
    : definition_(definition), gridMinLon_(DBL_MAX), gridMinLat_(DBL_MAX),
      gridMaxLon_(-DBL_MAX), gridMaxLat_(-DBL_MAX), wraparound_(false) {
  // init();
  EpsgConfig config;
  config.init();
}
...
Proj4Projection::Proj4Projection()
    : gridMinLon_(DBL_MAX), gridMinLat_(DBL_MAX), gridMaxLon_(-DBL_MAX),
      gridMaxLat_(-DBL_MAX), wraparound_(false) {
  // init();
  EpsgConfig config;
  config.init();
}
...
magics::Polyline &Proj4Projection::getSimplePCBoundingBox() const {
  ...
  PCEnveloppe_->intersect(box, newbox);
  if (newbox.empty()) {
      MagLog::warning() << "Proj4 : the sub-area is not valid : use global view
  instead" << endl;
  }
  else {
      PCEnveloppe_ = newbox.front();
...
void Proj4Projection::revert(const vector<std::pair<double, double>> &in,
  ...
  for (vector<std::pair<double, double>>::const_iterator pt = in.begin();
    ...
    if (error) {
      MagLog::error() << pj_strerrno(error) << " for " << pt->first << " "
                      << pt->second << endl;
      out.push_back(make_pair(HUGE_VAL, HUGE_VAL));
    } else {
...
MatrixHandler *
Proj4Projection::prepareData(const AbstractMatrix &matrix) const {
