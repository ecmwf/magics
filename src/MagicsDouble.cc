...
/*! \mainpage Magics++
 ...
 functionality: C++ (for higher-level applicatiosn such as Metview), C, Fortran
77 Python and MagML/MagJson. The Fortran interface is intended to be backwards
compatible with older versions (mainly the the 6.x series) of Magics.
...
- \ref compatibility "Compatibility to MAGICS 6 for deprecated parameters"
 ...
 - <a
href="http://www.ecmwf.int/services/computing/training/material/com_mag.html">Training
course</a>
...
MAGICS_EXPORT void pset2r_(char *name, double *data, int *dim, int *dim2,
                           int length) {
  std::string n(name, length);
  mag_set2r(n.c_str(), data, *dim, *dim2);
}

MAGICS_EXPORT void pset3r_(char *name, double *data, int *dim, int *dim2,
                           int *dim3, int length) {
  std::string n(name, length);
