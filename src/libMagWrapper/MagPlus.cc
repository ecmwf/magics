...
void replace_string(magics::MagRequest &request, const string &name,
...
void replace_default(magics::MagRequest &request, const string &name,
                     const string &to) {
  if (request.countValues(name) == 0) {
    request(name) = to;
    return;
...
void copy_string(magics::MagRequest &request, const string &from,
                 const string &to) {
...
template <class T>
void replace(magics::MagRequest &request, const string &name, T from,
             const string &newname, T to) {
  if (request.countValues(name) == 0) {
...
string get(magics::MagRequest &request, const string &param,
           const string &val) {
...
MagPlus::MagPlus()
    : root_(0), superpage_(-1), geographical_(true), mode_(interactif),
      currentMulti_(0) {
#ifdef MAGICS_QT
  qtDriver_ = 0;
  qtScene_ = 0;
...
#ifdef MAGICS_QT
void MagPlus::setQtScene(QGraphicsScene *scene) { qtScene_ = scene; }
...
bool MagPlus::page(magics::MagRequest &in) {
  ...
  if ((string)in("SUBPAGE_MAP_PROJECTION") != "NEXT") {
    ...
    pages_[i] = viewhelper;
    if (!id.empty()) {
      view->setInteractiveInfo(id.c_str(), in("ZOOM_NUMBER_OF_LEVELS"),
                               in("ZOOM_CURRENT_LEVEL"));
    }
    top()->insert(view);
...
bool MagPlus::cartesian(magics::MagRequest &in) {
  ...
  if (!id_.empty()) {
    view->setInteractiveInfo(id_.c_str(), in("ZOOM_NUMBER_OF_LEVELS"),
                             in("ZOOM_CURRENT_LEVEL"));
  }
  top()->insert(view);
  push(view);
  ...
  map<string, ObjectCreator>::iterator creator =
      sceneCreators_.find(projection);
...
bool MagPlus::cartesianGrid(magics::MagRequest &in) {
  ...
  if (haxis) {
    ...
    haxis->method_ = unique_ptr<AxisMethod>(
        MagTranslator<string, AxisMethod>()(in("X_AXIS_TYPE")));
    top()->push_back(haxis);
  }
  ...
  if (vaxis) {
    ...
    vaxis->method_ = unique_ptr<AxisMethod>(
        MagTranslator<string, AxisMethod>()(in("Y_AXIS_TYPE")));
    top()->push_back(vaxis);
  }
...
bool MagPlus::table(magics::MagRequest &in) {
  ...
  if (data) {
    if (data.getVerb() == "TABLE" || data.getVerb() == "NOTE" ||
        data.getVerb() == "GEOPOINTS") {
      string x = data("PATH");
      in("TABLE_FILENAME") = x;
    }
...
void MagPlus::notify(MagicsEvent &event) {
  ...
  for (vector<MagicsObserver *>::iterator observer = observers_.begin();
...
void MagPlus::unregisterObserver(MagicsObserver *observer) {
  observers_.erase(
      std::remove_if(observers_.begin(), observers_.end(),
                     bind2nd(std::equal_to<MagicsObserver *>(), observer)),
      observers_.end());
}
