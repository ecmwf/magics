...
public:
  ...
  void currentType(const map<string, string> &def) {
    type_ = def.find("value")->second;
  }
...
static void XMLCALL startFamilyElement(void *userData, const char *name,
                                       const char **atts) {
...
BufrIdentifiers::BufrIdentifiers(int centre) : centre_(centre) {
  ...
  file << getEnvVariable("MAGPLUS_HOME") << MAGPLUS_PATH_TO_SHARE_ << "bufr_"
       << centre << ".xml";
  deffile << getEnvVariable("MAGPLUS_HOME") << MAGPLUS_PATH_TO_SHARE_
          << "bufr_98.xml";
  char buf[BUFSIZ];
  ...
  if (!in) {
    ...
    MagLog::warning() << "No definition file for [" << centre
                      << "]: We use ECMWF definitions " << endl;
  }
  ...
  do {
    ...
    if (XML_Parse(parser, buf, len, done) == XML_STATUS_ERROR) {
      ostringstream s;
      s << "XmlMagException : " << XML_ErrorString(XML_GetErrorCode(parser))
        << " at line  " << XML_GetCurrentLineNumber(parser) << ends;
      cerr << s.str() << "\n";
    }
  } while (!done);
...
public:
  ...
  void init() {
    if (translator_.empty()) {
      ...
      translator_["pressure_tendency_characteristic"] =
  ...
  BufrAccessor(const string &descriptor) : descriptor_(descriptor) {
    ...
    if (token == translator_.end())
      MagLog::warning() << "Could not find eccodes key for " << descriptor_
                        << endl;
...
public:
  BufrMultiValueAccessor(const string &descriptor, int index)
      : BufrAccessor(descriptor), index_(index) {}
  ...
  virtual void operator()(const ObsDecoder &, MvObs &obs, double &val) const {
    ...
    MagLog::dev() << "BufrMultiValueAccessor-Descriptor--->" << descriptor_
                  << " INDEX--->" << index_ << " Value--->" << val << endl;
  }
  virtual void print() {}
...
public:
  ...
  void operator()(const ObsDecoder &, MvObs &obs, string &val) const {
    ...
    pair<string, string> type =
        make_pair(tostring(obs.messageType()), tostring(obs.messageSubtype()));
    ...
    if (value == types_.end()) {
      ...
      if (val == "") {
        MagLog::warning() << "BufrTypeAccessor> No template type for ["
                          << centre << ", " << type.first << ", " << type.second
                          << "][centre, type, subtype]" << endl;
        val = "position";
      }
      types_[type] = val;
    } else {
...
public:
  ...
  void operator()(const ObsDecoder &decoder, MvObs &obs, double &val) const {
    ...
    if (multilevel == multilevels_.end()) {
      MagLog::warning() << "BufrThicknessAccessor> Unknown observation type ["
                        << val << "]\n";
    ...
    if (type == 0 || type == 1) {
      ...
      // MagLog::dev()<< " look for --->" << table.ident(altitude_) << " at " <<
      // decoder.getLevel(); upper-air data
      if (type == 5 || type == 4)
        val = 0;
      else // Multi-level data {
        val = abs(
            obs.valueByPressureLevel(decoder.level_, table.ident(keyword_)) -
...
public:
  BufrMultiLevelAccessor(const string &descriptor, const string &surface,
                         const string &altitude)
      : surface_(surface), altitude_(altitude) {
    descriptor_ = descriptor;
  }
  BufrMultiLevelAccessor(const string &descriptor)
      : surface_(descriptor), altitude_(descriptor) {
    descriptor_ = descriptor;
  }
  ...
  void operator()(const ObsDecoder &decoder, MvObs &obs, double &val) const {
    ...
    map<int, bool>::const_iterator multilevel = multilevels_.find(type);
    if (multilevel == multilevels_.end()) {
      MagLog::warning() << "BufrMultiLevelAccessor> Unknown observation type ["
                        << val << "]\n";
      val = kBufrMissingValue;
    }
    if (type == 0 || type == 1) {
...
public:
  BufrTemperatureAccessor()
      : BufrMultiLevelAccessor("airTemperature", "airTemperatureAt2M",
                               "airTemperature") {}
...
public:
  BufrDewPointAccessor()
      : BufrMultiLevelAccessor("dewpointTemperature", "dewpointTemperatureAt2M",
                               "dewpointTemperature") {}
...
public:
  BufrWindSpeedAccessor()
      : BufrMultiLevelAccessor("windSpeed", "windSpeedAt10M", "windSpeed") {}
  virtual ~BufrWindSpeedAccessor() {}
};

class BufrWindDirectionAccessor : public BufrMultiLevelAccessor {
public:
  BufrWindDirectionAccessor()
      : BufrMultiLevelAccessor("windDirection", "windDirectionAt10M",
                               "windDirection") {}
  virtual ~BufrWindDirectionAccessor() {}
};

SimpleObjectMaker<BufrTypeAccessor, BufrAccessor> type_accessor("type");
SimpleObjectMaker<BufrTemperatureAccessor, BufrAccessor>
    temperature_accessor("temperature");
SimpleObjectMaker<BufrDewPointAccessor, BufrAccessor>
    dewpoint_accessor("dewpoint");
SimpleObjectMaker<BufrGeopotentialAccessor, BufrAccessor>
    geopotential_accessor("geopotential");
SimpleObjectMaker<BufrThicknessAccessor, BufrAccessor>
    thickness_accessor("thickness");

SimpleObjectMaker<BufrWindSpeedAccessor, BufrAccessor>
    wind_speed_accessor("wind_speed");
SimpleObjectMaker<BufrWindDirectionAccessor, BufrAccessor>
    wind_direction_accessor("wind_direction");

SimpleObjectMaker<BufrLowCloudAccessor, BufrAccessor>
    low_cloud_accessor("low_cloud");
SimpleObjectMaker<BufrMediumCloudAccessor, BufrAccessor>
    medium_cloud_accessor("medium_cloud");
SimpleObjectMaker<BufrHighCloudAccessor, BufrAccessor>
    high_cloud_accessor("high_cloud");

SimpleObjectMaker<BufrLowCloudHeightAccessor, BufrAccessor>
    low_cloud_nebulosity_accessor("low_cloud_height");
SimpleObjectMaker<BufrMediumCloudHeightAccessor, BufrAccessor>
    medium_cloud_nebulosity_accessor("medium_cloud_height");
SimpleObjectMaker<BufrHighCloudHeightAccessor, BufrAccessor>
    high_cloud_nebulosity_accessor("high_cloud_height");

SimpleObjectMaker<BufrLowCloudNebulosityAccessor, BufrAccessor>
    low_cloud_height_accessor("low_cloud_nebulosity");
SimpleObjectMaker<BufrMediumCloudNebulosityAccessor, BufrAccessor>
    medium_cloud_height_accessor("medium_cloud_nebulosity");
SimpleObjectMaker<BufrHighCloudNebulosityAccessor, BufrAccessor>
    high_cloud_heightaccessor("high_cloud_nebulosity");
...
void ObsDecoder::decode() {
  ...
  while (obs) {
    ...
    push_back(
        new UserPoint(location.x(), location.y(), obs.value("airTemperature")));
    obs.clearIterator(); // must do this before the msg handle is deleted by the
                         // next assignment
    obs = obsIterator();
  }
}
...
bool ObsDecoder::findInTypes(const string &val) {
  if (types_.empty()) // No restriction specified by the user --> we accept all
                      // the types.
    return true;

  for (stringarray::const_iterator type = types_.begin(); type != types_.end();
       ++type) {
...
void ObsDecoder::getInfo(const std::set<string> &tokens,
                         multimap<string, string> &values) {
  MvObsSet obsSet(file_name_.c_str());
  if (obsSet.messageCount() <= 0)
    return;
  for (std::set<string>::const_iterator token = tokens.begin();
    ...
    try {
      ...
      while (obs) {
        ...
        obs.clearIterator(); // must do this before the msg handle is deleted by
                             // the next assignment
        obs = obsIterator();
      }
    ...
    for (std::set<string>::const_iterator no = noduplicate.begin();
         no != noduplicate.end(); ++no) {
...
*/
void ObsDecoder::customisedPoints(const Transformation &transformation,
                                  const std::set<string> &tokens,
      ...
      // if (multilevels_[obs.messageType()] == false || checkLevel(level) )
      {
        ...
        if (transformation.in(point)) {
          CustomisedPoint *point =
              new CustomisedPoint(loc.x(), loc.y(), obs.findSomeIdent());
          point->type(type);

          for (auto accessor = accessors.begin(); accessor != accessors.end();
               ++accessor) {
