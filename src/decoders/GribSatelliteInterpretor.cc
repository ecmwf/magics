...
const double SAT_HEIGHT =
    42164.0;                  /* distance from Earth centre to satellite     */
...
// const double  SUB_LON    =     0.0;       /* longitude of sub-satellite point
// in radiant */

// const double  CFAC_NONHRV  =  -781648343;      /* scaling coefficients (see
// note above)  */ const double  LFAC_NONHRV  =  -781648343;      /* scaling
// coefficients (see note above)  */

// const double  CFAC_HRV     =   -2344945030.;   /* scaling coefficients (see
// note above)  */ const double  LFAC_HRV     =   -2344945030.;   /* scaling
// coefficients (see note above)  */
...
int geocoord2pixcoord(double latitude, double longitude, int coff, int loff,
                      double cfac, double lfac, double sublon, int *column,
                      int *row) {
  int ccc = 0, lll = 0;
  ...
  re = R_POL /
       sqrt(((double)1.0 - (double)0.00675701 * cos(c_lat) * cos(c_lat)));
  ...
  dotprod = r1 * (rl * cos(c_lat) * cos(lon - sublon)) - r2 * r2 -
...
/*
    ...
    Correct the information provided in the headers of certain satellite imagery
   that we have available. This is a very specific function.
*/
void GribSatelliteInterpretor::AdjustBadlyEncodedGribs(
    int satId, int chanId, long &nx, long &ny, long &dx, long &dy, long &xp,
    long &yp, double &slon, long &functionCode) const {
  ...
  {
    nx = ny = 900;
    dx = dy = 853; // obtained through trial-and-error to get the best match
                   // with the coastlines
    xp = yp = 450;
    functionCode = 1;                                  // wrongly encoded as 0
  } else if (satId == 54 && chanId == 3 && dx == 1179) // Meteosat 7, channel 3
  {
    dx = dy = 1184; // obtained through trial-and-error to get the best match
                    // with the coastlines
    xp = yp = 635;
  } else if (satId == 259 && chanId == 4 &&
             dx == 1185) // GOES-15 (West) channel 4
  {
    dx = dy = 880; // obtained through trial-and-error to get the best match
                   // with the coastlines
    xp = yp = 450;
  } else if (satId == 57 &&
             dx == 1732) // MSG (Meteosat second generation), non-HRV channels
  {
    dx = dy = 1811; // obtained through trial-and-error to get the best match
                    // with the coastlines
    xp = yp = 928;
  }
}
...
void GribSatelliteInterpretor::interpretAsMatrix(GribDecoder &grib) const {
  ...
  // GRIB edition 1 needs to be divided by 10^6, GRIB 2 is already in the right
  // units
  ...
  AdjustBadlyEncodedGribs(sat, chan, nx, ny, dx, dy, xp, yp, slon,
                          functionCode);
  ...
  double target_dx = grib.regular_resolution_; // resolution, in degrees of
                                               // output lat/lon matrix
  double target_dy = grib.regular_resolution_; // resolution, in degrees of
                                               // output lat/lon matrix
  ...
  //  TeEARTHRADIUS ))); double resy = (double)(abs(( atan( tan(prj) *
  //  (altitude-1.) ) * TeEARTHRADIUS )));
  double resx = (double)(abs((atan(tan(pri) * (altitude - 1.)))));
  ...
  MagLog::dev() << "calcul -->" << dlon << " (from->" << west << " to-->"
                << west + (nblon - 1) * dlon << ")" << endl;
  ...
  for (int j = 0; j < nblat; j++) {
    for (int i = 0; i < nblon; i++) {
      ...
      geocoord2pixcoord(lat, lon, coff, loff, cfac, lfac,
                        -lono, // sub-satellite longitude in radians; unclear
                               // why we have to negate it
                        &srcCol, &srcRow);
