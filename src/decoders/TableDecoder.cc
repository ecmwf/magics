...
void TableDecoder::dateSetting(vector<string> &dates, vector<double> &values,
                               DateTime &base) {
  if (dates.empty())
    return;
  base = DateTime(dates.front());
  for (vector<string>::iterator date = dates.begin(); date != dates.end();
       ++date) {
    DateTime d(*date);
...
void TableDecoder::prepareGeo() {
  ...
  for (map<string, string>::const_iterator it = meta.begin(); it != meta.end();
       it++) {
...
void TableDecoder::customisedPoints(const Transformation &transformation,
...
void TableDecoder::getReady(const Transformation &transformation) {
  /*
  try {
      for ( vector<string>::const_iterator x = date_x_.begin(); x !=
  date_x_.end(); ++x ) x_.push_back(transformation.x(*x));
      }

  catch (...) {}
  try {
      for ( vector<string>::const_iterator y = date_y_.begin(); y !=
  date_y_.end(); ++y ) y_.push_back(transformation.y(*y));
  }
  catch (...) {}
  */
}
...
void TableDecoder::visit(Transformation &transformation) {
  ...
  try {
    ...
    if (transformation.getAutomaticX()) {
      if (magCompare(this->x_type_, "date")) {
        double min = (this->x_values_.empty())
                         ? 0
                         : *std::min_element(this->x_values_.begin(),
                                             this->x_values_.end());
        double max = (this->x_values_.empty())
                         ? 24 * 3600
                         : *std::max_element(this->x_values_.begin(),
                                             this->x_values_.end());
    ...
    if (transformation.getAutomaticY()) {
      if (magCompare(this->y_type_, "date")) {
        double min = (this->y_values_.empty())
                         ? 0
                         : *std::min_element(this->y_values_.begin(),
                                             this->y_values_.end());
        double max = (this->y_values_.empty())
                         ? 24 * 3600
                         : *std::max_element(this->y_values_.begin(),
                                             this->y_values_.end());
...
void TableDecoder::customisedPoints(const std::set<string> &,
                                    CustomisedPointsList &out) {
  prepareXY();
  vector<double>::const_iterator x = x_values_.begin();
  vector<double>::const_iterator y = y_values_.begin();
...
string TableDecoder::legendText(double valMin, double valMax) {
  if (MetviewIcon::info("_datatype") == "TABLE_flextra") {
    ...
    for (unsigned int i = 0; i < idVec.size(); i++) {
      if (idVec[i] == valStr) {
        return dateVec[i].substr(dateVec[i].size() - 2, 2) + "/" +
               timeVec[i].substr(0, 2) + " " + levVec[i] + " m";
      }
    }
  }
  return string();
...
PointsHandler &TableDecoder::points(const Transformation &transformation,
                                    bool all) {
  ...
  if (all)
    ...
    this->pointsHandlers_.push_back(
        new BoxPointsHandler(*list, transformation, true));

  return *(this->pointsHandlers_.back());
}

bool TableDecoder::check(const Transformation &transformation, UserPoint *point,
                         stack<UserPoint> &duplicates) {
  ...
  if (duplicates.empty()) {
    ...
    return false; // point has been adjusted and is not in the transformation
                  // view
  }
  return true; // point and eventual duplicates! has been adjusted and is in the
               // transformation view!
}

void TableDecoder::customisedPoints(const Transformation &t,
  ...
  for (ValuesCollector::iterator point = points.begin(); point != points.end();
    ...
    if (minIdx >= 0)
      (*point).push_back(new ValuesCollectorData(
          at(minIdx)->x(), at(minIdx)->y(), at(minIdx)->value(), dist));
  }
}
