...
public:
    MtgPrecipitation() :
        MetgramParameter("precipitation", "Precipitation (mm/3h to 144 hours then mm/6h)", "143.128") {}
    ~MtgPrecipitation() {}
    const string& title() const { return title_; }
    const string& xml() const { return title_; }
    virtual double operator()(double value) const {
    ...
    for (map<int, int>::const_iterator step = rows.begin(); step != rows.end(); ++step) {
        ...
        for (map<string, int>::const_iterator info = columns.begin(); info != columns.end(); ++info) {
            ...
            if (!std::isnan(values[info->second]) && values[info->second] < missing) {
                ...
                if (rain)
                    (*point)["as_rain"] = 1;
                MagLog::dev() << " SPOTBASE returns " << info->first << " for " << (*this)(values[info->second])
                              << " at " << step->first << endl;
            }
            else {
...
void ClassicMtgDecoder::moreTitle(TextVisitor& title) {
    ...
    lines.push_back(station_ + " " + grid_.asLatitude() + " " + grid_.asLongitude() + " " + tostring(maground(detz_)) +
                    " m");
