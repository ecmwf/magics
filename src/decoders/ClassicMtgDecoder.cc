...
public:
  MtgTempe()
      : MetgramParameter("temperature", "Temperature (&#176;C)", "167.128") {}
  ~MtgTempe() {}
  const string &title() const { return title_; }
  const string &xml() const { return title_; }
  virtual double operator()(double value) const { return value - 273.15; }
  virtual spot_query_result *prepare(const ClassicMtgDecoder &,
...
public:
  ...
  virtual double operator()(double value) const { return value / 100; }
...
public:
  MtgHumidity()
      : MetgramParameter("humidity", "850 hPa Relative Humidity (%)",
                         "157.128") {}
  ~MtgHumidity() {}
  const string &title() const { return title_; }
  const string &xml() const { return title_; }
...
public:
  MtgCloudCover()
      : MetgramParameter("cloud_cover", "Cloud Amount (%)", "164.128") {}
  ~MtgCloudCover() {}
  const string &title() const { return title_; }
  const string &xml() const { return title_; }
  virtual double operator()(double value) const {
  ...
  virtual spot_query_result *prepare(const ClassicMtgDecoder &decoder,
...
public:
  MtgPrecipitation()
      : MetgramParameter("precipitation",
                         "Precipitation (mm/3h to 144 hours then mm/6h)",
                         "143.128") {}
  ~MtgPrecipitation() {}
  const string &title() const { return title_; }
  const string &xml() const { return title_; }
...
SimpleObjectMaker<MtgPrecipitation, MetgramParameter>
...
void MetgramParameter::interpretResult(spot_query_result *result,
                                       vector<CustomisedPoint *> &out,
                                       const string &key) {
  ...
  for (map<int, int>::const_iterator step = rows.begin(); step != rows.end();
    ...
    for (p = out.begin(); p != out.end(); ++p) {
      if ((**p)["year"] != next.date().year())
        continue;
      if ((**p)["month"] != next.date().month())
        continue;
      if ((**p)["day"] != next.date().day())
        continue;
      if ((**p)["hours"] != next.time().hours())
        continue;
      if ((**p)["minutes"] != next.time().minutes())
        continue;
      if ((**p)["seconds"] != next.time().seconds())
        continue;
      break;
    }
    ...
    for (map<string, int>::const_iterator info = columns.begin();
      ...
      if (!std::isnan(values[info->second]) && values[info->second] < missing) {
        ...
        if (rain)
          (*point)["as_rain"] = 1;
        MagLog::dev() << " SPOTBASE returns " << info->first << " for "
                      << (*this)(values[info->second]) << " at " << step->first
                      << endl;
      } else {
...
spot_query_result *MtgTempe::prepare(const ClassicMtgDecoder &decoder,
                                     vector<CustomisedPoint *> &out) {
  spot_query *query = decoder.newQuery();
...
spot_query_result *MetgramParameter::prepare(const ClassicMtgDecoder &decoder,
                                             vector<CustomisedPoint *> &out) {
  spot_query *query = decoder.newQuery();
...
spot_query_result *MtgMSL::prepare(const ClassicMtgDecoder &decoder,
                                   vector<CustomisedPoint *> &out) {
  spot_query *query = decoder.newQuery();
...
spot_query_result *MtgHumidity::prepare(const ClassicMtgDecoder &decoder,
                                        vector<CustomisedPoint *> &out) {
  spot_query *query = decoder.newQuery();
...
void ClassicMtgDecoder::customisedPoints(const std::set<string> &,
...
void ClassicMtgDecoder::moreTitle(TextVisitor &title) {
  ...
  lines.push_back(station_ + " " + grid_.asLatitude() + " " +
                  grid_.asLongitude() + " " + tostring(maground(detz_)) + " m");
  lines.push_back("ECMWF Forecast from " + out.str());

  string landsea =
      (mask_ >= 0.5) ? "Nearest land grid point" : "Nearest sea grid point";

  lines.push_back(landsea + " (" + resolution_ + ")");

  lines.push_back("");

  for (vector<string>::const_iterator line = lines.begin(); line != lines.end();
       ++line)
