...
ShapeDecoder::~ShapeDecoder() {
  ...
  for (iterator line = begin(); line != end(); ++line) {
    for (PointsList::iterator point = (*line)->begin(); point != (*line)->end();
         ++point) {
      delete *point;
      *point = 0;
    }
...
*/
void ShapeDecoder::customisedPoints(const std::set<string> &,
  ...
  try {
    ...
    for (i = 0; i < nEntities; i++) {
      ...
      for (vector<string>::const_iterator need = needs.begin();
           need != needs.end(); ++need) {
        if (*need == capital) {
          add = true;
          break;
      ...
      string name = (index != attributes.end())
                        ? DBFReadStringAttribute(hDBF, i, index->second)
                        : "?";
...
/*
   ...
   */
void ShapeDecoder::decode(const Transformation &transformation,
                          const string &filter, const vector<string> &values) {
  if (!this->empty())
    return;
  try {
    char szTitle[12];
    ...
    if (index == attributes.end() && !filter.empty()) {
      MagLog::info() << "ShapeDecoder: can not find attribute " << filter
                     << " -> Data will not be filtered!" << endl;
    }
    SHPObject *psShape = 0;
    for (i = 0; i < nEntities; i++) {
      int j;
      ...
      if (index != attributes.end()) {
        ...
        for (vector<string>::const_iterator val = values.begin();
             val != values.end(); ++val) {
          string ss = s.substr(0, val->length());
          if (magCompare(*val, ss)) {
            if (magCompare(ss, "ITA")) {
              MagLog::debug()
                  << "Enter special case for regions in Italy MAGP-1214"
                  << endl;
              map<string, int>::iterator index_class =
                  attributes.find("featurecla");
              string type =
                  DBFReadStringAttribute(hDBF, i, index_class->second);
              map<string, int>::iterator index =
                  filter.empty() ? attributes.end() : attributes.find(filter);
      ...
      if (add) {
        ...
        for (j = 0, iPart = 1; j < psShape->nVertices; j++) {
          ...
          if (in) {
            inlist->push_back(
                new UserPoint(psShape->padfX[j], psShape->padfY[j], i));
          }
          if (left) {
            leftlist->push_back(
                new UserPoint(psShape->padfX[j] - 360., psShape->padfY[j], i));
          }
          if (right) {
            rightlist->push_back(
                new UserPoint(psShape->padfX[j] + 360., psShape->padfY[j], i));
...
/*! \brief Decoder to read land and lakes
  ...
  */
void ShapeDecoder::decode(vector<magics::Polyline *> &data,
