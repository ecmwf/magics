...
bool NetcdfMatrixInterpretor::interpretAsMatrix(Matrix **matrix) {
  ...
  try {
    ...
    *matrix = matrix_;
    ...
    MagLog::debug() << "matrix_[" << matrix_->size() << ", " << scaling_ << ", "
                    << offset_ << "]"
                    << "\n";
...
void NetcdfMatrixInterpretor::visit(Transformation &transformation) {
  ...
  try {
    ...
    if (transformation.getAutomaticX()) {
      if (!baseDateX_.empty()) {
        transformation.setDataMinMaxX(columns_.front(), columns_.back(),
                                      baseDateX_);
...
void NetcdfMatrixInterpretor::customisedPoints(
...
void NetcdfMatrixInterpretor::visit(ValuesCollector &vcp, PointsList &) {
  ...
  // for (ValuesCollector::iterator point =  points.begin(); point !=
  // points.end(); ++point) {
  //	point->push_back(new ValuesCollectorData(point->x(),
  // point->y(),matrix_->nearest(point->y(), point->x()),-1.));
  //}
}

bool NetcdfMatrixInterpretor::interpretAsPoints(
    PointsList &points, const Transformation &transformation) {
  refDateX_ = transformation.getReferenceX();
  refDateY_ = transformation.getReferenceY();
  ...
  for (vector<double>::iterator row = rows_.begin(); row != rows_.end(); ++row)
    for (vector<double>::iterator column = columns_.begin();
