...
GribDecoder::~GribDecoder() {
  ...
  if (nearest_) grib_nearest_delete(nearest_);

  for (PointsList::iterator point = points_.begin(); point != points_.end();
       ++point) {
    delete *point;
    *point = 0;
  }
...
long computeStep(const GribDecoder& grib, const string& key) {
  ...
  double factor = 1;
  if (stepunit != stepUnits.end()) factor = stepunit->second;
  return step * factor;
}
...
string GribDecoder::getstring(const string& key, bool warnIfKeyAbsent,
                              bool cache) const {
  if (!valid_) return "";
  if (cache) {
  ...
  if (cache) sKeys_.insert(make_pair(key, val));
...
void GribDecoder::setDouble(const string& key, double val) const {
  ...
  if (component2_) err = grib_set_double(component2_, key.c_str(), val);
  if (err) {
    MagLog::warning() << "Grib API: can not find key [" << key << "]  - "
                      << grib_get_error_message(err) << "\n";
  }
...
void GribDecoder::scale(const string& metadata, double& scaling,
                        double& offset) {
  assert(false);
}
...
void GribDecoder::read() {
  ...
  const string representation =
      getstring("typeOfGrid");  // Make sure we only get one info
  ...
  try {
    ...
    if (u() == 0) {
      valid_ = false;
      ostringstream msg;
      msg << "Grib Decoder: Representation [" << representation
    ...
    ostringstream msg;
    msg << "Grib Decoder - read: Representation [" << representation
        << "] not supported";
...
void GribDecoder::read(const Transformation& transformation) {
  ...
  try {
    ...
    MagLog::error() << "Grib Decoder - read: Representation [" << representation
                    << "] not supported.\n"
                    << std::endl;
    ;
    valid_ = false;
...
void GribDecoder::decode2D() {
  ...
  try {
    ...
    MagLog::warning() << "Grib Decoder: Vector cobination of representations ["
...
void GribDecoder::customisedPoints(const AutomaticThinningMethod& thinning,
                                   const Transformation& transformation,
                                   const std::set<string>& request,
  ...
  try {
    ...
    double xstep = (transformation.getMaxPCX() - transformation.getMinPCX()) /
                   (thinning.x() - 1);
    double ystep = (transformation.getMaxPCY() - transformation.getMinPCY()) /
                   (thinning.y() - 1);
    ...
    MagLog::error() << "Grib Decoder - customisedPoints: Representation ["
                    << representation << "] not supported.\n"
                    << std::endl;
    ;
    throw MagicsException("Grib Decoder: Representation [] not supported.");
...
void GribDecoder::newPoint(const Transformation& transformation, double lat,
                           double lon, double uc, double vc, double cc,
                           vector<CustomisedPoint*>& points, double grid) {
...
void GribDecoder::customisedPoints(const Transformation& transformation,
                                   CustomisedPointsList& out, double thinx,
  ...
  if (thiny && thinning_) {
    ...
    for (pos = positions.begin(); pos != positions.end(); ++pos) {
      ...
      if (w > xComponent_->data_.size()) continue;
      ...
      if (!cached) {
        ...
        if (colourComponent_)
          add->insert(
              make_pair("colour_component", colourComponent_->data_[w]));
    ...
    for (map<double, int>::iterator y = xComponent_->yIndex_.begin();
         y != xComponent_->yIndex_.end(); ++y) {
      ...
      for (int i = 0; i < x.nbPoints_; i++) {
        ...
        if (u != missing && v != missing) {
          ...
          pair<double, double> value = (*wind_mode_)(u, v);
          transformation.populate(lon, lat, 0, pos);
          for (vector<UserPoint>::iterator p = pos.begin(); p != pos.end();
               ++p) {
            CustomisedPoint* add = new CustomisedPoint(p->x(), p->y(), "");
            add->insert(make_pair("x_component", value.first));
            add->insert(make_pair("y_component", value.second));
            if (colourComponent_)
              add->insert(make_pair("colour_component",
                                    colourComponent_->data_[index]));
            out.push_back(add);
          }
        }
        index++;
...
void GribDecoder::customisedPoints(const BasicThinningMethod& thinning,
                                   const Transformation& transformation,
  ...
  try {
    ...
    customisedPoints(transformation, points, gap * thinning.factor(),
...
grib_handle* GribDecoder::open(grib_handle* grib, bool sendmsg) {
  ...
  if (!file) {
    ostringstream error;
    error << "file can not be opened [" << file_name_ << "] "
  ...
  if (loop_) {
    ...
    grib_handle* handle =
        codes_handle_new_from_file(0, file, PRODUCT_GRIB, &error);
  ...
  if (!handle) {
    if (sendmsg) {
      ostringstream error;
      error << "can not access position [" << current_position_ << " in "
            << file_name_ << "]" << std::endl;
      MagLog::error() << "can not access position [" << current_position_
                      << " in " << file_name_ << "]" << std::endl;
      MagLog::broadcast();
      valid_ = false;
      throw MagicsException(error.str());
      return 0;
...
bool GribDecoder::verify(const string& val) const {
  ...
  for (map<string, string>::const_iterator w = where.begin(); w != where.end();
       ++w) {
    string val = getstring(w->first);
    if (magCompare(val, w->second) == false) return false;
  }
  return true;
}
...
void GribDecoder::decodePoints() {
  ...
  if (Data::dimension_ == 1) {
    ...
    try {
      ...
      MagLog::warning() << "Grib Decoder: Representation [" << representation
                        << "] not supported.\n"
                        << std::endl;
      ;
      scaling = 1;
    ...
    if (!iter) {
      MagLog::error()
          << "Grib Iterator not yet supported on this kind of GRIB\n";
      MagLog::broadcast();
  ...
  while (grib_iterator_next(iter1, &lat1, &lon1, &val1) &&
         grib_iterator_next(iter2, &lat2, &lon2, &val2)) {
...
GribLoop::~GribLoop() {
  for (vector<GribDecoder*>::const_iterator g = gribs_.begin();
       g != gribs_.end(); ++g) {
    delete *g;
  }
...
Data* GribLoop::current() { return currentgrib_; }
...
bool GribLoop::hasMore() {
  if (file_ == 0) {
    ...
    if (!file_) {
      MagLog::error() << "file can not be opened [" << path_ << "] "
                      << std::strerror(errno) << std::endl;
  ...
  if (currentDim_ == dimension_.end()) return false;
  ...
  if (*currentDim_ == 1) {
    if (dim_.empty()) {
      ...
      if (currentPos_ == dim_.end()) return false;
...
class GribTag : public XmlNodeVisitor {
 public:
  ...
  string baseDate(const XmlNode& node) {
    ...
    if (format.empty()) format = "%A %d %B %Y %H%M UTC";
  ...
  string validDate(const XmlNode& node) {
    ...
    const long hour = grib_.getLong("hour");
    const long mn = grib_.getLong("minute");
    const long step = computeStep(
        grib_, "stepRange");  // default is in hours. Set 'stepUnits' to change.
  ...
  string dataDate(const XmlNode& node) {
    ...
    string format = node.getAttribute("format");
    if (format.empty()) return tostring(day);
  ...
  void visit(const XmlNode& node) {
    if (magCompare(node.name(), "grib_info")) {
      ...
      if (val.empty()) val = node.getAttribute("default");
...
void GribDecoder::nearestGridpoints(double* inlats, double* inlons,
                                    double* outlats, double* outlons,
                                    double* values, double* distances, int nb,
                                    const string& representation,
                                    double missing) {
  ...
  if (representation == "regular_ll" || representation == "reduced_ll" ||
      representation == "regular_gg" || representation == "reduced_gg") {
  ...
  for (int i = 0; i < nb; i++) {
    if (nearHandle) {
      ...
      // grib_nearest_find
      grib_nearest_find(nearHandle, field_, inlats[i], inlons[i],
                        GRIB_NEAREST_SAME_GRID, outlats4, outlons4, outvals4,
                        outdist4, outindexes, &len);
      vector<double> vdistances(outdist4, outdist4 + 4);
      int closestIndex =
          distance(vdistances.begin(),
                   min_element(vdistances.begin(), vdistances.end()));
...
void GribDecoder::visit(ValuesCollector& points) {
  ...
  if (Data::dimension_ == 1) {
    ...
    nearestGridpoints(inlats, inlons, outlats, outlons, values, distances, nb,
                      representation, missing);

    for (int i = 0; i < nb; i++) {
      points[i].push_back(new ValuesCollectorData(outlons[i], outlats[i],
                                                  values[i], distances[i]));
      if (scaled)
        points[i].back()->setScaledValue(scaling * values[i] + offset);
      if (values[i] == missing) points[i].back()->setMissing(true);
    }
  } else {
    ...
    nearestGridpoints(inlats, inlons, outlats, outlons, x, distances, nb,
                      representation, missing);
    nearestGridpoints(inlats, inlons, outlats, outlons, y, distances, nb,
                      representation, missing);

    for (int i = 0; i < nb; i++) {
      points[i].push_back(
          wind_mode_->values(outlons[i], outlats[i], x[i], y[i], distances[i]));
...
const DateDescription& GribDecoder::timeStamp() {
  ...
  timeStamp_ =
      DateDescription(helper.get("grib", "valid-date"), index_, internalIndex_);
  dataLevel_ = LevelDescription::level(helper.get("grib", "typeOfLevel"),
                                       tonumber(helper.get("grib", "level")),
...
void GribDecoder::visit(MetaDataCollector& step) {
  ...
  try {
    ...
    if (name_.empty()) {
      ...
      need.push_back("<grib_info key='level'/>");
      need.push_back(
          "<grib_info key='start-date' format='%Y-%m-%d %H:%M:00'/>");
      need.push_back("<grib_info key='end-date' format='%Y-%m-%d %H:%M:00'/>");
    }

    for (map<string, string>::iterator key = step.begin(); key != step.end();
         ++key) {
      // If key is not found in information we use ecCodes
      if (information_.find(key->first) == information_.end()) {
        // Compute stats
        if (step.attribute(key->first).group() ==
            MetaDataAttribute::StatsGroup) {
          stats_.clear();
        ...
        // We use ecCodes
        else if (step.attribute(key->first).source() ==
                     MetaDataAttribute::AnySource ||
                 step.attribute(key->first).source() ==
                     MetaDataAttribute::GribApiSource) {
          if (step.attribute(key->first).type() !=
              MetaDataAttribute::NumberType) {
            need.push_back("<grib_info key='" + key->first + "'/>");
          } else {
            need.push_back("<grib_info key='" + key->first +
                           "' readAsLong='yes'/>");
          }
        } else if (key->first == "scaling_formula" ||
                   key->first == "scaled_units") {
          double scaling, offset;
          string oriUnits, derivedUnits;
          string representation = getstring("typeOfGrid");
          try {
            unique_ptr<GribInterpretor> interpretor_(
                SimpleObjectMaker<GribInterpretor>::create(representation));
            interpretor_->scaling(*this, scaling, offset, oriUnits,
                                  derivedUnits);
            if (scaling == 1 && offset == 0) {
              ...
              information_["scaling_formula"] =
                  "(value * " + tostring(scaling) + ") " + offsetSign + " " +
                  tostring(fabs(offset));
              information_["scaled_units"] = derivedUnits;
            }
          } catch (NoFactoryException&) {
            MagLog::warning() << "Grib Decoder: Representation ["
                              << representation << "] not supported.\n"
                              << std::endl;
            ;
          ...
          if (representation == "reduced_gg") {
            if (getLong("global") == 1) {
              information_["isOctahedral"] =
                  (getLong("isOctahedral") == 1) ? "yes" : "no";
            }
          }
        }
      }
    ...
    if (!need.empty()) {
      TagHandler helper;
      GribTag tag1(*this, helper);
      for (vector<string>::const_iterator t = need.begin(); t != need.end();
           ++t) {
        tag1.decode(*t);
      }
      ...
      if (members) {
        name_ =
            helper.get("grib", "shortName") + " " + helper.get("grib", "level");
      ...
      for (map<string, string>::iterator key = step.begin(); key != step.end();
           ++key) {
        if (information_.find(key->first) == information_.end()) {
          if (step.attribute(key->first).source() ==
                  MetaDataAttribute::AnySource ||
              step.attribute(key->first).source() ==
                  MetaDataAttribute::GribApiSource) {
...
MatrixHandler& GribDecoder::direction() {
  ...
  if (directionDone_) return *(matrixHandlers_.back());
  directionDone_ = true;
  ...
  for (vector<double>::iterator d = directions.begin(); d != directions.end();
       ++d) {
...
void GribDecoder::decode(const Transformation& transformation) {
  ...
  need.push_back("<grib_info id=\'" + id_ +
                 "\'  key='start-date' format='%Y-%m-%d %H:%M:00'/>");
  need.push_back("<grib_info id=\'" + id_ +
                 "\' key='end-date' format='%Y-%m-%d %H:%M:00'/>");
  GribTag tag1(*this, helper);
  ...
  name_ = helper.get("grib" + id_, "shortName") + "-" +
          helper.get("grib" + id_, "level");
  name_ = iconName_;
...
void GribDecoder::visit(TextVisitor& title) {
  ...
  for (vector<string>::const_iterator t = titles.begin(); t != titles.end();
       ++t) {
    tag.decode(*t);
  }
}
...
void GribDecoder::decodeRaster(const Transformation& transformation) {
  ...
  try {
    ...
    MagLog::error() << "Grib Decoder: Raster representation [" << representation
                    << "] not supported.\n";
    throw MagicsException(
        "Grib Decoder: Raster representation [] not supported.");
  }
}
...
class GribLocalCriter : public MatchCriteria {
 public:
  ...
  bool verify(const GribDecoder& grib, const string& param, const string& val) {
    ...
    MagLog::debug() << "I am verifing " << param
                    << " for a GribDecoder : " << criter << " ==  " << val
                    << "???"
                    << "\n";
    return (criter == val);
  }
...
class GribObstatHandler : public TitleFieldHandler {
 public:
  ...
  void operator()(TitleField&, vector<string>& /*title*/,
                  const GribDecoder& grib) {}
};
...
class GribParamHandler : public TitleFieldHandler {
 public:
  GribParamHandler() {}
  ~GribParamHandler() {}
  virtual void operator()(TitleField&, vector<string>& title,
                          const GribDecoder& grib) {
...
class GribKeyHandler : public TitleFieldHandler {
 public:
  GribKeyHandler() {}
  ~GribKeyHandler() {}
  void operator()(TitleField& field, vector<string>& title,
                  const GribDecoder& grib) {
    char x[256];
...
class GribBaseDateHandler : public TitleFieldHandler {
 public:
  ...
  void operator()(TitleField& field, vector<string>& title,
    ...
    if (type == 2) {  //     Verifying time of forecast
      long step =
          grib.getLong("step");  // needs steps in second!   // default is in
...
class GribLevelHandler : public TitleFieldHandler {
 public:
  ...
  ~GribLevelHandler() {}
  typedef string (GribLevelHandler::*Builder)(const string& def,
...
double GribDecoder::uComponent(int index) { return xValues_[index]; }

double GribDecoder::vComponent(int index) { return yValues_[index]; }
...
static SimpleObjectMaker<GribParamCriter, MatchCriteria> gribparamcriter(
    "parameter");
static SimpleObjectMaker<GribParamHandler, TitleFieldHandler> gribparamhandler(
    "parameter");
static SimpleObjectMaker<GribBaseDateHandler, TitleFieldHandler>
    gribbasedatehandler("base_date");
static SimpleObjectMaker<GribValidDateHandler, TitleFieldHandler>
    gribvaliddatehandler("valid_date");
static SimpleObjectMaker<GribStepHandler, TitleFieldHandler> gribstephandler(
    "step");
static SimpleObjectMaker<GribEpsNumberInfoHandler, TitleFieldHandler>
    gribepsnumberhandler("eps_number_info");

static SimpleObjectMaker<GribTimeHandler, TitleFieldHandler> gribTimehandler(
    "time");
static SimpleObjectMaker<GribLevelHandler, TitleFieldHandler> gribLevelhandler(
    "level");
static SimpleObjectMaker<NewLineHandler, TitleFieldHandler> newlinehandler(
    "newline");
static SimpleObjectMaker<GribKeyHandler, TitleFieldHandler> gribkeyhandler(
    "gribapi");

static SimpleObjectMaker<GribStreamHandler, TitleFieldHandler>
    gribstreamhandler("stream");
static SimpleObjectMaker<GribClassHandler, TitleFieldHandler> gribclasshandler(
    "class");
static SimpleObjectMaker<GribTypeHandler, TitleFieldHandler> gribtypehandler(
    "type");
static SimpleObjectMaker<GribLocalDefHandler, TitleFieldHandler>
    griblocaldefhandler("localdef");
static SimpleObjectMaker<GribCentreHandler, TitleFieldHandler>
    gribcentrehandler("centre");
static SimpleObjectMaker<GribProductHandler, TitleFieldHandler>
    gribproducthandler("product");
static SimpleObjectMaker<GribUnitHandler, TitleFieldHandler> gribunithandler(
    "units");
static SimpleObjectMaker<GribExpverHandler, TitleFieldHandler>
    gribexpverhandler("expver");
static SimpleObjectMaker<GribPlotTypeHandler, TitleFieldHandler>
    gribplottypehandler("plot_type");
static SimpleObjectMaker<SatelliteHandler, TitleFieldHandler> satellitehandler(
    "satellite");
static SimpleObjectMaker<ChannelHandler, TitleFieldHandler> channelhandler(
    "channel");
static SimpleObjectMaker<GribBaseDateHandler, TitleFieldHandler> datehandler(
    "date");

static SimpleObjectMaker<GribObsDiagCriter, MatchCriteria> gribobsdiagriter(
    "observationDiagnostic");
static SimpleObjectMaker<GribObsDiagHandler, TitleFieldHandler>
    gribobsdiaghandler("observationDiagnostic");
static SimpleObjectMaker<GribObstatHandler, TitleFieldHandler>
    gribobstathandler("obstat");

static SimpleObjectMaker<GribLocalCriter, MatchCriteria> gribstreamcriter(
    "stream");
static SimpleObjectMaker<GribLocalCriter, MatchCriteria> gribtypecriter("type");
static SimpleObjectMaker<GribLocalCriter, MatchCriteria> gribclasscriter(
    "class");
static SimpleObjectMaker<GribLocalCriter, MatchCriteria>
    typeOfgeneratingProcess("typeOfGeneratingProcess");
static SimpleObjectMaker<GribLocalCriter, MatchCriteria> timeRangeIndicator(
    "timeRangeIndicator");
static SimpleObjectMaker<GribLocalCriter, MatchCriteria> dataType("dataType");

#include "GribRegularInterpretor.h"
static SimpleObjectMaker<GribRegularInterpretor, GribInterpretor> regular_ll(
    "regular_ll");
static SimpleObjectMaker<GribReducedLatLonInterpretor, GribInterpretor>
    reduced_ll("reduced_ll");
static SimpleObjectMaker<GribRegularGaussianInterpretor, GribInterpretor>
    regular_gg("regular_gg");
static SimpleObjectMaker<GribReducedGaussianInterpretor, GribInterpretor>
    reduced_gg("reduced_gg");
static SimpleObjectMaker<GribRotatedInterpretor, GribInterpretor> rotated_ll(
    "rotated_ll");
static SimpleObjectMaker<GribLambertAzimutalInterpretor, GribInterpretor>
    lambert_azimuthal_equal_area("lambert_azimuthal_equal_area");
static SimpleObjectMaker<GribLambertInterpretor, GribInterpretor> lambert(
    "lambert");
static SimpleObjectMaker<GribPolarStereoInterpretor, GribInterpretor> polar(
    "polar_stereographic");

#include "GribSatelliteInterpretor.h"
static SimpleObjectMaker<GribSatelliteInterpretor, GribInterpretor> satellite(
    "space_view");
