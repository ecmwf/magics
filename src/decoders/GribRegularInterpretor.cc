...
void GribInterpretor::new_index(const GribDecoder &grib) {
  ...
  */
  ...
  helper_ =
      vector<vector<Index>>((indexLon_ + 1) * (indexLat_ + 1), vector<Index>());
...
double
GribRegularInterpretor::longitudeIncrement(const GribDecoder &grib) const {
...
double
GribReducedLatLonInterpretor::XResolution(const GribDecoder &grib) const {
...
double
GribReducedGaussianInterpretor::XResolution(const GribDecoder &grib) const {
  long res = grib.getLong("numberOfParallelsBetweenAPoleAndTheEquator");
  double west = grib.getDouble("longitudeOfFirstGridPointInDegrees");
...
void GribReducedGaussianInterpretor::interpretAsMatrix(
  ...
  d = 0;
  {
    ...
    bool first = true;
    for (vector<vector<double>>::iterator row = rows.begin(); row != rows.end();
         ++row) {
      vector<double> p;
      p.reserve(row->size());
      if (v) {
...
double
GribLambertAzimutalInterpretor::XResolution(const GribDecoder &grib) const {
  return 0.0125;
}
...
PaperPoint
GribRotatedInterpretor::reference(const GribDecoder &grib,
                                  const Transformation &transformation) {
  double lon = grib.getDouble("longitudeOfFirstGridPointInDegrees");
  double lat = grib.getDouble("latitudeOfFirstGridPointInDegrees");
