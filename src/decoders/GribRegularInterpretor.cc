...
void GribInterpretor::new_index(const GribDecoder& grib) {
  ...
  */
  if (grib.getLong("jScansPositively") == 1 ||
      grib.getLong("iScansPositively") == 0) {
    MagLog::error() << " Scanning mode not yet supported" << endl;
    return;
  }

  helper_ = vector<vector<Index> >((indexLon_ + 1) * (indexLat_ + 1),
                                   vector<Index>());
  ...
  int ilat, ilon;
  {
    ...
    while (grib_iterator_next(iter, &lat, &lon, &u)) {
      ...
      if (lat < -90 || lat > 90 || lon < -180 || lon > 360) {
        if (flag_error) {
          MagLog::warning()
              << "Check Grib Iterator: Position is not in the expected range ["
              << lat << ", " << lon << "]" << endl;
          flag_error = false;
        }
        continue;
...
Index GribInterpretor::nearest(double ulat, double ulon) {
  Index index(-1, 0, 0);
  if (ulat < minlat_) return index;
  if (ulat > maxlat_) return index;
  if (ulon < minlon_) return index;
  if (ulon > maxlon_) return index;

  if (ulat == 1000. || ulon == 1000.) return index;
  if (ulat >= 90 || ulat <= -90) return index;

  int ilat = floor((ulat + 90) / indexStep_);
  int ilon = floor(ulon / indexStep_);
  int lat1, lat2;

  if (ilon == indexLon_) ilon = 0;
  ...
  for (int clat = lat1; clat <= lat2; clat++) {
    ...
    for (set<int>::iterator lon = lonn.begin(); lon != lonn.end(); ++lon) {
      ...
      for (vector<Index>::iterator point = points.begin();
...
void GribInterpretor::scaling(GribDecoder& grib, double& scaling,
                              double& offset, string& originalUnits,
...
int GribInterpretor::nearest(double lon, double lat, double& nlon,
                             double& nlat) {
  assert(false);
  return -1;
}

void GribInterpretor::raw(
    GribDecoder& grib, const Transformation& transformation,
  ...
  while (grib_iterator_next(uiter, &lat, &lon, &u) &&
    ...
    if (transformation.in(lon, lat) || transformation.in(lon - 360, lat)) {
      ...
      if (u != missing) u = (u * factor) + offset;
      if (v != missing) v = (v * factor) + offset;

      if (lat != last) {
        last = lat;
        points.push_back(
            make_pair(lat, vector<pair<double, CustomisedPoint*> >()));
  ...
  for (vector<pair<double, vector<pair<double, CustomisedPoint*> > > >::iterator
           ilat = points.begin();
    ...
    CustomisedPoint* dup =
        new CustomisedPoint(first->longitude() + 360, first->latitude(), "");
...
void GribInterpretor::raw(
    GribDecoder& grib, const Transformation& transformation, const string& key,
  ...
  while (grib_iterator_next(iter, &lat, &lon, &value)) {
    if (transformation.in(lon, lat) || transformation.in(lon - 360, lat)) {
      if (value != missing) value = (value * factor) + offset;
...
void GribRegularInterpretor::interpretAsMatrix(GribDecoder& grib,
...
void GribRegularInterpretor::index(const GribDecoder& grib) {
  ...
  try {
    if (jPointsAreConsecutive) {
      for (vector<double>::iterator lat = latitudes.begin();
           lat != latitudes.end(); ++lat) {
        index_.insert(make_pair(*lat, map<double, int>()));
      }
      for (vector<double>::iterator lon = longitudes.begin();
...
void GribRegularInterpretor::interpretAsMatrix(GribDecoder& grib) const {
  ...
  if (v != NULL) {
    ...
    int offset = 0;
    int l = 0;
    for (vector<double>::iterator lat = u->rowsAxis().begin();
         lat != u->rowsAxis().end(); ++lat) {
  ...
  try {
    ...
    } else  // otherwise, just copy the array of values as they are
    {
      if (v != NULL) {
        ...
        grib_get_double_array(grib.uHandle(), "values", &u->data_.front(),
                              &aux);
        grib_get_double_array(grib.vHandle(), "values", &v->front(), &aux);
        grib_get_double_array(grib.vHandle(), "values", &v->data_.front(),
                              &aux);
        if (c) {
          c->data_.resize(nb);
          grib_get_double_array(grib.cHandle(), "values", &c->front(), &aux);
          grib_get_double_array(grib.cHandle(), "values", &c->data_.front(),
                                &aux);
        }
      } else
...
void GribRegularGaussianInterpretor::latitudes(
    const GribDecoder& grib, vector<double>& latitudes) const {
  ...
  for (int i = 0; i < 2 * res; i++) {
    ...
    if (array[i] < north && array[i] > south) latitudes.push_back(array[i]);
  }
  delete[] array;

  if (scanning == 1) std::reverse(latitudes.begin(), latitudes.end());
}
...
PaperPoint GribInterpretor::reference(const GribDecoder& grib,
  ...
  if (transformation.in(point)) return transformation(point);
  int err;
  grib_iterator* iter = grib_iterator_new(grib.handle(), 0, &err);
...
void GribRegularInterpretor::interpretAsRaster(
    const GribDecoder& grib, RasterData& raster,
  ...
  for (int j = nblat - 1; j >= 0; j--)
    for (int i = 0; i < nblon; i++) raster.push_back(box(j, i));
}

void GribReducedGaussianInterpretor::interpretAsRaster(
    const GribDecoder& grib, RasterData& raster,
    const Transformation& transformation) const {
  MagLog::dev() << "GribRegularInterpretor::interpretAsRaster"
                << "\n";
...
void GribReducedGaussianInterpretor::interpretAsMatrix(
    GribDecoder& grib, const Transformation& transformation) const {
  interpretAsMatrix(grib);
}
...
void GribReducedGaussianInterpretor::interpretAsMatrix(
  ...
  for (int i = 0; i < nblat; i++) {
    ...
    if (global) {
      ...
      grib_get_reduced_row_p(globalPointsInThisRow, west, east, &numPts,
                             &lonfirst, &lonlast);
      add = 2 * dx;
    }
  ...
  if (v) {
    ...
    if (c) grib_get_double_array(grib.cHandle(), "values", data3, &nb);
  } else
  ...
  d = 0;
  {
    ...
    bool first = true;
    for (vector<vector<double> >::iterator row = rows.begin();
         row != rows.end(); ++row) {
      vector<double> p;
      p.reserve(row->size());
      if (v) {
        u->xIndex_.push_back(
            InfoIndex(row->front(), row->back(), row->size(), d));
      }
      ...
      if (interpolate == GribDecoder::nearest_valid) {
        ...
        last = 0;
        c = p.size() - 1;
        for (vector<double>::reverse_iterator val = p.rbegin(); val != p.rend();
             ++val) {
          if (*val != missing) {
            last = c;
          }
...
void GribReducedLatLonInterpretor::interpretAsMatrix(GribDecoder& grib) const {
  ...
  for (size_t i = 0; i < res; i++) {
    ...
    if (v) matrix->xIndex_.push_back(InfoIndex(west, east, pl[i], d));

    if (pl[i] == 0) {
      // add missing data
      for (int x = 0; x < nblon; x++) matrix->push_back(missing);
    } else {
      vector<double> p;
      vector<double> lons;
...
double GribLambertAzimutalInterpretor::XResolution(
    const GribDecoder& grib) const {
  return 0.0125;
}
...
PaperPoint GribRotatedInterpretor::reference(
    const GribDecoder& grib, const Transformation& transformation) {
  double lon = grib.getDouble("longitudeOfFirstGridPointInDegrees");
  double lat = grib.getDouble("latitudeOfFirstGridPointInDegrees");
...
void GribLambertAzimutalInterpretor::interpretAsMatrix(
    GribDecoder& grib) const {
  ...
  try {
    ...
    for (int i = 0; i < im; i++) columns.push_back(i);
    for (int i = 0; i < jm; i++) rows.push_back(i);
...
void GribRotatedInterpretor::interpretAsMatrix(GribDecoder& grib) const {
  ...
  Matrix* matrix =
      grib.u(new RotatedMatrix(nblat, nblon, southPoleLat_, southPoleLon_));
  Matrix* matrix2 =
      grib.v(new RotatedMatrix(nblat, nblon, southPoleLat_, southPoleLon_));
  ...
  if (matrix2 != NULL) {
    ...
    int offset = 0;
    int l = 0;
    for (vector<double>::iterator lat = matrix->rowsAxis().begin();
         lat != matrix->rowsAxis().end(); ++lat) {
...
void GribRotatedInterpretor::raw(
    GribDecoder& grib, const Transformation& transformation, const string& key,
    map<double, map<double, CustomisedPoint*> >& points) const {
  double factor, offset;
  scaling(grib, factor, offset);
...
void GribLambertInterpretor::interpretAsMatrix(GribDecoder& grib) const {
  ...
  try {
    ...
    for (int i = 0; i < im; i++) lon.push_back(minlon + (i * inci));
    // for the lon we take the fisrt column :
    for (int i = 0; i < jm; i++) lat.push_back(minlat + (i * incj));
    ...
    for (int r = 0; r < jm - 1; r++) {
      for (int c = 0; c < im - 1; c++) {
        ...
        if (low == helper.end() || up == helper.end()) break;
        for (Helper::iterator it = low; it != up; ++it) {
          ...
          if (llow == lons.end() || lup == lons.end()) break;
          ;
...
PaperPoint GribLambertAzimutalInterpretor::reference(const GribDecoder&,
...
void GribPolarStereoInterpretor::interpretAsMatrix(GribDecoder& grib) const {
  ...
  for (int i = 0; i < im; i++) {
    for (int j = 0; j < jm; j++) (*matrix)[i + (j * im)] = missing;
  }
  matrix->missing(missing);
  matrix->setMapsAxis();
...
void GribInterpretor::interpolate(const GribDecoder& grib,
                                  Matrix& matrix) const {
  int err;

  grib_iterator* iter = grib_iterator_new(grib.handle(), 0, &err);
  if (err) {
    MagLog::warning() << "Grib Iterator not available : Chech Grib Api Version "
                      << grib_get_api_version() << endl;
    return;
  }
