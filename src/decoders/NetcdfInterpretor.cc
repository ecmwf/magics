...
NetcdfInterpretor* NetcdfGuessInterpretor::guess() const {
  ...
  if (delegate_) return delegate_;
  cout << "PRINT--> Trying Orca " << endl;
  delegate_ = NetcdfOrcaInterpretor::guess(*this);

  if (delegate_) return delegate_;

  MagLog::warning()
      << "Could not guess the type of netcdf: Use default -->matrix" << endl;
...
void NetcdfInterpretor::setDimensions(const stringarray& value,
  ...
  for (stringarray::const_iterator val = value.begin(); val != value.end();
       ++val) {
    tokens.clear();
...
double NetcdfInterpretor::missing(Netcdf& netcdf) const {
  double missing = netcdf.getAttribute(missing_attribute_,
                                       std::numeric_limits<double>::max());
  return missing;
}

bool NetcdfInterpretor::reference_date(Netcdf& netcdf, const string& var,
                                       const string& refdate, string& basedate,
  ...
  if (factor != factors.end())
    std::transform(coords.begin(), coords.end(), coords.begin(),
                   Multiply(factor->second, missing_value));
  std::transform(coords.begin(), coords.end(), coords.begin(),
                 Plus(diff, missing_value));
  return true;
}

bool NetcdfInterpretor::cf_date(Netcdf& netcdf, const string& var,
                                const string& refdate, string& basedate,
                                vector<double>& coords,
                                vector<double>& originals) {
...
string NetcdfInterpretor::getAttribute(const string& var, const string& attr,
                                       const string& def) {
  Netcdf netcdf(path_, dimension_method_);
  if (var.empty()) return netcdf.getAttribute(attr, def);
...
void NetcdfInterpretor::visit(TextVisitor& title) {
  ...
  string name =
      netcdf.getVariableAttribute(field_, "standard_name", string(""));
  name = netcdf.getVariableAttribute(field_, "long_name", name);

  title.addAutomaticTitle(netcdf.getAttribute("title", name));
}

void NetcdfInterpretor::getAttributes(Netcdf& nc, const string& varName,
                                      string& keys, string& values) {
  try {
    NetVariable var = nc.getVariable(varName);
    bool first = true;
    for (map<string, NetAttribute>::iterator it = var.attributes_.begin();
         it != var.attributes_.end(); it++) {
      string val;
      it->second.get(val);
...
void NetcdfGuessInterpretor::visit(MetaDataCollector& info) {
  for (MetaDataCollector::iterator key = info.begin(); key != info.end();
       ++key) {
