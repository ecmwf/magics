...
void BinningObject::listx(vector<double> &vals, double min, double max) {
  ...
  for (vector<double>::iterator val = x_list_.begin(); val != x_list_.end();
       ++val) {
...
Matrix *BinningObject::operator()(PointsList &points) {
  ...
  if (binner != binners_x_.end()) {
    ...
    if (matrix->columnsAxis().empty()) {
      MagLog::warning() << " could not find any binns: return to count method"
                        << endl;
      countx(matrix->columnsAxis(), minx, maxx);
    }
  } else {
    MagLog::warning() << " could not find the method " << x_
                      << " for binning: return to count method" << endl;
    countx(matrix->columnsAxis(), minx, maxx);
  }
  ...
  if (binner != binners_y_.end()) {
    ...
    MagLog::warning() << " could not find the method " << y_
                      << " for binning: return to count method" << endl;
