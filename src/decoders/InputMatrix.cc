...
InputMatrix::InputMatrix()
    : matrix_(0), u_(0), v_(0), speed_(0), direction_(0) {
...
void InputMatrix::filter(Matrix &data) {
  ...
  for (unsigned int i = 0; i < data.size(); i++)
    if (data[i] <= suppress_below_ || data[i] >= suppress_above_ ||
        std::isnan(data[i]) || std::isinf(data[i]))
      data[i] = data.missing();
}
void InputMatrix::visit(MetaDataCollector &visitor) { metadata(visitor); }

void InputMatrix::metadata(MetaDataCollector &visitor) const {
  ParamJSon data = ParamJSon(metadata_);

  for (map<string, string>::iterator key = visitor.begin();
       key != visitor.end(); ++key) {
...
PointsHandler &InputMatrix::points(const Transformation &transformation) {
  this->pointsHandlers_.push_back(
      new BoxPointsHandler(this->matrix(), transformation, true));
...
void InputMatrix::customisedPoints(const BasicThinningMethod &thinning,
