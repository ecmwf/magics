...
void XYList::customisedPoints(const std::set<string> &needs,
...
void XYList::customisedPoints(const Transformation &, const std::set<string> &s,
                              CustomisedPointsList &out) {
...
void XYList::getReady(const Transformation &transformation) {
  try {
    if (transformation.xAxisType() == "date") {
      ...
      if (x_date_.empty() && !x_base_.empty()) {
        ...
        for (vector<double>::const_iterator x = x_.begin(); x != x_.end();
             ++x) {
      ...
      if (!x_date_.empty())
        x_.clear();
      for (vector<string>::const_iterator x = x_date_.begin();
           x != x_date_.end(); ++x)
  ...
  try {
    ...
    if (transformation.yAxisType() == "date") {
      if (y_date_.empty() && !y_base_.empty()) {
        ...
        for (vector<double>::const_iterator y = y_.begin(); y != y_.end();
             ++y) {
      ...
      if (!y_date_.empty())
        y_.clear();
      for (vector<string>::const_iterator y = y_date_.begin();
           y != y_date_.end(); ++y)
  ...
  try {
    if (transformation.xAxisType() == "date") {
      ...
      if (x2_date_.empty() && !x2_base_.empty()) {
        ...
        for (vector<double>::const_iterator x = x2_.begin(); x != x2_.end();
             ++x) {
          DateTime date(base + Second((*x) * off));
      ...
      if (!x2_date_.empty())
        x2_.clear();
      for (vector<string>::const_iterator x = x2_date_.begin();
           x != x2_date_.end(); ++x)
  ...
  try {
    ...
    if (transformation.yAxisType() == "date") {
      if (y2_date_.empty() && !y2_base_.empty()) {
        ...
        for (vector<double>::const_iterator y = y2_.begin(); y != y2_.end();
             ++y) {
          DateTime date(base + Second((*y) * off));
      ...
      if (!y2_date_.empty())
        y2_.clear();
      for (vector<string>::const_iterator y = y2_date_.begin();
           y != y2_date_.end(); ++y)
  ...
  try {
    for (vector<string>::const_iterator x = x_upper_date_.begin();
         x != x_upper_date_.end(); ++x)
      x_upper_.push_back(transformation.x(*x));
  } catch (...) {
  }
  try {
    for (vector<string>::const_iterator y = y_upper_date_.begin();
         y != y_upper_date_.end(); ++y)
      y_upper_.push_back(transformation.y(*y));
  } catch (...) {
  }

  try {
    for (vector<string>::const_iterator x = x_lower_date_.begin();
         x != x_lower_date_.end(); ++x)
      x_lower_.push_back(transformation.x(*x));
  } catch (...) {
  }
  try {
    for (vector<string>::const_iterator y = y_lower_date_.begin();
         y != y_lower_date_.end(); ++y)
...
void XYList::visit(Transformation &transformation) {
  ...
  try {
    ...
    if (transformation.getAutomaticX()) {
      transformation.setMinMaxX(
          *std::min_element(this->x_.begin(), this->x_.end()),
          *std::max_element(this->x_.begin(), this->x_.end()));
    }
    if (transformation.getAutomaticY()) {
      transformation.setMinMaxY(
          *std::min_element(this->y_.begin(), this->y_.end()),
