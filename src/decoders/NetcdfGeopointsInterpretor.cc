...
static void setDim(Netcdf& netcdf, const string& name, vector<double>& values,
                   map<string, string>& first, map<string, string>& last) {
  try {
    netcdf.get(name, values, first, last);
  } catch (...) {
    int dim = netcdf.getDimension(name);
    for (int i = 0; i < dim; ++i) values.push_back(i);
  }
}

bool NetcdfGeopointsInterpretor::interpretAsPoints(
    PointsList& list, const Transformation& projection) {
  ...
  try {
    ...
    string units;
    if (magCompare(netcdf.getVariableAttribute(latitude_, "units", units),
                   string("radians"))) {
    ...
    if (magCompare(netcdf.getVariableAttribute(longitude_, "units", units),
                   "radians")) {
...
void NetcdfGeopointsInterpretor::visit(ValuesCollector& vcp,
                                       PointsList& points) {
  if (field_.empty()) {
  ...
  for (ValuesCollector::iterator it = vcp.begin(); it != vcp.end(); ++it) {
    ...
    for (int i = 0; i < idxV.size(); i++) {
      int idx = idxV[i];
      double d = magics::geoDistanceInKm(points.at(idx)->y(),
                                         points.at(idx)->x(), y, x);
    ...
    if (minIdx >= 0)
      (*it).push_back(new ValuesCollectorData(
          points.at(minIdx)->x(), points.at(minIdx)->y(),
...
bool NetcdfXYpointsInterpretor::interpretAsPoints(
    PointsList& list, const Transformation& projection) {
  Netcdf netcdf(path_, dimension_method_);
  ...
  try {
    ...
    while (x != xs.end() && y != ys.end()) {
      ...
      if (!same(*x, xmissing) || !same(*y, ymissing)) {
        ...
        if (projection.in(*xy)) list.push_back(xy);
      }
      x++;
      y++;
    }
...
bool NetcdfXYpointsInterpretor::interpretAsPoints(
    PointsList& list, const std::set<string>& needs) {
  Netcdf netcdf(path_, dimension_method_);
  ...
  try {
    ...
    while (x != xs.end() && y != ys.end()) {
      ...
      if (!same(*y, ymissing) || !same(*x, xmissing)) {
        list.push_back(new UserPoint(
            *x, *y, value, (same(*y, ymissing) || same(*x, xmissing))));
      }

      x++;
      y++;
    }
    if (needs.find("area") == needs.end()) return true;
    vector<double> y2s;
    vector<double> x2s;
...
void NetcdfXYpointsInterpretor::visit(Transformation& transformation) {
  ...
  try {
    refDateX_ =
        (transformation.getAutomaticX()) ? "" : transformation.getReferenceX();
    refDateY_ =
        (transformation.getAutomaticY()) ? "" : transformation.getReferenceY();
    PointsList points;
    interpretAsPoints(points);

    if (transformation.getAutomaticX()) {
      if (!this->baseDateX_.empty()) {
        transformation.setDataMinMaxX(points.minX(), points.maxX(),
                                      this->baseDateX_);
      }
...
void NetcdfXYpointsInterpretor::customisedPoints(
    const Transformation& transformation, const std::set<string>& needs,
...
void NetcdfXYpointsInterpretor::visit(MetaDataCollector& mdc) {
  ...
  if (attrIt != attrs.end()) {
    ...
    if (str == "MVERTPROFVIEW") isVprof = true;
  }
...
void NetcdfXYpointsInterpretor::visit(ValuesCollector& vcp,
                                      PointsList& points) {
  if (field_.empty()) {
    vcp.setHasValue(false);
  }
