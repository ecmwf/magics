...
void InputData::dateSetting(vector<string> &dates, vector<double> &values,
                            DateTime &base, bool update) {
  if (dates.empty())
    return;
  if (update)
...
void InputData::customisedPoints(const Transformation &transformation,
...
void InputData::customisedPoints(const std::set<string> &,
                                 CustomisedPointsList &out) {
  prepare();
  vector<double>::const_iterator x = x_values_.begin();
  vector<double>::const_iterator y = y_values_.begin();
...
void InputData::getReady(const Transformation &transformation) {
  try {
    for (vector<string>::const_iterator x = date_x_.begin(); x != date_x_.end();
         ++x)
      x_.push_back(transformation.x(*x));
  } catch (...) {
  }
  try {
    for (vector<string>::const_iterator y = date_y_.begin(); y != date_y_.end();
         ++y)
...
void InputData::visit(Transformation &transformation) {
  ...
  try {
    ...
    if (transformation.getAutomaticX()) {
      double min = INT_MAX;
      double max = -INT_MAX;
      for (vector<double>::iterator x = x_values_.begin(); x != x_values_.end();
           ++x) {
        if (same(*x, x_missing_))
          continue;
        if (min > *x)
      ...
      for (vector<double>::iterator x = x2_values_.begin();
           x != x2_values_.end(); ++x) {
        if (same(*x, x_missing_))
          continue;
        if (min > *x)
    ...
    if (transformation.getAutomaticY()) {
      double min = INT_MAX;
      double max = -INT_MAX;
      for (vector<double>::iterator y = y_values_.begin(); y != y_values_.end();
           ++y) {
        if (same(*y, y_missing_))
          continue;
        if (min > *y)
      ...
      for (vector<double>::iterator y = y2_values_.begin();
           y != y2_values_.end(); ++y) {
        if (same(*y, y_missing_))
          continue;
        if (min > *y)
...
PointsHandler &InputData::points(const Transformation &transformation,
                                 bool all) {
  prepare();
  ...
  if (magCompare(x_type_, "date") || magCompare(y_type_, "date")) {
    ...
    if (all)
      ...
      pointsHandlers_.push_back(
          new BoxPointsHandler(*this, transformation, false));
  }
