...
void EpsXmlInput::visit(Transformation &transformation) {
  ...
  transformation.setDataMinMaxX((minstep_ - dateOffset_) * 3600,
                                (maxstep_ + dateOffset_) * 3600, base_);
...
void EpsXmlInput::customisedPoints(const std::set<string> &,
                                   CustomisedPointsList &out) {
  MagLog::dev() << "EpsXmlInput::customisedPoints-->need to be implemented"
                << std::endl;
...
void EpsXmlInput::visit(TextVisitor &title) {
  ...
  if (long_title_) {
    ...
    line << station_ << "(" << position.asLatitude() << ", "
