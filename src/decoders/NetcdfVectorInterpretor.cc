...
void NetcdfVectorInterpretor::customisedPoints(const std::set<string> &,
                                               CustomisedPointsList &) {
  MagLog::dev()
      << "NetcdfVectorInterpretor::customisedPoints()--> to be implemented!"
      << endl;
}
...
void NetcdfVectorInterpretor::customisedPoints(
...
void NetcdfGeoVectorInterpretor::customisedPoints(
    const Transformation &transformation, const std::set<string> &,
    CustomisedPointsList &list, int thinning) {
  Netcdf netcdf(path_, dimension_method_);
  try {
...
NetcdfInterpretor *
NetcdfGeoVectorInterpretor::guess(const NetcdfInterpretor &from) {
  ...
  if (latitude_x.size() && longitude_x.size() && latitude_y == latitude_x &&
...
void NetcdfGeoPolarMatrixInterpretor::customisedPoints(
    const Transformation &, const std::set<string> &,
