...
MagDate::MagDate(long year, long month, long day)
...
long MagDate::parse(const string &s) {
  ...
  /*
   *   Stephan: Removed exception - caused Metview to not plot the field (but
   * title) - see MAGP-455
   */
  if (err)
    //		throw MagicsException(string("Invalid date ") + s);
    MagLog::warning() << "The date read looks invalid: " << s
...
MagDate::operator tm() const {
  ...
  tm_date.tm_wday =
      (day() + (13 * mm - 27) / 5 + yy + yy / 4 - yy / 100 + yy / 400) % 7;
  return tm_date;
}
...
long MagDate::yyyymmdd() const { return julianToMagDate(julian_); }
