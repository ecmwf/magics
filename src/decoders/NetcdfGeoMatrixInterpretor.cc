...
string NetcdfGeoMatrixInterpretor::proj4Detected(Netcdf& netcdf) {
  ...
  if (proj4.size()) return proj4;
  string mapping =
      netcdf.getVariableAttribute(field_, "grid_mapping", string(""));
  if (mapping.size())
...
UserPoint* NetcdfGeoMatrixInterpretor::newPoint(const string& proj4, double lon,
                                                double lat, double val) {
  double x = lon;
  double y = lat;
...
void NetcdfGeoMatrixInterpretor::visit(Transformation& transformation) {
  ...
  if (transformation.getAutomaticX()) {
    transformation.setMinMaxX(matrix_->columnsAxis().front(),
                              matrix_->columnsAxis().back());
  }
  if (transformation.getAutomaticY()) {
    transformation.setMinMaxY(matrix_->rowsAxis().front(),
                              matrix_->rowsAxis().back());
  }
}
  ...
  try {
    ...
    for (unsigned int lat = 0; lat < latitudes.size(); lat++) {
      for (unsigned int lon = 0; lon < longitudes.size(); lon++) {
        ...
        list.push_back(newPoint(proj4, longitudes[lon], latitudes[lat],
                                (values[val] * scaling_) + offset_));
      }
    }
...
void NetcdfGeoMatrixInterpretor::statsData(
...
void NetcdfGeoMatrixInterpretor::customisedPoints(
  ...
  try {
    ...
    for (unsigned int lat = 0; lat < latitudes.size(); lat += thinning) {
      for (unsigned int lon = 0; lon < longitudes.size(); lon += thinning) {
        ...
        if (same(xcomponent[val], missing_x)) continue;
        if (same(ycomponent[val], missing_y)) continue;
        vector<UserPoint> points;
        transformation.populate(longitudes[lon], latitudes[lat], 0, points);
        for (vector<UserPoint>::iterator pt = points.begin();
             pt != points.end(); ++pt) {
...
NetcdfInterpretor* NetcdfGeoMatrixInterpretor::guess(
    const NetcdfInterpretor& from) {
  ...
  if (variable.empty()) variable = from.x_component_;
  ...
  string projection_y_coordinate =
      netcdf.detect(variable, "projection_y_coordinate");
  string projection_x_coordinate =
      netcdf.detect(variable, "projection_x_coordinate");

  cout << "PRINT-->projection_y_coordinate" << projection_y_coordinate << " "
       << projection_x_coordinate << endl;

  if (projection_y_coordinate.size() && projection_y_coordinate.size()) {
    NetcdfGeoMatrixInterpretor* interpretor = new NetcdfGeoMatrixInterpretor();
    interpretor->NetcdfInterpretor::copy(from);
    cout << "PRINT-->PROJ4 " << interpretor->proj4Detected(netcdf) << endl;
