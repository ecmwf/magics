...
NetDimension::NetDimension(Netcdf *netcdf, const string &name, int index,
                           int variable)
    : parent_(netcdf), name_(name), first_(0), index_(index),
      variable_(variable) {
...
template <class T> int find(const T &value, vector<T> &values) {
...
template <> map<nc_type, Accessor<int> *> *Accessor<int>::accessors_ = 0;
} // namespace magics
