...
string EpsParameter::x(const string &prefix, const string &name) const {
  ...
  //
  //	}
  return (names.find(name) == names.end()) ? prefix + lowerCase(name)
                                           : prefix + names.find(name)->second;
}
...
void EpsParameter::steps(const vector<double> &steps) {
  for (vector<double>::const_iterator step = steps.begin(); step != steps.end();
       ++step)
...
public:
  Eps2mTemperature(const string &name, const string &title, const string &code)
      : EpsParameter(name, title, code) {}
  Eps2mTemperature(const string &code)
      : EpsParameter(
            "min", "2m Temperature (deg C) (No reduction ( working on it...) ",
            code) {}
  Eps2mTemperature()
      : EpsParameter(
            "min", "2m Temperature (deg C) (No reduction ( working on it...) ",
  ...
  const string &title() const {
    ...
    title << "2m Temperature "
          << " (&#176;C) " << reduction_ << " from " << maground(detz_)
          << " m (" << detResolution_ << ") and " << maground(epsz_) << " m ("
          << epsResolution_ << ")";
    title_ = title.str();
    return title_;
  }
  ...
  virtual double operator()(double value, const string &data) const {
    ...
    map<string, CorrectionFunction>::const_iterator correction =
        map_.find(data);
    return (correction != map_.end())
               ? (this->*correction->second)(value)-273.15
               : value - 273.15;
  }
...
public:
  ...
  const string &title() const {
    ostringstream title;
    title << "2m min/max temperature (&#176;C) " << reduction_ << " from "
  ...
  spot_query_result *prepare(const SpotDecoder &decoder,
                             vector<CustomisedPoint *> &out) {
...
public:
  EpsPrecipitation(const string &code = "143.128")
      : EpsParameter("precip", "Total Precipitation (mm/24h)", code) {
    step_ = 0;
  }
  ~EpsPrecipitation() {}
...
public:
  EpsCloudCover(const string &code = "164.128")
      : EpsParameter("cloud-cover", "Daily mean of Total Cloud Cover (okta)",
                     code) {}
  ~EpsCloudCover() {}
...
public:
  ...
  string x(const string &prefix, const string &val) const {
    return (prefix == "clim_") ? prefix + val
                               : prefix + tostring(2 * tonumber(val));
  }
...
public:
  ...
  const string &title() const {
    ...
    title_ = "Geopotential 500hPa - Probability for 2.5 dam intervals      "
             "Range : " +
             tostring(maground(range)) + " dam";
    return title_;
  }
  virtual double operator()(double value, const string &) const {
    return value / 100;
...
public:
  ...
  const string &title() const {
    double range = maxy_ - miny_;
    title_ = "Temperature 850hPa - Probability for 1&#176;C intervals      "
             "Range :  " +
             tostring(maground(range)) + " &#176;C";
    return title_;
  }
  virtual double operator()(double value, const string &) const {
    return value - 273.15;
...
public:
  ClimPrecipitation()
      : EpsParameter("clim-precip", "Precipitation", "143.128") {}
  ~ClimPrecipitation() {}
  const string &title() const {
...
public:
  EpsWindFF(const string &code = "165.128")
      : EpsParameter("10m-wind", "Daily mean of 10m Wind Speed (m/s)", code) {}
  ~EpsWindFF() {}
  const string &title() const {
...
public:
  EpsWindDD()
      : EpsParameter("10m-wind-dd", "Daily mean of 10m Wind", "249.140") {}
  ~EpsWindDD() {}
...
static SimpleObjectMaker<EfiMaxTemperature, EpsParameter>
    efi_2mt_max("efi-2mt-max");
static SimpleObjectMaker<EfiMinTemperature, EpsParameter>
    efi_2mt_min("efi-2mt-min");
static SimpleObjectMaker<EfiMeanTemperature, EpsParameter>
    efi_2mt_mean("efi-2mt-mean");
static SimpleObjectMaker<ClimMaxTemperature, EpsParameter>
    clim_2mt_max("clim-2mt-max");
static SimpleObjectMaker<ClimMinTemperature, EpsParameter>
    clim_2mt_min("clim-2mt-min");
static SimpleObjectMaker<ClimMeanTemperature, EpsParameter>
    clim_2mt_mean("clim-2mt-mean");

static SimpleObjectMaker<EfiWindGust, EpsParameter>
    efi_wind_gust("efi-wind-gust");
static SimpleObjectMaker<EfiWind, EpsParameter> efi_wind("efi-wind");
static SimpleObjectMaker<ClimWindGust, EpsParameter>
    clim_wind_gust("clim-wind-gust");
static SimpleObjectMaker<ClimWind, EpsParameter> clim_wind("clim-wind");

static SimpleObjectMaker<EfiPrecipitation, EpsParameter>
    efi_precip("efi-precip");
static SimpleObjectMaker<ClimPrecipitation, EpsParameter>
    clim_eps_precip("clim-precip");

static SimpleObjectMaker<Eps2mTemperature, EpsParameter> mt("2mt");
static SimpleObjectMaker<ClimEpsMaxTemperature, EpsParameter>
    tempe_clim_max("clim-eps-2mt-max");
static SimpleObjectMaker<ClimEpsMinTemperature, EpsParameter>
    tempe_clim_min("clim-eps-2mt-min");
static SimpleObjectMaker<ClimEpsTemperature, EpsParameter>
    tempe_clim("clim-eps-2mt");
...
static SimpleObjectMaker<EpsTemperature, EpsParameter>
    temperature("temperature");
static SimpleObjectMaker<ClTemperature, EpsParameter>
    cltemperature("clim-temperature");
static SimpleObjectMaker<EpsWindFF, EpsParameter> wind("10m-wind");
static SimpleObjectMaker<ClimEpsWindFF, EpsParameter>
    clim_wind_ff("clim-10m-wind");
...
static SimpleObjectMaker<ClimEpsCloudCover, EpsParameter>
    clim_cloud("clim-eps-cloud-cover");

static SimpleObjectMaker<EpsPrecipitation, EpsParameter> precip("precip");
static SimpleObjectMaker<ClimEpsPrecipitation, EpsParameter>
    clim_precip("clim-eps-precip");

spot_query_result *EpsParameter::prepare(const SpotDecoder &decoder,
                                         vector<CustomisedPoint *> &out) {
  spot_query *query = decoder.newQuery();
...
template <class T> inline void read(const string &str, T &i) {
...
string convert(const string &name) {
  ...
  return (names.find(name) == names.end()) ? lowerCase(name)
...
void EpsParameter::interpretResult(spot_query_result *result,
                                   vector<CustomisedPoint *> &out) {
  size_t len = 1024;
  char tmp[1024];
  ...
  for (map<int, int>::const_iterator step = rows.begin(); step != rows.end();
       ++step) {
    ...
    for (map<string, int>::const_iterator info = columns.begin();
         info != columns.end(); ++info) {
      if (!steps_.empty() && steps_.find(step->first) == steps_.end()) {
        ok = false;
        continue;
      }

      MagLog::debug() << step->first << " -> " << info->first << " = "
                      << values[info->second] << "->"
                      << (*this)(values[info->second], info->first) << endl;
      (*point)["step"] = x(step->first) * 3600;
      stepvalues(x(step->first) * 3600, xpos);

      map<string, EpsParameter::SpecificFunction>::const_iterator function =
          specifics_.find(type_);
      (*point)["width"] = 1 * 3600;
      if (type_ == "eps10")
        specific10(*point);
      if (type_ == "eps15")
      ...
      if (!std::isnan(values[info->second])) {
        (*point)[x(prefix_, info->first)] =
            (*this)(values[info->second], info->first);
        // MagLog::debug() <<  x(info->first) << "----->" <<
        // (*this)(values[info->second], info->first) << endl;
        ...
        MagLog::warning() << " SPOTBASE returns nan for " << info->first
                          << " : data ignored for step " << (*point)["shift"]
                          << endl;
        ok = false;
      }
    }
  ...
  for (vector<CustomisedPoint *>::iterator point = out.begin();
       point != out.end(); ++point) {
  ...
  if (threshold_ < 30) {
    ...
    int i = maxs.size() - 1;
    int n = maxs.size();
    for (vector<double>::reverse_iterator val = maxs.rbegin();
         val != maxs.rend(); ++val) {
      p2 = (*val) / maxs.back();

      if (p2 < (1 - i / n) * percentile_ && (p1 - p2) > threshold_ / n &&
          *val >= limit) {
        mm = *val;
      }
      p1 = p2;
...
void EpsgramDecoder::moreTitle(TextVisitor &title) const {
  ...
  if (long_title_point_) {
    station = station + string(" ") + grid_.asLatitude() + " " +
              grid_.asLongitude() + landsea + height;
  }
...
void SpotDecoder::customisedPoints(const std::set<string> &,
...
void SpotDecoder::decode(bool check) {
  ...
  if (!parameter_) {
    ...
    MagLog::warning() << "Eps Data access: parameter [" << param_
                      << "] unknown\n";
    return;
  }
#endif
...
MatrixHandler &EpsgramDecoder::matrix() {
  ...
  for (vector<CustomisedPoint *>::const_iterator point = points_.begin();
       point != points_.end(); ++point) {
    for (IntervalMap<int>::iterator interval = array.begin();
         interval != array.end(); ++interval) {
      interval->second = 0;
    }
    ...
    for (IntervalMap<int>::iterator interval = array.begin();
         interval != array.end(); ++interval) {
      matrix_[column + row * steps.size()] =
          interval->second * 2; // in percentage!
      row++;
    }
    column++;
  }
...
spot_query *SpotDecoder::newQuery() const { return newQuery(database_); }
...
void EfigramDecoder::decode() {
  ...
  for (vector<string>::iterator date = dates_.begin(); date != dates_.end();
       ++date) {
    SpotDecoder::steps_.clear();
    SpotDecoder::steps_.push_back(*step);
    int legend_step = *step + 12;
    SpotDecoder::database_ =
        EfigramDecoderAttributes::database_ + "/" + *date + "/epsdb";
  ...
  // Now the Clim!
  {
    ...
    SpotDecoder::database_ =
        EfigramDecoderAttributes::clim_database_ + "/" + clim_date_ + "/epsdb";
...
void EfigramDecoder::visit(LegendVisitor &legend) {
  legend.columns_ = 5;
  for (vector<string>::iterator entry = legends_.begin();
       entry != legends_.end(); ++entry) {
...
void EfigramDecoder::visit(TextVisitor &title) {
  ...
  if (long_title_) {
    ...
    MagLog::debug() << "HELOOOO-->Date/time ---> [" << date_ << ", " << time_
                    << ", " << step_ << "]\n";
    ostringstream out;
    ...
    const std::time_put<char> &tfac = use_facet<time_put<char>>(loc);
    string format = "%A %e %B %Y %H UTC";
    tfac.put(out, out, ' ', &cfrom, format.c_str(),
             format.c_str() + format.length());
    out << " to ";
    tfac.put(out, out, ' ', &cto, format.c_str(),
             format.c_str() + format.length());

    ostringstream line;
    UserPoint position(SpotDecoder::longitude_, SpotDecoder::latitude_);

    line << "Forecast and M-Climate cumulative distribution functions with EFI "
            "values at "
         << position.asLatitude() << "/" << position.asLongitude() << endl;
