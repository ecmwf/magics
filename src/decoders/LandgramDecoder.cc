...
void LandgramDecoder::customisedPoints(const std::set<string> &,
  ...
  for (unsigned int step = 0; step < steps_.size(); step++) {
    ...
    for (unsigned int height = 0; height < heights_.size(); height++) {
      ...
      point->insert(
          make_pair(key.str(), values_[(step * heights_.size()) + height]));
    }
...
void LandgramDecoder::decode() {
  ...
  for (vector<double>::const_iterator height = heights_.begin();
       height != heights_.end(); ++height) {
    matrix_.rowsAxis().push_back(*height);
  }
  MagLog::dev() << " step-->" << std::endl;
  for (vector<double>::const_iterator step = steps_.begin();
...
void LandgramDecoder::visit(const XmlNode &node) {
  ...
  if (node.name() == "forecast") {
    ...
    first_++;
    MagLog::dev() << "step --> " << tonumber(node.getAttribute("value"))
                  << endl;
    ...
    MagLog::dev() << "height --> " << tonumber(node.getAttribute("height"))
                  << " = value " << tonumber(node.getAttribute("value"))
                  << endl;
    node.visit(*this);
  }
}
