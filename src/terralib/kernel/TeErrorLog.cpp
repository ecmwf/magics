...
TeErrorLog::TeErrorLog() : logFile_(0) {
  ...
  errorMessage_[PROJECTION_NOT_AVAILABLE] =
      "Projection not available in TerraLib";
  ...
  errorMessage_[NOT_ENOUGH_ROWS] =
      "Less rows in MID file than regions in MIF file";
  errorMessage_[NOT_POLYGON_FILE] = "Not a polygon (planar areas) file";
  errorMessage_[PLINE_ERROR] = "Wrongly defined PLINE attribute";
  errorMessage_[NO_DTM_QUOTE_VALUE] =
      "Unable to define an attribute as a DTM quote file";
  ...
  errorMessage_[ATTRIBUTE_INDEX_MISMATCH] =
      "Mismatch between attribute index and number of geometries";
  ...
  errorMessage_[LINE_IS_NOT_RING] =
      "Tried to a create a ring from a non-closed line";

  // Error codes for tables

  errorMessage_[ROW_MISMATCH] =
      "Unable to insert a row into a table (attribute mismatch)";
  errorMessage_[NO_SUCH_ATTRIBUTE] = "Attribute does not exist in the table";

  // Error codes for Raster stuff
  errorMessage_[UNKNOWN_DECODER_TECHNOLOGY_TYPE] =
      "Decoder technology not specified";
  errorMessage_[UNKNOWN_RASTER_FORMAT] = "Raster format not supported";

  // Error codes for layer stuff
  errorMessage_[GEOMETRY_REPRESENTATION_MISMATCH] =
      "Geometry representation has not been created";
...
TeErrorLog::~TeErrorLog() { endSession(); }
string TeErrorLog::message(const TeErrorType msgCode) {
...
void TeErrorLog::insert(TeErrorType code, const string &msg) {}

void TeErrorLog::startSession(const string &errLogFileName) { endSession(); }

int TeErrorLog::endSession() { return 0; }
