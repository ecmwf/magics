...
// Name						Units  Long  Lat  Par1  Par2 Sca
// Eas Nor
...
const char **TeGetProjInfo() { return teProjInfo; }
...
TeProjection *TeProjectionFactory::make(const TeProjectionParams &par) {
  ...
  if (par.name == "UTM")
    return new TeUtm(par.datum, par.lon0, par.lat0, par.offx, par.offy, punits,
                     par.scale, par.hemisphere);

  if (par.name == "LambertConformal")
    return new TeLambertConformal(par.datum, par.lon0, par.lat0, par.offx,
                                  par.offy, par.stlat1, par.stlat2, punits);

  if (par.name == "Albers")
    return new TeAlbers(par.datum, par.lon0, par.lat0, par.offx, par.offy,
                        par.stlat1, par.stlat2, punits);
  ...
  if (par.name == "Polyconic")
    return new TePolyconic(par.datum, par.lon0, par.lat0, par.offx, par.offy,
                           punits);

  if (par.name == "Mercator")
    return new TeMercator(par.datum, par.lon0, par.lat0, par.offx, par.offy,
                          par.stlat1, punits);
  ...
  if (par.name == "CylindricalEquidistant")
    return new TeCylindricalEquidistant(par.datum, par.lon0, par.offx, par.offy,
                                        par.stlat1, punits);

  if (par.name == "PolarStereographic")
    return new TePolarStereographic(par.datum, par.lon0, par.offx, par.offy,
                                    punits, par.hemisphere);

  // FAMI
  if (par.name == "Satellite")
    return new TeSatelliteProjection(par.datum, par.offx, par.offy, par.pri,
                                     par.prj, par.pis, par.pjs, par.lat0,
...
void TeProjection ::ChangeLL(double &lon1, double &lat1) {
  ...
  if (GPdatum.xShift() == TeMAXFLOAT ||
      GPdestination->GPdatum.xShift() == TeMAXFLOAT) {
    x2 = x1;
    y2 = y1;
    z2 = z1;
  } else {
  ...
  equad2 -= (GPdestination->GPdatum.flattening()) *
            (GPdestination->GPdatum.flattening());
  lat2 = lat1;
  do {
    a1 = sin(lat2);
    a1 *= a1;
...
TeCoord2D TeUtm ::PC2LL(TeCoord2D &ptpc) {
  double k0,    // Scale factor
      ...
      elinquad, // Ancillary variables
      e1, aux1, aux2, aux3, aux4, aux5, m, mi, aux6, aux7, aux8, lat1, c1, t1,
      quoc, r1, d, aux9, aux10, aux11, aux12, ptpcx, ptpcy;
...
TeCoord2D TeLambertConformal ::LL2PC(TeCoord2D &ptll) {
  ...
  double equad, // Squared eccentricity
      ...
      m1, m2, aux1, aux2, aux0, t1, t2, t0, n, efe, ro0, aux, t, ro, teta,
      ptllx, ptlly;
  ...
  aux1 =
      sqrt(((double)1 - e * sin(GPstlat1)) / ((double)1 + e * sin(GPstlat1)));
  aux2 =
      sqrt(((double)1 - e * sin(GPstlat2)) / ((double)1 + e * sin(GPstlat2)));
  aux0 = sqrt(((double)1 - e * sin(GPlat0)) / ((double)1 + e * sin(GPlat0)));
  t1 = ((1. - tan(GPstlat1 / (double)2)) / (1. + tan(GPstlat1 / (double)2))) /
       pow(aux1, e);
  t2 = ((1. - tan(GPstlat2 / (double)2)) / (1. + tan(GPstlat2 / (double)2))) /
       pow(aux2, e);
  t0 = ((1. - tan(GPlat0 / (double)2)) / (1. + tan(GPlat0 / (double)2))) /
       pow(aux0, e);
  ...
  t = ((1. - tan(ptlly / (double)2)) / (1. + tan(ptlly / (double)2))) /
      pow(aux, e);
  ro = Prd * efe * pow(t, n);
  teta = n * (ptllx - GPlon0);
  ptllx = ro * sin(teta);
...
TeCoord2D TeLambertConformal ::PC2LL(TeCoord2D &ptpc) {
  double equad, // Squared eccentricity
      pi,       // PI value
      e, m1, m2, aux1, aux2, aux0, t1, t2, t0, n, efe, ro0, ro, teta, t, xx,
      aux3, aux4, aux5, ptpcx, ptpcy;
...
TeCoord2D TePolyconic ::LL2PC(TeCoord2D &ptll) {
  ...
  aux01 = (1. - equad / 4. - 3. * equad * equad / 64. -
           5. * equad * equad * equad / 256.) *
          GPlat0;
  aux02 = (3. * equad / 8 + 3. * equad * equad / 32. +
           45. * equad * equad * equad / 1024.) *
          sin((double)2 * GPlat0);
  aux03 = (15. * equad * equad / 256. + 45. * equad * equad * equad / 1024.) *
          sin((double)4 * GPlat0);
  ...
  if (ptlly == 0.) {
    ...
    aux1 = (1. - equad / 4. - 3. * equad * equad / 64. -
            5. * equad * equad * equad / 256.) *
           ptlly;
    aux2 = (3. * equad / 8 + 3. * equad * equad / 32. +
            45. * equad * equad * equad / 1024.) *
           sin((double)2 * ptlly);
    aux3 = (15. * equad * equad / 256. + 45. * equad * equad * equad / 1024.) *
           sin((double)4 * ptlly);
...
TeCoord2D TePolyconic ::PC2LL(TeCoord2D &ptpc) {
  double A, B, C,
      equad, // Squared eccentricity
      aux01, // Ancillary variables
      aux02, aux03, aux04, m0, mn, mnl, ma, cp, lat1, lat2, aux05, aux06, aux07,
      aux08, aux09, aux10, aux11, aux12, aux21, aux22, aux23, aux24, ptpcx,
      ptpcy;
  ...
  if (ptpcy == (-m0)) {
    ...
    do {
      ...
      // mn calculation
      aux21 = (1. - equad / 4. - 3. * equad * equad / 64. -
               5. * equad * equad * equad / 256.) *
              lat2;
      aux22 = (3. * equad / 8. + 3. * equad * equad / 32. +
               45. * equad * equad * equad / 1024.) *
              sin((double)2 * lat2);
      aux23 =
          (15. * equad * equad / 256. + 45. * equad * equad * equad / 1024.) *
          sin((double)4 * lat2);
      aux24 = (35. * equad * equad * equad / 3072.) * sin((double)6 * lat2);
      mn = Prd * (aux21 - aux22 + aux23 - aux24);

      // mnl calculation
      aux05 = 1. - equad / 4. - 3. * equad * equad / 64. -
              5. * equad * equad * equad / 256.;
      aux06 = 2. *
              (3. * equad / 8. + 3. * equad * equad / 32. +
               45. * equad * equad * equad / 1024.) *
              cos((double)2 * lat2);
      aux07 =
          4. *
          ...
          cos((double)4 * lat2);
      aux08 =
          6. * (35. * equad * equad * equad / 3072.) * cos((double)6 * lat2);
...
TeCoord2D TeAlbers ::LL2PC(TeCoord2D &ptll) {
  ...
  double equad, // Squared eccentricity
      e,        // Ancillary variables
      m1, m2, aux1, aux10, aux11, aux12, aux2, aux20, aux21, aux22, q0, q1, q2,
      q, n, c, ro0, teta, ro, ptllx, ptlly;
...
TeCoord2D TeAlbers ::PC2LL(TeCoord2D &ptpc) {
  ...
  double equad, // Squared eccentricity
      ...
      m2, aux10, aux11, aux12, aux20, aux21, aux22, q0, q1, q2, n, c, ro0, ro,
      q, aux, beta, aux1, aux2, aux3, teta, ptpcx, ptpcy;
  ...
  beta = asin(q / (1. - aux));
  aux1 = (equad / 3. + 31. * equad * equad / 180. +
          517. * equad * equad * equad / 5040.) *
         sin(2. * beta);
  aux2 = (23. * equad * equad / 360. + 251. * equad * equad * equad / 3780.) *
         sin(4. * beta);
...
// FAMI
void TePolarStereographic ::LL2PC(double xi, double yi, double &xo,
                                  double &yo) const {
  double k0, // Scale factor
      e,     // Eccentricity
      pi,    // Auxillary variables
...
void TePolarStereographic ::LL2PC(ostream &java) const {
  ...
  // java << "\tyll = xx + " << aux3 << "*Math.sin(2*xx) + " << aux4 <<
  // "*Math.sin(4*xx) + "<< aux5 << "*Math.sin(6*xx);\n";
...
TeCoord2D TePolarStereographic ::PC2LL(TeCoord2D &ptpc) {
  ...
  ptpcy = xx + aux3 * sin((double)2 * xx) + aux4 * sin((double)4 * xx) +
          aux5 * sin((double)6 * xx);
...
TeCoord2D TeSatelliteProjection::LL2PC(TeCoord2D &ptll) {
  ...
  if (x < 0.0) {
    ...
    v = (tan(dp) * tan(dp) * cos(dp) * cos(dp) * Rm2) +
        (tan(dl) * tan(dl) * Rd2);
...
TeCoord2D TeSatelliteProjection::PC2LL(TeCoord2D &ptpc) {
  double dl, dp, x, y, z, x1, x2, equad, n, d, sn, a, b, c, Rd2, Rm, Rm2, Rs2,
      v, ptpcx, ptpcy, yla, xlo, resx, resy;
  ...
  equad =
      2. * GPdatum.flattening() - GPdatum.flattening() * GPdatum.flattening();
  ...
  do {
    ...
    d = GPdatum.radius() / sqrt((double)1 - equad * pow(sin(yla), (double)2)) -
        n;
  } while (fabs(d) > 0.001);
...
TeProjection *TeGetTeProjectionFromSProj(const string &proj4) {
  ...
  for (unsigned int i = 0; i < proj4.size(); i++) {
    ...
    // ellipsoid figures using the option +ellps=acronym. But we are not
    // decoding them yet because of our problem with the shiftings from SAD69
  }
...
string TeGetWKTFromTeProjection(TeProjection *teproj) {
  ...
  TeProjInfo pjInfo = TeProjectionInfo(teproj->name());
  if (pjInfo.hasLon0)
    wkt += ",PARAMETER[\"central_meridian\"," +
           Te2String(teproj->lon0() * TeCRD, 6) + "]";

  if (pjInfo.hasLat0)
    wkt += ",PARAMETER[\"latitude_of_origin\"," +
           Te2String(teproj->lat0() * TeCRD, 6) + "]";

  if (pjInfo.hasStlat1)
    wkt += ",PARAMETER[\"standard_parallel_1\"," +
           Te2String(teproj->stLat1() * TeCRD, 6) + "]";

  if (pjInfo.hasStlat2)
    wkt += ",PARAMETER[\"standard_parallel_2\"," +
           Te2String(teproj->stLat2() * TeCRD, 6) + "]";
  ...
  if (pjInfo.hasOffy)
    wkt +=
        ",PARAMETER[\"false_northing\"," + Te2String(teproj->offY(), 6) + "]";
  ...
  while (*pszInput != '\0') {
    if (*pszInput == '"') {
      isInQuote = !isInQuote;
                ...
                *pszInput == '(' || *pszInput == ')')) {
      break;
    } else if (!isInQuote && (*pszInput == ' ' || *pszInput == '\t' ||
