...
void step(set<double> &values, set<double> &labels, double from, double to,
          double ref, double step, int freq) {
  if (ref > to)
    return;
  int l = 0;
...
void TephiGrid::visit(DrawingVisitor &out) {
  ...
  if (isotherm_) {
    ...
    MagFont font(isotherm_label_font_, isotherm_label_style_,
                 isotherm_label_size_);
    font.colour(*isotherm_label_colour_);

    double theta = magics::theta(xy.x() + 273.15, (xy.y()) * 100.);

    step(isotherms, labels, tmin, tmax, isotherm_reference_, isotherm_interval_,
         isotherm_label_frequency_);

    for (std::set<double>::iterator t = isotherms.begin(); t != isotherms.end();
         ++t) {
      Polyline poly;
      Colour colour = (*t != isotherm_reference_) ? *isotherm_colour_
                                                  : *isotherm_reference_colour_;
      poly.setColour(colour);
      poly.setThickness(isotherm_thickness_);
      poly.setLineStyle(isotherm_style_);

      poly.push_back(tephi(UserPoint(
          *t, pressureFromTheta(thmin + 273.15, *t + 273.15) / 100.)));
      poly.push_back(tephi(UserPoint(
          *t, pressureFromTheta(thmax + 273.15, *t + 273.15) / 100.)));
  ...
  if (dry_adiabatic_) {
    ...
    MagFont font(dry_adiabatic_label_font_, dry_adiabatic_label_style_,
                 dry_adiabatic_label_size_);
    font.colour(*dry_adiabatic_label_colour_);

    step(dry, labels, thmin, thmax, dry_adiabatic_reference_,
         dry_adiabatic_interval_, dry_adiabatic_label_frequency_);
    ...
    for (std::set<double>::iterator t = dry.begin(); t != dry.end(); ++t) {
      ...
      poly.push_back(tephi(UserPoint(
          tmin, magics::pressureFromTheta(*t + 273.15, tmin + 273.15) / 100)));
      poly.push_back(tephi(UserPoint(
          tmax, magics::pressureFromTheta(*t + 273.15, tmax + 273.15) / 100)));
      ...
      if (label == labels.end())
        continue;
      UserPoint point(
          tp_ll.x(), pressureFromTheta(*t + 273.15, tp_ll.x() + 273.15) / 100.);
      if (tephi.in(point)) {
        Text *text = new Text();
        text->setAngle(3.14 / 4);
  ...
  if (isobar_) {
    ...
    step(isobars, labels, pmin, pmax, isobar_reference_, isobar_interval_,
         isobar_label_frequency_);

    for (std::set<double>::iterator p = isobars.begin(); p != isobars.end();
         ++p) {
      Polyline poly;
      ...
      for (double t = tmin; t < tmax; t += 1) {
        ...
        if (label == labels.end())
          continue;
        if (xy.x() >= maxpcx) {
          map<double, PaperPoint>::iterator label =
              pressureRightLabels_.find(*p);
  ...
  if (saturated_adiabatic_) {
    MagFont font(saturated_adiabatic_label_font_,
                 saturated_adiabatic_label_style_,
    ...
    step(sat, labels, thmin, thmax, saturated_adiabatic_reference_,
         saturated_adiabatic_interval_, saturated_adiabatic_label_frequency_);

    for (std::set<double>::iterator thetaw = sat.begin(); thetaw != sat.end();
         ++thetaw) {
      if (*thetaw > 40.)
        continue;
      Polyline poly;
...
void TephiGrid::visit(LeftAxisVisitor &out) {
  ...
  for (map<double, PaperPoint>::iterator label = pressureLeftLabels_.begin();
       label != pressureLeftLabels_.end(); ++label) {
    Text *text = new Text();
...
void TephiGrid::visit(RightAxisVisitor &out) {
  ...
  for (map<double, PaperPoint>::iterator label = pressureRightLabels_.begin();
       label != pressureRightLabels_.end(); ++label) {
    Text *text = new Text();
    text->setText(tostring(label->first));
    text->setFont(font);
    text->setBlanking(true);
...
void TephiGrid::visit(BottomAxisVisitor &out) {
  MagFont font(mixing_ratio_label_font_, mixing_ratio_label_style_,
               mixing_ratio_label_size_);
  font.colour(*mixing_ratio_label_colour_);

  for (map<double, PaperPoint>::iterator label = mixingLabels_.begin();
       label != mixingLabels_.end(); ++label) {
    Text *text = new Text();
    text->setText(tostring(label->first));
    text->setFont(font);
    text->setBlanking(true);
  ...
  font = MagFont(isotherm_label_font_, isotherm_label_style_,
                 isotherm_label_size_);
  font.colour(*isotherm_label_colour_);

  for (map<double, PaperPoint>::iterator label = infoLabels_.begin();
       label != infoLabels_.end(); ++label) {
    Text *text = new Text();
    text->setText(tostring(label->first));
    text->setFont(font);
    text->setBlanking(true);
...
void TephiGrid::visit(SceneLayer &layer, vector<LayoutVisitor *> &visitors) {
  ...
  for (vector<LayoutVisitor *>::iterator visitor = visitors.begin();
       visitor != visitors.end(); ++visitor) {
    tephi->set(*visitor);
    (*visitor)->visit(*this);
  }
}
