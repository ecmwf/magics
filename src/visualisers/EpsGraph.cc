...
string writeDate(DateTime &date, const string &format) {
  ...
  tm convert = date;
  facet.put(visitor, visitor, ' ', &convert, format.c_str(),
...
void tick(double min, double max, vector<double> &ticks) {
  ...
  while (nb < 20) {
    ...
    if (wmax - wmin != inc &&
        (inc / ws == 1 || inc / ws == 2 || inc / ws == 5 || inc / ws == 10)) {
...
public:
  ...
  virtual void set(const PaperPoint &point,
...
public:
  ...
  virtual void set(const PaperPoint &point,
    ...
    for (vector<Colour>::const_iterator colour = colours_.begin();
         colour != colours_.end(); ++colour) {
      magics::Polyline *box = new magics::Polyline();
      box->setColour(*colour);
      box->setFilled(true);
      box->setShading(new FillShadingProperties());
...
public:
  EpsControl(const string &model, double resolution, const string &type,
             double height)
  ...
  EpsControl(const string &title, double height)
      : LegendEntry(" "), legend_size_(height) {
    title_ = title;
  }
  ...
  void set(const PaperPoint &point, BasicGraphicsObjectContainer &visitor) {
    ...
    MagLog::dev() << "Legend at Point[" << point.x() << ", " << point.y() << "]"
                  << endl;

    Text *text = new Text();
    text->addText(title_, Colour("red"),
                  legend_size_); // should be customisable
    text->setJustification(MLEFT);
...
public:
  EpsForecast(const string &model, double resolution, const string &type,
              double height)
      : LegendEntry(" "), legend_size_(height) {
  ...
  void set(const PaperPoint &point, BasicGraphicsObjectContainer &visitor) {
    ...
    Text *text = new Text();
    text->addText(title_, Colour("blue"),
                  legend_size_); // should be customisable
    text->setJustification(MLEFT);
    (*text).push_back(PaperPoint(x + 1 * 0.04, y));
    visitor.push_back(text);
...
void EpsGraph::pushControl(magics::Polyline *control,
                           BasicGraphicsObjectContainer &visitor) {
...
void EpsGraph::pushForecast(magics::Polyline *forecast,
                            BasicGraphicsObjectContainer &visitor) {
  const Transformation &transformation = visitor.transformation();
...
void EpsGraph::operator()(Data &data, BasicGraphicsObjectContainer &visitor) {
  ...
  for (const auto &point : points) {
    ...
    double max = (point->find("max") != point->end())
                     ? point->find("max")->second
                     : point->find("maximum")->second;
    double min = (point->find("min") != point->end())
                     ? point->find("min")->second
                     : point->find("minimum")->second;
    ...
    Colour colour75_90 =
        (size >= 3) ? Colour(quantiles_colour_[2]) : colour10_25;
    ...
    (*top).push_back(PaperPoint(x, epsmax > transformation.getMaxY()
                                       ? transformation.getMaxY()
                                       : epsmax));
    ...
    for (const auto &point :
         points) // NB this shadows point in the enclosing scope
    {
      double max = (point->find("max") != point->end())
                       ? point->find("max")->second
                       : point->find("maximum")->second;
...
void EpsLight::operator()(Data &data, BasicGraphicsObjectContainer &visitor) {
  /*
      ...
      hsl.light_ += (0.99 - light)*((point["total"]-
     point[direction.first])/point["total"]);
  */
  ...
  for (const auto &point : points) {
    ...
    for (vector<Colour>::iterator colour = colours.begin();
         colour != colours.end(); ++colour) {
      for (int i = 0; i < 4; i++) {
        magics::Polyline *box = new magics::Polyline();
        box->setColour(*colour);
        box->setFilled(true);
...
void EpsGraph::visit(LegendVisitor &legend) {
  ...
  if (control_) {
    ...
    if (legend_control_text_.empty())
      ...
      legend.add(new EpsControl(control_legend_, resolution_,
                                legend_resolution_, legend_size_));
    else
  ...
  if (forecast_) {
    if (legend_forecast_text_.empty())
      legend.add(new EpsForecast(deterministic_legend_, resolution_,
                                 legend_resolution_, legend_size_));
    else
...
void triangle2(const pair<string, float> &direction, CustomisedPoint &point,
  ...
  text->push_back(PaperPoint(pos + 11 * 3600 * cos(direction.second),
                             11.5 * 3600 * sin(direction.second)));
  visitor.push_back(text);
}

void triangle3(const Colour &colour, const Colour &border,
               const pair<string, float> &direction, CustomisedPoint &point,
               BasicGraphicsObjectContainer &visitor, double pos, double max) {
  Hsl hsl = colour.hsl();

  float light = hsl.light_;

  hsl.light_ += (0.99 - light) *
                ((point["total"] - point[direction.first]) / point["total"]);
...
void EpsCloud::triangle(const pair<string, float> &direction,
...
void triangle(const pair<string, float> &direction, CustomisedPoint &point,
              BasicGraphicsObjectContainer &visitor, double pos, double) {
  if (!point[direction.first])
    return;
  double shift = 3.14 * 0.125;

  // cvisitor << "Triangle---->" << direction.first << "=" <<
  // point[direction.first] << " " << scale << endl;
  magics::Polyline *poly = new magics::Polyline();
...
void EpsWind::operator()(Data &data, BasicGraphicsObjectContainer &visitor) {
  ...
  for (const auto &point : points) {
    double total = 0;
    for (map<string, float>::const_iterator direction = directions.begin();
      ...
      if (point->find(direction->first) == point->end()) {
        ...
        double val = 0;
        for (vector<string>::const_iterator key = classification.begin();
             key != classification.end(); ++key) {
          CustomisedPoint::const_iterator value =
              point->find(direction->first + "_" + *key);
          if (value != point->end()) {
    ...
    for (float angle = 0; angle <= 2; angle += 0.1)
      grid->push_back(
          PaperPoint(x + (l100 * cos(3.14 * angle)), l100 * sin(3.14 * angle)));
    // iset->push_back(grid);
...
void EpsWave::visit(LegendVisitor &legend) {
  MagFont font(legend.font_, legend.font_style_,
               tonumber(legend.font_dimension_));
  font.colour(Colour("Rgb(0.2, 0.2, 0.2)"));
  vector<Colour> colours;
...
void EpsBar::visit(LegendVisitor &legend) {
  MagLog::dev() << " EpsBar::visit(LegendVisitor&) --> to be implemented! "
                << endl;
}
...
void triangle5(const pair<string, float> &direction, CustomisedPoint &point,
               BasicGraphicsObjectContainer &visitor, double pos, double max) {
  vector<Colour> colours;
  colours.push_back(Colour("greenish_blue"));
  colours.push_back(Colour("yellow_green"));
  ...
  for (vector<string>::const_iterator force = forces.begin();
...
void EpsWave::operator()(Data &data, BasicGraphicsObjectContainer &visitor) {
  ...
  for (const auto &point : points) {
    ...
    if (point->find("control") != point->end()) {
      ...
      control->push_back(PaperPoint(x + (l100 * sin(angle * (3.14 / 180.))),
                                    l100 * cos(angle * (3.14 / 180.))));
    ...
    if (point->find("hres") != point->end()) {
      ...
      hres->push_back(PaperPoint(x + (l100 * sin(angle * (3.14 / 180.))),
                                 l100 * cos(angle * (3.14 / 180.))));
...
void CdfGraph::setExtended(const string &base) {
  ...
  static vector<pair<string, double>> values = {
      {"min", 0},           {"ten", 10},    {"twenty_five", 25}, {"fifty", 50},
      {"seventy_five", 75}, {"ninety", 90}, {"max", 99}};
  for (auto &val : values) {
    ostringstream key;
...
void CdfGraph::operator()(Data &data, BasicGraphicsObjectContainer &visitor) {
  ...
  // visitor << "CdfGraph::preparePlot(Data<PaperPoint>& data, visitor&)" <<
  // endl;
  ...
  for (const auto &point : points) {
    ...
    for (auto key = keys_.begin(); key != keys_.end(); ++key) {
      ...
      if (step != point->end()) {
        MagLog::dev() << key->first << ":" << step->second << "-->"
                      << key->second << endl;
  ...
  double w = 1.5 * (transformation.getMaxX() - transformation.getMinX()) /
  ...
  for (vector<BasicGraphicsObject *>::reverse_iterator object = sorter.rbegin();
       object != sorter.rend(); ++object)
...

{
  ...
  for (vector<string>::reverse_iterator colour = usedColours_.rbegin();
       colour != usedColours_.rend(); ++colour) {
    magics::Polyline *efi = new magics::Polyline();
...
void EpsShade::operator()(Data &data, BasicGraphicsObjectContainer &visitor) {
  ...
  for (vector<PaperPoint>::iterator point = one.begin(); point != one.end();
       ++point)
    backbottom->push_back(*point);
  for (vector<PaperPoint>::reverse_iterator point = ninetynine.rbegin();
       point != ninetynine.rend(); ++point)
    backtop->push_back(*point);
  for (vector<PaperPoint>::iterator point = ten.begin(); point != ten.end();
       ++point)
    first->push_back(*point);
  for (vector<PaperPoint>::reverse_iterator point = ninty.rbegin();
       point != ninty.rend(); ++point)
    first->push_back(*point);
  if (!first->empty())
    first->push_back(first->front());
  for (vector<PaperPoint>::iterator point = twentyfive.begin();
       point != twentyfive.end(); ++point)
    second->push_back(*point);
  for (vector<PaperPoint>::reverse_iterator point = seventyfive.rbegin();
       point != seventyfive.rend(); ++point)
    second->push_back(*point);
  if (!second->empty())
    second->push_back(second->front());

  for (vector<PaperPoint>::iterator point = onemin.begin();
       point != onemin.end(); ++point)
    backbottommin->push_back(*point);
  for (vector<PaperPoint>::reverse_iterator point = ninetyninemin.rbegin();
       point != ninetyninemin.rend(); ++point)
    backtopmin->push_back(*point);
  for (vector<PaperPoint>::iterator point = tenmin.begin();
       point != tenmin.end(); ++point)
    firstmin->push_back(*point);
  for (vector<PaperPoint>::reverse_iterator point = nintymin.rbegin();
       point != nintymin.rend(); ++point)
    firstmin->push_back(*point);
  if (!firstmin->empty())
    firstmin->push_back(firstmin->front());
  for (vector<PaperPoint>::iterator point = twentyfivemin.begin();
       point != twentyfivemin.end(); ++point)
    secondmin->push_back(*point);
  for (vector<PaperPoint>::reverse_iterator point = seventyfivemin.rbegin();
       point != seventyfivemin.rend(); ++point)
    secondmin->push_back(*point);
  if (!secondmin->empty())
    secondmin->push_back(secondmin->front());

  for (vector<PaperPoint>::iterator point = onemax.begin();
       point != onemax.end(); ++point)
    backbottommax->push_back(*point);
  for (vector<PaperPoint>::reverse_iterator point = ninetyninemax.rbegin();
       point != ninetyninemax.rend(); ++point)
    backtopmax->push_back(*point);
  for (vector<PaperPoint>::iterator point = tenmax.begin();
       point != tenmax.end(); ++point)
    firstmax->push_back(*point);
  for (vector<PaperPoint>::reverse_iterator point = nintymax.rbegin();
       point != nintymax.rend(); ++point)
    firstmax->push_back(*point);
  if (!firstmax->empty())
    firstmax->push_back(firstmax->front());
  for (vector<PaperPoint>::iterator point = twentyfivemax.begin();
       point != twentyfivemax.end(); ++point)
    secondmax->push_back(*point);
  for (vector<PaperPoint>::reverse_iterator point = seventyfivemax.rbegin();
       point != seventyfivemax.rend(); ++point)
    secondmax->push_back(*point);
  if (!secondmax->empty())
...
void EpsDirection::operator()(Data &data,
                              BasicGraphicsObjectContainer &visitor) {
  CustomisedPointsList points;
  std::set<string> request;
  const Transformation &transformation = visitor.transformation();
  ...
  for (const auto &point : points) {
    ...
    double angle =
        ((2 * 3.14) - (((*point)[keyword_] - 90) / 180) * 3.14) + 3.14;
...
void EpsWind::triangle(const pair<string, float> &direction,
                       CustomisedPoint &point,
                       BasicGraphicsObjectContainer &visitor, double pos,
                       double max) {
  Hsl hsl = colour_->hsl();
...
void EpsPlume::visit(LegendVisitor &legend) {
  if (!legend_)
    return;
  if (shading_) {
    for (vector<Colour>::iterator entry = shading_legend_.begin();
         entry != shading_legend_.end(); ++entry) {
      magics::Polyline *box = new magics::Polyline();
...
void EpsPlume::timeserie(Data &data, BasicGraphicsObjectContainer &visitor) {
  ...
  if (shading_) {
    for (vector<double>::iterator level = shading_levels_.begin();
         level != shading_levels_.end(); ++level)
  ...
  for (const auto &point : points) {
    ...
    for (map<string, double>::const_iterator value = point->begin();
         value != point->end(); ++value) {
      if (alldigit(value->first)) {
        if (line_) {
          map<string, magics::Polyline *>::iterator iline =
              lines.find(value->first);
          if (iline == lines.end()) {
            magics::Polyline *line = new magics::Polyline();
            line->setColour(*line_colour_);
            line->setThickness(line_thickness_);
  ...
  for (int i = 0; i < shading_levels_.size() / 2; i++) {
    ...
    visitor.push_back(line);
    for (vector<PaperPoint>::iterator point = shading[bottom].begin();
         point != shading[bottom].end(); ++point)
      line->push_back(*point);

    for (vector<PaperPoint>::reverse_iterator point = shading[top].rbegin();
         point != shading[top].rend(); ++point)
      line->push_back(*point);
  ...
  if (line_)
    for (map<string, magics::Polyline *>::const_iterator line = lines.begin();
         line != lines.end(); ++line) {
...
void EpsPlume::verticalprofile(Data &data,
                               BasicGraphicsObjectContainer &visitor) {
  CustomisedPointsList points;
  std::set<string> request;
  const Transformation &transformation = visitor.transformation();
...
void EpsPlume::operator()(Data &data, BasicGraphicsObjectContainer &visitor) {
  ...
  if (method == methods_.end()) {
    MagLog::warning() << "Could not find method [" << method_
                      << "] for plotting : no plot could be done." << endl;
    return;
  }
...
void CapeBox::box(CustomisedPoint &point,
                  BasicGraphicsObjectContainer &visitor) {
  ...
  */
  ...
  if (median != point.end()) {
    ...
    box->push_back(
        transformation(UserPoint(x->second - box_width_, upper->second)));
    box->push_back(
        transformation(UserPoint(x->second + box_width_, upper->second)));
    box->push_back(
        transformation(UserPoint(x->second + box_width_, lower->second)));
    box->push_back(
        transformation(UserPoint(x->second - box_width_, lower->second)));
    box->push_back(
        transformation(UserPoint(x->second - box_width_, upper->second)));
    ...
    box->push_back(
        transformation(UserPoint(x->second - box_width_, min->second)));
    box->push_back(
        transformation(UserPoint(x->second + box_width_, min->second)));
    visitor.push_back(box);
    ...
    box->push_back(
        transformation(UserPoint(x->second - box_width_, max->second)));
    box->push_back(
        transformation(UserPoint(x->second + box_width_, max->second)));
    visitor.push_back(box);
    ...
    auto median = point.find("median");
    box->push_back(
        transformation(UserPoint(x->second - box_width_, median->second)));
    box->push_back(
        transformation(UserPoint(x->second + box_width_, median->second)));
    visitor.push_back(box);
