...
public:
  FullArray(MatrixHandler &data, IntervalMap<int> &range,
            const Transformation &transformation, int, int, float,
            const string &);
  ...
  double value(int row, int column) const { return data_(row, column); }
  double row(int row, int column) const { return data_.row(row, column); }
  ...
  double column(int row, int column) const { return data_.column(row, column); }
...
int IsoShading::shadingIndex(double value) { return technique_->index(value); }

int IsoShading::leftIndex(double value) { return technique_->leftIndex(value); }
