...
void NoCoastPlotting::efas(DrawingVisitor& visitor)
{
    ...
    data["extended"] = PATH("efas/ExtendedDomain/lines")
        data["current"]
        = PATH("efas/CurrentDomain/lines")

            map<string, string>::iterator file
        = data.find(lowerCase(efas_domain_));
...
void CoastPlotting::operator()(DrawingVisitor& parent)
{
    ...
    if (magCompare(NoCoastPlottingAttributes::resolution_, "high") || magCompare(NoCoastPlottingAttributes::resolution_, "full")) {
        string resol = "10m";
