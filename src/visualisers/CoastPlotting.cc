...
void CoastPlotting::visit(LegendVisitor &legend) { return; }
...
void CoastPlotting::operator()(PreviewVisitor &parent) {
  ...
  for (vector<magics::Polyline *>::iterator poly = preview.coast_.begin();
       poly != preview.coast_.end(); ++poly) {
  ...
  frame->push_back(
      PaperPoint(transformation.getMinX(), transformation.getMinY()));
  frame->push_back(
      PaperPoint(transformation.getMaxX(), transformation.getMinY()));
  frame->push_back(
      PaperPoint(transformation.getMaxX(), transformation.getMaxY()));
  frame->push_back(
      PaperPoint(transformation.getMinX(), transformation.getMaxY()));
  frame->push_back(
      PaperPoint(transformation.getMinX(), transformation.getMinY()));
...
void NoCoastPlotting::operator()(DrawingVisitor &parent) {
  const Transformation &transformation = parent.transformation();
  transformation.coastSetting(coastSet_, parent.layout().absoluteWidth(),
                              parent.layout().absoluteHeight());
...
void NoCoastPlotting::efas(DrawingVisitor &visitor) {
  ...
  if (file == data.end()) {
    MagLog::warning() << " Can not find the EFAS domain " << efas_domain_
                      << ": revert to default [current]" << endl;
  ...
  for (ShapeDecoder::const_iterator river = efas.begin(); river != efas.end();
       ++river) {
...
void NoCoastPlotting::user(DrawingVisitor &visitor) {
  ...
  for (ShapeDecoder::const_iterator river = user.begin(); river != user.end();
       ++river) {
    magics::Polyline poly;
...
void NoCoastPlotting::rivers(DrawingVisitor &visitor) {
  ...
  for (ShapeDecoder::const_iterator river = rivers.begin();
...
void NoCoastPlotting::layers(map<string, Action> &methods, const string &val,
...
void CoastPlotting::release(vector<magics::Polyline *> &polys) {
  for (vector<magics::Polyline *>::iterator poly = polys.begin();
       poly != polys.end(); ++poly) {
    delete *poly;
  }
  polys.clear();
...
/*! \brief Method to set resource files for GIS information
  ...
  \note We have to use '10m' resolution for administrative Provinces since only
  this resolution contains data from OUTSIDE the USA and Canada
*/
void CoastPlotting::operator()(DrawingVisitor &parent) {
  ...
  coastSet_["administrative_boundaries"] =
  ...
  // clean and reproject !
  for (vector<magics::Polyline *>::iterator coast = coast_.begin();
       coast != coast_.end(); ++coast) {
    (*coast)->southClean(transformation.addSouth());
    (*coast)->reproject(transformation);
  }
  for (vector<magics::Polyline *>::iterator ocean = ocean_.begin();
       ocean != ocean_.end(); ++ocean) {
...
void CoastPlotting::landonly(Layout &out) {
  ...
  for (vector<magics::Polyline *>::iterator coast = clips.begin();
       coast != clips.end(); ++coast) {
...
void CoastPlotting::seaonly(Layout &out) {
  ...
  for (vector<magics::Polyline *>::iterator coast = clip.begin();
       coast != clip.end(); ++coast) {
    setSeaShading(**coast);
    out.push_back(*coast);
  }
}
...
void CoastPlotting::decode(const Layout &parent) {
  ...
  for (vector<magics::Polyline *>::iterator coast = coastlines.begin();
       coast != coastlines.end(); ++coast) {
  ...
  if (sea_) {
    ...
    for (vector<magics::Polyline *>::iterator coast = oceans.begin();
         coast != oceans.end(); ++coast) {
      (*coast)->close();
...
void CoastPlotting::clip(const Transformation &transformation,
                         const vector<magics::Polyline *> &in,
                         vector<magics::Polyline *> &out) const {
  magics::Polyline &box = transformation.getPCBoundingBox();
  for (vector<magics::Polyline *>::const_iterator poly = in.begin();
       poly != in.end(); ++poly) {
    (*poly)->clip(box, out);
  }
}

void CoastPlotting::clipAndClose(const Transformation &transformation,
                                 const vector<magics::Polyline *> &in,
                                 vector<magics::Polyline *> &out) const {
...
void NoCoastPlotting::visit(MetaDataCollector &meta) {
  ...
  for (map<string, string>::const_iterator entry = meta_.begin();
       entry != meta_.end(); ++entry) {
