...
void ArrowPlotting::operator()(bool north, const PaperPoint &point, double x,
                               double y, double val) {
  ...
  if (!levels_->empty() &&
      (value < levels_->front() || value > levels_->back()))
    return;
...
void ArrowPlotting::prepare(BasicGraphicsObjectContainer &out, double res) {
  ...
  if (magCompare(unit_system_, "automatic")) {
    ...
    // adjust the scaling factor!
    // this->unit_velocity_ /=
    // out.transformation().unitToCm(out.absoluteWidth(), out.absoluteHeight());
    MagLog::dev() << "unit_velocity_-->" << this->unit_velocity_ << endl;
  }
...
void ArrowPlotting::finish(BasicGraphicsObjectContainer &out) {
  ...
  vector<Arrow *> arrows;
  for (map<Colour, Arrow *>::iterator arrow = northArrows_.begin();
       arrow != northArrows_.end(); ++arrow)
    arrows.push_back(arrow->second);

  for (map<Colour, Arrow *>::iterator arrow = southArrows_.begin();
       arrow != southArrows_.end(); ++arrow)
    arrows.push_back(arrow->second);

  std::sort(arrows.begin(), arrows.end(), SortHelper());

  // Now we feed the task...
  for (vector<Arrow *>::iterator arrow = arrows.begin(); arrow != arrows.end();
       ++arrow) {
    if (!(*arrow)->empty()) {
...
void ArrowPlotting::visit(LegendVisitor &legend) {
  ...
  // const double scale =
  // legend.transformation().unitToCm(legend.absoluteWidth(),
  // legend.absoluteHeight());
  const double scale = 1.;

  const double speed =
      (this->unit_velocity_) ? this->unit_velocity_ : this->maxVelocity_;
...
void ArrowPlotting::visit(Data &data, PointsHandler &points,
                          HistoVisitor &visitor) {
