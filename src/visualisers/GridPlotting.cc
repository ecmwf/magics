...
void NoGridPlotting::longitudes(vector<double> &lons, int freq) const {
  ...
  i = 0;
  for (double lon = lonReference_ + lonIncrement_; lon < max;
       lon += lonIncrement_) {
    i++;
    if ((i % freq) == 0) {
      lons.push_back(lon);
...
void NoGridPlotting::latitudes(vector<double> &lats, int freq) const {
  ...
  i = 0;
  for (double lat = latReference_ + latIncrement_; lat < max;
       lat += latIncrement_) {
    i++;
    if (i % freq == 0) {
      lats.push_back(lat);
...
const vector<double> &
NoGridPlotting::longitudes(const Transformation &transformation) const {
...
const vector<double> &
NoGridPlotting::latitudes(const Transformation &transformation) const {
  transformation.boundingBox(minx_, miny_, maxx_, maxy_);
...
void GridPlotting::operator()(DrawingVisitor &out) {
  ...
  grid->push_back(
      PaperPoint(transformation.getMinPCX(), transformation.getMinPCY()));
  grid->push_back(
      PaperPoint(transformation.getMinPCX(), transformation.getMinPCY()));
  out.push_back(grid);
