...
void Wind::print(ostream &out) const { out << "Wind"; }
...
void Wind::operator()(Data &data, BasicGraphicsObjectContainer &parent) {
  ...
  for (const auto &point : points) {
    ...
    PaperPoint xy =
        transformation(UserPoint(point->longitude(), point->latitude()));
    if (transformation.in(xy) || point->tile()) {
      pair<double, double> component =
          std::make_pair((*point)["x_component"], (*point)["y_component"]);
      transformation.reprojectComponents(x, y, component);
      (*this->type_)(north, xy, component.first, component.second,
...
void Wind::visit(LegendVisitor &legend) { (*this->type_).visit(legend); }
...
void Wind::visit(Data &data, HistoVisitor &visitor) {
  ...
  (*this->type_)
      .visit(data, data.points(*visitor.dataLayoutTransformation(), false),
             visitor);
}
