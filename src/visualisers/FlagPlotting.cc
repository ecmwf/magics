...
void FlagPlotting::operator()(bool north, const PaperPoint &point, double x,
                              double y, double val) {
  if (legendOnly_)
    return;
...
void FlagPlotting::visit(Data &data, PointsHandler &points,
...
void FlagPlotting::finish(BasicGraphicsObjectContainer &out) {
  ...
  vector<Flag *> flags;
  for (map<Colour, Flag *>::iterator flag = northFlags_.begin();
       flag != northFlags_.end(); ++flag)
    flags.push_back(flag->second);

  for (map<Colour, Flag *>::iterator flag = southFlags_.begin();
       flag != southFlags_.end(); ++flag)
    flags.push_back(flag->second);

  std::sort(flags.begin(), flags.end(), SortHelper());

  // Now we feed the task...
  for (vector<Flag *>::iterator flag = flags.begin(); flag != flags.end();
       ++flag) {
    if (!(*flag)->empty()) {
      out.push_back(*flag);
    }
