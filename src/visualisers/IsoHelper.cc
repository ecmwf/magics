...
IsoHelper::IsoHelper(int n, vector<magics::Polyline *> &lines,
                     IsoData &segments)
    : n_(n), lines_(lines), data_(segments) {}

void IsoHelper::concatenate_back(vector<magics::Polyline *> &lines,
                                 magics::Polyline *poly) {
  double x = poly->back().x();
  double y = poly->back().y();

  for (vector<magics::Polyline *>::iterator line = lines.begin();
       line != lines.end(); ++line) {
...
void IsoHelper::concatenate_front(vector<magics::Polyline *> &lines,
                                  magics::Polyline *poly) {
...
void IsoHelper::run() {
  deque<
      std::pair<double, pair<std::pair<double, double>, pair<double, double>>>>
      todo;
  int last = 0;
  ...
  while (true) {
    { // Block
      ...
      if (!data_.more_ && data_.segments_.empty()) {
        for (map<double, vector<magics::Polyline *>>::iterator level =
                 helpers_.begin();
             level != helpers_.end(); ++level)
          for (vector<magics::Polyline *>::iterator line =
                   level->second.begin();
               line != level->second.end(); ++line)
      ...
      }
      {
        ...
        if (data_.segments_.size() > 2000 || !data_.more_) {
          todo.clear();
          std::copy(data_.segments_.begin(), data_.segments_.end(),
                    back_inserter(todo));
    ...
    {
      AutoLock<Mutex> locklines(locklines_);
      for (deque<std::pair<double, pair<std::pair<double, double>,
                                        pair<double, double>>>>::iterator x =
               todo.begin();
           x != todo.end(); ++x) {
        pair<std::pair<double, double>, pair<double, double>> &segment =
            x->second;
        ...
        map<double, vector<magics::Polyline *>>::iterator h =
            helpers_.find(x->first);
        if (h == helpers_.end()) {
