...
void TaylorGrid::visit(DrawingVisitor &out) {
  ...
  MagLog::dev() << "TaylorGrid::prepareGraphics()-->needs to be implemented! "
                << endl;
  ...
  for (std::set<double>::iterator value = values.begin(); value != values.end();
       ++value) {
  ...
  for (vector<double>::const_iterator angle = labels.begin();
       angle != labels.end(); ++angle) {
