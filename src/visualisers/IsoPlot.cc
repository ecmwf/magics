...
IsoPlot::IsoPlot() { setTag("isoline"); }
...
public:
  ...
  void push_back(int index, vector<PaperPoint> &points) {
    ...
    if (helper->second->size() == 0) {
      ...
      for (int i = 0; i < size; i++) {
        int j = (i + 1) % size;
        helper->second->push_back(points[i].x_, points[i].y_, points[j].x_,
                                  points[j].y_);
      }
    } else {
      // intersect !
      ...
      for (vector<PaperPoint>::iterator pt = points.begin(); pt != points.end();
           ++pt) {
        pts.push_back(*pt);
      }
      ...
      /*
                  if (output.size() == 1) {
                      ...
                      for ( auto pt = output.front()->begin();  pt !=
         output.front()->end(); ++pt ) { tmp.push_back(*pt);
                          }
      ...
      if (output.size() == 1) {
        vector<PaperPoint> tmp;
        for (auto pt = output.front()->begin(); pt != output.front()->end();
             ++pt) {
          tmp.push_back(*pt);
        }
        push_back(index, tmp);
  ...
  void reshape(CellBox *parent) {
    if (parent == this)
      return;
    for (map<int, SegmentJoiner *>::iterator entry = helper_.begin();
         entry != helper_.end(); ++entry) {
  ...
  void feed(IsoPlot &owner, BasicGraphicsObjectContainer &out) {
    for (vector<Polyline *>::iterator poly = polylines_.begin();
         poly != polylines_.end(); ++poly) {
      (*owner.shading_)(*poly);
      out.push_back(*poly);
    }
    polylines_.clear();
  ...
  void finish() {
    ...
    for (map<int, SegmentJoiner *>::iterator index = helper_.begin();
         index != helper_.end(); ++index) {
      vector<vector<Point>> result;
      ...
      for (vector<vector<Point>>::iterator j = result.begin();
           j != result.end(); ++j) {
        if (reverse) {
          if (!joiner.isHole((*j))) {
      ...
      for (vector<vector<Point>>::iterator j = polys.begin(); j != polys.end();
           ++j) {
        poly = new Polyline();
        ...
        for (vector<Point>::iterator point = j->begin(); point != j->end();
             ++point)
          poly->push_back(PaperPoint(point->x_, point->y_));

        for (std::list<vector<Point>>::iterator h = holes.begin();
             h != holes.end();) {
          ...
          if (joiner.pointInPoly((*h).front(), *j)) {
            poly->newHole();
            for (vector<Point>::iterator point = h->begin(); point != h->end();
                 ++point)
...
void CellBox::shade(const IsoPlot &owner) { shade(owner, this); }
  ...
  void run() {
    {
      ...
      (objects_.shading_) ? objects_.cell_.shade(objects_.parent_)
                          : objects_.cell_.contour(objects_.parent_);
    }
...
void IsoPlot::isoline(Cell &cell, CellBox *parent) const {
  ...
  for (int p = 0; p < 2; p++) {
    ...
    for (int l = cell.min_; l != cell.max_; ++l) {
      ...
      const int out = cases[cell.coef(p1, contour)][cell.coef(p2, contour)]
                           [cell.coef(p3, contour)];
      if (out != 0) {
        levels.push_back(l);
      }
    }
    ...
    if (levels.empty() && parent) {
      ...
      parent->push_back(index, cell.column(p1), cell.row(p1), cell.column(p2),
                        cell.row(p2));
      parent->push_back(index, cell.column(p2), cell.row(p2), cell.column(p3),
                        cell.row(p3));
      parent->push_back(index, cell.column(p3), cell.row(p3), cell.column(p1),
                        cell.row(p1));
    } else {
      CellBox *box = (parent) ? new CellBox() : 0;

      for (vector<int>::const_iterator l = levels.begin(); l != levels.end();
           ++l) {
        int level = *l;
        ...
        current = cases[cell.coef(p1, contour)][cell.coef(p2, contour)]
                       [cell.coef(p3, contour)];
        ...
        if (add == 2 && needIsolines()) {
          ...
          const int t = (*l) % threads_;
          {
            AutoLock<MutexCond> lockproducer(producerMutex_);
            {
              ...
              segments_[t]->segments_.push_back(make_pair(
                  levels_[*l],
...
void IsoPlot::isoline(MatrixHandler &data,
  ...
  int r = 1;
  for (vector<double>::const_iterator level = levels_.begin();
       level != levels_.end(); ++level) {
  ...
  CellArray *array =
      shading_->array(data, range, transformation, parent.widthResolution(),
                      parent.heightResolution(), resolution_, technique_);
  if (!array)
    return;
  if (shading_->shadingMode())
    range.insert(
        make_pair(Interval(levels_.back(), levels_.back() + EPSILON), r - 1));
...

  {
    ...
    // int i = 1;
    {
      IsoProducerData *data =
          new IsoProducerData(shading_->shadingMode(), *this, *(view[i]));
  ...
  delete array;
  for (vector<IsoData *>::iterator segment = segments_.begin();
 ...
 * Create the isolines...
 */
void IsoPlot::operator()(MatrixHandler &data,
                         BasicGraphicsObjectContainer &parent) {
  prepare(data);
  if (legend_only_) {
    if (rainbow_) {
  ...
  for (vector<vector<Polyline *> *>::const_iterator lines = lines_.begin();
       lines != lines_.end(); ++lines) {
    for (vector<Polyline *>::const_iterator poly = (*lines)->begin();
         poly != (*lines)->end(); ++poly) {
      if ((*poly)->empty())
        continue;
...
void NoIsoPlot::operator()(MatrixHandler &data,
                           BasicGraphicsObjectContainer &parent) {
  // Create the isolines...
  prepare(data);
...
void IsoPlot::visit(Data &data, LegendVisitor &legend) {
  ...
  case LegendMethod::DISJOINT: {
    ...
    if (shading_->hasLegend())
      return;
    if (rainbow_) {
      for (vector<double>::iterator level = levelSelection_->begin();
           level != levelSelection_->end(); ++level) {
  ...
  case LegendMethod::HISTOGRAM: {
    ...
    IntervalMap<int> &histogram =
        helper.histogram(beans, data.points(transformation, false));
    int total = 0;
    for (IntervalMap<int>::const_iterator interval = histogram.begin();
         interval != histogram.end(); ++interval) {
      total += interval->second;
    }
    bool first = true;
    for (IntervalMap<Colour>::const_iterator interval = beans.begin();
         interval != beans.end(); ++interval) {
...
void NoIsoPlot::visit(Data &data, LegendVisitor &legend) {
  ...
  case LegendMethod::HISTOGRAM: {
    ...
    Histogram helper;
    IntervalMap<int> &histogram =
        helper.histogram(beans, data.points(legend.transformation(), true));
    int total = 0;
    for (IntervalMap<int>::const_iterator interval = histogram.begin();
         interval != histogram.end(); ++interval) {
      total += interval->second;
...
CellArray::CellArray(MatrixHandler &data, IntervalMap<int> &range,
                     const Transformation &transformation, int width,
  ...
  double stepx =
      (transformation.getMaxPCX() - transformation.getMinPCX()) / (columns_);
  double stepy =
      (transformation.getMaxPCY() - transformation.getMinPCY()) / (rows_);
  {
    ...
    for (vector<std::pair<double, double>>::iterator xy = xypoints.begin();
         xy != xypoints.end(); ++xy) {
      double value;
      if (geo->second == -1000) {
        value = missing;
        ...
        value = (magCompare(technique, "nearest"))
                    ? data.nearest(geo->second, geo->first)
      ...
      if (value != missing) {
        ...
        // cout << "MISSING VALUE-->" << geo->second << ", " << geo->first <<
        // endl;
      }
      points_[i] = value;
      i++;
...
GridArray::GridArray(MatrixHandler &data, IntervalMap<int> &range,
                     const Transformation &transformation, int width,
                     int height, float resol, const string &technique)
...
GridCell::GridCell(const CellArray &data, int row, int column,
...
void IsoPlot::setThicknessAndStyle() {
  ...
  for (vector<string>::iterator style = rainbowStyleList_.begin();
       style != rainbowStyleList_.end(); ++style) {
