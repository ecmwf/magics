...
OneLineClass::OneLineClass() : Len(0), X(0x0), Y(0x0) {}
...
int CalcStreamlines(int density, const float *dir, const GSStruct *gs,
                    OneLineClass **&str_lines, int &linenum) {
  // Input arguments:
  // density: density of the streamlines 1: highest density 2,3,... lower
  // density dir: wind direction field (in radian) gs: pointer of the grid
  // geometry (lat-lon values in degree)
  ...
  // density2 and start_index_[ij] are only for the more equable density of
  // lines int density2 = density * 4; for(int start_index_j=0;
  // start_index_j<=density*3; start_index_j+=density) for(int start_index_i=0;
  // start_index_i<=density*3; start_index_i+=density)
  for (int density2 = density * 64; density2 >= density; density2 /= 2)
    for (int j = 0; j < ny; j += density2)
      for (int i = 0; i < nx; i += density2) {
        ...
        if (used)
          continue;
        for (int c = act_cell - min_dist * nx; c <= act_cell + min_dist * nx;
             c += nx)
          if (c > -1 && c < gridsize && cell_used[c] > 0) {
            used = 1;
            break;
        ...
        for (int direction = -1; direction <= 1; direction += 2) {
          ...
          // Some help variables for the very variable wind direction (light
          // wind)
          int cycle_cont = 0;
          ...
          do {
            ...
            if (cell_used[act_cell] &&
                cell_used_by[cell_used[act_cell] - 1][act_cell] == act_line)
              break;
            ...
            // act_cell <=> j*nx + i
            cell_x =
                ShiftPeriod(startx + dx_orig * (act_cell % nx), x_min, x_per);
            ...
            if (act_side == 1) {
              ...
              if (act_dir > 0 && act_dir < M__PI) {
                ...
                if (cycle_cont && (prev_dir > M__PI_2 && prev_dir < M3_PI_2) ==
              ...
              if (act_dir == 0) {
                ...
                if (new_y <= cell_y + dy_2e && new_y >= cell_y - dy_2e) {
                  // Westward
                  ...
                  if (act_dir > M3_PI_2) {
                    // Northward
                    x[pos] =
                        ShiftPeriod(x[pos - direction] +
                                        (cell_y + dy_2 - y[pos - direction]) *
                                            tan_dir / cos_cell_y,
                                    x_min, x_per);
                    y[pos] = cell_y + dy_2;
              ...
              if (act_dir < M__PI_2 || act_dir > M3_PI_2) {
                // Compare the neighbouring wind directions
                if (cycle_cont && (prev_dir > 0 && prev_dir < M__PI) ==
              ...
              if (act_dir == M__PI_2) {
                ...
                if (new_x <= cell_x + dx_2e && new_x >= cell_x - dx_2e) {
                  // Southward
                  ...
                  if (act_dir < M__PI) {
                    ...
                    if (gs_geo)
                      y[pos] = y[pos - direction] +
                               CalcLonDist(cell_x + dx_2, x[pos - direction]) /
                                   tan_dir * cos_cell_y;
                    else
                      y[pos] = y[pos - direction] +
                               fabs(cell_x + dx_2 - x[pos - direction]) /
                                   tan_dir * cos_cell_y;
                    ...
                    if (gs_geo)
                      y[pos] = y[pos - direction] -
                               CalcLonDist(x[pos - direction], cell_x - dx_2) /
                                   tan_dir * cos_cell_y;
                    else
                      y[pos] = y[pos - direction] -
                               fabs(x[pos - direction] - cell_x + dx_2) /
                                   tan_dir * cos_cell_y;
            ...
            act_dir); if( y[pos] != y[pos] ) printf("y%d nan act_side = %d
            ...
            // if( max_used > 1 ) Hey it should be always checked!
            {
              ...
              for (int l = 0; l < cell_used[act_cell]; l++) {
                ...
                const OneLineClass &line =
                    *str_lines[cell_used_by[l][act_cell]];
                int lp = cell_used_by_point[l][act_cell] -
                         line_start_array[cell_used_by[l][act_cell]];
                // if( line.X[lp] == x[pos] && line.Y[lp] == y[pos] )
                // This condition makes more equable line density
                // CalcSpereDist would be better (but slower and it changes with
                // latitude)
                if (lp >= line.Len)
                  printf("Error in CalcStreamlines %d > %d\n", lp, line.Len);
                if (line.X[lp] < x[pos] + max_dx &&
                    line.Y[lp] < y[pos] + max_dy &&
                    line.X[lp] > x[pos] - max_dx &&
                    line.Y[lp] > y[pos] - max_dy) {
                  // If previously this line was too close to an other one stop
                  // it immediately
                  if (stop_need == 2)
                    stop_now = 1;
                  stop_need =
                      2; // instead of 1 (2 means we give an other chance)
                  break;
                }
          ...
          while (act_cell >= 0 && act_cell < gridsize && pos > -1 &&
                 len < gridsize);
