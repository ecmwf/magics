...
void ColourTechnique::prepare(LevelSelection &out, LevelSelection &levels,
                              bool rainbow) {
  if (levels.empty())
    return;
  clear();
  ...
  for (LevelSelection::const_iterator val = levels.begin(); val != levels.end();
       ++val) {
    if (rainbow) {
  ...
  if (!rainbow)
    bands_.insert(
        make_pair(Interval(levels.back(), levels.back() + EPSILON), left));

  MagLog::dev() << levels.back() << "<<" << left << "<<"
                << levels.back() + EPSILON << endl;
}
...
void ColourTechnique::colours(vector<string> &colours) const {
  for (IntervalMap<Colour>::const_iterator band = bands_.begin();
       band != bands_.end(); ++band)
...
void ColourTechnique::visit(LegendVisitor &legend) {
  for (IntervalMap<Colour>::const_iterator band = bands_.begin();
    ...
    // MagLog::dev()<< "From " << min << " to " << max << "-->" << band->second
    // << endl;
...
void PaletteColourTechnique::set(LevelSelection &out, LevelSelection &in,
  ...
  if (colours_.empty()) {
    MagLog::warning() << "Could not load palette " << palette_
                      << ": using a default one " << endl;
