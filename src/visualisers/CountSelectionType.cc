...
void CountSelectionType::calculate(double min, double max, bool) {
  ...
  double toleranceProportion =
      (double)tolerance_ /
      (double)(levelCount_ + 1 +
               i); // tolerance as a proportion of the number of levels
                   // we add 'i' here because it will be subtracted at the end
                   // (because this is the number of levels we will not actually
                   // plot, ie the min/max contours).
  if (same(step, 0)) {
    // Only one isoline!
    push_back(mini);
    return;
  ...
  for (double val = first; val < maxi; val += inc) {
    // special case - if the value is close to zero then set it to precisely
    // zero to avoid later formatting issues
    if (fabs(val) < epsilon)
      push_back(0.0);
  ...
  // last resort - if we have too many/few contours, then just do a 'blind'
  // division so as to ensure the correct number, even if they are not nice

  if (si - i < levelCount_ - tolerance_ || si - i > levelCount_ + tolerance_) {
    MagLog::debug() << "Not using nice levels[" << levelCount_ << ", "
                    << tolerance_ << "]-->" << size() << endl;
    clear();
  ...
  MagLog::debug() << "count=" << levelCount_ << ", tolerance=" << tolerance_
                  << ", result=" << size() << endl;
