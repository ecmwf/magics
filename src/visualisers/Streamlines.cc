...
bool Streamlines::operator()(Data &data, BasicGraphicsObjectContainer &parent) {
  ...
  // pres des poles et de jointure moche pour les projections 'proj4
  // (overlapped)'
  ...
  PaperPoint pt2 =
      transformation(UserPoint((x1 + x2) / 2 + 360.0, (y1 + y2) / 2));
  ...
  if (right - left < 350) {
    if (!overlapping) {
      ...
      // A priori moins couteux, mais la grille n'est tracée qu'une seule fois
      // si maxx - minxx > 360.0. Mais c'est déja le cas pour les "overlapped"
      while (left > maxx) {
        left -= 360.0;
        right -= 360.0;
      }
  ...
  for (int l = 0; l < size; l++) {
    ...
    // Il est difficile de calculer une distmax qui marche avec toutes les
    // projections
    ...
    for (int i = 0; i < result[l]->Len;) {
      ...
      while (i < result[l]->Len) {
        PaperPoint pt =
            transformation(UserPoint(result[l]->X[i], result[l]->Y[i]));
        i++;
        // On elimine ainsi les points invalides
        // ( Pour lesquels la projection a "échoué" )
        // if (!transformation.in(pt) || => NON il peut manquer un segment sur
        // les bords
        ...
        if (distmax > 0 && !(prevpt == NOPT)) {
          //              if( pt.distance(prevpt) > distmax ) { NON ->
          //              distance() très couteux
