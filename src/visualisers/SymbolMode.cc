...
void SymbolMode::visit(Data &data, HistoVisitor &visitor) {
  ...
  Histogram helper;
  helper.visit(beans, data,
               data.points(*visitor.dataLayoutTransformation(), false),
               visitor);
}
...
*/
void SymbolMode::print(ostream &out) const { out << "SymbolMode"; }
...
void SymbolIndividualMode::properties() const {
  ...
  properties_.connectLineColour_ = (parent_->automatic_connect_colour_)
                                       ? *colour_
                                       : *parent_->connect_colour_;
...
void SymbolTableMode::prepare() {
  ...
  if (!marker_.empty()) {
    symbol_.clear();
    for (vector<int>::const_iterator marker = marker_.begin();
         marker != marker_.end(); ++marker)
  ...
  for (doublearray::const_iterator min = min_.begin(); min != min_.end();
       ++min) {
    map_[Interval(*min, *max)] =
        SymbolProperties(Colour(*colour), *height, *symbol);
...
void SymbolTableMode::visit(LegendVisitor &legend) {
  ...
  for (interval = map_.begin(); interval != map_.end(); ++interval) {
    ...
    legend.add(new SimpleSymbolEntry(interval->first.min_, interval->first.max_,
                                     symbol));
  }
  legend.last();
}
...
void SymbolTableMode::buildBins(const IntervalMap<SymbolProperties> &in,
                                IntervalMap<Colour> &out) {
  for (IntervalMap<SymbolProperties>::const_iterator interval = in.begin();
       interval != in.end(); ++interval)
    out.insert(make_pair(Interval(interval->first.min_, interval->first.max_),
                         interval->second.colour_));
}
...
void SymbolIndividualMode::adjust(double, double, bool, const Transformation &,
                                  double) {}
void SymbolTableMode::adjust(double, double, bool, const Transformation &,
                             double) {}
...
void SymbolTableMode::visit(Data &data, LegendVisitor &legend) {
  ...
  case LegendMethod::CONTINUOUS: {
    for (IntervalMap<SymbolProperties>::const_iterator interval = map_.begin();
  ...
  case LegendMethod::DISJOINT: {
    for (IntervalMap<SymbolProperties>::iterator interval = map_.begin();
         interval != map_.end(); ++interval) {
      Symbol *symbol = new Symbol();
      (*symbol).setColour(interval->second.colour_);
      (*symbol).setSymbol(interval->second.marker_);
      ...
      legend.add(new SimpleSymbolEntry(
          interval->second.label_.empty() ? str : interval->second.label_,
          symbol));
    }
    break;
  }
  case LegendMethod::HISTOGRAM: {
    ...
    IntervalMap<int> &histogram =
        helper.histogram(beans, data.points(legend.transformation(), false));
