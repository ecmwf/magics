...
void MetgramBar::print(ostream &out) const { out << "MetgramBar[]"; }

void MetgramCurve::print(ostream &out) const { out << "MetgramCurve[]"; }

void MetgramFlags::print(ostream &out) const { out << "MetgramFlags[]"; }

DateTime readDate(CustomisedPoint &point) {
  MagDate date1((long)point["year"], (long)point["month"], (long)point["day"]);
  magics::MagTime time((long)point["hours"], (long)point["minutes"],
                       (long)point["seconds"]);
...
void MetgramBar::operator()(CustomisedPointsList &points,
  ...
  MagDate date((long)(*points.front())["year"],
               (long)(*points.front())["month"],
               (long)(*points.front())["day"]);
  MagTime time((long)(*points.front())["hours"],
               (long)(*points.front())["minutes"],
...
void MetgramCurve::operator()(CustomisedPointsList &points,
                              BasicGraphicsObjectContainer &visitor) {
  if (points.empty())
    return;
  ...
  for (const auto &point : points) {
    MagDate date((long)(*point)["year"], (long)(*point)["month"],
                 (long)(*point)["day"]);
    MagTime time((long)(*point)["hours"], (long)(*point)["minutes"],
                 (long)(*point)["seconds"]);
...
void MetgramFlags::operator()(CustomisedPointsList &points,
                              BasicGraphicsObjectContainer &visitor) {
  if (points.empty())
    return;
  MagDate date((long)(*points.front())["year"],
  ...
  for (const auto &point : points) {
    ...
    if (point->find(component1_) != point->end() &&
        point->find(component2_) != point->end()) {
      PaperPoint pos(x, 0);
      flags->push_back(
          ArrowPoint((*point)[component1_], (*point)[component2_], pos));
    }
  }
  if (!flags->empty())
