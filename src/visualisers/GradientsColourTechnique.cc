...
void GradientsColourTechnique::set(LevelSelection &out, LevelSelection &in,
                                   ColourTable &table, int nb) const {
  // First make sure that
  ColourTableDefinitionCompute helper;

  if (colours_.size() < 2) {
    MagLog::warning() << " No enough colours given to the gradients method"
                      << endl;
    return;
  }
  ...
  if (stops.empty()) {
    MagLog::warning()
        << " No intervals given to the gradients method, guessing ..." << endl;
    double min = in.front();
    double max = in.back();
  ...
  for (int col = 1; col < colours_.size(); ++col) {
    ...
    // right
    ColourTableDefinitionCompute helper(left, right, technique_,
                                        technique_direction_);
