...
Filter::Filter(MatrixHandler &matrix, int nrows, int ncols)
    : matrix_(matrix), nrows_(nrows), ncols_(ncols) {
  flr_ = 0;
  fbuf_ = NULL;
...
Filter::~Filter() { FreeImageBuffers(); }
...
MinMaxFilter::MinMaxFilter(MatrixHandler &matrix, int nrows, int ncols,
                           int flag)
