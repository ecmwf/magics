...
void Boundaries::operator()(const map<string, string> &setting,
  ...
  const string administrative_boundaries =
      setting.find("administrative_boundaries")->second;
  ...
  if (predefined_list.empty()) {
    predefined_list["south_america"] = {"ARG", "BRA", "BOL", "HL",
                                        "COL", "GUY", "PRY", "PER",
                                        "SUR", "URY", "VEN", "ECU"};
    predefined_list["oceania"] = {"AUS", "CCK", "CXR", "HMD", "NFK",
                                  "NCL", "NZL", "PNG", "SLB", "VUT"};
    predefined_list["asia"] = {
        "AFG", "BGD", "CHN", "TLS", "IND", "IDN", "IRN", "IRQ", "ISR", "JPN",
        "KAZ", "PRK", "KOR", "KGZ", "LAO", "MYS", "MNG", "MMR", "NPL", "PAK",
        "SAU", "SYR", "TJK", "TUR", "TKM", "UZB", "YEM", "ZMB", "ZWE"};
    predefined_list["north_america"] = {"CAN", "CUB", "MEX", "USA"};
    predefined_list["africa"] = {
        "DZA", "AGO", "CAF", "TCD", "COG", "COD", "BWA", "CMR", "EGY",
        "ERI", "ETH", "GAB", "GMB", "CIV", "GHA", "GIN", "KEN", "LBY",
        "MLI", "MRT", "MDG", "MAR", "MOZ", "NAM", "NER", "NGA", "RWA",
        "SEN", "SOM", "ZAF", "SDN", "TZA", "TGO"};
    predefined_list["europe"] = {
        "AUT", "BLR", "RUS", "CZE", "FRO", "FIN", "ALA", "FRA", "DEU", "GRC",
        "SJM", "ITA", "POL", "ROU", "SRB", "ESP", "UKR", "GBR", "GIB"};
  }

  vector<string> admin;

  for (vector<string>::const_iterator a = administrative_list_.begin();
       a != administrative_list_.end(); ++a) {
  ...
  if (admistrative_) {
    ...
    for (ShapeDecoder::const_iterator boundary = admistrative.begin();
         boundary != admistrative.end(); ++boundary) {
      Polyline poly;
      poly.setColour(administrative_colour_->automatic()
                         ? *colour_
                         : *administrative_colour_);
  ...
  for (ShapeDecoder::const_iterator boundary = boundaries.begin();
  ...
  if (disputed_) {
    ...
    for (ShapeDecoder::const_iterator boundary = disputed.begin();
         boundary != disputed.end(); ++boundary) {
      Polyline poly;
      poly.setColour(disputed_colour_->automatic() ? *colour_
                                                   : *disputed_colour_);
