...
static void step(set<double> &values, set<double> &labels, double from,
                 double to, double ref, double step, int freq) {
  if (ref > to)
    return;
  int l = 0;
...
void SkewtGrid::visit(DrawingVisitor &out) {
  ...
  if (dry_adiabatic_) {
    ...
    for (std::set<double>::iterator th = dry.begin(); th != dry.end(); ++th) {
      ...
      UserPoint point(tLabel,
                      magics::pressureFromTheta(*th + 273.15, tLabel + 273.15) /
                          100.);
  ...
  if (isobar_) {
    ...
    for (std::set<double>::iterator p = isobars.begin(); p != isobars.end();
      ...
      if (label != labels.end()) {
        ...
        map<double, PaperPoint>::iterator labelIt =
        ...
        // Right label
        xy.x(0 +
             10); // right labels have their own box with x coordinates 0->100
...
void SkewtGrid::visit(BottomAxisVisitor &out) {
  ...
  for (map<double, PaperPoint>::iterator label = tBottomLabels_.begin();
       label != tBottomLabels_.end(); ++label) {
    Text *text = new Text();
    text->setText(tostring(label->first));
    text->setFont(font);
    text->setBlanking(true);
...
void SkewtGrid::visit(TopAxisVisitor &out) {
  ...
  for (map<double, PaperPoint>::iterator label = tTopLabels_.begin();
       label != tTopLabels_.end(); ++label) {
    Text *text = new Text();
    text->setText(tostring(label->first));
    text->setFont(font);
    text->setBlanking(true);
