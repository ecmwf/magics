...
void ObsPlotting::operator()(Data &data, BasicGraphicsObjectContainer &out) {
  ...
  for (multimap<string, string>::const_iterator type = types.find("type");
       type != types.end(); ++type) {
    try {
      ...
      MagLog::warning() << " Magics++ has no observation template for: "
                        << type->second << "\n"
...
static SimpleObjectMaker<ObsStationTriangle, ObsItem>
    ObsStationTriangle("obs_station_triangle");
static SimpleObjectMaker<ObsStationRing, ObsItem>
    ObsStationRing("obs_station_ring");
...
static SimpleObjectMaker<ObsCloudAndWind, ObsItem>
    ObsCloudAndWind("obs_cloud_wind");
static SimpleObjectMaker<ObsTemperature, ObsItem>
    ObsTemperature("obs_temperature");
static SimpleObjectMaker<ObsPressure, ObsItem> ObsPressure("obs_pressure");
static SimpleObjectMaker<ObsPressureTendency, ObsItem>
    ObsPressureTendency("obs_pressure_tendency");
static SimpleObjectMaker<ObsDewPoint, ObsItem> ObsDewPoint("obs_dewpoint");
static SimpleObjectMaker<ObsVisibility, ObsItem>
    ObsVisibility("obs_visibility");
static SimpleObjectMaker<ObsPresentWeather, ObsItem>
    ObsPresentWeather("obs_present_weather");
static SimpleObjectMaker<ObsPressureLevel, ObsItem>
    ObsPressureLevel("obs_pressure_level");
static SimpleObjectMaker<ObsIdentifier, ObsItem>
    ObsIdentifier("obs_identification");
static SimpleObjectMaker<ObsCloud, ObsItem> ObsCloud("obs_cloud");
static SimpleObjectMaker<ObsPastWeather, ObsItem>
    ObsPastWeather("obs_past_weather");
...
static SimpleObjectMaker<ObsSeaTemperature, ObsItem>
    SeaTempe("obs_sea_temperature");
