...
int CellShading::index(double value) { return map_.find(value, -1); }
...
void CellShading::operator()(IsoPlot *iso, MatrixHandler &data,
  ...
  UserPoint nextdata(point.x() + data.XResolution(),
                     point.y() + data.YResolution());
  ...
  MagLog::debug() << "Creation of a bitmap [rows, columns] --> [" << rows
                  << ", " << columns << "]" << endl;
...
void DumpShading::operator()(IsoPlot *, MatrixHandler &data,
                             BasicGraphicsObjectContainer &parent) {
  // Here we have to work using the parentput projection.
...
bool CellShading::prepare(LevelSelection &levels,
  ...
  map_[Interval(levels.back(), std::numeric_limits<double>::max())] =
      levels.size();
  colours_.push_back(Colour("none"));
  return false;
}
...
void CellShading::visit(LegendVisitor &legend, const ColourTechnique &) {
  ...
  // }
  bool first = true;
  for (IntervalMap<int>::const_iterator interval = map_.begin();
    ...
    for (vector<double>::iterator val = legend.values_list_.begin();
         val != legend.values_list_.end(); ++val) {
      if (min <= *val && *val < max) {
...
void CellShading::print(ostream &out) const { out << "CellShading"; }
...
CellArray *CellShading::array(MatrixHandler &matrix, IntervalMap<int> &range,
  ...
  if (adaptive_) {
    shading_ = "grid";
    return new GridArray(matrix, range, transformation, width, height,
                         resolution, "middle");
  } else {
    shading_ = "cell";
    return new CellArray(matrix, range, transformation, width, height,
