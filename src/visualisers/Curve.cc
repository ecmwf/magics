...
void setHandler(const vector<string> &keys, vector<T> &values,
                ListPolicy policy, const T &def, map<string, O> &handler) {
  if (values.empty())
    values.push_back(def);

  typename vector<T>::const_iterator value = values.begin();
  MagTranslator<T, O> translator;
  for (vector<string>::const_iterator key = keys.begin(); key != keys.end();
       ++key) {
...
void Curve::operator()(Data &data, BasicGraphicsObjectContainer &task) {
  ...
  if (magCompare(style_setting_, "advanced")) {
    string style = "solid";
    setHandler(colour_keys_, colour_list_, colour_policy_, colour_->name(),
               colourHandler_);
    setHandler(thickness_keys_, thickness_list_, thickness_policy_, thickness_,
               thicknessHandler_);
    ...
    string colour =
        get(colourHandler_, info[colour_key_], currentColour_.name());
    currentColour_ = Colour(colour);
    currentThickness_ =
        get(thicknessHandler_, info[thickness_key_], currentThickness_);
  ...
  PaperPoint last_current =
      transformation(UserPoint((**point)["x"], (**point)["y"]));
  ...
  while (point != points.end()) {
    ...
    if (!this->missing(**point)) {
      if (last_out) {
        bool result;
        std::map<string, MissingMethod>::iterator method =
            missingMethods_.find(lowerCase(missing_mode_));
      ...
      if (magCompare(plot_method_, "stepped")) {
        if (current.x_ != last_current.x_ && current.y_ != last_current.y_) {
          toadd =
              PaperPoint((current.x_ + last_current.x_) / 2., last_current.y_);
...
void Curve::set(const PaperPoint &point, BasicGraphicsObjectContainer &legend,
                LegendEntry &entry) {
  magics::Polyline *curve = new magics::Polyline();
  curve->setColour(currentColour_);
  curve->setLineStyle(currentStyle_);
...
void CurveArea::legend(magics::Polyline &curve) { shade_->legend(curve); }
void Curve::symbol(vector<PaperPoint> &points,
  ...
  symbol->outline(outline_, *outline_colour_, outline_thickness_,
                  outline_style_);

  const Transformation &transformation = out.transformation();

  for (vector<PaperPoint>::const_iterator point = points.begin();
...
void Curve::legend_symbol(PaperPoint &point,
...
bool Curve::ignore(const PaperPoint &, const PaperPoint &,
                   const vector<PaperPoint> &, BasicGraphicsObjectContainer &) {
  return true; // need new line!
}

bool Curve::join(const PaperPoint &p1, const PaperPoint &p2,
                 const vector<PaperPoint> &,
                 BasicGraphicsObjectContainer &task) {
...
bool Curve::drop(const PaperPoint &p1, const PaperPoint &p2,
                 const vector<PaperPoint> &points,
                 BasicGraphicsObjectContainer &task) {
  magics::Polyline *curve = new magics::Polyline();
  (*curve).setColour(*missing_colour_);
  (*curve).setLineStyle(missing_style_);
