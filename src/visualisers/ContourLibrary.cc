...
bool ContourLibrary::checkId(MetaDataCollector &metaId,
                             MetaDataCollector &metaKey) {
  // Obstat
...
void ContourLibrary::getStyle(MetaDataCollector &meta, MagDef &attributes,
                              StyleEntry &) {
  MagLog::dev() << "ContourLibrary::set-->" << endl;

  // Obstat
  if (info_) {
    for (map<string, string>::iterator it = meta.begin(); it != meta.end();
         it++) {
...
void EcChartData::callback(const string &name,
...
map<string, string> EcChartData::getMap(const int key) { return data_[key]; }

void EcChartSetData::callback(const string &name,
                              const json_spirit::Value &value) {
...
vector<int> EcChartSetData::getSet(const string &key) { return data_[key]; }

EcChartLibrary::EcChartLibrary()
    : contours_("contours"), default_set_("default") {
...
void EcChartLibrary::setCriteria(MetaDataCollector &request,
...
// se the map to set the contour!
void EcChartLibrary::getStyle(MetaDataCollector &data, MagDef &contour,
                              StyleEntry &) {
  ...
  if (result_set.size() > 1) {
    ...
    if (data["paramId"] != "" && ikey->second.hasKey(data["paramId"])) {
      ...
      vector<int> second_set =
          vector<int>(ikey->second.getSet(data["paramId"]));
      it = std ::set_intersection(first_set.begin(), first_set.end(),
  ...
  // for each GRIB key (excluding paramId and units that have just been checked,
  // OPTIONAL)
  ...
  if (result_set.size() > 0) {
    ...
    for (map<string, string>::const_iterator i = cont.begin(); i != cont.end();
         ++i) {
      contour[i->first] = i->second;
      MagLog::debug() << " EcChartLibrary::getAttributes contour[" << i->first
                      << "]= " << i->second << endl;
    }
  } else
    MagLog::info() << " EcChartLibrary::getAttributes: NO CONTOUR MATCHED!"
                   << endl;
}
...
void WebLibrary::setCriteria(MetaDataCollector &request,
                             const string &criteria) {
  request[criteria] = "";
  MetaDataAttribute attribute;
  attribute.setSource(MetaDataAttribute::GribApiSource);
...
void WebLibrary::getStyle(MetaDataCollector &data, MagDef &contour,
                          StyleEntry &info) {
  MagDef style;
...
// set the map to set the contour!
void WebLibrary::getScaling(MetaDataCollector &data, double &scaling,
                            double &offset) {
  MagDef values;
  StyleEntry info;
  scaling = 1;
  ...
  MagLog::debug() << "Need " << need->second << " get " << unit->second
                  << "--->APPLY " << scaling << " and " << offset << endl;
}
