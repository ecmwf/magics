...
public:
  Hasher(const SegmentJoiner::SegList &v) {
    ...
    for (SegmentJoiner::SegList::const_iterator i = v.begin(); i != v.end();
         ++i) {
      const Segment &s = (*i);
      double x = s.from_.x_;
      double y = s.from_.y_;
    ...
    miss = 0;
    // Compute min/max coordinates
    for (SegmentJoiner::SegList::const_reverse_iterator i = v.rbegin();
         i != v.rend(); ++i) {
      const Segment &s = (*i);
      double x = s.from_.x_;
      double y = s.from_.y_;
  ...
  hash_t operator()(const Point &p) const {
    return (((hash_t)((p.x_ + rvx_) * sfx_) << (bits_)) |
            ((hash_t)((p.y_ + rvy_) * sfy_))) &
           hashMask_;
  }
...
static double compute(SegmentJoiner::SegList &v, list<deque<Segment>> &lines) {
  ...
  // << "HASH " << hashSize << " H " << H.size() << " " << double(H.size()) /
  // double(hashSize)*100.0 << "%, avg= " << double(v.size())/double(H.size())
  // << endl;
      ...
      while (from) {
        ...
        if (s1.ok_) {
          ...
          while (to) {
            ...
            if (s2.ok_) {
              if (s1.from_ == s2.to_) {
                // ASSERT(!(s1.before_ && !s2.after_) || (s1.before_ &&
                // s2.after_));
  ...
  for (std::list<deque<Segment>>::iterator k = lines.begin(); k != lines.end();
       ++k) {
    deque<Segment> &p = (*k);
    if (p.size() >= 2) {
      Segment &f = p.front();
...
double SegmentJoiner::computeSegmentLines(list<deque<Segment>> &result) {
  if (dirty_) {
    // In case the algo is ran again, the segments internal states need
    // resetting
    dirty_ = false;
    for (SegmentJoiner::SegList::iterator j = segments_.begin();
         j != segments_.end(); ++j) {
      Segment &s = (*j);
      s.ok_ = true;
      s.before_.clear();
...
bool SegmentJoiner::pointInPoly(const Point &p, const vector<Point> &poly) {
  ...
  for (int i = 0; i < poly.size(); i++) {
    if ((poly[i].y_ > p.y_) != (poly[j].y_ > p.y_))
      if (p.x_ < (poly[j].x_ - poly[i].x_) * (p.y_ - poly[i].y_) /
                         (poly[j].y_ - poly[i].y_) +
                     poly[i].x_) {
        n++;
      }
...
void SegmentJoiner::punchHoles(vector<vector<Point>> &result) {
  ...
  for (vector<vector<Point>>::iterator j = result.begin(); j != result.end();
       ++j) {
    if (!isHole((*j))) {
...
void SegmentJoiner::check(list<deque<Segment>> &lines) {
  ...
  for (std::list<deque<Segment>>::iterator j = lines.begin(); j != lines.end();
       ++j) {
    deque<Segment> &p = (*j);

    for (std::list<deque<Segment>>::iterator k = lines.begin();
         k != lines.end(); ++k) {
      deque<Segment> &q = (*k);
      if (k != j) {
...
void SegmentJoiner::push_back(const Point &from, const Point &to) {
  Segment add(from, to);
  checker check(add);
  deque<Segment>::iterator done =
      std::remove_if(segments_.begin(), segments_.end(), check);
