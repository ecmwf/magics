...
struct Radius {
  Radius(double radius, CustomisedPoint &reference)
      : radius_(radius), reference_(reference) {}
  ~Radius() {}
  bool operator()(const std::unique_ptr<CustomisedPoint> &point) {
    return operator()(point.get());
  }
...
void clean(CustomisedPointsList &filter, CustomisedPointsList::iterator point) {
  double radius = 1;
  CustomisedPointsList::iterator last =
...
void Cities::operator()(const map<string, string> &,
  ...
  const double radius = 2. * font_size_ *
                        task.transformation().unitToCm(task.absoluteWidth(),
                                                       task.absoluteHeight());
  ...
  Symbol::TextPosition position =
      (pos != positions.end()) ? pos->second : Symbol::M_ABOVE;
  ...
  for (vector<CustomisedPoint *>::iterator point = filter.begin();
       point != filter.end(); ++point) {
    vector<CustomisedPoint *>::iterator last =
        std::remove_if(filter.begin(), filter.end(), Radius(radius, **point));
    filter.erase(last, filter.end());
  }
