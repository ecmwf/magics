...
AxisItem::AxisItem(double position, const string &label, int level)
    : position_(position), label_(label), level_(level), colour_("undef"),
      height_(DBL_MIN), font_("undef"), style_("undef") {}

AxisItem::AxisItem(double position, const string &label, int level,
                   const string &colour, double height)
    : position_(position), label_(label), level_(level), colour_(colour),
      height_(height), font_("undef"), style_("undef") {}
...
AxisHyperItem::AxisHyperItem(double position, vector<double> &values)
    : AxisItem(position, "") {
...
AxisDateItem::AxisDateItem(double position, DateTime date, int level,
                           const string &colour, double height)
    : AxisItem(position, "", level, colour, height), date_(date),
...
void AxisDateItem::format(const string &format, int digit) const {
  ...
  tm convert = date_;
  facet.put(out, out, ' ', &convert, format.c_str(),
