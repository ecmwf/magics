...
void Histogram::bean(PointsHandler &points) {
  ...
  while (points.more()) {
    try {
      ...
      // MagLog::debug() <<  points.current().value() << " is not in range..."
      // << endl;
    }
...
IntervalMap<int> &Histogram::histogram(const IntervalMap<Colour> &beans,
                                       PointsHandler &points) {
  for (IntervalMap<Colour>::const_iterator entry = beans.begin();
       entry != beans.end(); ++entry) {
    counter_.insert(
        make_pair(Interval(entry->first.min_, entry->first.max_), 0));
  }
  bean(points);
  return counter_;
}

void Histogram::visit(const IntervalMap<Colour> &beans, Data &data,
                      PointsHandler &points, HistoVisitor &visitor) {
  ...
  vector<int> y;
  vector<double> x;
  for (IntervalMap<int>::const_iterator count = counter_.begin();
       count != counter_.end(); ++count) {
    y.push_back(count->second);
    x.push_back(count->first.min_);
    x.push_back(count->first.max_);
    MagLog::dev() << "[" << count->first.min_ << "-" << count->first.max_
                  << "]-->" << count->second << endl;
  }
  ...
  for (vector<double>::iterator tick = ticks.begin(); tick != ticks.end();
       ++tick) {
    Text *val = new Text();
  ...
  int barCnt = 0;
  int barLabelFreq =
      1 + (counter_.size() / 10) - ((counter_.size() % 10 == 0) ? 1 : 0);

  for (IntervalMap<int>::const_iterator count = counter_.begin();
       count != counter_.end(); ++count, barCnt++) {
    Polyline *grid = new Polyline();
    grid->setColour(Colour("grey"));
    grid->setThickness(1);
    grid->setLineStyle(M_DOT);
    ...
    cols << sep << colour.red() << ":" << colour.green() << ":"
         << colour.blue();
