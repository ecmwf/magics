...
void SimplePolylineVisualiser::basic(Data &data,
  ...
  while (points.more()) {
    ...
    if (!points.current().missing()) {
      ...
      if (shade_ && points.current().value() >= min_ &&
          points.current().value() <= max_) {
...
void SimplePolylineVisualiser::
operator()(Data &data, BasicGraphicsObjectContainer &parent) {
  ...
  if (function != map_.end())
    ...
    MagLog::warning() << "Could not find method " << method_
                      << ": Use default visualisation" << endl;
    basic(data, parent);
  }
}
...
void SimplePolylineVisualiser::setup() {
  ...
  for (int i = 0; i < max; i++) {
    colour_map_[Interval(colour_level_list_[i], colour_level_list_[i + 1])] =
        Colour(*colour);
    ++colour;
  ...
  for (int i = 0; i < max; i++) {
    style_map_[Interval(style_level_list_[i], style_level_list_[i + 1])] =
        translator(*style);
    ++style;
  ...
  for (int i = 0; i < max; i++) {
    thickness_map_[Interval(thickness_level_list_[i],
                            thickness_level_list_[i + 1])] = *thickness;
    ++thickness;
    if (thickness == thickness_list_.end())
      thickness = (thickness_policy_ == M_CYCLE) ? thickness_list_.begin()
                                                 : --thickness;
  }
  max = transparency_level_list_.empty() ? 0
                                         : transparency_level_list_.size() - 1;

  for (int i = 0; i < max; i++) {
    alpha_map_[Interval(transparency_level_list_[i],
                        transparency_level_list_[i + 1])] = i;
  }
}

void SimplePolylineVisualiser::smooth(Data &data,
                                      BasicGraphicsObjectContainer &parent) {
  setup();
  if (legend_only_)
    return;
  ...
  for (map<double, vector<CustomisedPoint *>>::iterator where = work.begin();
       where != work.end(); ++where) {
    for (int i = 1; i < where->second.size(); i++) {
      ...
      if (add && alpha(*point) == 1) {
        ...
        if (magCompare(pivot_marker_name_, "cyclone")) {
          string name;
          if (point->latitude() > 0) {
            name = (x < 117) ? "WMO_TropicalCycloneNHLT"
                             : "WMO_TropicalCycloneNHGE";
          } else {
            name = (x < 117) ? "WMO_TropicalCycloneSHLT"
                             : "WMO_TropicalCycloneSHGE";
          }
...
void SimplePolylineVisualiser::visit(Data &, LegendVisitor &legend) {
  ...
  for (interval = colour_map_.begin(); interval != colour_map_.end();
       ++interval) {
