...
void ShadingTechnique::operator()(IsoPlot *iso, MatrixHandler &data,
                                  BasicGraphicsObjectContainer &parent) {
  if (!done_)
    iso->isoline(data, parent);
}

CellArray *PolyShadingTechnique::array(MatrixHandler &matrix,
...
CellArray *GridShading::array(MatrixHandler &matrix, IntervalMap<int> &range,
                              ...
                              const string &technique) {
  done_ = true;
  return new GridArray(matrix, range, transformation, width, height, resolution,
                       position_);
}
