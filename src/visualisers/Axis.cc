...
void HorizontalAxis::label(HorizontalAxisVisitor &axis) {
  ...
  for (AxisItems::const_iterator x = items_.begin(); x != items_.end(); ++x) {
    ...
    double height = ((*x)->height() == DBL_MIN || (*x)->height() == 0)
                        ? label_height_
                        : (*x)->height();
...
void VerticalAxis::label(VerticalAxisVisitor &axis) {
  ...
  for (AxisItems::const_iterator y = items_.begin(); y != items_.end(); ++y) {
    ...
    double height = ((*y)->height() == DBL_MIN || (*y)->height() == 0)
                        ? label_height_
                        : (*y)->height();
    double pos;
    ...
    text->setJustification(justification);
    Colour newcolour =
        (label_colour_->automatic()) ? *line_colour_ : *label_colour_;
...
string Axis::number(const AxisItem &in) { return in.label(); }
...
string Axis::basic(const AxisItem &item) { return item.label(); }

string Axis::createLabel(const AxisItem &in) {
  MagLog::dev() << "Axis::createLabel " << in
                << " with method : " << label_type_ << endl;

  map<string, LabelHelper>::iterator helper =
      labelHelpers_.find(lowerCase(label_type_));
  if (helper != labelHelpers_.end())
    return (this->*helper->second)(in);
  else {
    MagLog::warning() << "Could not the method " << label_type_
                      << " to setup axis labels..." << endl;
    return in.label();
  }
}
...
void HorizontalAxis::title(HorizontalAxisVisitor &out) {
  ...
  text->push_back(
      PaperPoint(x, out.offsetTickLabel(title_height_, title_position_)));
...
void HorizontalAxis::grid(DrawingVisitor &out) const {
  ...
  if (!grid_) {
    if (grid_reference_level_ != INT_MAX) {
      for (AxisItems::const_iterator x = items_.begin(); x != items_.end();
           ++x) {
        pos = (*x)->position();
        if (pos == grid_reference_level_) {
          ...
          Colour colour = !grid_reference_colour_->automatic()
                              ? *grid_reference_colour_
                              : *grid_colour_;
...
void VerticalAxis::grid(DrawingVisitor &out) const {
  ...
  if (!grid_) {
    if (grid_reference_level_ != INT_MAX) {
      for (AxisItems::const_iterator y = items_.begin(); y != items_.end();
           ++y) {
        pos = (*y)->position();
        if (pos == grid_reference_level_) {
          Polyline *grid = new Polyline();
...
void VerticalAxis::minortick(VerticalAxisVisitor &axis) {
  ...
  for (AxisItems::const_iterator y = items_.begin(); y != items_.end(); ++y) {
    ...
    Colour calcol =
        (minor_tick_colour_->automatic()) ? *tick_colour_ : *minor_tick_colour_;
    tick->setColour(calcol);
