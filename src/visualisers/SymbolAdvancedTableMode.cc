...
map<string, SymbolAdvancedTableMode::TextHandler>
    SymbolAdvancedTableMode::textHandlers_;
...
void SymbolAdvancedTableMode::adjust(double min, double max, bool scale,
  ...
  while (true) {
    ...
    MagLog::debug() << "[" << *level << ", " << *(level + 1) << "]=" << *marker
    ...
    if (index)
      properties = SymbolProperties(colourMethod_->right(*level), height,
                                    *marker, *text);
    else
      properties = SymbolProperties(colourMethod_->right(*level), height,
                                    *marker_name, *text);
    ...
    properties.connectLineColour_ = (parent_->automatic_connect_colour_)
                                        ? colourMethod_->right(*level)
  ...
  MagLog::debug() << "[" << *level << ", " << (*level) + EPSILON
                  << "]=" << colourMethod_->right(*level) << *marker
                  << "(marker)" << *text << "(text)" << endl;
...
void SymbolAdvancedTableMode::visit(Data &data, LegendVisitor &legend) {
  ...
  case LegendMethod::DISJOINT: {
    for (IntervalMap<SymbolProperties>::iterator interval = first;
         interval != last; ++interval) {
