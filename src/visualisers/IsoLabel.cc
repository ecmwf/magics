...
void IsoLabel::operator()(magics::Polyline &object, double l) const {
  ...
  Colour colour =
      (colour_ == "contour_line_colour") ? object.getColour() : Colour(colour_);
  std::map<string, Method>::const_iterator method =
      methods_.find(lowerCase(type_));

  string text;
  if (method == methods_.end()) {
    MagLog::warning() << "contour_label_type: " << type_
                      << " is unknown : use number instead" << endl;
    text = number(l);
  } else
...
string IsoLabel::text(double) const { return text_; }

string IsoLabel::both(double l) const { return number(l) + " " + text_; }
