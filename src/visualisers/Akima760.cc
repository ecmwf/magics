...
Akima760::Akima760(const AbstractMatrix &matrix,
                   const Akima760MethodAttributes &attr)
  ...
  double aux = (this->matrix_.regular_column(monoColumns_ - 1) -
                this->matrix_.regular_column(0)) /
               attr_.resolutionX_;
  if ((double)(int(aux)) != aux)
    aux += 1.; // next integer number
  ncols_ =
      int(aux + 1.); // must include the first and the last input coordinates

  aux = (this->matrix_.regular_row(monoRows_ - 1) -
         this->matrix_.regular_row(0)) /
        attr_.resolutionY_;
  if ((double)(int(aux)) != aux)
    aux += 1.; // next integer number
  nrows_ =
...
double Akima760::regular_row(int i) const {
  ...
  return (aux > this->matrix_.regular_row(monoRows_ - 1)
              ? this->matrix_.regular_row(monoRows_ - 1)
              : aux);
}

double Akima760::row(int i, int j) const { return regular_row(i); }
...
double Akima760::regular_column(int j) const {
  ...
  return (aux > this->matrix_.regular_column(monoColumns_ - 1)
              ? this->matrix_.regular_column(monoColumns_ - 1)
              : aux);
}

double Akima760::column(int i, int j) const { return regular_column(j); }
...
void Akima760::rgpd3p() {
  ...
  double B00 = 0., B00X = 0., B00Y = 0., B01 = 0., B10 = 0., B11 = 0., CX1 = 0.,
         CX2 = 0., CX3 = 0., CY1 = 0., CY2 = 0., CY3 = 0., DISF = 0., DNM = 0.,
         DZ00 = 0., DZ01 = 0., DZ02 = 0., DZ03 = 0., DZ10 = 0., DZ11 = 0.,
         DZ12 = 0., DZ13 = 0., DZ20 = 0., DZ21 = 0., DZ22 = 0., DZ23 = 0.,
         DZ30 = 0., DZ31 = 0., DZ32 = 0., DZ33 = 0., DZX10 = 0., DZX20 = 0.,
         DZX30 = 0., DZXY11 = 0., DZXY12 = 0., DZXY13 = 0., DZXY21 = 0.,
         DZXY22 = 0., DZXY23 = 0., DZXY31 = 0., DZXY32 = 0., DZXY33 = 0.,
         DZY01 = 0., DZY02 = 0., DZY03 = 0., EPSLN = 0., PEZX = 0., PEZXY = 0.,
         PEZY = 0., SMPEF = 0., SMPEI = 0., SMWTF = 0., SMWTI = 0., SX = 0.,
         SXX = 0., SXXY = 0., SXXYY = 0., SXY = 0., SXYY = 0., SXYZ = 0.,
         SXZ = 0., SY = 0., SYY = 0.,
         //              SYZ,SZ,VOLF,WT,X0,X1,X2,X3,XX1,XX2,XX3,Y0,Y1,Y2,
      SYZ = 0., SZ = 0., VOLF = 0., WT = 0., X0 = 0., X1 = 0., X2 = 0., X3 = 0.,
         Y0 = 0., Y1 = 0., Y2 = 0., Y3 = 0., Z00 = 0., Z01 = 0., Z02 = 0.,
         Z03 = 0., Z10 = 0., Z11 = 0., Z12 = 0., Z13 = 0., Z20 = 0., Z21 = 0.,
         Z22 = 0.,
         //              Z23,Z30,Z31,Z32,Z33,ZXDI,ZXYDI,ZYDI,ZZ0,ZZ1,ZZ2;
      Z23 = 0., Z30 = 0., Z31 = 0., Z32 = 0., Z33 = 0., ZXDI = 0., ZXYDI = 0.,
         ZYDI = 0.0;
  ...
  //*     .. Local Arrays .
  double B00XA[4], B00YA[4], B01A[4], B10A[4], CXA[3][4], CYA[3][4], SXA[4],
      SXXA[4], SYA[4], SYYA[4], XA[3][4], YA[3][4], Z0IA[3][4], ZI0A[3][4];
...
#define Z3F(XX1, XX2, XX3, ZZ0, ZZ1, ZZ2)                                      \
  (((ZZ2 - ZZ0) * (XX3 - XX1) / XX2 - (ZZ1 - ZZ0) * (XX3 - XX2) / XX1) *       \
       (XX3 / (XX2 - XX1)) +                                                   \
   ZZ0)
    ...
    for (IX0 = 0; IX0 < NXDF; IX0++) {
      ...
      for (IPEX = 0; IPEX < 4; IPEX++) {
        ...
        if (NXDF >= 4) {
          ...
          X2 = this->matrix_.regular_column(IX2) - X0;
          Z20 = this->matrix_(IY0, IX2);
          X3 = 2 * this->matrix_.regular_column(2) -
               this->matrix_.regular_column(1) - X0;
          Z30 = Z3F(X1, X2, X3, Z00, Z10, Z20);
        } else if (NXDF == 2) {
          X2 = 2 * this->matrix_.regular_column(1) -
               this->matrix_.regular_column(0) - X0;
          Z20 = Z2F(X1, X2, Z00, Z10);
          X3 = 2 * this->matrix_.regular_column(0) -
               this->matrix_.regular_column(1) - X0;
      ...
      for (IPEY = 0; IPEY < 4; IPEY++) {
        ...
        if (NYDF >= 4) {
          ...
          Y2 = this->matrix_.regular_row(IY2) - Y0;
          Z02 = this->matrix_(IY2, IX0);
          Y3 = 2 * this->matrix_.regular_row(2) - this->matrix_.regular_row(1) -
               Y0;
          Z03 = Z3F(Y1, Y2, Y3, Z00, Z01, Z02);
        } else if (NYDF == 2) {
          Y2 = 2 * this->matrix_.regular_row(1) - this->matrix_.regular_row(0) -
               Y0;
          Z02 = Z2F(Y1, Y2, Z00, Z01);
          Y3 = 2 * this->matrix_.regular_row(0) - this->matrix_.regular_row(1) -
               Y0;
      ...
      for (IPEX = 0; IPEX < 4; IPEX++) {
        ...
        for (IPEY = 0; IPEY < 4; IPEY++) {
          ...
          //* Calculates EPSLN
          EPSLN = (Z00 * Z00 + Z01 * Z01 + Z02 * Z02 + Z03 * Z03 + Z10 * Z10 +
                   Z11 * Z11 + Z12 * Z12 + Z13 * Z13 + Z20 * Z20 + Z21 * Z21 +
                   Z22 * Z22 + Z23 * Z23 + Z30 * Z30 + Z31 * Z31 + Z32 * Z32 +
                   Z33 * Z33) *
                  1.0E-12;
...
void Akima760::rgplnl(double XII, double YII, int IXDI, int IYDI,
                      double &ZII) const {
  ////*
...
void Akima760::boundRow(double r, double &row1, int &index1, double &row2,
                        int &index2) const {
...
void Akima760::boundColumn(double r, double &column1, int &index1,
