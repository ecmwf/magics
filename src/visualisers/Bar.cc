...
void Bar::vertical(CustomisedPointsList &points,
                   BasicGraphicsObjectContainer &out) {
  const Transformation &transformation = out.transformation();
  double y = transformation.getMinY() +
             (transformation.getMaxY() - transformation.getMinY()) * 0.2;
  ...
  for (CustomisedPointsList::const_iterator point = points.begin();
    ...
    map<string, Renderer>::iterator renderer =
        renderers_.find(lowerCase(bar_style_));

    if (renderer != renderers_.end()) {
      (this->*renderer->second)(x, top, bottom, out);
    } else {
      MagLog::warning() << "Coud not find the method " << bar_style_
                        << " to plot the bar\n"
...
void Bar::horizontal(CustomisedPointsList &points,
                     BasicGraphicsObjectContainer &out) {
  const Transformation &transformation = out.transformation();
  double x = transformation.getMinX() +
             (transformation.getMaxX() - transformation.getMinX()) * 0.2;
...
void Bar::fullbar(double x, double a, double b,
                  BasicGraphicsObjectContainer &out) {
  if (magCompare(orientation_, "horizontal"))
    fullbar_y(x, a, b, out);
  else
...
void Bar::linebar(double x, double a, double b,
                  BasicGraphicsObjectContainer &out) {
  if (magCompare(orientation_, "horizontal"))
    linebar_y(x, a, b, out);
  else
...
void Bar::fullbar_y(double y, double left, double right,
                    BasicGraphicsObjectContainer &out) {
  const Transformation &transformation = out.transformation();
...
void Bar::fullbar_x(double x, double top, double bottom,
                    BasicGraphicsObjectContainer &out) {
  const Transformation &transformation = out.transformation();
...
void Bar::linebar_x(double x, double top, double bottom,
                    BasicGraphicsObjectContainer &out) {
  const Transformation &transformation = out.transformation();
  double left = x - width_ / 2;
  double right = x + width_ / 2;
...
void Bar::linebar_y(double y, double left, double right,
                    BasicGraphicsObjectContainer &out) {
  const Transformation &transformation = out.transformation();
  double bottom = y - width_ / 2;
  double top = y + width_ / 2;
...
void GraphFlag::operator()(Data &data, BasicGraphicsObjectContainer &out) {
  ...
  for (const auto &point : points) {
    ...
    x = (*point)["x"];
    y = (*point)["y"];
    missing = (point->find("intensity") != point->end()) ? (*point)["missing"]
                                                         : -99999999;
    double lat = 10;
    if (point->find("latitude") != point->end())
      lat = (*point)["latitude"];
    flags->setHemisphere(lat > 0 ? NORTH : SOUTH);
    if (point->find("intensity") != point->end() &&
        point->find("direction") != point->end()) {
