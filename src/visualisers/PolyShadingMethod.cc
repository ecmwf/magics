...
void PolyShadingMethod::visit(LegendVisitor &legend,
                              const ColourTechnique &colour) {
  ...
  legend.last(); // Flag the last entry as being the last! To get a nice
                 // labelling in countinuous mode!!!
  helper.setLabels();
}
...
void PolyShadingMethod::prepare(LevelSelection &levels,
...
void DotPolyShadingMethod::prepare(LevelSelection &levels,
                                   const ColourTechnique &colours) {
  if (levels.empty())
    return;
...
void HatchPolyShadingMethod::prepare(LevelSelection &levels,
                                     const ColourTechnique &colours) {
  int index = 1;
