...
void WindPlotting::setAdvanced(double &min, double &max) {
  ...
  while (true) {
    if (level + 1 == levels_->end())
      break;
    MagLog::debug() << "[" << *level << ", " << *(level + 1)
                    << "]=" << colourMethod_->right(*level) << endl;
...
void WindPlotting::adjust(CustomisedPointsList &points,
  ...
  for (const auto &point : points) {
    if (transformation.in(point->longitude(), point->latitude()))
      values.push_back(value((*point)["x_component"], (*point)["y_component"],
  ...
  if (set == settingMethods_.end()) {
    MagLog::warning() << advanced_method_
                      << " is not valid value for  'wind_advanced_method': "
...
double WindPlotting::parameter(double, double, double col) { return col; }
...
double WindPlotting::value(double x, double y, double col) {
  ...
  map<string, ColouringMethod>::const_iterator method =
      colouringMethods_.find(value);
  if (method == colouringMethods_.end()) {
    MagLog::warning()
        << colour_method_
        << " is not valid value for  'wind_advanced_colour_parameter': reset "
           "to default[speed]"
        << endl;
