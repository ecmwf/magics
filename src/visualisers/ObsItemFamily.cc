...
void ObsPressureTendency::operator()(CustomisedPoint& point, ComplexSymbol& symbol) const {
    ...
    Colour colour =
        owner_->pressure_tendency_colour_->automatic() ? *owner_->colour_ : *owner_->pressure_tendency_colour_;
    TextItem* object = new TextItem();
    object->x(column_);
    object->y(row_);
    ...
    if (value->second < 100) {
        ...
        if (w == presentweather.end()) {
            MagLog::warning() << "OBS > Present Weather " << value->second
                              << " not recognised yet, please contact Magics team" << endl;
        }
        else
            ww = w->second;
    }
...
void ObsIdentifier::operator()(CustomisedPoint& point, ComplexSymbol& symbol) const {
    ...
    TextItem* time = new TextItem();
#ifdef OBS_DEBUG_
    //FMagLog::debug() << "Identification for " << point.identifier() << "at[" << column_ << ", " << row_ << "]" << endl;
#endif
...
void ObsCloud::operator()(CustomisedPoint& point, ComplexSymbol& symbol) const {
    if (clouds_.empty()) {
        ...
        clouds_[12] = "CH_2";  // 12 12 CIRRUS SPISSATUS, IN PATCHES OR ENTANGLED SHEAVES, WHICH USUALLY DO NOT INCREASE
                               // AND SOMETIMES SEEM TO BE THE REMAINS OF THE UPPER PART OF A CUMULONIMBUS; OR CIRRUS
                               // CASTELLANUS OR FLOCCUS
        clouds_[13] = "CH_3";  // 13 13 CIRRUS SPISSATUS CUMULONIMBOGENITUS
        clouds_[14] = "CH_4";  // 14 14 CIRRUS UNCINUS OR FIBRATUS, OR BOTH, PROGRESSIVELY INVADING THE SKY; THEY
                               // GENERALLY THICKEN AS A WHOLE
        clouds_[15] = "CH_5";  // 15 15 CIRRUS (OFTEN IN BANDS) AND CIRROSTRATUS, OR CIRROSTRATUS ALONE, PROGRESSIVELY
                               // INVADING THE SKY; THEY GENERALLY THICKEN AS A WHOLE, BUT THE CONTINUOUS VEIL DOES NOT
                               // REACH 45 DEGREES ABOVE THE HORIZON
        clouds_[16] = "CH_6";  // 16 16 CIRRUS (OFTEN IN BANDS) AND CIRROSTRATUS, OR CIRROSTRATUS ALONE, PROGRESSIVELY
                               // INVADING THE SKY; THEY GENERALLY THICKEN AS A WHOLE; THE CONTINUOUS VEIL EXTENDS MORE
                               // THAN 45 DEGREES ABOVE THE HORIZON, WITHOUT THE SKY BEING TOTALLY COVERED
        ...
        clouds_[24] = "CM_4";  // 24 24 PATCHES (OFTEN LENTICULAR) OF ALTOCUMULUS TRANSLUCIDUS, CONTINUALLY CHANGING AND
        ...
        clouds_[32] =
            "CL_2";  // 32 32 CUMULUS MEDIOCRIS OR CONGESTUS, TOWERING CUMULUS (TCU), WITH OR WITHOUT CUMULUS OF SPECIES
                     // FRACTUS OR HUMILIS OR STRATOCUMULUS, ALL HAVING THEIR BASES AT THE SAME LEVEL
        ...
        clouds_[37] = "CL_7";  // 37 37 STRATUS FRACTUS OR CUMULUS FRACTUS OF BAD WEATHER,* OR BOTH (PANNUS), USUALLY
                               // BELOW ALTOSTRATUS OR NIMBOSTRATUS
        clouds_[38] = "CL_8";  // 38 38 CUMULUS AND STRATOCUMULUS OTHER THAN STRATOCUMULUS CUMULOGENITUS, WITH BASES AT
                               // DIFFERENT LEVELS
        clouds_[39] = "CL_9";  // 39 39 CUMULONIMBUS CAPILLATUS (OFTEN WITH AN ANVIL), WITH OR WITHOUT CUMULONIMBUS
        ...
        // 60 60 CH CLOUDS INVISIBLE OWING TO DARKNESS, FOG, BLOWING DUST OR SAND, OR OTHER SIMILAR PHENOMENA, OR
        // BECAUSE OF A CONTINUOUS LAYER OF LOWER CLOUDS 61 61 CM CLOUDS INVISIBLE OWING TO DARKNESS, FOG, BLOWING DUST
        // OR SAND, OR OTHER SIMILAR PHENOMENA, OR BECAUSE OF CONTINUOUS LAYER OF LOWER CLOUDS 62 62 CL CLOUDS INVISIBLE
