...
void ObsWind::operator()(CustomisedPoint &point, ComplexSymbol &symbol) const {
  ...
  Colour colour = owner_->wind_colour_->automatic() ? *owner_->colour_
                                                    : *owner_->wind_colour_;
  ...
  MagLog::debug() << "OBS ITEM - ObsWind - Lon/Lat: " << point.longitude()
                  << " / " << point.latitude()
                  << "\n\twind_speed:     " << point[speed_]
                  << "\n\twind_direction: " << point[direction_]
                  << "\n\tcloud amount:   " << point["total_cloud"] << " -> "
                  << origin << std::endl;
  ...
  std::pair<double, double> wind =
      std::make_pair(speed->second, direction->second);
...
void ObsCloudAndWind::operator()(CustomisedPoint &point,
                                 ComplexSymbol &symbol) const {
  if (!owner_->wind_visible_)
    return;
  ...
  MagLog::debug() << "total_cloud-->" << point["total_cloud"] << "--->"
                  << total_cloud << endl;
...
void ObsPressure::visit(std::set<string> &tokens) {
  if (!owner_->pressure_visible_)
    return;

  tokens.insert("msl_pressure");
}

void ObsPressure::operator()(CustomisedPoint &point,
                             ComplexSymbol &symbol) const {
  if (!owner_->pressure_visible_)
    return;
  ...
  Colour colour = owner_->pressure_colour_->automatic()
                      ? *owner_->colour_
                      : *owner_->pressure_colour_;
  ...
  MagLog::debug() << "\tPressure:  " << value->second << " = " << pressure
                  << " -> " << os.str() << "\n";
...
void ObsPressureLevel::visit(std::set<string> &tokens) {
  if (!owner_->upper_air_visible_)
    return;
  tokens.insert("pressure");
}

void ObsPressureLevel::operator()(CustomisedPoint &point,
                                  ComplexSymbol &symbol) const {
  if (!owner_->upper_air_visible_)
    return;
  ...
  Colour colour = owner_->upper_air_colour_->automatic()
                      ? *owner_->colour_
                      : *owner_->upper_air_colour_;
  font.colour(colour);
...
void ObsPressureTendency::visit(std::set<string> &tokens) {
  if (!owner_->pressure_tendency_visible_)
    return;
  tokens.insert("pressure_tendency_amount");
  tokens.insert("pressure_tendency_characteristic");
}

void ObsPressureTendency::operator()(CustomisedPoint &point,
                                     ComplexSymbol &symbol) const {
  if (!owner_->pressure_tendency_visible_)
    return;
  CustomisedPoint::const_iterator value =
      point.find("pressure_tendency_amount");
  if (value == point.end())
    return;

  Colour colour = owner_->pressure_tendency_colour_->automatic()
                      ? *owner_->colour_
                      : *owner_->pressure_tendency_colour_;
  TextItem *object = new TextItem();
  object->x(column_);
  object->y(row_);
  ...
  ostringstream os;
  if (value->second >= 0)
    os << setw(2) << setfill('0') << value->second * .1;
  else
    os << "-" << setw(2) << setfill('0') << value->second * -.1;
...
void ObsDewPoint::visit(std::set<string> &tokens) {
  if (!owner_->dewpoint_visible_)
    return;
  tokens.insert("dewpoint");
}

void ObsDewPoint::operator()(CustomisedPoint &point,
                             ComplexSymbol &symbol) const {
  if (!owner_->dewpoint_visible_)
    return;
  ...
  TextItem *object = new TextItem();
  MagFont font("sansserif");
  Colour colour = owner_->dewpoint_colour_->automatic()
                      ? *owner_->colour_
                      : *owner_->dewpoint_colour_;
  font.colour(colour);
  font.size(owner_->size_);
  object->x(column_);
  object->y(row_);
  ...
  MagLog::debug() << "\tDewPoint--->" << point["dewpoint_2meters"] << " = "
                  << tempe << "\n";
...
*/
void ObsVisibility::operator()(CustomisedPoint &point,
                               ComplexSymbol &symbol) const {
  if (!owner_->visibility_visible_)
    return;
  ...
  if (value == point.end())
    return;
  Colour colour = owner_->visibility_colour_->automatic()
                      ? *owner_->colour_
                      : *owner_->visibility_colour_;
  TextItem *object = new TextItem();
  object->x(column_);
  object->y(row_);
...
void ObsPresentWeather::operator()(CustomisedPoint &point,
                                   ComplexSymbol &symbol) const {
  if (!owner_->present_ww_visible_)
    return;
  ...
  if (value->second < 100) {
    ...
    if (w == presentweather.end()) {
      MagLog::warning() << "OBS > Present Weather " << value->second
                        << " not recognised yet, please contact Magics team"
                        << endl;
    } else
      ww = w->second;
  }
  ...
  Colour colour = owner_->present_ww_colour_->automatic()
                      ? *owner_->colour_
                      : *owner_->present_ww_colour_;
  object->colour(colour);
...
void ObsTemperature::operator()(CustomisedPoint &point,
                                ComplexSymbol &symbol) const {
  ...
  Colour colour = owner_->temperature_colour_->automatic()
                      ? *owner_->colour_
                      : *owner_->temperature_colour_;
...
void ObsTimePlot::visit(std::set<string> &) {
  if (!owner_->time_plot_visible_)
    return;
}

void ObsTimePlot::operator()(CustomisedPoint &point,
                             ComplexSymbol &symbol) const {
  if (!owner_->time_plot_visible_)
    return;
  ...
  MagLog::debug() << "\tTimePlot: " << value->second << "at[" << column_ << ", "
                  << row_ << "]" << endl;

  Colour colour = owner_->time_plot_colour_->automatic()
                      ? *owner_->colour_
                      : *owner_->time_plot_colour_;
...
void ObsHeight::visit(std::set<string> &tokens) {
  if (!owner_->height_visible_)
    return;
  tokens.insert("geopotential");
}

void ObsHeight::operator()(CustomisedPoint &point,
                           ComplexSymbol &symbol) const {
  if (!owner_->height_visible_)
    return;
  ...
  MagLog::debug() << "\tGeopotential: " << geop << "at[" << column_ << ", "
                  << row_ << "]" << endl;
  Colour colour = owner_->height_colour_->automatic() ? *owner_->colour_
                                                      : *owner_->height_colour_;
...
void ObsThickness::visit(std::set<string> &tokens) {
  if (!owner_->thickness_visible_)
    return;
  tokens.insert("thickness");
}

void ObsThickness::operator()(CustomisedPoint &point,
                              ComplexSymbol &symbol) const {
  if (!owner_->thickness_visible_)
    return;
...
#ifdef OBS_DEBUG_
  MagLog::debug() << "\tThickness: " << thickness << "at[" << column_ << ", "
                  << row_ << "]" << endl;
#endif
  Colour colour = owner_->thickness_colour_->automatic()
                      ? *owner_->colour_
                      : *owner_->thickness_colour_;
  TextItem *object = new TextItem();
  MagFont font("sansserif");
  font.colour(colour);
  font.size(owner_->size_);
...
void ObsIdentifier::operator()(CustomisedPoint &point,
                               ComplexSymbol &symbol) const {
  if (!owner_->identifier_visible_)
    return;
  TextItem *time = new TextItem();
#ifdef OBS_DEBUG_
  MagLog::debug() << "Identification for " << point.identifier() << "at["
                  << column_ << ", " << row_ << "]" << endl;
#endif
  Colour colour = owner_->identifier_colour_->automatic()
                      ? *owner_->colour_
                      : *owner_->identifier_colour_;
  MagFont font("sansserif");
  font.colour(colour);
  font.size(owner_->size_);
  time->x(column_);
...
void ObsPastWeather::operator()(CustomisedPoint &point,
                                ComplexSymbol &symbol) const {
  ...
  Colour colour = owner_->past_ww_colour_->automatic()
                      ? *owner_->colour_
                      : *owner_->past_ww_colour_;
...
void ObsCloud::operator()(CustomisedPoint &point, ComplexSymbol &symbol) const {
  if (clouds_.empty()) {
    ...
    // 10 10 NO CH CLOUDS
    clouds_[11] = "CH_1"; // 11 11 CIRRUS FIBRATUS, SOMETIMES UNCINUS, NOT
                          // PROGRESSIVELY INVADING THE SKY
    clouds_[12] = "CH_2"; // 12 12 CIRRUS SPISSATUS, IN PATCHES OR ENTANGLED
                          // SHEAVES, WHICH USUALLY DO NOT INCREASE AND
                          // SOMETIMES SEEM TO BE THE REMAINS OF THE UPPER PART
                          // OF A CUMULONIMBUS; OR CIRRUS CASTELLANUS OR FLOCCUS
    clouds_[13] = "CH_3"; // 13 13 CIRRUS SPISSATUS CUMULONIMBOGENITUS
    clouds_[14] =
        "CH_4"; // 14 14 CIRRUS UNCINUS OR FIBRATUS, OR BOTH, PROGRESSIVELY
                // INVADING THE SKY; THEY GENERALLY THICKEN AS A WHOLE
    clouds_[15] =
        "CH_5"; // 15 15 CIRRUS (OFTEN IN BANDS) AND CIRROSTRATUS, OR
                // CIRROSTRATUS ALONE, PROGRESSIVELY INVADING THE SKY; THEY
                // GENERALLY THICKEN AS A WHOLE, BUT THE CONTINUOUS VEIL DOES
                // NOT REACH 45 DEGREES ABOVE THE HORIZON
    clouds_[16] = "CH_6"; // 16 16 CIRRUS (OFTEN IN BANDS) AND CIRROSTRATUS, OR
                          // CIRROSTRATUS ALONE, PROGRESSIVELY INVADING THE SKY;
                          // THEY GENERALLY THICKEN AS A WHOLE; THE CONTINUOUS
                          // VEIL EXTENDS MORE THAN 45 DEGREES ABOVE THE
                          // HORIZON, WITHOUT THE SKY BEING TOTALLY COVERED
    clouds_[17] = "CH_7"; // 17 17 CIRROSTRATUS COVERING THE WHOLE SKY
    clouds_[18] = "CH_8"; // 18 18 CIRROSTRATUS NOT PROGRESSIVELY INVADING THE
                          // SKY AND NOT ENTIRELY COVERING IT
    clouds_[19] = "CH_9"; // 19 19 CIRROCUMULUS ALONE, OR CIRROCUMULUS
                          // PREDOMINANT AMONG THE CH CLOUDS
    // 20 20 NO CM CLOUDS
    ...
    clouds_[24] =
        "CM_4"; // 24 24 PATCHES (OFTEN LENTICULAR) OF ALTOCUMULUS TRANSLUCIDUS,
                // CONTINUALLY CHANGING AND OCCURRING AT ONE OR MORE LEVELS
    clouds_[25] = "CM_5"; // 25 25 ALTOCUMULUS TRANSLUCIDUS IN BANDS, OR ONE OR
                          // MORE LAYERS OF ALTOCUMULUS TRANSLUCIDUS OR OPACUS,
                          // PROGRESSIVELY INVADING THE SKY; THESE ALTOCUMULUS
                          // CLOUDS GENERALLY THICKEN AS A WHOLE
    clouds_[26] =
        "CM_6"; // 26 26 ALTOCUMULUS CUMULOGENITUS (OR CUMULONIMBOGENITUS)
    clouds_[27] = "CM_7"; // 27 27 ALTOCUMULUS TRANSLUCIDUS OR OPACUS IN TWO OR
                          // MORE LAYERS, OR ALTOCUMULUS OPACUS IN A SINGLE
                          // LAYER, NOT PROGRESSIVELY INVADING THE SKY, OR
                          // ALTOCUMULUS WITH ALTOSTRATUS OR NIMBOSTRATUS
    clouds_[28] = "CM_8"; //		28 28 ALTOCUMULUS CASTELLANUS OR FLOCCUS
    clouds_[29] = "CM_9"; // 29 29 ALTOCUMULUS OF A CHAOTIC SKY, GENERALLY AT
                          // SEVERAL LEVELS
    // 30 30 NO CL CLOUDS
    clouds_[31] = "CL_1"; // 31 31 CUMULUS HUMILIS OR CUMULUS FRACTUS OTHER THAN
                          // OF BAD WEATHER,* OR BOTH
    clouds_[32] =
        "CL_2"; // 32 32 CUMULUS MEDIOCRIS OR CONGESTUS, TOWERING CUMULUS (TCU),
                // WITH OR WITHOUT CUMULUS OF SPECIES FRACTUS OR HUMILIS OR
                // STRATOCUMULUS, ALL HAVING THEIR BASES AT THE SAME LEVEL
    clouds_[33] = "CL_3"; // 33 33 CUMULONIMBUS CALVUS, WITH OR WITHOUT CUMULUS,
                          // STRATOCUMULUS OR STRATUS
    clouds_[34] = "CL_4"; // 34 34 STRATOCUMULUS CUMULOGENITUS
    clouds_[35] =
        "CL_5"; // 35 35 STRATOCUMULUS OTHER THAN STRATOCUMULUS CUMULOGENITUS
    clouds_[36] = "CL_6"; // 36 36 STRATUS NEBULOSUS OR STRATUS FRACTUS OTHER
                          // THAN OF BAD WEATHER,* OR BOTH
    clouds_[37] =
        "CL_7"; // 37 37 STRATUS FRACTUS OR CUMULUS FRACTUS OF BAD WEATHER,* OR
                // BOTH (PANNUS), USUALLY BELOW ALTOSTRATUS OR NIMBOSTRATUS
    clouds_[38] =
        "CL_8"; // 38 38 CUMULUS AND STRATOCUMULUS OTHER THAN STRATOCUMULUS
                // CUMULOGENITUS, WITH BASES AT DIFFERENT LEVELS
    clouds_[39] = "CL_9"; // 39 39 CUMULONIMBUS CAPILLATUS (OFTEN WITH AN
                          // ANVIL), WITH OR WITHOUT CUMULONIMBUS CALVUS,
                          // CUMULUS, STRATOCUMULUS, STRATUS OR PANNUS
    // 0 40 CH
    // 41 41 CM
    // 42 42 CL
    // 59 59 CLOUD NOT VISIBLE OWING TO DARKNESS, FOG, DUSTSTORM, SANDSTORM, OR
    // OTHER ANALOGOUS PHENOMENA 60 60 CH CLOUDS INVISIBLE OWING TO DARKNESS,
    // FOG, BLOWING DUST OR SAND, OR OTHER SIMILAR PHENOMENA, OR BECAUSE OF A
    // CONTINUOUS LAYER OF LOWER CLOUDS 61 61 CM CLOUDS INVISIBLE OWING TO
    // DARKNESS, FOG, BLOWING DUST OR SAND, OR OTHER SIMILAR PHENOMENA, OR
    // BECAUSE OF CONTINUOUS LAYER OF LOWER CLOUDS 62 62 CL CLOUDS INVISIBLE
    // OWING TO DARKNESS, FOG, BLOWING DUST OR SAND, OR OTHER SIMILAR PHENOMENA
  }
  ...
  if (value != point.end()) {
    ...
    if (low != clouds_.end()) {
      ...
      MagLog::debug() << "\tLow Cloud--->" << value->second << "-->"
                      << low->second << "\n";
  ...
  if (value != point.end()) {
    ...
    if (medium != clouds_.end()) {
      ...
      MagLog::debug() << "\tMedium Cloud--->" << value->second << " --> "
                      << medium->second << "\n";
  ...
  if (value != point.end()) {
    ...
    if (high != clouds_.end()) {
      ...
      MagLog::debug() << "\tHigh Cloud--->" << value->second << "-->"
                      << high->second << "\n";
...
void ObsDemoItem1::operator()(CustomisedPoint &point,
                              ComplexSymbol &symbol) const {
...
void ObsDemoItem2::operator()(CustomisedPoint &point,
                              ComplexSymbol &symbol) const {
  string text;
...
void ObsEra::visit(std::set<string> &tokens) { tokens.insert(key_); }
...
void ObsNumber::visit(std::set<string> &tokens) { tokens.insert(key_); }

void ObsNumber::operator()(CustomisedPoint &point,
                           ComplexSymbol &symbol) const {
  CustomisedPoint::iterator pt = point.find(key_);
  if (pt == point.end())
    return;
...
void ObsString::visit(std::set<string> &tokens) {
  ...
  for (vector<string>::const_iterator token = keys.begin(); token != keys.end();
       ++token) {
    vector<string> key;
    tend(*token, key);
    if (!key.empty()) {
...
void ObsString::operator()(CustomisedPoint &point,
                           ComplexSymbol &symbol) const {
  string text = format_;
  for (std::set<string>::iterator key = keys_.begin(); key != keys_.end();
       ++key) {
...
void ObsSeaTemperature::visit(std::set<string> &tokens) {
  if (!owner_->sea_temperature_visible_)
    return;
  tokens.insert("sea_temperature");
}

void ObsSeaTemperature::operator()(CustomisedPoint &point,
                                   ComplexSymbol &symbol) const {
  if (!owner_->sea_temperature_visible_)
    return;
  ...
  if (value == point.end())
    return;
  Colour colour = owner_->sea_temperature_colour_->automatic()
                      ? *owner_->colour_
                      : *owner_->sea_temperature_colour_;
  TextItem *object = new TextItem();
  MagFont font("sansserif");
  font.colour(colour);
  font.size(owner_->size_);
...
void ObsWave::operator()(CustomisedPoint &point, ComplexSymbol &symbol) const {
  ...
  Colour colour = owner_->waves_colour_->automatic() ? *owner_->colour_
                                                     : *owner_->waves_colour_;

  if (period == point.end() || height == point.end())
    return;

  MagLog::debug() << "\theight: " << height->second << " period "
                  << period->second << "\n";
