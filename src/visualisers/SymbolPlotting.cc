...
double SymbolPlotting::height(const Transformation &transformation,
                              double height) {
...
void SymbolPlotting::operator()(const PaperPoint &point,
  ...
  try {
    ...
    map<SymbolProperties, Symbol *>::const_iterator symb =
        symbols_.find(properties);
...
void SymbolPlotting::operator()(Data &data, BasicGraphicsObjectContainer &out) {
  ...
  if (!valid) {
    MagLog::warning() << type_ << " not yet implemented : reset to marker "
                      << endl;
    type_ = "marker";
  }
  mode_->set(type_);
  ...
  try {
    ...
    // to enable proper clipping of the line! Othewise wee just nedde to get the
    // point from the visible area.

    PointsHandler &points = data.points(transformation, connect_);

    // Some Mode need to know the min and max of the data, in order to adjust
    // the computation of the levels
    (*mode_).adjust(points.min(), points.max(), scaling_method_, transformation,
                    scaling_factor_);
    if (legend_only_)
      return;
    ...
    vector<Symbol *> work;
    for (map<SymbolProperties, Symbol *>::iterator symbol = symbols_.begin();
         symbol != symbols_.end(); ++symbol)
      work.push_back(symbol->second);

    std::sort(work.begin(), work.end(), SortHelper());

    // Now we feed the task...     f
    for (vector<Symbol *>::iterator symbol = work.begin(); symbol != work.end();
         ++symbol) {
      if (!(*symbol)->empty()) {
