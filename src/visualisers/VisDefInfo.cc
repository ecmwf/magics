...
VisDefInfoBase::VisDefInfoBase(string fConf, DataType dataType)
    : fConf_(fConf), dataType_(dataType) {
  loaded_ = false;
  dataTypeName_[GribType] = "GRIB";
}

VisDefInfoBase::~VisDefInfoBase() { clear(); }
...
void VisDefInfoBase::collectKeys() {
  keys_.clear();
  for (vector<VisDefInfoItem *>::iterator it = items_.begin();
       it != items_.end(); it++) {
    for (map<string, vector<string>>::const_iterator itK =
             (*it)->keys().begin();
         itK != (*it)->keys().end(); itK++) {
...
ObstatVisDefInfo::ObstatVisDefInfo(string fConf, DataType dataType)
    : VisDefInfoBase(fConf, dataType) {
...
void ObstatVisDefInfo::loadItems() {
  ...
  // Obs        fgdep       bcor       Obstdv       fgdStdv    BcorSTDV Count
  // Unit
  const char *statIdDef[] = {"2", "4", "18", "3", "5", "19", "1"};
  vector<string> statId(statIdDef,
                        statIdDef + sizeof(statIdDef) / sizeof(char *));
  ...
  if (in.fail()) {
    MagLog::warning()
        << "ObstatVisDefInfo::loadItems() --> Cannot open visdef file "
        << fConf_ << "\n";
    return;
  }
...
void ObstatVisDefInfo::getAttributes(MetaDataCollector &meta,
