...
MvObsSet::MvObsSet(const char *aName)
    : _unpacked(false), _msgCount(0), _ecH(NULL), _minTime(2247, 6, 20),
      _maxTime(1799, 12, 31), useSkipExtraAttributes_(true),
...
void MvObsSet::_init(const char *aName) {
  ...
  if (_IO_mode ==
      WRITE) // should be two classes!!!!!!! (but this a working one...)
  {
#ifdef MV_BUFRDC_TEST
    _message = 0;
    ...
    #ifdef METVIEW_PREPBUFR
          _firstObs = next();     //-- check if PrepBUFR file (contains BUFR
    tables) _isPrepBufrFile = ( _firstObs.messageType() == cMSG_TYPE_BUFR_TABLES
    ); rewind();

          //  1) test if PrepBUFR tables have already been extracted:
    MV_PREPBUFR_TABLES_EXTRACTED
    ...
    #else
          cout << "MvObsSet::_init - PrepBUFR support NOT available!!!!" <<
    endl; #endif
    */ //FAMI20171005
  }
...
bool MvObsSet::Open(const char *aFileName) {
  ...
  if (!_ecFile) {
    std::cerr << " >>> MvObsSet::Open - ERROR opening file \'" << aFileName
...
MvObs MvObsSet::gotoMessage(long offset, int msgCnt) {
  ...
  if (_ecH != NULL || err != CODES_SUCCESS) {
    if (_ecH == NULL) {
      std::cout << "Failed reading next BUFR msg: unable to create handle for "
                   "message = "
                << _msgNumber << std::endl;
      codes_handle_delete(_ecH);
      _ecH = 0;
  ...
  return MvObs(&_ecH, 1, _unpacked, useSkipExtraAttributes_,
               cacheCompressedData_); // subset number = 1
}
...
// e Analyse the PREPBufr code inside the loop.
  ...
  while (readAnotherMessage) {
    ...
    if (myError == -3) // Bufr too small
    {
      ...
      if (!_message) {
        cerr << "MvObsSet::next: Cannot allocate memory for next BUFR message"
             << endl;
    ...
    if (myError) {
      if (myError != EOF_STATUS) {
        cerr << "MvObsSet::next: Failed reading next BUFR msg, returned status="
             << myError << endl;
      }
      _IO_buffer_OK = false;
      break;
    ...
    //--
    MvBufr tmpBufr(_message, _msgLen,
                   _msgNumber); //-- make BUFR octets into an object
    if (tmpBufr.subsetCount() == 0) {
      ...
      //-- get next msg and cross your fingers it fits into current _message
      // array
      _msgLen = MAX_MESSAGE_LENGTH;
      myError = _readbufr(_bufrFile, _message, &_msgLen);
...
bool MvObsSet::write(const char *aMsg, int aMsgLen) {
  ...
  // FAMI20171017 remove this line temporarily, because the ecCodes write
  // command below is also increasing the message count FAMI20171017
  // _msgNumber++;
  return true;
}
#endif
...
bool MvObsSet::write(const void *aMsg, const size_t aMsgLen) {
  ...
  if (fwrite(aMsg, 1, size, _ecFile) != aMsgLen) {
    std::cout << "ERROR -  MvObsSet::write(const void*,int) -> could not write "
                 "a field"
              << std::endl;
    return false;
  }
...
bool MvObsSet::write(MvObs &anObs) {
  ...
  if (clone_handle == NULL) {
    std::cout << "ERROR -  MvObsSet::write(MvObs&) -> could not clone field"
              << std::endl;
    return false;
  }
  ...
  if (codes_get_message(clone_handle, &buffer, &size)) {
    std::cout << "ERROR -  MvObsSet::write(MvObs&) -> could not create a "
                 "buffer message"
              << std::endl;
...
int MvObsSet::obsCount() {
  ...
  _obsCount = 0;
  while ((ecH = codes_handle_new_from_file(NULL, _ecFile, PRODUCT_BUFR,
                                           &err)) != NULL ||
         err != CODES_SUCCESS) {
    if (ecH == NULL) {
...
void MvObsSet::searchMinMaxTime() {
  std::cout << "MvObsSet::searchMinMaxTime() -> not implemented yet"
            << std::endl;
  exit(0);
...
MvDate MvObsSet ::maxDate() {
  searchMinMaxTime();
  double timfloat =
      10000.0L * (double)_maxTime.GetYear() +
      100.0L * (double)_maxTime.GetMonth() + (double)_maxTime.GetDay() +
...
MvObsSetIterator::MvObsSetIterator(MvObsSet &s)
    : _NoFiltersSet(true), useObsTime_(false), _SelectValueCount(0),
      _MsgTypeCount(0), _MsgSubtypeCount(0), _TimeFilterState(kTFS_notSet),
      _SelectState(SF_notSet), ObsSet(&s), observer_(0),
      filterProgressStep_(20) {}

//___________________________________________________________________
MvObsSetIterator::~MvObsSetIterator() {}

bool MvObsSetIterator::checkOptionSize(std::size_t num,
                                       const std::string &functionName) {
  if (num >= MAX_FILTER_LIST_ARRAY_SIZE) {
    std::cerr << ">>> MvObsIterator::" + functionName + " array overflow!!!"
              << std::endl;
    return false;
  }
  return true;
...
MvObs MvObsSetIterator::nextMessage() { return ObsSet->next(false); }
...
// e  if( mars.debug && ( ( ObsSet->messageNumber() == 1 ) ||
// change_me_with_debugger ) ) e    current.printAllValues();
#endif
}
...
void MvObsSetIterator::setTimeRange(const TDynamicTime &anObsTime,
                                    short deltaInMinutes) {
...
void MvObsSetIterator::setTimeRange(const TDynamicTime &aBeginTime,
                                    const TDynamicTime &anEndTime) {
...
// setTimeRangeWithoutDate
// add 941229/vk
//
// parameters: in format HHMM, i.e. 1200 == 12.00, 15 == 0.15, i.e.
// 100*hour+min!!!
...
void MvObsSetIterator::select(const std::string &aDescriptor, double aValue) {
  ...
  if (_SelectValueCount > 0 && aDescriptor != _SelectDescriptor) {
    std::cerr << ">>> MvObsIterator::select: changing the descriptor while "
                 "building the list!!!"
              << std::endl;
  }
...
void MvObsSetIterator::selectRange(const std::string &aDescriptor,
...
void MvObsSetIterator::excludeRange(const std::string &aDescriptor,
                                    double firstValue, double secondValue) {
  _SelectDescriptor = aDescriptor;
  _SelectValue[0] = firstValue < secondValue ? firstValue : secondValue;
  _SelectValue[1] = secondValue > firstValue ? secondValue : firstValue;
...
void MvObsSetIterator::setXSectionLine(const MvLocation &aStartPoint,
...
void MvObsSetIterator::setArea(const MvLocation &aCorner1,
                               const MvLocation &aCorner2) {
...
void MvObsSetIterator::setMessageType(int aMsgType) {
  if (_MsgTypeCount >= MAX_FILTER_LIST_ARRAY_SIZE) {
    std::cerr << ">>> MvObsIterator::setMessageType: array overflow!!!"
              << std::endl;
    return;
  }
...
void MvObsSetIterator::setMessageSubtype(int aMsgSubtype) {
  if (_MsgSubtypeCount >= MAX_FILTER_LIST_ARRAY_SIZE) {
    std::cerr << ">>> MvObsIterator::setMessageSubtype: array overflow!!!"
              << std::endl;
    return;
  }
...
void MvObsSetIterator::setOriginatingCentreAsStr(const std::string &centre) {
  if (!checkOptionSize(originatingCentreStr_.size(),
                       "setOriginatingCentreAsStr"))
    return;
...
bool MvObsSetIterator::selectOk(MvObs *anObs) const {
  ...
  // We can assume that this key is from the  data section so we need to expand
  // the message
  anObs->expand();
...
std::ostream &operator<<(std::ostream &aStream,
                         const MvObsSetIterator &aFilter) {
  int i;
