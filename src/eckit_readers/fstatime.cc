...
TStaticTime ::TStaticTime(void) { _setCurrent(); }
...
const char *TStaticTime::CharHhMm() const {
  ostrstream ss(charValueString, sizeof(charValueString));
  ss << setfill('0') << setw(2) << GetHour() << ":" << setw(2) << GetMin()
     << ends;
...
const char *TStaticTime::CharValue(void) const {
  ostrstream ss(charValueString, sizeof(charValueString));
  ss << CharDate() << " " << setfill('0') << setw(2) << GetHour() << ":"
     << setw(2) << GetMin() << ":" << setw(2) << GetSec() << ends;
...
const char *TStaticTime::ShorterCharValue(void) const {
  ostrstream ss(charValueString, sizeof(charValueString));
  ss << setfill('0') << setw(2) << (GetYear() % 100) << setw(2) << GetMonth()
     << setw(2) << GetDay() << " " << setw(2) << GetHour() << setw(2)
     << GetMin() << ends;
...
void TStaticTime ::SetDate(const short year, const short month,
                           const short day) {
  SetYear(year);
  SetMonth(month);
  SetDay(day);
...
void TStaticTime ::SetTime(const short hour, const short minute,
                           const short sec) {
  SetHour(hour);
  SetMin(minute);
  SetSec(sec);
...
{
  cout << fDay << "." << fMonth << "." << fYear << " " << fHour << ":" << fMin
       << ":" << fSec; //<< "\n";
}
//__________________________________________________________________
