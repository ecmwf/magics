...
void TableReader::setFieldContainer(int index, string &name,
                                    vector<double> &container,
                                    double outputMissingIndicator) {
  if (index == -1) // is the user wanting to find a field with the given name?
  {
    ...
    {
      setError("Cannot find field with name " + name +
               " -  will not be stored.");
      return;
    }
  ...
  resizeDecoders(index +
                 1); // e.g. if index==2, then we need 3 decoders (0-indexed)

  // add the decoder to the appropriate place in our list

  TableDoubleVectorElementDecoder *decoder =
      new TableDoubleVectorElementDecoder(container, outputMissingIndicator);
  TableElementDecoder *genericDecoder =
      dynamic_cast<TableElementDecoder *>(decoder);
...
void TableReader::setFieldContainer(int index, string &name,
  ...
  TableStringVectorElementDecoder *decoder =
      new TableStringVectorElementDecoder(container, outputMissingIndicator);
  TableElementDecoder *genericDecoder =
      dynamic_cast<TableElementDecoder *>(decoder);
  decoderSets_[index].push_back(genericDecoder); //.push_back(genericDecoder);
  names_[index] = &name;
...
void TableReader::splitLineConsecutiveDelimiters(char *line,
                                                 vector<char *> &tokens) {
  char *current = line;
  char *token = current;
  ...
  while (*current != '\0') {
    if (*current == delimiter_) // reached the end of a token?
    {
      *current = '\0'; // yes - terminate the token string
      ...
      while (*current != '\0' &&
             *current == delimiter_) // skip all consecutive delimiters
        current++;
      current--; // step back one character

      if (token[0] != '\0') // when treating consecutive delimiters as one, we
  ...
  if (token[0] !=
      '\0') // safeguard against stray delimiters at the end of the line
    tokens.push_back(token); // store a pointer to the last token
}
...
TableReader::eTableReaderFieldType TableReader::guessFieldType(char *str) {
  if (str) {
    ...
    while (*pch) {
      int ch = *pch; // any character not part of a proper number format means
                     // this is a string
...
int TableReader::nextLineTokens(char *line, size_t sizeOfLine,
                                vector<char *> &tokens) {
  // skip blank lines
  ...
  */
  ...
  if (f_.getline(line, sizeOfLine)) {
    ...
    if ((numread >= 2) && (line[numread - 1] == '\0') &&
        (line[numread - 2] ==
         '\r')) // remove extraneous newline characters from Windows
      line[numread - 2] = '\0';
...
bool TableReader::readUserMetaData(char *line, size_t sizeOfLine,
                                   string &errorMessage) {
  int currentRow = 1;
  char oldDelimiter = delimiter_; // store because we will over-ride it
  bool oldConsecutive =
      consecutiveDelimitersAsOne_; // store because we will over-ride it
  ...
  for (vector<int>::size_type r = 0; r < userMetaDataRows_.size();
       r++) // for each line of meta-data...
  {
    skipLines(userMetaDataRows_[r] - currentRow, line,
              sizeOfLine); // get to the right line
    setDelimiter(' ');
...
bool TableReader::getMetaData(string &errorMessage) {
  ...
  numRecords_ = numLines - headerRow() - dataRowOffset() +
                1; // how many actual records are there?
  ...
  dataStart_ = f_.tellg(); // make a note of where the data starts (may be
                           // changed if we have a header line)

  int numLinesToCheck =
      1 + (headerRow() > 0 ? 1 : 0); // how many lines to read?
  ...
  for (int i = 0; i < numLinesToCheck; i++) {
    ...
    if (nextLineTokens(line, sizeof(line), tokens) > 0) {
      if (i == 0) {
        resizeDecoders(tokens.size()); // ensure our array of decoders is big
                                       // enough to handle all the fields
      }
      ...
      if ((i == 0) && headerRow() > 0) // parse a header row?
      {
        ...
        skipLines(dataRowOffset() - 1, line,
                  sizeof(line)); // go to start of data
      ...
      else // this is a data row
      {
        if (types_.size() != tokens.size()) {
          errorMessage = "Not the same number of elements in the first 2 lines "
                         "of table. Check file format parameters.";
        ...
        for (vector<eTableReaderFieldType>::size_type j = 0; j < tokens.size();
             j++) {
...
bool TableReader::read(string &errorMessage) {
  ...
  if (!getMetaData(errorMessage)) // ensure we have the meta-data. This will
                                  // also open the file (f_)
    return false;
  ...
  for (vector<TableElementDecoders>::iterator decoderSet = decoderSets_.begin();
    ...
    for (TableElementDecoders::iterator decoder = (*decoderSet).begin();
         decoder != (*decoderSet).end(); ++decoder) {
      if (*decoder != NULL) {
  ...
  f_.clear();
  f_.seekg(dataStart_,
           ios::beg); // set to start of the data (determined by getMetaData())
  ...
  for (int i = 0; i < numRecords_; i++) {
    ...
    if (nextLineTokens(line, sizeof(line), tokens) > 0) {
      if (tokens.size() != decoderSets_.size()) // only do something if we have
                                                // the right number of tokens
      {
        ...
        } else // otherwise, the disparity in numbers indicates a problem
        {
          char msg[128];
          sprintf(msg,
                  "TableReader: record %d has %u elements, but the first "
                  "record has %u. Failed to read.",
                  i + 1, (unsigned int)tokens.size(),
                  (unsigned int)decoderSets_.size());
          errorMessage = msg;
          f_.close(); // close the input file
          return false;
        }
      ...
      // decode the data with their various decoders - one set of decoders for
      // each field

      vector<char *>::iterator s = tokens.begin();

      for (vector<TableElementDecoders>::iterator decoderSet =
               decoderSets_.begin();
           decoderSet != decoderSets_.end(); ++decoderSet) {
        for (TableElementDecoders::iterator decoder = (*decoderSet).begin();
             decoder != (*decoderSet).end(); ++decoder) {
