...
public:
  Station(const XmlNode &node) {
    ...
    if (template_.empty())
      template_ = string(getenv("MAGPLUS_HOME")) +
                  "/share/templates/10_days_epsgram.xml";
  ...
  void visit(const XmlNode &node) {
    if (node.name() == "station") {
      ...
      for (XmlNode::DataIterator data = node.firstData();
           data != node.lastData(); ++data)
        station << *data;
      ...
      try {
        ...
        if (!test.good()) {
          ...
          template_ =
              string(getenv("MAGPLUS_HOME")) + "/share/templates/" + template_;
          MagLog::dev() << "try to find template in system directory-> "
                        << template_ << endl;
          ifstream defaut(template_.c_str());
          if (!defaut.good()) {
            MagLog::error()
                << "Can not open template file " << template_ << endl;
            return;
          }
          defaut.close();
        }
        ...
        for (map<string, string>::iterator param = parameters_.begin();
             param != parameters_.end(); ++param)
